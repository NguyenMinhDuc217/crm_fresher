...
class CRMEntity {
    ...
    // IMPORTANT: do not apply caching to this function as it will cause issue record not updated when editing
	...
	function initialize() {
        ...
        // Modified by Hieu Nguyen on 2021-05-31 to prevent recursive loop
		$moduleModel = Vtiger_Module::getInstance($moduleName);
        
		if ($moduleModel && $moduleModel->isentitytype !== '1') {
			return;
		}
        // End Hieu Nguyen
	...
	function saveentity($module, $fileid = '') {
		checkRecordLimitBeforeSaveEntity($this, $module);	// Added by Hieu Nguyen on 2022-04-06 to check record limit before saving
		...
		if(count($_FILES)) {
			...
			// Added by Phuc on 2020.01.14 to validate file ext
			foreach ($_FILES as $file) {
				if (!empty($file['name'])) {
					...
					if (!in_array($fileExt, $GLOBALS['validationConfig']['allowed_upload_file_exts'])) {
						throw new Exception(vtranslate('LBL_FILE_EXT_DO_NOT_ALLOW'));
					}
				}
			}
			// Ended by Phuc
	...
	function uploadAndSaveFile($id, $module, $file_details, $attachmentType='Attachment') {
		...
		$mimeType = $file_details['type'];  // Modified by Hieu Nguyen on 2020-08-27 to fix bug can not upload strange file type like DWG
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		$ownerid = $this->column_fields['assigned_user_id'];
        $mainOwnerId = $this->column_fields['main_owner_id'];   // Added by Hieu Nguyen on 2020-08-03 to save main owner id with new entity
		...
		if ($this->mode == 'edit') {
			...
			if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
				// Modified query by Hieu Nguyen on 2020-08-03 to save main_owner_id
                $sql = "UPDATE vtiger_crmentity SET smownerid = ?, main_owner_id = ?, smgroupid = ?, modifiedby = ?, description = ?, modifiedtime = ? WHERE crmid = ?";
				$params = [$ownerid, $mainOwnerId, $groupid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id];
                // End Hieu Nguyen
			} else {
				...
				if (is_array($columname) && in_array("description", $columname)) {
                    ...
                    // Modified query by Hieu Nguyen on 2020-08-03 to save main_owner_id
					$sql = "UPDATE vtiger_crmentity SET smownerid = ?, main_owner_id = ?, smgroupid = ?, modifiedby = ?, modifiedtime = ? WHERE crmid = ?";
					$params = [$ownerid, $mainOwnerId, $groupid, $current_user->id, $adb->formatDate($date_var, true), $this->id];
                    // End Hieu Nguyen
				}
			}
            ...
            // Modified query by Hieu Nguyen on 2020-08-03 to save main_owner_id
			$sql = "INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid, smgroupid, setype, description, modifiedby, createdtime, modifiedtime, source, main_owner_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$params = [$current_id, $current_user->id, $ownerid, $groupid, $module, $description_val, $current_user->id, $created_date_var, $modified_date_var, $source, $mainOwnerId];
            // End Hieu Nguyen
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		if ($insertion_mode == 'edit') {
			...
			$acl = Vtiger_AccessControl::loadUserPrivileges($current_user->id);
			if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
				// Modified query by Hieu Nguyen on 2018-12-15 to allow saving data into the fields that are not display on the form
				$sql = "select fieldname,columnname,uitype,generatedtype,
										typeofdata from vtiger_field where tabid in (" . generateQuestionMarks($tabid) . ") and tablename=? and displaytype in (1,3,6) group by columnname";
				...
				if (count($profileList) > 0) {
					...
					$sql = "SELECT vtiger_field.fieldname,vtiger_field.columnname,vtiger_field.uitype,vtiger_field.generatedtype,vtiger_field.typeofdata FROM vtiger_field
						...
						AND vtiger_def_org_field.visible = 0 and vtiger_field.tablename=? and vtiger_field.displaytype in (1,3,6) group by columnname";
			...
			// Modified query by Hieu Nguyen on 2018-12-15 to allow saving data into the fields that are not display on the form
			$sql = "select fieldname,columnname,uitype,generatedtype,typeofdata from vtiger_field where tabid=? and tablename=? and displaytype in (1,3,4,6)";
			$params = array($tabid, $table_name);
		}
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if (($insertion_mode == 'edit' && $skipUpdateForField == false) || $_REQUEST['imgDeleted']) {
						$skipUpdateForField = false;
						$uploadedFileNames = array();
                        // Modified by Hieu Nguyen on 2021-06-11 to fix bug ambiguous column name when getting image from non-person modules
						$getImageNamesSql = "SELECT vtiger_attachments.name 
                            FROM vtiger_seattachmentsrel 
                            INNER JOIN vtiger_attachments ON (vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid) 
                            LEFT JOIN {$table_name} ON ({$table_name}.{$tablekey} = vtiger_seattachmentsrel.crmid) 
	...
	function retrieve_entity_info($record, $module, $allowDeleted = false) {
		...
		$moduleName = ($module == 'Events') ? 'Calendar' : $module;	// Moved this line here by Hieu Nguyen on 2022-01-18
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				// Modified by Hieu Nguyen on 2022-01-18 to clear value of parent_id when that id is actually the related value of Account and Lead fields
				$columnAlias = $this->createColumnAliasForField($fieldinfo);

				if ($columnAlias == 'vtiger_seactivityrelparent_id') {
					$column_clause .= " MAX(
							CASE
								WHEN vtiger_seactivityrel_seentity.setype NOT IN ( 'Accounts', 'Leads' )
								THEN vtiger_seactivityrel_seentity.crmid
								ELSE '' 
							END
						) AS vtiger_seactivityrelparent_id,";
				}
				else {
					$column_clause .=  "{$fieldinfo['tablename']}.{$fieldinfo['columnname']} AS {$columnAlias},";
				}
				// End Hieu Nguyen
			}
			...
			// Added by Hieu Nguyen on 2022-01-18 to clear value of parent_id when that id is actually the related value of Account and Lead fields
			if ($moduleName == 'Calendar') {
				$from_clause .= " LEFT JOIN vtiger_crmentity AS vtiger_seactivityrel_seentity ON vtiger_seactivityrel.crmid = vtiger_seactivityrel_seentity.crmid";
			}
			// End Hieu Nguyen

            // Modified by Hieu Nguyen on 2020-05-25 to fix bug Vtiger_Record_Model::getInstanceById('<Id>', '<Module>') return record even when the record is not matching with specified <Module>
			$where_clause .= " vtiger_crmentity.setype = ? AND vtiger_crmentity.crmid = ?";
			$params[] = $moduleName;
			$params[] = $record;
            // End Hieu Nguyen
			...
			// Added by Hieu Nguyen on 2022-01-18 to remove duplicated rows using GROUP BY statement
			if ($moduleName == 'Calendar') {
				$sql = str_replace($limit_clause, "GROUP BY vtiger_activity.activityid {$limit_clause}", $sql);
			}
			// End Hieu Nguyen
			...
			if (!$result || $adb->num_rows($result) < 1) {
				...
				foreach ($cachedModuleFields as $fieldinfo) {
					...
					if (isset($resultrow[$fieldkey])) {
						...
						// Added by Phu Vo on 2021.06.20 to fix text issue on retrieve and save CRMEntity (without Record Model)
						if (is_string($fieldvalue)) $fieldvalue = decode_html($fieldvalue);
						// End Phu Vo

						// Added by Hieu Nguyen on 2022-01-18 to clear value of parent_id when that id is actually the related value of Account and Lead fields
						if ($moduleName == 'Calendar' && $fieldkey == 'vtiger_seactivityrelparent_id' && !empty($fieldvalue)) {
							$referenceModule = getSalesEntityType($fieldvalue);

							if (in_array($referenceModule, ['Accounts', 'Leads'])) {
								$fieldvalue = '';
							}
						}
						// End Hieu Nguyen
		...
		// Added by Hieu Nguyen on 2022-01-19 to store the raw fetched row so that we can compare with the modified data later 
        $this->fetchedRow = array_merge([], $this->column_fields->getColumnFields());
        // End Hieu Nguyen
	}
	...
	function save($module_name, $fileid = '') {
		...
		// Modified by Hieu Nguyen on 2019-01-03 to support event handling for both saving from the form and from importing
		global $batchSaveConfig;
		
		if(!self::isBulkSaveMode() || (self::isBulkSaveMode() && $batchSaveConfig['trigger_entity_events'])) {
		...
		if($em) {
			// Added by Hieu Nguyen on 2022-10-17 to fix bug full_name is empty when handling event vtiger.entity.aftersave
			if ($entityData->focus->isPerson) {
				$entityData->set('full_name', $entityData->get('label'));
			}
...
//Added by Richie
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$temporaryTableCrmentityrel = self::setupTemporaryTableCrmentityrel($id);

		$query .= " FROM $other->table_name";
		$query .= " INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $other->table_name.$other->table_index";
		$query .= " INNER JOIN {$temporaryTableCrmentityrel} ON ({$temporaryTableCrmentityrel}.relcrmid = vtiger_crmentity.crmid)";
		$query .= $more_relation;
		$query .= " LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid";
		$query .= " LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid";
		$query .= " WHERE vtiger_crmentity.deleted = 0";
		if($related_module == 'Leads' && $currentModule != 'Campaigns') { // Modified by Phu Vo to exclude module Campaigns
	...
	// Added by Tung Nguyen on 2022.04.29 auto setup and return temp table name to join with instead of using the original table vtiger_crmentityrel -> no more using OR condition inside JOIN statement to boost performance
	// Ex: instead of "INNERT JOIN vtiger_crmentityrel AS rel ON (rel.crmid = {$record} OR rel.relcrmid = {$record})", use the following way:
	// 	$tempTableCrmentityrel = CRMEntity::setupTemporaryTableCrmentityrel($record);
	// 	$sql .= "INNERT JOIN {$tempTableCrmentityrel} AS rel ON (rel.crmid = {$record})"
	// => The query will run much faster even when tables have large data :)
	static function setupTemporaryTableCrmentityrel($crmIds = null) {
		global $adb;

		$tableName = 'vtiger_crmentityrel_' . $GLOBALS['current_user']->id;
		...
		if (!empty($crmIds)) {
			$condition['crmid'] = " AND crmid IN ({$crmIds})";
			$condition['relcrmid'] = " AND relcrmid IN ({$crmIds})";
		}

		// Modified query create temp table by Hieu Nguyen on 2022-12-15 to create temp table with 4 column: crmid, module, relcrmid, relmodule like the orignal vtiger_crmentityrel table
		$query = "CREATE TEMPORARY TABLE IF NOT EXISTS {$tableName} (crmid INT(11), module VARCHAR(100), relcrmid INT(11), relmodule VARCHAR(100),  PRIMARY KEY (crmid, relcrmid)) IGNORE
			SELECT * FROM (
				(SELECT crmid, module, relcrmid, relmodule FROM vtiger_crmentityrel WHERE 1 = 1 {$condition['crmid']})
				UNION
				(SELECT relcrmid, relmodule, crmid, module FROM vtiger_crmentityrel WHERE 1 = 1 {$condition['relcrmid']})
			) AS temp";
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				$ui10_modules_query = $adb->pquery("SELECT relmodule FROM vtiger_fieldmodulerel WHERE relmodule NOT IN ('Events') AND fieldid=?", array($field_id));	// Modified by Hieu Nguyen on 2022-10-18 to prevent duplicate alias for relate table from Comment to Calendar/Events
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
		...
		if ($pritablename == 'vtiger_crmentityrel') {
			...
			// Modify by Tung Nguyen on 2022-10-10 to boost performance when load report chart with related list n-n
			$condition = " {$tableName}.{$column_name} = {$tmpname}.relcrmid";
					
			$temporaryTableCrmentityrel = self::setupTemporaryTableCrmentityrel();
			$query = " LEFT JOIN {$temporaryTableCrmentityrel} AS $tmpname ON ($condvalue={$tmpname}.crmid AND {$tmpname}.relmodule='{$secmodule}')";
			// Ended by Tung Nguyen
	...
	function getNonAdminAccessQuery($module, $user, $parentRole, $userGroups) {
        ...
        // Added by Hieu Nguyen on 2021-01-27 to get custom group ids for temp table. This update wil help increase performance a lot:
        //  1. Boost non-admin permission query performance as we don't need to modify join condition in function getNonAdminAccessControlQuery() 
        //  2. Boost admin query performance as we dont't need to add DISTINCT keyword in the main query SELECT statement (this keyword causes significant performance decreased with large data)
        $customGroupsQuery = "
            (
                SELECT DISTINCT groupid
                FROM (
                    (SELECT ug.userid AS member_id, ug.groupid FROM vtiger_users2group AS ug INNER JOIN vtiger_groups AS g ON (g.groupid = ug.groupid AND g.is_custom = 1))
                    UNION
                    (SELECT gg.containsgroupid AS member_id, gg.groupid FROM vtiger_group2grouprel AS gg INNER JOIN vtiger_groups AS g ON (g.groupid = gg.groupid AND g.is_custom = 1))
                ) AS cgm 
                -- INNER JOIN ({$query}) AS t ON (t.id = cgm.member_id)
                WHERE cgm.member_id IN (SELECT id FROM ({$query}) as t)
            )";
        $query .= " UNION {$customGroupsQuery}";
        // End Hieu Nguyen
	 ...
	 * @param <type> $userGroups
	 */
    // Modified by Hieu Nguyen on 2020-08-07 to get non-custom groups only for access query to boost performance
	function getNonAdminUserGroupAccessQuery($userGroups) {
		$query = " UNION (SELECT groupid FROM vtiger_groups WHERE is_custom = 0 AND groupid IN (". implode(",", $userGroups) ."))";
		return $query;
	}
	...
	function getNonAdminAccessControlQuery($module, $user, $scope = '') {
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2]
            ...
            // Reversed join condition to vtiger default by Hieu Nguyen on 2021-01-27 to fix performance issue
            if ($scope == '') {

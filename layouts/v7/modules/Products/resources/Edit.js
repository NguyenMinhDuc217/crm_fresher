...
},
{
	...
	registerRecordPreSaveEvent: function(form) {
		...
		app.event.on(Vtiger_Edit_Js.recordPresaveEvent, function(e, data) {
			var isActiveEle = form.find('input[name="active"]');
			var recordId = jQuery('input[name="record"]').val();
	...
	registerTaxEvents : function(container) {
		...
		container.on('change','.taxes', function(e){
			...
			// Modified by Vu Mai on 2023-01-13 to prevent checkbox in popup quick create hide input tax in EditView when popup show at Editview
			if (element.is(":checked")) {
				element.closest('.fieldLabel').next('.fieldValue').find('#'+taxIdSelector).removeClass('hide').addClass('show');
			} else {
				element.closest('.fieldLabel').next('.fieldValue').find('#'+taxIdSelector).removeClass('show').addClass('hide');
			}
			// End Vu Mai
		});
	},

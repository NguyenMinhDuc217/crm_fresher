					...
					{foreach item=LISTVIEW_ENTRY key=RECORD_ID from=$LISTVIEW_ENTRIES}
						<tr class="listViewEntries" data-id="{$RECORD_ID}" data-recordurl="index.php?module=Portal&view=Detail&record={$RECORD_ID}">
								...
								<div class="table-actions">
									...
									<span class="more dropdown action">
										<span href="javascript:;" class="dropdown-toggle" data-toggle="dropdown">
											<i class="far fa-ellipsis-v-alt icon"></i>
										</span>
							...
							<td class="listViewEntryValue {$WIDTHTYPE}" nowrap>
                                {* Modified by Hieu Nguyen on 2021-03-29 to open link in new tab when clicking on the bookmark name *}
								<a href="{if substr($LISTVIEW_ENTRY->get('portalurl'), 0, 4) neq 'http'}//{/if}{$LISTVIEW_ENTRY->get('portalurl')}" target="_blank" sl-processed="1">{$LISTVIEW_ENTRY->get('portalname')}</a>
                                {* End Hieu Nguyen *}
							</td>
							<td class="listViewEntryValue {$WIDTHTYPE}" nowrap>

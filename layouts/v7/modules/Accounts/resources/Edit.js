	...
	copyAddress : function(swapMode, container){
		...
		if(swapMode == "false"){
			for(var key in addressMapping) {
				...
				if (fromElement.next('.coordinate').length > 0) {
					if (toElement.next('.coordinate').length == 0) {
						$('<div class="coordinate hide"></div>').insertAfter(toElement);
					}
		
					if (toElement.next().find('.longitute').length == 0) {
						toElement.next().append('<input type="hidden" name="' + toElement.attr('name') + '_longitude" class="longitute"/>');
					}
		
					if (toElement.next().find('.latitude').length == 0) {
						toElement.next().append('<input type="hidden" name="' + toElement.attr('name') + '_latitude" class="latitude"/>');
					}

					toElement.next().find('.longitute').val(fromElement.next().find('.longitute').val());
            		toElement.next().find('.latitude').val(fromElement.next().find('.latitude').val());
				}
				// Ended by Phuc
			}
		} else if(swapMode){

								...
								{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
												...
												<div {if count($IMAGE_DETAILS) > 0}class="images-container"{/if}>
													{foreach key=ITER item=IMAGE_INFO from=$IMAGE_DETAILS}
														<div class="swipe-image-holder-lg" style="margin-top:5px;" title="{$IMAGE_INFO.name}">
															...
															{else}
																<a href="resources/images/default-user-avatar.png" class="swipebox image animate-bg-zoomout" data-image-id="{$IMAGE_INFO.id}" onclick="event.preventDefault()" style="background-image:url('resources/images/default-user-avatar.png')">
																</a>
															{/if}
														</div>
													{/foreach}
									...
									{else}
										...
										{else}
											{assign var=COUNTER value=$COUNTER+1}
										{/if}
										<td class="fieldLabel {$FIELD_NAME} {$WIDTHTYPE} textOverflowEllipsis" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
											<span class="muted">
										...
										<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                                            ...
                                            {else}
                                                ...
                                                {if $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && $FIELD_MODEL->isAjaxEditable() eq 'true'}
                                                    ...
                                                    <span class="action pull-right"><a href="#" onclick="return false;" class="editAction far fa-pen"></a></span>
                                                {/if}
                                            {/if}
                                            {* End Hieu Nguyen *}
										</td>

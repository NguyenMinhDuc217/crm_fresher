...
{strip}
	...
	<html>
		<head>
			<!-- Added By Kelvin Thang -- Date:2018-10-11-->
            ...
            {* Modified by Hieu Nguyen on 2019-11-07 *}
			<link rel="stylesheet" href="modules/Users/resources/UserSetup.css"></link>
            {* End Hieu Nguyen *}
		</head>
		<body>
			<div class="container">
                {* Added by Hieu Nguyen on 2019-11-07 to load the real index *}
				...
				<form class="form" method="POST" action="index.php?module=Users&action=UserSetupSave">
					...
					<div class="modal" {if false && $IS_FIRST_USER}style="width: 700px;"{/if}> {* FirstUser information gather - paused *}
						<div class="modal-header">
							<h3>{vtranslate('LBL_USER_SETUP_POPUP_TITLE', $MODULE)}</h3> {* Changed the popup title label by Hieu Nguyen on 2019-11-07 *}
						</div>
							...
							<div class="row">
								{if false && $IS_FIRST_USER} {* FirstUser information gather - paused *}
									<div class="span4">
                                        ...
                                        {* Moved system curreny here by Hieu Nguyen on 2019-11-07 *}
										<div class="controls" id="base_currency_name_controls">
											<select name="base_currency_name" id="base_currency_name" placeholder="{vtranslate('LBL_BASE_CURRENCY', $MODULE)}" data-errormessage="{vtranslate('LBL_CHOOSE_BASE_CURRENCY', $MODULE)}" class="validate[required]" style="width:250px;">
												<option value=""></option>
												{foreach key=header item=currency from=$BASE_CURRENCIES}
											...
											<span rel="tooltip" title="{vtranslate('LBL_OPERATING_CURRENCY', $MODULE)}" id="base_currency_name_tooltip" class="icon-info-sign"></span>
											<div style="padding-top:10px;"></div>
										</div>
                                        {* End Hieu Nguyen *}
										<div class="controls">
								...
								{/if}
								<div class="span6"> {* Modified from span4 to span6 by Hieu Nguyen on 2019-11-07 to make the form wider *}
                                    ...
                                    <div class="controls" id="currency_id_controls">
                                        <select name="currency_id" id="currency_id" placeholder="{vtranslate('Currency', $MODULE)}" class="validate[required]" style="width:250px;">
                                            {foreach key=CURRENCY_ID item=CURRENCY from=$USER_CURRENCIES}
                                                <option value="{$CURRENCY_ID}" {if $CURRENCY_ID eq $CURRENT_USER_MODEL->get('currency_id')}selected{/if}>{$CURRENCY['name']|@getTranslatedCurrencyString} ({$CURRENCY['symbol']})</option> 
                                            {/foreach}
                                        </select>&nbsp;
                                        <span rel="tooltip" title="{vtranslate('Currency', $MODULE)}" id="currency_id_tooltip" class="icon-info-sign"></span>
                                        <div style="padding-top:10px;"></div>
                                    </div>
                                    <div class="controls" id="currency_symbol_placement_controls">
                                        <select name="currency_symbol_placement" id="currency_symbol_placement" placeholder="{vtranslate('Symbol Placement', $MODULE)}" class="validate[required]" style="width:250px;">
                                            {foreach key=header item=currency from=$ENUM_LIST['currency_symbol_placement_dom']}
                                                <option value="{$header}" {if $header eq $CURRENT_USER_MODEL->get('currency_symbol_placement')}selected{/if}>{$header|@getTranslatedCurrencyString}</option> 
                                            {/foreach}
                                        </select>&nbsp;
                                        <span rel="tooltip" title="{vtranslate('Symbol Placement', $MODULE)}" id="currency_symbol_placement_tooltip" class="icon-info-sign"></span>
                                        <div style="padding-top:10px;"></div>
                                    </div>
                                    {* End Hieu Nguyen *}
								</div>

	...
	*/
	restoreUser : function(userId) {
		app.helper.showConfirmationBox({
			...
			// Modified by Hieu Nguyen on 2022-03-31 to hide loading and display error in 5s
			app.request.post(params).then(
				...
				if(err === null) {
					...
					var params = {
						submitHandler: function(form) {
                            ...
                            // Added by Hieu Nguyen on 2020-01-06 to validate password
                            if (!PasswordValidator.validate(new_password.val())) {
                                return false;
                            }
                            // End Hieu Nguyen
	...
	loadListViewRecords : function(urlParams) {
		...
		app.request.get({data:usersListParams}).then(function(err, res){
			...
			container.find('div.list-content').html(res);
	...
	getListSearchParams : function(includeStarFilters) {
		...
		listViewTable.find('.listSearchContributor').each(function(index,domElement){
			...
			if(fieldName in currentSearchParams) {
                ...
                // Modified by Hieu Nguyen on 2021-01-15 to seperate first name, last name and email fields
	...
	registerEvents : function() {
		...
		// this.registerEditLink(); // Commented out by Hieu Nguyen on 2020-04-20 as this function is not used anymore
	}
});

						...
						<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}&parentblock=LBL_USER_MANAGEMENT' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
							...
							{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
											...
											<span class="value textOverflowEllipsis">
												<div style="margin-left: -13px;">
													...
													{if $IMAGE_DETAILS[0]['id'] eq null}
														<div class='col-lg-2'>
															<i class="far fa-user userDefaultIcon"></i>
														</div>
													{/if}
                                ...
                                <td class="{$WIDTHTYPE}" nowrap>
                                    <span class="fieldValue">
                                        <span class="value textOverflowEllipsis">
                                            {if in_array($LISTVIEW_HEADER->getName(), ['first_name', 'last_name', 'user_name'])}
                                                <a href="index.php?module=Users&view=Detail&parent=Settings&record={$LISTVIEW_ENTRY->getId()}">{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}</a>
                                            {else}
                                                {$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
                                            {/if}
                                        </span>
								...
								<div class="emptyRecordsContent">
									<center>
										{if $SEARCH_VALUE eq 'Active'}
											...
											{vtranslate('LBL_NO')} {vtranslate($MODULE, $MODULE)} {vtranslate('LBL_FOUND')},{if $IS_MODULE_EDITABLE} <a href="{$MODULE_MODEL->getCreateRecordUrl()}"> {vtranslate('LBL_CREATE_USER',$MODULE)}</a>{/if}
										{else}

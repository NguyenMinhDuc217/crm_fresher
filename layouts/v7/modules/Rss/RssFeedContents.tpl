    ...
    <tr class="listViewEntries" data-id='{$RECORD->getId()}'>
        {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
            ...
            <td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
                <a class='feedLink' href="#" data-url="{$LISTVIEW_ENTRY['link']}">{$LISTVIEW_ENTRY[$LISTVIEW_HEADERNAME]}</a>
            {if $LISTVIEW_HEADER@last}
            </td>
                <td nowrap class="{$WIDTHTYPE}">
                    <span class="actions">
                        <span class="actionImages pull-right">
                            <a href="{$LISTVIEW_ENTRY['link']}" target="_BLANK"><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="far fa-external-link"></i></a>&nbsp;
                        </span>
                    </span>
                </td>
            {/if}

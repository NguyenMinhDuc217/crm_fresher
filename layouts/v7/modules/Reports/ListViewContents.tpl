...
{strip}
	...
	<div class="col-sm-12 col-xs-12 ">
		...
		<div class="essentials-toggle" title="{vtranslate('LBL_LEFT_PANEL_SHOW_HIDE', 'Vtiger')}">
			<span class="essentials-toggle-marker far {if $LEFTPANELHIDE eq '1'}fa-chevron-right{else}fa-chevron-left{/if} cursorPointer"></span>
		</div>
							...
							<tr class="searchRow">
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
									<th>
                                        ...
                                        {* Added by Hieu Nguyen on 2019-06-19 to filter owner field with autcomplete UI *}
                                        {elseif $LISTVIEW_HEADER_KEY == 'owner'}
											<input type="text" name="{$LISTVIEW_HEADER_KEY}" class="listSearchContributor inputElement" data-fieldinfo='{$FIELD_INFO|escape}'
                                                {if $SEARCH_DETAILS[$LISTVIEW_HEADER_KEY]['searchValue']} data-selected-tags='{ZEND_JSON::encode(Vtiger_Owner_UIType::getSelectedOwnersForSearchView($SEARCH_DETAILS[$LISTVIEW_HEADER_KEY]['searchValue']))}' {/if}
                                            />
                                        {* End Hieu Nguyen *}
						...
						{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
							<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY  from=$LISTVIEW_HEADERS}
									...
									{* Modified by Hieu Nguyen on 2018-12-28 *}
									{assign var=LISTVIEW_ENTRY_VALUE value=html_entity_decode($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}
									{* End Hieu Nguyen *}
									...
									<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADERNAME}" title="{$LISTVIEW_ENTRY_RAWVALUE}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="">
										<span class="fieldValue">
											{* Modified by Phu Vo on 2021.27.07 to remove spaces between HTML elements *}
											{strip}
												<span class="value textOverflowEllipsis">
													{if $LISTVIEW_HEADERNAME eq 'reporttype'}

														{* Modified by Phu Vo on 2019.09.27 to Display Report type label as report type at List View module Report *}
														{if $LISTVIEW_ENTRY_VALUE eq 'summary' || $LISTVIEW_ENTRY_VALUE eq 'tabular'}
															<span title="{vtranslate('LBL_DETAIL_REPORT', $MODULE)}">
																<span class='far fa-list-ul' style="font-size:13px;"></span>
																<span> {vtranslate('LBL_DETAIL_REPORT', $MODULE)}</span>
															</span>
														{elseif $LISTVIEW_ENTRY_VALUE eq 'chart'}
															<span title="{vtranslate('LBL_CHART_REPORT', $MODULE)}">
																<span class='far fa-pie-chart fa-2x' style="font-size:13px;"></span>
																<span> {vtranslate('LBL_CHARTS', $MODULE)}</span>
															</span>
														{/if}
														{* End Phu Vo *}

													{* Added by Hieu Nguyen on 2020-03-23 to render link for report name and owner name *}
													{else if $LISTVIEW_HEADERNAME eq 'reportname'}
														<a href="index.php?module=Reports&view={if $LISTVIEW_ENTRY->get('reporttype') == 'chart'}ChartDetail{else}Detail{/if}&record={$LISTVIEW_ENTRY->getId()}">{Vtiger_Util_Helper::tosafeHTML(decode_html($LISTVIEW_ENTRY_VALUE))}</a>
													{else if $LISTVIEW_HEADERNAME eq 'owner'}
														<a href="index.php?module=Users&parent=Settings&view=Detail&record={$LISTVIEW_ENTRY->get('owner_id')}" target="_blank">{Vtiger_Util_Helper::tosafeHTML(decode_html($LISTVIEW_ENTRY_VALUE))}</a>
													{* End Hieu Nguyen *}
								...
								<td colspan="{$COLSPAN_WIDTH}">
									<div class="emptyRecordsDiv">
										<div class="emptyRecordsContent">
											...
											{vtranslate('LBL_NO')} {vtranslate($MODULE, $MODULE)} {vtranslate('LBL_FOUND')}.{if $IS_MODULE_EDITABLE} <a href="{$MODULE_MODEL->getCreateRecordUrl()}"> {vtranslate('LBL_CREATE')} </a> {if Users_Privileges_Model::isPermitted($MODULE, 'Import') && $LIST_VIEW_MODEL->isImportEnabled()} {vtranslate('LBL_OR', $MODULE)} <a href="#" onclick="return Vtiger_Import_Js.triggerImportAction()"> {vtranslate('LBL_IMPORT', $MODULE)} </a>{vtranslate($MODULE, $MODULE)}{else}{vtranslate($SINGLE_MODULE, $MODULE)}{/if}{/if}
										</div>
									</div>
								</td>
							</tr>

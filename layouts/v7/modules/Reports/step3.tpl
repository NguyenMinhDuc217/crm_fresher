...
{strip}
    <form class="form-horizontal recordEditView" id="report_step3" method="post" action="index.php">
        ...
        <input type="hidden" name="set_limit" value="{$REPORT_MODEL->get('set_limit')}">
        <input type="hidden" name="sort_column" value="{$REPORT_MODEL->get('sort_column')}">
        <input type="hidden" name="sort_order" value="{$REPORT_MODEL->get('sort_order')}">
        <input type="hidden" name="limit_records" value="{$REPORT_MODEL->get('limit_records')}">
        {* End Hieu Nguyen *}
        ...
        {* Refactored HTML structure by Hieu Nguyen on 2022-03-24 *}
        <div>
            <div class="row-fluid">

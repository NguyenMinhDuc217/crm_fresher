...
<div class="reportContents">
    <form class="form-horizontal recordEditView" id="report_step1" method="post" action="index.php">
        ...
        <div class="padding20">
                    ...
                    <div class="col-lg-4">
                        <select class="select2-container select2 col-lg-12 inputElement" id="primary_module" name="primary_module" data-rule-required="true"
                            ...
                            {foreach key=RELATED_MODULE_KEY item=RELATED_MODULE from=$MODULELIST}
                                {* Added by Hieu Nguyen on 2021-08-18 to hide forbidden modules in this package and hidden module from Create Report form *}
                                {if empty($RECORD_ID) && (isForbiddenFeature($RELATED_MODULE_KEY) || isHiddenModule($RELATED_MODULE_KEY))}{continue}{/if}
                                {* End Hieu Nguyen *}
            ...
            <div class="row">
                <div class="form-group">
                    <label class="col-lg-3 control-label textAlignLeft">{vtranslate('LBL_SELECT_RELATED_MODULES', $MODULE, ['%max' => 2])}</label>
                    <div class="col-lg-4">
                        ...
                        <select class="select2 col-lg-12 inputElement" id="secondary_module" multiple name="secondary_modules[]" data-placeholder="{vtranslate('LBL_SELECT_RELATED_MODULES', $MODULE, ['%max' => 2])}"
            ...
            <div class='row'>
                <div class='form-group'>
                    ...
                    <div class='col-lg-4 share-report'>
                        {* Modified by Hieu Nguyen on 2019-06-19 to display members field in automplete mode *}
                        ...
                        <input type="text" autocomplete="off" class="inputElement select2" style="width: 100%" data-fieldtype="owner" data-fieldname="members" data-name="members" name="members" 
                            {if $SELECTED_MEMBERS_GROUP} data-selected-tags='{ZEND_JSON::encode(Vtiger_Owner_UIType::getSelectedOwnersFromOwnersString($SELECTED_MEMBERS_GROUP))}' {/if}
                        />
                        {* End Hieu Nguyen *}
                    </div>
                </div>
            ...
            {if $VIEW == 'Edit' && $CURRENT_USER->isAdminUser()} {* Only admin user can set the custom handler file *}
                <div class="row">
                    <div class="form-group">
                        <label class="col-lg-3 control-label textAlignLeft">{vtranslate('LBL_CUSTOM_HANDLER_FILE', $MODULE)}</label>
                        <div class="col-lg-4">
                            <input type="text" class="inputElement" data-rule-required="false" name="custom_handler_file" value="{$REPORT_MODEL->get('custom_handler_file')}"/>
                        </div>
                    </div>
                </div>
            {/if}

...
Reports_Edit_Js("Reports_Edit1_Js",{},{
	...
	submit : function(){
		...
		// Added by Hieu Nguyen on 2022-03-28 to validate form before submitting
		var customHandlerFileInput = form.find('[name="custom_handler_file"]');

		if (customHandlerFileInput[0] != null) {	// This field will not exist in Chart Report or when user is not Admin
			var filePath = customHandlerFileInput.val().trim();

			if (filePath != '') {
				var folderPath = filePath.substring(0, filePath.lastIndexOf('/'));

				if (folderPath != 'modules/Reports/custom') {
					app.helper.showErrorNotification({ 'message': app.vtranslate('Reports.JS_INVALID_CUSTOM_HANDLER_FILE_ERROR_MSG') });

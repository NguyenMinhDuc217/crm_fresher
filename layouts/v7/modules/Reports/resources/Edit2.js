...
Reports_Edit_Js("Reports_Edit2_Js",{},{
	...
	registerLimitRecords: function (container) {
		let setLimitInput = container.find('[name="set_limit"]');
		let limitWrapper = container.find('#limit-wrapper');
		let limitRecordsInput = container.find('[name="limit_records"]');
		let defaultSortRow = container.find('.row.default-sort');
		let selectedSortColumn = defaultSortRow.data('sortColumn') || 'none';
		let selectedSortOrder = defaultSortRow.data('sortOrder');

		// Init default sort column
		if (defaultSortRow.find('select[name="selectstep2dropdown_defaultsort"]')[0] != null) {
			defaultSortRow.find('select[name="selectstep2dropdown_defaultsort"]').attr('name', 'sort_column');
			defaultSortRow.find('select[name="sort_column"]').val(selectedSortColumn);

			defaultSortRow.find('select[name="sort_column"]').find('option').each(function () {
				let value = jQuery(this).val();
				let dataType = value.split(':').pop();

				if (value != 'none' && jQuery.inArray(dataType, ['DT', 'D', 'T', 'I', 'N']) < 0) {
					jQuery(this).remove();
				}
			});
		}

		// Init default sort order
		if (defaultSortRow.find('input[name="row_defaultsort"]')[0] != null) {
			defaultSortRow.find('input[name="row_defaultsort"]').attr('name', 'sort_order');
			defaultSortRow.find('input[name="sort_order"]:first').val('ASC');
			defaultSortRow.find('input[name="sort_order"]:last').val('DESC');

			if (selectedSortColumn != '' && selectedSortColumn != 'none') {
				defaultSortRow.find('input[name="sort_order"]').attr('disabled', false);	// Remove default disabled behavior
				defaultSortRow.find('input[name="sort_order"][value="'+ selectedSortOrder +'"]').attr('checked', true);
			}
		}

		// Handle toggle event
		setLimitInput.on('change', function () {
			if (jQuery(this).is(':checked')) {
				limitWrapper.show();
			}
			else {
				limitWrapper.hide();
				container.find('select[name="sort_column"]').val('none').trigger('change');
				limitRecordsInput.val('');
			}
		});
	},

	// Added by Hieu Nguyen on 2022-03-10
	registerGroupAndSortFields: function (container) {
		let fieldInputs = container.find('select.selectedSortFields');

		fieldInputs.on('change', function () {
			let sortOrderInputs = jQuery(this).closest('.sortFieldRow').find('input.sortOrder');

			if (jQuery(this).val() != 'none') {
				sortOrderInputs.attr('disabled', false);

				if (!jQuery(sortOrderInputs[0]).is(':checked') && !jQuery(sortOrderInputs[1]).is(':checked')) {	
					jQuery(sortOrderInputs[0]).attr('checked', true);
				}
			}
			else {
				sortOrderInputs.attr('checked', false).attr('disabled', true);
			}
		});
	},
	...
	calculateValues : function(){
		...
		jQuery.each(selectedSortFieldsRows,function(index,element){
			...
			// Added by Hieu Nguyen on 2022-03-23 to prevent getting default sort field as group&sort fields
			if (currentElement.find('select.selectedSortFields').attr('name') == 'sort_column') {
	...
	submit : function(){
		...
		// Added by Hieu Nguyen on 2022-03-24 to validate form before submitting
		let container = this.getContainer();

		if (container.find('[name="set_limit"]').is(':checked')) {
			let sortColumnInput = container.find('select[name="sort_column"]');
			let limitRecordsInput = container.find('[name="limit_records"]');

			if (sortColumnInput.val() == 'none') {
				app.helper.showErrorNotification({ 'message': app.vtranslate('Reports.JS_LIMIT_DEFAULT_SORT_COLUMN_EMPTY_ERROR_MSG') });
				return aDeferred.reject();
			}

			if (limitRecordsInput.val() <= 0) {
				app.helper.showErrorNotification({ 'message': app.vtranslate('Reports.JS_LIMIT_INVALID_NUMBER_OF_RECORDS_ERROR_MSG') });
				return aDeferred.reject();
			}
		}
		// End Hieu Nguyen
	...
	registerEvents : function(){
		...
		this.registerLimitRecords(container);		// Added by Hieu Nguyen on 2022-03-10
		this.registerGroupAndSortFields(container);	// Added by Hieu Nguyen on 2022-03-10
        ...
        this.arrangeSelectChoicesInOrder(); // Moved this line here by Hieu Nguyen on 2021-06-11 to fix bug selected columns not re-arranged when edit report
	}
});

...
Reports_Detail_Js("Reports_ChartDetail_Js", {
		...
		jQuery('.generateReportChart').on('click', function (e) {
			...
			app.request.post({data: postData}).then(
					function (error, data) {
						...
						// thisInstance.registerEventForChartGeneration();  // Commented out by Hieu Nguyen on 20221-06-02 to disable default chart rendering
						// jQuery('.reportActionButtons').addClass('hide'); // Commented out by Hieu Nguyen on 2020-04-09 to prevent hiding the report filter buttons
					}
			);
		});
	},
	...
	registerEventForPinChartToDashboard: function () {
        ...
        jQuery('button.pinToDashboard').closest('.btn-group').find('.dashBoardTab').on('click', function (e) {
            var dashBoardTabId = jQuery(this).data('tabId');
            var recordId = thisInstance.getRecordId();
	...
	registerEvents: function () {
		this._super();
		// this.registerEventForChartGeneration(); // Commented out by Hieu Nguyen on 20221-06-02 to disable default chart rendering

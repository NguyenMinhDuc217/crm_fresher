            ...
            <optgroup label='{vtranslate($PRIMARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$PRIMARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        {* Added by Hoang Duc 2020-05-08 to add date option for groupby*}
                        <option value="{$FIELD_KEY}:Ymd">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $PRIMARY_MODULE_NAME)})</option>
                        {* End Added by Hoang Duc 2020-05-08 to add date option for groupby*}
            ...
            <optgroup label='{vtranslate($SECONDARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$SECONDARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        {* Added by Hoang Duc 2020-05-08 to add date option for groupby*}
                        <option value="{$FIELD_KEY}:Ymd">{vtranslate($SECONDARY_MODULE_NAME, $SECONDARY_MODULE_NAME)} {vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $SECONDARY_MODULE_NAME)})</option>
                        {* End Added by Hoang Duc 2020-05-08 to add date option for groupby*}

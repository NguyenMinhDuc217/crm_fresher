...
{strip}
    <form class="form-horizontal recordEditView" id="report_step2" method="post" action="index.php">
        ...
        {* Added by Hieu Nguyen on 2018-12-08 *}
        <input type="hidden" name="custom_handler_file" value="{$REPORT_MODEL->get('custom_handler_file')}">
        {* End Hieu Nguyen *}

        <input type="hidden" class="step" value="2" />

        {* Modified HTML structure by Hieu Nguyen on 2022-03-24 to support limit records *}
        <div class="padding20">
            <!-- Display Columns -->
            <div class="form-group">
                <h4><strong>{vtranslate('LBL_CHOOSE_COLUMNS', $MODULE, ['%max' => 25])}</strong></h4>
                <div class="col-lg-12">
            ...
            <!-- Limit Records -->
            <div class="form-group">
                <h4><strong>{vtranslate('LBL_LIMIT_RECORDS', $MODULE)} <input type="checkbox" name="set_limit" {if $REPORT_MODEL->get('set_limit') == 1}checked{/if} /></strong></h4>
                <div id="limit-wrapper" class="col-lg-12" style="{if $REPORT_MODEL->get('set_limit') != 1}display: none;{/if}">
                    {vtranslate('LBL_LIMIT_DISPLAY_DEFAULT_SORT_COLUMN', $MODULE)} <i class="far fa-info-circle" title="{vtranslate('LBL_LIMIT_DISPLAY_DEFAULT_SORT_COLUMN_TOOLTIP', $MODULE)}" data-toggle="tooltip"></i>
                    <div class="row sortFieldRow default-sort" data-sort-column="{$REPORT_MODEL->get('sort_column')}" data-sort-order="{$REPORT_MODEL->get('sort_order')}">
                        {include file='RelatedFields.tpl'|@vtemplate_path:$MODULE ROW_VAL='defaultsort'}
                    </div>
                    <br>
                    {vtranslate('LBL_LIMIT_DISPLAY_NUMBER_OF_RECORDS', $MODULE)}&nbsp;&nbsp;
                    <input type="number" name="limit_records" class="inputElement" value="{$REPORT_MODEL->get('limit_records')}" style="width: 80px;" />
                </div>
            ...
            <!-- Grouping -->
            <div class="form-group">
                <h4><strong>{vtranslate('LBL_GROUPING', $MODULE)}</strong></h4>
                <div class="col-lg-12">
                    <div class="row-fluid">
                        <label class="col-lg-3">{vtranslate('LBL_GROUP_BY',$MODULE)}</label>
                        <label class="col-lg-9">{vtranslate('LBL_SORT_ORDER',$MODULE)}</label>
                    </div>
                    <div class="row-fluid">
            ...
            <div class="row block padding1per">
                <h4><strong>{vtranslate('LBL_CALCULATIONS', $MODULE)}</strong></h4>
                <div class="col-lg-12">

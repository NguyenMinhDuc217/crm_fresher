...
{strip}
    ...
    <span class="col-lg-9">
        <div class="row">
            <span class="col-lg-6 sort-direction">
                {assign var=ROW value='row_'|cat:$ROW_VAL}
                
                {* Added wrapper label tag by Hieu Nguyen on 2022-10-24 to make the radio button easier to select *}
                <label class="cursorPointer"><input type="radio" name="{$ROW}" class="sortOrder" value="Ascending" {if $SELECTED_SORT_FIELD_VALUE eq 'Ascending'}checked{/if} {if !$SELECTED_SORT_FIELD_KEY || $SELECTED_SORT_FIELD_KEY eq 'none'}disabled{/if} />&nbsp;<span>{vtranslate('LBL_ASCENDING',$MODULE)}</span></lable>&nbsp;&nbsp;
                <label class="cursorPointer"><input type="radio" name="{$ROW}" class="sortOrder" value="Descending" {if $SELECTED_SORT_FIELD_VALUE eq 'Descending'}checked{/if} {if !$SELECTED_SORT_FIELD_KEY || $SELECTED_SORT_FIELD_KEY eq 'none'}disabled{/if} />&nbsp;<span>{vtranslate('LBL_DESCENDING',$MODULE)}</span></label>
                {* End Hieu Nguyen *}
            </span>
        </div>
    </span>

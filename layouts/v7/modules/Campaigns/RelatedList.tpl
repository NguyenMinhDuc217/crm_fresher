							...
							<div class="btn-group">
								...
								{else}
									...
									<button type="button" module="{$RELATED_MODULE_NAME}"  class="btn addButton btn-default
										...
										{if $IS_SEND_EMAIL_BUTTON eq true} disabled="disabled" {/if}>{if ($IS_SELECT_BUTTON neq true) && ($IS_SEND_EMAIL_BUTTON neq true)}<i class="far fa-plus"></i>{/if}&nbsp;&nbsp;{$RELATED_LINK->getLabel()}</button>
								{/if}
							</div>
						{/foreach}&nbsp;
					</div>
					<div class='col-lg-4 col-md-4 col-sm-4'>
						{* Comment out by Phu Vo on 2019.10.25 to prevent add Customer using this filter
					...
					<table id="listview-table"  class="table listview-table">
						<thead>
							<tr class="listViewHeaders">
								{* Comment out by Vu mai on 2022-12-28 to remove checkbox is not used *}
							...
							<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'>
								...
								<td style="width:100px">
									<span class="actionImages">
										<a name="relationEdit" data-url="{$RELATED_RECORD->getEditViewUrl()}" href="javascript:void(0)"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen"></i></a> &nbsp;&nbsp;
								...
								{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
									...
									<td class="{$WIDTHTYPE} relatedListEntryValues" data-field-name="{$RELATED_HEADERNAME}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>   {* Modified by Hieu Nguyen on 2020-01-16 to show field name attribue *}
								...
								<td class="{$WIDTHTYPE}" nowrap>
									<span class="currentStatus more dropdown action">
										...
										<a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-arrow-down alignMiddle editRelatedStatus"></i></a>

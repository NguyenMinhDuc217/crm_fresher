...
* All Rights Reserved.
*************************************************************************************}
{* -- Modified by Kelvin Thang on 2020-06-19 -- Fixed wrong path summary content, change UI column and view widget-- *}
{strip}
    ...
    <div class="left-block col-lg-4">
        ...
        {if $DOCUMENT_WIDGET_MODEL}
            ...
            {* Modified by Phu Vo on 2021.11.19 to fix error on summary view document widget *}
                            ...
                            <div class="pull-right">
                                <div class="dropdown">
                                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                                        <span class="far fa-plus" title="{vtranslate('LBL_NEW_DOCUMENT', $MODULE_NAME)}"></span>&nbsp;{vtranslate('LBL_NEW_DOCUMENT', 'Documents')}&nbsp; <i class="far fa-angle-down"></i>
                                    </button>
                                    <ul class="dropdown-menu">
                                        <li class="dropdown-header"><i class="far fa-upload"></i> {vtranslate('LBL_FILE_UPLOAD', 'Documents')}</li>
                                        <li id="VtigerAction">
                                        ...
                                        <li class="dropdown-header"><i class="far fa-link"></i> {vtranslate('LBL_LINK_EXTERNAL_DOCUMENT', 'Documents')}</li>
                                        <li id="shareDocument"><a href="javascript:Documents_Index_Js.createDocument('E',{$PARENT_ID},'{$MODULE_NAME}')"><i class="far fa-external-link"></i>{vtranslate('LBL_FROM_SERVICE', 'Documents', {vtranslate('LBL_FILE_URL', 'Documents')})}</a></li>
                                        <li role="separator" class="divider"></li>
                                        <li id="createDocument"><a href="javascript:Documents_Index_Js.createDocument('W',{$PARENT_ID},'{$MODULE_NAME}')"><i class="far fa-file-text"></i>{vtranslate('LBL_CREATE_NEW', 'Documents', {vtranslate('SINGLE_Documents', 'Documents')})}</a></li>
                                    </ul>
                                </div>
                            </div>
                        {/if}
    ...
    <div class="middle-block col-lg-8">
        {* Added by Phu Vo on 2020.11.17 *}
        {* Modified by Hieu Nguyen on 2022-06-28 to show campaign log *}
        {if Campaigns_Logic_Helper::hasCampaignLog($RECORD->get('campaigntype'))}
            {if $EMAIL_LOGS_WIDGET_MODEL}
                {include file="modules/Campaigns/tpls/EmailLogsWidget.tpl"}
            {/if}

            {if $MESSAGE_STATISTICS_WIDGET_MODEL}
                {include file="modules/Campaigns/tpls/MessageStatisticsWidget.tpl"}
            {/if}
        {/if}
        {* End Hieu Nguyen *}
        {* End Phu Vo *}

        {* Added by Vu Mai on 2022-11-30 to show campaign summary when campaigntype is telesales *}
        {if $RECORD->get('campaigntype') == 'Telesales'}
            <div id="telesales-campaign-summary">
                {$TELESALES_CAMPAIGN_SUMMARY}
            </div> 
        {/if}
        {* End Vu Mai *}

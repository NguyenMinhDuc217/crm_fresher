        ...
        <div id="convertLeadContainer" class='modelContainer modal-content'>
            ...
            {else}
                ...
                <form class="form-horizontal" id="convertLeadForm" method="post" action="index.php">
                    ...
                    {* Added by Phu Vo on 2019.06.27 add field main owner id as hidden field *}
                    <input type="hidden" name="main_owner_id" value="{$RECORD->get('main_owner_id')}">
                    {* End add field main owner id as hidden field *}
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-10 moduleContent" style="border:1px solid #CCC;">
                                    <div class="accordion-group convertLeadModules">
                                        <input type="hidden" class="picklistDependency"value='{Vtiger_Util_Helper::toSafeHTML($CONVERT_LEAD_DEPENDENCIES_LIST[$MODULE_NAME])}'/> {* Added by Phuc on 2020.06.18 to add dependencies list in background *}
                                        <div class="header accordion-heading">
                                            ...
                                            <div id="{$MODULE_NAME}_FieldInfo" class="{$MODULE_NAME}_FieldInfo accordion-body collapse fieldInfo {if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts"} in {elseif !$CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Contacts"} in {/if}">
                                                ...
                                                {foreach item=FIELD_MODEL from=$MODULE_FIELD_MODEL}
                                                    <div class="row">
                                                        <div class="fieldLabel {$FIELD_MODEL->getName()} col-lg-4"> {* Modified by Phu Vo on 2019.06.27 add field name *}
                                                            <label class='muted pull-right'>
                                                                {vtranslate($FIELD_MODEL->get('label'), $MODULE_NAME)}&nbsp;
                                                                {if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if} 
                                                            </label>
                                                        </div>
                                                        <div class="fieldValue {$FIELD_MODEL->getName()} col-lg-8"> {* Modified by Phu Vo on 2019.06.27 add field name *}

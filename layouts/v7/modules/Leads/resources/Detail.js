...
Vtiger_Detail_Js("Leads_Detail_Js", {
    ...
    // Modified by Phu Vo on 2021.03.27 to support use convert lead logic in another screen
	getInstance: function(){
		if (Leads_Detail_Js.detailInstance == false) {
			instance = new window.Leads_Detail_Js();
			Leads_Detail_Js.detailInstance = instance;
		}

		return Leads_Detail_Js.detailInstance;
	},

    /* function to trigger Convert Lead action
     * @param: Convert Lead url, currentElement.
     */
    convertLead: function (convertLeadUrl, callback = null) {
        var instance = Leads_Detail_Js.detailCurrentInstance || Leads_Detail_Js.getInstance();
        ...
        if (jQuery.isEmptyObject(Leads_Detail_Js.cache)) {
            ...
            app.request.get({url: convertLeadUrl}).then(function (err, data) {
                app.helper.hideProgress();

                if (data) {
                    Leads_Detail_Js.cache = data;
                    instance.displayConvertLeadModel(data, callback);
                }
            });
        } else {
            instance.displayConvertLeadModel(Leads_Detail_Js.cache, callback);
        }
    }
    // End Phu Vo
     ...
     */
    displayConvertLeadModel: function (data, callback = null) { // Modified by Phu Vo on 2021.03.27 to support callback
        var instance = this;
        ...
        if (errorElement.length != '0') {
            ...
            var callBackFunction = function (data) {
                ...
                if (typeof callback == 'function') callback(data); // Modified by Phu Vo on 2021.03.27 to trigger callback

                // Added by Phu Vo on 2019.06.27 to trigger post event
                app.event.trigger('post.convertLeadModal.load', jQuery(data));
                // End trigger post event
            }
    ...
    checkingModuleSelection: function (element) {
        ...
        // Custom by Phuc on 2019.10.25 to display panels which have checked box
        var moduleHeaderBlock = jQuery(element).parent().parent();

        if (jQuery(element).is(':checked')) {
            instance.removeDisableAttr(moduleBlock);
            
            jQuery(moduleBlock).addClass('in');
            jQuery(moduleBlock).attr('aria-expanded', true);
            
            jQuery(moduleHeaderBlock).removeClass('collapsed');
            jQuery(moduleHeaderBlock).attr('aria-expanded', true);
        } else {
            instance.addDisableAttr(moduleBlock);

            jQuery(moduleBlock).removeClass('in');
            jQuery(moduleBlock).attr('aria-expanded', false);

            jQuery(moduleHeaderBlock).addClass('collapsed');
            jQuery(moduleHeaderBlock).attr('aria-expanded', false);
        }
        // Ended by Phuc
    },

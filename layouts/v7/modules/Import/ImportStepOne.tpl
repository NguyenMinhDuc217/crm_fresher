...
********************************************************************************/
-->*}
<!-- Modified By Kelvin Thang -- Date: 2018-06-28-->
<div class ="importBlockContainer show" id = "uploadFileContainer">
    <table class = "table table-borderless" cellpadding = "30" >
        <span>
			...
			{else}
				<h4>{'LBL_IMPORT_FROM_XLSX_FILE'|@vtranslate:$MODULE} </h4>
			{/if}

            {* Modified by Hieu Nguyen on 2021-09-07 to display url to download template file *}
            <div class="pull-right" style="margin-top: -30px">
                <span class="redColor"><i class="far fa-info-circle"/> {vtranslate('LBL_IMPORT_FILE_HINT', 'Import')}</span>
                {if !in_array($FORMAT, ['vcf', 'ics']) && $TEMPLATE_FILE}
                    <a href="{getGlobalVariable('site_URL')}/{$TEMPLATE_FILE}">{vtranslate('LBL_DOWNLOAD_IMPORT_TEMPLATE_FILE', 'Import')} <i class="far fa-download"/></a>
                {/if}
            </div>
            {* End Hieu Nguyen *}
        </span>
        ...
        <tr id="file_type_container" style="height:50px">
			...
			{else}
				<td>{'LBL_SELECT_XLSX_FILE'|@vtranslate:$MODULE}</td>
			{/if}
            <td data-import-upload-size="{$IMPORT_UPLOAD_SIZE}" data-import-upload-size-mb="{$IMPORT_UPLOAD_SIZE_MB}">
                <div>
                    ...
                    <input type="hidden" id="type" name="type" value="{if $FORMAT eq ''}xlsx{else}{$FORMAT}{/if}" />

                    <input type="hidden" name="is_scheduled" value="1" />
                    <div class="fileUploadBtn btn btn-primary">
                        <span><i class="far fa-laptop"></i> {vtranslate('Select from My Computer', $MODULE)}</span>
        ...
        {* Added by Hieu Nguyen on 2021-05-11 to determine excel file *}
        {assign var="IS_EXCEL_FILE" value=(strpos($FORMAT, 'xls') !== false || strpos($FORMAT, 'xlsx') !== false)}
        {* End Hieu Nguyen *}

        <!-- Modified By Kelvin Thang -- Date: 2018-06-28-->
        {if $FORMAT eq 'csv' || $IS_EXCEL_FILE} {* Modified condition by Hieu Nguyen on 2021-05-11 *}
        ...
        <!-- Modified By Kelvin Thang -- Date: 2018-06-28-->
        {if $FORMAT eq 'csv' || $IS_EXCEL_FILE} {* Modified condition by Hieu Nguyen on 2021-05-11 *}
            <tr id="delimiter_container" style="height:50px; {if $IS_EXCEL_FILE} display: none; {/if}">

...
if (typeof (Vtiger_Import_Js) == 'undefined') {
    ...
    Vtiger_Import_Js = {
            ...
            if (Vtiger_Import_Js.sanitizeFieldMapping() && Vtiger_Import_Js.validateCustomMap()) {
                ...
                app.request.post({data: formData}).then(function(err, response) {
                    ...
                    if(!err){
                        ...
                        // Modified by Hieu Nguyen on 2020-09-10 to display the right message in the right situation
                        else if (jQuery('#scheduleImportMsg').length == 0 && jQuery(response).find('.app-exception').length == 0) {
                            app.helper.showSuccessNotification({ message: app.vtranslate('Import.JS_IMPORT_COMPLETED_MSG') });
                        }
                        // End Hieu Nguyen
                    }
                });
        ...
        showPopup: function(url) {
            ...
            // Modified by Hieu Nguyen on 2018-01-10 to fix bug Import result popup disappear when click on row
            popupInstance.showPopup(params, '', function() {
                $('#popupPageContainer:visible').find('.listViewEntries').removeClass('listViewEntries');
            });
            // End Hieu Nguyen
        ...
        handleFileTypeChange: function() {
            ...
            /*var fileTypeSupported = ['csv', 'xlsx', 'xls'];*/
        ...
        uploadFilter: function(elementId, allowedExtensions) {
            ...
            if (obj) {
                ...
                // Modified by Hieu Nguyen on 2021-05-11 to change delimeter format from verticle slash to comma
                var validExtensions = allowedExtensions.toLowerCase().split(',');
                // End Hieu Nguyen

                /*
                if(fileType == 'csv'){
                    jQuery('#import_file').attr("data-file-formats","csv");
        ...
        // Added by Hieu Nguyen on 2020-09-10 to mark import process as finished
        finishImport: function (importId, callback) {
            var params = {
				module: app.getModuleName(),
                view: 'Import',
                mode: 'finishImport',
        ...
        showImportActionStepOne: function(format) {
            ...
            app.helper.showProgress();
            app.request.post({data: params}).then(function(err, data) { // Modified by Hieu Nguyen on 2021-03-18 to use POST instead of GET for import action
                app.helper.loadPageContentOverlay(data);
                app.helper.hideProgress();
				if (jQuery('#scheduleImportStatus').length > 0) {
					app.event.one('post.overlayPageContent.hide', function(container) {

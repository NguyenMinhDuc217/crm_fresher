    ...
    <table class = "table table-borderless" id="duplicates_merge_configuration">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <select id="selected_merge_fields" size="10" name="selected_merge_fields" multiple class="txtBox" style="width: 100%">
                                {foreach key=_FIELD_NAME item=_FIELD_INFO from=$DUPLICATE_CHECK_FIELDS} {* Refactored variable name by Hieu Nguyen on 2021-05-11 *}
                                    <option value="{$_FIELD_NAME}">{$_FIELD_INFO->getFieldLabelKey()|@vtranslate:$FOR_MODULE}</option>
                                {/foreach}
                            </select>
                        </td>
        ...
        <tr>
            <td>
                ...
                {* Modified by Hieu Nguyen on 2021-05-10 to to warn user about skipping this step for Accounts *}
                {if $FOR_MODULE == 'Accounts'}
                    <br/>
                    <div class="redColor"><i class="far fa-exclamation-triangle"></i> {vtranslate('LBL_IMPORT_STEP2_SKIP_WARNING_FOR_ACCOUNTS', $MODULE)}</div>
                {/if}
                {* End Hieu Nguyen *}
            </td>
        </tr>

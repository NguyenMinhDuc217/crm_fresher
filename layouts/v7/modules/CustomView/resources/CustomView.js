...
jQuery.Class("Vtiger_CustomView_Js",{
			...
			function(error,data){
               if(error === null){
				...
				aDeferred.resolve(data);
					...
					app.helper.hideProgress();
					aDeferred.reject();
					app.helper.showErrorNotification({ 'message': error.message });	// Modified by Hieu Nguyen on 2021-09-01 to display error message return from server
				}
			}
		);
		return aDeferred.promise();
	...
	registerFilterCreateEvents : function() {
        ...
        // Modified by Hieu Nguyen on 2020-02-28
		this.registerToggleShareList();
		//this.registerOnlyAllUsersInSharedList();    // Commented out this line as new UI does not have this case
        this.initSharingList();
        // End Hieu Nguyen
			...
			customViewForm.vtValidate({
				submitHandler : function(form){
					var form = jQuery(form); 
						  ...
						  var selectedOptions = selectElement.val() || []; // #2633: Modified by Phu Vo on 2021.07.12 to fix null value issue
	...
	registerToggleShareList : function() {
		jQuery('[data-toogle-members]').on('change',function (e) {
			if($(this).is(':checked')) {
				jQuery('#sharing-list').addClass('fadeInx');                
			}
			else {
				jQuery('#sharing-list').removeClass('fadeInx');
			}
		});
	},
    // End Hieu Nguyen

    // Added by Hieu Nguyen on 2020-02-28 to render sharing list with new UI
    initSharingList: function () {
        var formContent = jQuery('.customview-content');

        // Handle shared type toggle
        formContent.find('[name="shared_type"]').on('change', function (e) {
			if ($(this).val() == 'selected_users') {
				formContent.find('#selected-users').addClass('fadeInx');
                formContent.find('[name="selected_users"]').select2('open');                
			}
			else {
				formContent.find('#selected-users').removeClass('fadeInx');
			}
		});

        // Init selected users input as select2
        CustomOwnerField.initCustomOwnerFields(formContent.find('[name="selected_users"]'));
    },
    // End Hieu Nguyen
	...
	registerEvents : function() {
		...
		jQuery(document).on('post.CreateFilter.click',function(e,params){
			self.doOperation(params.url).then(function(data){
                ...
                // Added by Hieu Nguyen on 2019-06-27 to trigger event post.advancedFilter.load
                app.event.trigger('post.listViewFilter.load', form);
                // End Hieu Nguyen
			})
		});
	}

		...
		<form id="CustomView" style="height:100%">
			<div class="modal-content" style="height:100%">
				...
				<div class="modal-body" style="height:100%">
					<div class="customview-content row" style="height:100%">
						...
						<div class="form-group">
							<label>{vtranslate('LBL_CHOOSE_COLUMNS', $MODULE, ['%max' => 15])}</label>
                        ...
                        {* Modified by Hieu Nguyen to make share users list as autocomplete field to boost performance *}
                        ...
                        <div id="sharing-list" class="row op0 {if $LIST_SHARED}fadeInx{/if}" style="padding-left: 30px">
                            {assign var="SHARED_TYPE" value=$CUSTOMVIEW_MODEL->get('shared_type')}

                            <div>
                                <label><input type="radio" name="shared_type" value="all_users" {if !$SHARED_TYPE || $SHARED_TYPE == 'all_users'}checked{/if}> {vtranslate('LBL_SHARE_TO_ALL_USERS', 'CustomView')}</label>&nbsp;&nbsp;
                                <label><input type="radio" name="shared_type" value="selected_users" {if $SHARED_TYPE == 'selected_users'}checked{/if}> {vtranslate('LBL_SHARE_TO_SELECTED_USERS', 'CustomView')}</label>
                            </div>
                            <div id="selected-users" class="op0 {if $SHARED_TYPE == 'selected_users'}fadeInx{/if}" style="margin-top: 5px">
                                <input type="text" autocomplete="off" class="inputElement select2" style="width: 80%" data-fieldtype="owner" data-fieldname="selected_users" data-name="selected_users" name="selected_users" 
                                    {if $SHARED_TYPE == 'selected_users' && $SELECTED_USERS} data-selected-tags='{ZEND_JSON::encode(Vtiger_Owner_UIType::getCurrentOwners(join(',', $SELECTED_USERS)))}' {/if}

...
{assign var=FINAL_DETAILS value=$RELATED_PRODUCTS.1.1.final_details}
...
<div class="details block">
    ...
    <div class="lineItemTableDiv">
        ...
        <div class=" header">
            <div class="col-lg-12">
                ...
                <div class="lineItemBlockHeader pull-right" style=" margin-right: 16px;">
                    {assign var=CURRENCY_INFO value=$RECORD->getCurrencyInfo()}
                    <strong>{vtranslate('LBL_CURRENCY', $MODULE_NAME)} :</strong> {vtranslate($CURRENCY_INFO['currency_name'],$MODULE_NAME)}({$CURRENCY_INFO['currency_symbol']})
                </div>

                <div class="lineItemBlockHeader pull-right " style=" margin-right: 16px;">
                    <strong>{vtranslate('LBL_TAX_MODE', $MODULE_NAME)} :</strong> {vtranslate($FINAL_DETAILS.taxtype, $MODULE_NAME)}
                </div>
        ...
        <div class="blockData">
            ...
            <!-- Added By Kelvin Thang -- OnlineCRM -- Date: 2018-09-17 -- change construct display -->
            {foreach key=section_no item=RELATED_PRODUCTS_SECTION from=$RELATED_PRODUCTS}
                <div class="lineItemsTable">
                    <table class="table table-bordered " width="100%">
                        <tbody>
                            {if count($RELATED_PRODUCTS) neq 1}
                                <tr>
                                    <td colspan="5">
                                        {assign var=sectionNumTabName value=array_column($RELATED_PRODUCTS_SECTION, 'sectionNumTabName')}
                                        <strong><span>{vtranslate('LBL_SECTION_NAME',$MODULE)}</span></strong> :
                                        <strong><span>{$sectionNumTabName.0}</span></strong>
                            ...
                            <tr>
                                {if $IMAGE_VIEWABLE}
                                    <td class="lineItemFieldName" width="10%">
                                        <strong>{vtranslate({$LINEITEM_FIELDS['image']->get('label')},$MODULE)}</strong>
                                    </td>
                                {/if}
                                {if $PRODUCT_VIEWABLE}
                                    <td class="lineItemFieldName" width="25%">
                                        <strong>{vtranslate({$LINEITEM_FIELDS['productid']->get('label')},$MODULE_NAME)}</strong>
                                    </td>
                                {/if}
                            ...
                            {foreach key=INDEX item=LINE_ITEM_DETAIL from=$RELATED_PRODUCTS_SECTION}
                                <tr>
                                    {if $IMAGE_VIEWABLE}
                                            ...
                                            {if $LINE_ITEM_DETAIL["productDeleted$INDEX"]}
                                                <div class="redColor deletedItem">
                                                    ...
                                                    {else}
                                                        ...
                                                        {$replaceParams = ['%product_type' => vtranslate($LINE_ITEM_DETAIL["entityType$INDEX"], $LINE_ITEM_DETAIL["entityType$INDEX"])]}
                                    ...
                                    {*{/if}*} {*-- Comment out by Kelvin Thang on 2020-29-02 --@To: Hide TOTAL HEADERS when field list price is hidden --*}
                                        ...
                                        <td>
                                            ...
                                            {/if}
                                            <div align = "right" class="individual-discount-value">{$LINE_ITEM_DETAIL["totalAfterDiscount$INDEX"]}</div>
                                            {if $FINAL_DETAILS.taxtype neq 'group'}
    ...
    <table class="table table-bordered lineItemsTable">
        ...
        <tr>
            <td width="83%">
                <div align="right" class="fieldTotal">
        ...
        <!-- Comment by Vu Mai on 2023-02-14 to hide charge tax field-->
        <!-- {if $SH_PERCENT_VIEWABLE}
            <tr>
                <td width="83%">
                    <div align="right">
        ...
        <!-- Removed By Kelvin Thang -- OnlineCRM -- Date: 2018-07-23-->
        <!--
        <tr>
            <td width="83%">
                <div align="right">
        ...
        <tr>
            <td width="83%">
                <div align="right">
                    <!-- Modified By Kelvin Thang -- OnlineCRM -- Date: 2018-07-23-->
                    <!--<strong>{vtranslate('LBL_ADJUSTMENT',$MODULE_NAME)}</strong>-->
                    {vtranslate('LBL_ADJUSTMENT',$MODULE_NAME)}
                </div>
            </td>
            <td>
                <div align="right">
        ...
        {* Remove module invoice $MODULE eq 'Purchase Order' by Phuc on 2019.10.07*}

...
Vtiger_Edit_Js("Inventory_Edit_Js", {
    ...
    /*-- Modified By Kelvin Thang -- Date: 2018-09-17*/
    ...
    getBrowserName: function() {
        ...
        var g = typeof InstallTrigger !== "undefined";
        var c = /constructor/i.test(window.HTMLElement) || (function(h) {
                return h.toString() === "[object SafariRemoteNotification]";
            })(!window["safari"] || safari.pushNotification);
        var d = (false || !!document.documentMode);
        var e = !d && !!window.StyleMedia;
        var f = !!window.chrome && !!window.chrome.webstore;
        var a = (f || b) && !!window.CSS;
        return b ? "Opera" : g ? "Firefox" : c ? "Safari" : f ? "Chrome" : d ? "IE" : e ? "Edge" : "Don't know";
    },
    ...
    initializeVariables : function() { 
        ...
        this.lineItemsHolder = jQuery('.lineitemTableContainer');
	...
	getLineItemContentsContainer : function() {
		if(this.lineItemContentsContainer == false) {
			...
			this.setLineItemContainer(jQuery('.lineitemTableContainer'));
		}
    ...
    formatListPrice : function(lineItemRow, listPriceValue) { 
         ...
         * Date: 2018-07-16
         * convert data Currency To User Format
         */
        listPrice = app.convertCurrencyToUserFormat(listPrice, false);
	...
	getQuantityValue : function(lineItemRow){
        var qty = app.unformatCurrencyToUser(lineItemRow.find('.qty').val());
	...
	getListPriceValue : function(lineItemRow) {
         ...
         * unformat data Currency To User Format
         */
        var listPrice = app.unformatCurrencyToUser(lineItemRow.find('.listPrice').val());
		return parseFloat(listPrice);
	},
	...
	getLineItemTotal : function(lineItemRow) {
        ...
        if(lineItemTotal){
             ...
             * unformat data Currency To User Format
             */
            lineItemTotal = app.unformatCurrencyToUser(lineItemTotal);
	...
	getDiscountTotal : function(lineItemRow) {
		...
		if(element.length > 0) {
             ...
             * unformat data Currency To User Format
             */
            var discountTotal = app.unformatCurrencyToUser(element.text());
	...
	getTotalAfterDiscount : function(lineItemRow) {
		...
		if(element.length > 0) {
             ...
             * unformat data Currency To User Format
             */
            var totalAfterDiscount = app.unformatCurrencyToUser(element.text());
	...
	setLineItemTaxTotal : function(lineItemRow, taxTotal) {
		taxTotal = app.convertCurrencyToUserFormat(taxTotal);
	...
	getLineItemTaxTotal : function(lineItemRow){
        ...
        if(lineItemTax){
             ...
             * unformat data Currency To User Format
             */
            lineItemTax = app.unformatCurrencyToUser(lineItemTax);
    ...
    formatLineItemNetPrice : function(netPriceEle) {
        ...
        if(lineItemNetPrice){
             ...
             * unformat data Currency To User Format
             */
            lineItemNetPrice = app.unformatCurrencyToUser(lineItemNetPrice);
	...
	getNetTotal : function() {
        ...
        if(netTotal){
             ...
             * unformat data Currency To User Format
             */
            netTotal = app.unformatCurrencyToUser(netTotal);
	...
	getFinalDiscountTotal : function() {
		...
		if(discountTotal){
             ...
             * unformat data Currency To User Format
             */
            discountTotal = app.unformatCurrencyToUser(discountTotal);
	...
	getGroupTaxTotal : function() {
        ...
        if(groupTax){
             ...
             * unformat data Currency To User Format
             */
            groupTax = app.unformatCurrencyToUser(groupTax);
    ...
    getChargesTotal : function() {
         ...
         * unformat data Currency To User Format
         */
        var chargesTotal = app.unformatCurrencyToUser(chargesElement.val());
        return parseFloat(chargesTotal);
	},
    ...
    getChargeTaxesTotal : function() {
         ...
         * unformat data Currency To User Format
         */
        var chargeTaxesTotal = app.unformatCurrencyToUser(taxElement.val());
        return parseFloat(chargeTaxesTotal);
	},
    ...
    getDeductTaxesTotal : function() {
         ...
         * unformat data Currency To User Format
         */
        var deductTaxesTotal = app.unformatCurrencyToUser(taxElement.val());
        return parseFloat(deductTaxesTotal);
	},
	...
	getPreTaxTotal : function() {
		if(this.preTaxTotalEle.length > 0){
             ...
             * unformat data Currency To User Format
             */
            var preTaxTotal = app.unformatCurrencyToUser(this.preTaxTotalEle.text());
            return parseFloat(preTaxTotal);
        }
	},
    ...
    getAdjustmentValue : function() {
         ...
         * unformat data Currency To User Format
         */
        var adjustmentValue = app.unformatCurrencyToUser(this.adjustmentEle.val());
	...
	getGrandTotal : function() {
        ...
        if(grandTotal){
             ...
             * unformat data Currency To User Format
             */
            grandTotal = app.unformatCurrencyToUser(grandTotal);
	...
	getClosestLineItemRow : function(element){
        ...
        var a = element.closest('tr.'+this.lineItemDetectingClass);
        return a;
	},
    ...
    updateRowNumberForRow : function(lineItemRow, expectedSequenceNumber, currentSequenceNumber){ 
        ...
        /*-- Added By Kelvin Thang -- Date: 2018-09-17 -- update for section*/
        lineItemRow.find('[name="section_' + currentSequenceNumber + '"]').attr("name", "section_" + expectedSequenceNumber);
	...
	calculateLineItemTotal : function (lineItemRow) { 
         ...
         * convert data Currency To User Format
         */
        lineItemTotal = app.convertCurrencyToUserFormat(lineItemTotal);
	...
	calculateDiscountForLineItem : function(lineItemRow) { 
		...
		var discountValue = app.unformatCurrencyToUser(discountRow.find('.discountVal').val());
        ...
        var productTotal = app.unformatCurrencyToUser( this.getLineItemTotal(lineItemRow) );
         ...
         * convert data Currency To User Format
         */
		var lineItemDiscount = '('+app.convertCurrencyToUserFormat(discountValue, false)+')';
         ...
         * convert data Currency To User Format
         */
		jQuery('.productTotalVal', lineItemRow).text(app.convertCurrencyToUserFormat(productTotal, false));
		this.setDiscountTotal(lineItemRow, app.convertCurrencyToUserFormat(discountValue, false))
			.calculateTotalAfterDiscount(lineItemRow);
    },
	...
	calculateTotalAfterDiscount: function(lineItemRow) { 
         ...
         * convert data Currency To User Format
         */
        totalAfterDiscount = app.convertCurrencyToUserFormat(totalAfterDiscount,false)
        margin = app.convertCurrencyToUserFormat(margin,false)
	...
	calculateTaxForLineItem : function(lineItemRow) { 
		...
		jQuery.each(taxPercentages,function(index,domElement){
             ...
             * convert data Currency To User Format
             */
			individualTaxRow.find('.taxTotal').val(app.convertCurrencyToUserFormat(individualTaxTotal, false));
		});
        ...
        jQuery.each(taxPercentages, function(index, domElement){
             ...
             * unformat data Currency To User Format
             */
			var total = app.unformatCurrencyToUser(jQuery('.taxTotal', taxRow).val());
			...
			if (compoundOn) {
				...
				jQuery.each(compoundOn, function(index, id) {
					if(!isNaN(jQuery('.taxTotal'+id, lineItemRow).val())) {
                         ...
                         * unformat data Currency To User Format
                         */
						amount = parseFloat(amount) + parseFloat(app.unformatCurrencyToUser(jQuery('.taxTotal'+id, lineItemRow).val()));
					}
				});
                 ...
                 * convert data Currency To User Format
                 */
				taxRow.find('.taxTotal').val(app.convertCurrencyToUserFormat(total, false));
			}
         ...
         * unformat data Currency To User Format
         */
		/*app.convertCurrencyToUserFormat(taxTotal, false)*/
	...
	calculateLineItemNetPrice : function(lineItemRow) {
         ...
         * convert data Currency To User Format
         */
        netPrice = app.convertCurrencyToUserFormat(netPrice, false);
	...
	calculateNetTotal : function() {
        ...
        netTotalValue = app.convertCurrencyToUserFormat(netTotalValue, false);
    ...
    calculateFinalDiscount : function() { 
         ...
         * unformat data Currency To User Format
         */
		discountValue = app.unformatCurrencyToUser(discountValue);
         ...
         * convert data Currency To User Format
         */
		var overallDiscount = '('+app.convertCurrencyToUserFormat(discountValue, false)+')';
		if(discountType == Inventory_Edit_Js.percentageDiscountType){
            ...
            if(this.prevSelectedCurrencyConversionRate){
                 ...
                 * unformat data Currency To User Format
                 */
                var conversionRate = app.unformatCurrencyToUser(this.conversionRateEle.val());
	...
	calculatePreTaxTotal : function() {
         ...
         * convert data Currency To User Format
         */
        preTaxTotal = app.convertCurrencyToUserFormat(preTaxTotal, false);
		this.setPreTaxTotal(preTaxTotal);
	},
    ...
    calculateCharges : function() {
		...
		chargesBlockContainer.find('.chargePercent').each(function(index, domElement){
             ...
             * convert data Currency To User Format
             */
			element.closest('tr').find('.chargeValue').val(app.convertCurrencyToUserFormat(value, false));
		});
		...
		chargesBlockContainer.find('.chargeValue').each(function(index, domElement){
             ...
             * unformat data Currency To User Format
             */
			var chargeElementValue = app.unformatCurrencyToUser(jQuery(domElement).val());
         ...
         * convert data Currency To User Format
         */
        chargesTotal = app.convertCurrencyToUserFormat(chargesTotal, false);
        ...
        */
        ...
        this.chargesTotalEle.val(chargesTotal);
        this.chargesTotalDisplay.text(chargesTotal);
		jQuery('#SHChargeVal').text(chargesTotal);
...
//		this.calculateGrandTotal();
    ...
    calculateChargeTaxes : function() {
		...
		chargesBlockContainer.find('.chargeTaxPercentage').each(function(index, domElement){
            ...
            if(isNaN(element.val())) {
                 ...
                 * convert data Currency To User Format
                 */
				chargeAmount = app.unformatCurrencyToUser(chargeAmount);
				var value = Math.abs(chargeAmount * app.unformatCurrencyToUser(element.val()))/100;
			}
            /**
             * Added by Kelvin Thang
             * Date: 2018-07-16
		...
		chargesBlockContainer.find('.chargeTaxPercentage').each(function(index, domElement){
			...
			if (compoundOn) {
				...
				var chargeAmount = app.unformatCurrencyToUser(self.chargesContainer.find('[name="charges['+chargeId+'][value]"]').val());
				...
				jQuery.each(compoundOn, function(index, id) {
					...
					if(!isNaN(chargeTaxEle.val())) {
                         ...
                         * unformat data Currency To User Format
                         */
						chargeAmount = chargeAmount + app.unformatCurrencyToUser(chargeTaxEle.val());
					}
				});
		...
		chargesBlockContainer.find('.chargeTaxValue').each(function(index, domElement){
             ...
             * unformat data Currency To User Format
             */
			chargesTotal = app.unformatCurrencyToUser(chargesTotal) + app.unformatCurrencyToUser(chargeElementValue);
		});
         ...
         */
        chargesTotal = app.convertCurrencyToUserFormat(chargesTotal, false);
        jQuery('#chargeTaxTotal').text(chargesTotal);
        this.chargeTaxesTotal.val(chargesTotal);
    ...
    calculateGrandTotal : function(){ 
         ...
         * convert data Currency To User Format
         */
        grandTotal = app.convertCurrencyToUserFormat(grandTotal, false);

		//alert(grandTotal);
		this.setGrandTotal(grandTotal);

        // Added by Hoang Duc 2019-10-08 to trigger grandTotal calculated
        app.event.trigger('post.grandTotalCalculated.load', app.unformatCurrencyToUser(grandTotal));
        // End Hoang Duc
	},
    ...
    calculateGroupTax : function() {
         ...
         * convert data Currency To User Format
         */
		this.setGroupTaxTotal(app.convertCurrencyToUserFormat(groupTaxTotal, false));
	},
    ...
    calculateDeductTaxes : function() {
		...
		this.dedutTaxesContainer.find('.deductTaxPercentage').each(function(index, domElement){
			...
			element.closest('tr').find('.deductTaxValue').val(app.convertCurrencyToUserFormat(value, false));
         ...
         * convert data Currency To User Format
         */
        deductTaxesTotalAmount = app.convertCurrencyToUserFormat(deductTaxesTotalAmount, false);

        /*this.deductTaxesTotal.text(parseFloat(deductTaxesTotalAmount).toFixed(this.numOfCurrencyDecimals));*/
        this.deductTaxesTotal.text(deductTaxesTotalAmount);
					...
					taxDiv +=
							'<tr>'+
								...
								'<td style="text-align: right;">'+
									'<input type="text" name="'+taxName+'_percentage'+rowNumber+'" data-rule-positive=true data-rule-inventory_percentage=true  id="'+taxName+'_percentage'+rowNumber+'" value="'+taxInfo.taxpercentage+'" class="taxPercentage col-sm-6" data-compound-on='+taxInfo.compoundOn+' data-regions-list="'+taxInfo.regionsList+'">&nbsp;%'+
								'</td>'+
    ...
    mapResultsToFields: function(parentRow,responseData){
		...
		for(var id in responseData){
			...
			jQuery('input.listPrice', parentRow).val(app.convertCurrencyToUserFormat(unitPrice, false));
	...
	// Modified by Tung Nguyen on 2022-10-31 to change logic add line item inventory
    postLineItemSelectionActions: function (itemRow, selectedLineItemsData, lineItemSelectedModuleName) {
        for (var index in selectedLineItemsData) {
			let lineItemTab = jQuery(itemRow).closest('.lineItemTab');
			let rowData = selectedLineItemsData[index];

            if (index != 0) {
                if (lineItemSelectedModuleName == 'Products') {
					this.addLineItemDetail(lineItemTab.find('.addProduct'), rowData);
                } else if (lineItemSelectedModuleName == 'Services') {
					this.addLineItemDetail(lineItemTab.find('.addService'), rowData);
                }
            } else {
                itemRow.find('.lineItemType').val(lineItemSelectedModuleName);
                this.mapResultsToFields(itemRow, rowData);
            }
        }
    },
    ...
    /*-- Modified By Kelvin Thang -- Date: 2018-09-17 -- update element for Line Items Tab Section*/
	// Modified by Tung Nguyen on 2022-10-31 to helper add new row table inventory
    registerAddProductService : function() {
        jQuery('.addProduct, .addService').on('click', e => {
			this.addLineItemDetail($(e.currentTarget)).find('.lineItemPopup').trigger('click');
		});
    },

	// Added by Tung Nguyen on 2022-10-31 to helper add new row table inventory
	addLineItemDetail: function (currentTarget, data) {
		var self = this;
		...
		/*-- Modified By Kelvin Thang -- Date: 2018-09-17 -- update element for Line Items Tab Section*/
		var lineItemRow = currentTarget.closest(".lineItemTab").find(".lineItemRow:last");
		if (lineItemRow.length <= 0) {
			var appendTo = currentTarget.closest(".lineItemTab").find(".lineItemHeaders")
		}else {
			var appendTo = currentTarget.closest(".lineItemTab").find(".lineItemRow:last");
		}
		newLineItem = newLineItem.insertAfter(appendTo);
		...
		/*-- Added By Kelvin Thang -- Date: 2018-09-17 -- update element for Line Items Tab Section Attributes*/
		self.addLineItemCustomFieldAttributes(newLineItem);
		/*var i = currentTarget.closest(".lineItemTab").find(".lineItemsSection");
		var f = i.data("section-num");
		var d = newLineItem.data("row-num");
		newLineItem.find('[name="section_' + d + '"]').val(f);
		newLineItem.find(".qty").trigger("focusout");*/

		var lineItemsSection = currentTarget.closest(".lineItemTab").find(".lineItemsSection");
		var lineItemsSectionNum = lineItemsSection.data("section-num");
		var lineItemNum = newLineItem.data("row-num");
		newLineItem.find('[name="section_' + lineItemNum + '"]').val(lineItemsSectionNum);
		newLineItem.find(".qty").trigger("focusout");

		/*var j = self.customLineItemMappedFields;
		for (var n in j) {
			newLineItem.find('[name*="' + n + '"]').trigger("focusout");
		}*/
		self.updateLineItemElementByOrder();
		self.getForm().find(".saveButton").removeAttr("disabled");
    ...
    registerProductAndServiceSelector : function() {
        var self = this;
        var a = this.getForm();
        $('.lineItemPopup').on("click", function(e) {
            var a = 1;
        });
    ...
    registerQuantityChangeEvent : function() {
		...
		this.lineItemsHolder.on('focusout','.qty',function(e){ 
			...
			if(typeof quantityInStock  != 'undefined') {
                 ...
                 * Date: 2018-07-24
                 */
                var qty = app.unformatCurrencyToUser(element.val());
				if(parseFloat(qty) > parseFloat(quantityInStock)) {
					lineItemRow.find('.stockAlert').removeClass('hide').find('.maxQuantity').text(app.convertCurrencyToUserFormat(quantityInStock, false));
				}else{
	...
	*/
    ...
    registerLineItemTaxShowEvent : function() {
		...
		this.lineItemsHolder.on('click','.individualTax',function(e){
            ...
            var popOverTitle = taxUI.find('.popover_title').find('.variable').text(app.convertCurrencyToUserFormat(self.getTotalAfterDiscount(lineItemRow))).closest('.popover_title').text();
    ...
    registerLineItemDiscountShowEvent: function () {
        ...
        this.lineItemsHolder.on('click', '.individualDiscount', function (e) {
            ...
            var popOverTitle = discountUI.find('.popover_title').find('.variable').text(app.convertCurrencyToUserFormat(productTotal, false)).closest('.popover_title').text();
            element.popover({
                'content': discountUI,
                'html': true,
                'placement': 'top',
    ...
    registerFinalDiscountShowEvent : function(){
       ...
       this.finalDiscountEle.on('click', function(e){
           let popOverEleOpacity = popOverEle.css('opacity'); // Added by Hoang Duc 2019-10-08 to fix popover remove input element when hide
		   self.getForm().find('.popover.lineItemPopover').css('opacity', 0).css('z-index', '-1');

           if(popOverEleOpacity === '0') { // Modified by Hoang Duc 2019-10-08 to fix popover remove input element when hide
        ...
        chargesTrigger.on('click', function(e){
            ...
            if(popOverEleOpacity === '0') { // Modified by Hoang Duc 2019-10-08 to fix popover remove input element when hide
               ...
               popOverEle.css('opacity',1).css('z-index','');
                ...
                /* Comment by Hoang Duc 2019-10-08 to fix popover remove input element when hide */
                // chargesTrigger.popover('hide');
                /* End Comment by Hoang Duc 2019-10-08 to fix popover remove input element when hide */
    ...
    makeLineItemsSortable: function() {
        ...
        this.lineItemsHolder.sortable({
            ...
            "update": function(e,ui) {
                ...
                jQuery.each(lineItemTab, function(index, element) {
                    element = jQuery(element);
                    var lineItemRow = element.find(".lineItemRow");
                    var lineItemsSectionNum = element.find(".lineItemsSection").attr("data-section-num");
                    if (lineItemRow.length > 0) {
                        element.find(".qty").trigger("focusout");
                        element.find('[name*="cf_"]').trigger("focusout");
                        jQuery.each(lineItemRow, function(indexRow, elementRow) {
                            elementRow = jQuery(elementRow);
                            var elementRowNum = elementRow.attr("data-row-num");
                            self.updateRowNumberForSection(element, lineItemsSectionNum, elementRowNum);
                        });
                    } else {
                        element.find(".subTotal").text(0);
                    }
                    self.checkLineItemRow();
                });
            }
            ...
            var element = self.getClosestLineItemRow(event);
...
//			};
	...
	referenceSelectionEventHandler : function(data,container){
		...
		/** Beign: Added By Kelvin Thang on 2020-07-29 - Update request #1128 - Autofill data Contact into SalesOrder */
        if (app.getModuleName() == "SalesOrder") {
            var selectedId = data['record'];
            var selectedModule = data['source_module'];

            if (selectedModule == 'Contacts' && selectedId != '') {
                var editViewController = Vtiger_Edit_Js.getInstance();
                var params = {'source_module': selectedModule, 'record': selectedId};

                editViewController.getRecordDetails(params).then(function (res) {
                    $('[name="receiver_phone"]').val(res.data.mobile); // Modified by Phu Vo on 2020.10.30
                    $('[name="receiver_name"]').val(res.data.label);
                });
            }
        }
		/** Ended: Added By Kelvin Thang on 2020-07-29 - Update request #1128 - Autofill data Contact into SalesOrder */
	},
     ...
     * Added By Kelvin Thang
     * Date: 2018-09-17
     * add Line Item Custom Field Attributes
     */
    addLineItemCustomFieldAttributes: function(container) {
        container = jQuery(container);
        var LineItemMappedFields = this.customLineItemMappedFields;
        for (var elementName in LineItemMappedFields) {
            var element = jQuery('[name*="' + elementName + '"]', container);
            var moduleName = container.find(".lineItemPopup").data("module-name");
            var check = (moduleName == "Products") ? LineItemMappedFields[elementName]["productsfieldname"] : LineItemMappedFields[elementName]["servicesfieldname"];
            if (check) {
                element.attr("data-reffieldname", check);
            }
        }
    },
     ...
     * Added By Kelvin Thang
     * Date: 2018-09-17
     * get New Line Items Section
     */
    getNewLineItemsSection: function() {
        var form = this.getForm();
        var ItemsSectionClone = form.find(".lineItemsSectionCloneCopy").get(0);
        ItemsSectionClone = jQuery(ItemsSectionClone).clone(true, true);
        ItemsSectionClone.removeClass("hide lineItemsSectionCloneCopy").addClass("lineItemsSection");
        var lineItemsSectionNum = form.find(".lineItemsSection").length + 1;
        this.updateRowNumberForSection(ItemsSectionClone, lineItemsSectionNum);
        return ItemsSectionClone;
    },

    /**
     * Added By Kelvin Thang
     * Date: 2018-09-17
     * update Row Number For Section
     */
    updateRowNumberForSection: function(itemSectionClone, itemSectionNum, itemSectionTemplate) { 
        if (typeof itemSectionTemplate == "undefined") {
            itemSectionTemplate = 0;
        }
        var sectionName = "section_name";
        var sectionNameTemplate = sectionName + itemSectionTemplate;
        var sectionNameNew = sectionName + itemSectionNum;
        itemSectionClone.find('[name="' + sectionNameTemplate + '"]').attr("name", sectionNameNew);
        itemSectionClone.attr("id", "section" + itemSectionNum).attr("data-section-num", itemSectionNum);
        var lineItemRow = itemSectionClone.closest(".lineItemTab").find(".lineItemRow");
        jQuery.each(lineItemRow, function(e, td) {
            td = jQuery(td);
            var dataRowNum = td.attr("data-row-num");
            td.find('[name="section_' + dataRowNum + '"]').val(itemSectionNum);
        });
    },
     ...
     * Added By Kelvin Thang
     * Date: 2018-09-17
     * Add Event For Line Items Section
     */
    registerEventForLineItemsSection: function() { 
        var form = this.getForm();
        var self = this;
        form.on("click", ".addSection", function(i) { 

            var lineitemTableContainer = form.find(".lineitemTableContainer").find(".lineItemTab:last");
            ...
            lineitemTableContainer = jQuery("<br>").insertAfter(lineitemTableContainer);
            lineItemTabClone = lineItemTabClone.insertAfter(lineitemTableContainer);

            var checkAddProduct = lineItemTabClone.find(".addProduct").length;

            if (form.find(".lineItemTab").length == 2) {
                var lineItemTab = form.find(".lineItemTab:first");
                var lineItemsSection = lineItemTab.find(".lineItemsSection");
                lineItemsSection.removeClass("hide");
            }

            var lineItemsSectionNew = self.getNewLineItemsSection();
            ...
            if (checkAddProduct > 0) {
				self.addLineItemDetail(lineItemTabClone.find('.addProduct'));
            } else {
				self.addLineItemDetail(lineItemTabClone.find('.addService'));
            }
			// Ended by Tung Nguyen
        });

        form.on("click", ".deleteSection", function(f) {
            var currentTarget = jQuery(f.currentTarget);
            var customMessage = app.vtranslate("LBL_DELETE_CONFIRMATION");
            app.helper.showConfirmationBox({
                "message": customMessage
            }).then(function() {
                currentTarget.closest(".lineItemTab").remove();
                self.checkLineItemRow();
                self.lineItemDeleteActions(f);
                self.updateLineItemElementByOrder();
                var lineItemsSection = form.find(".lineItemsSection");
                jQuery.each(lineItemsSection, function(num, tr) {
                    tr = jQuery(tr);
                    var id = tr.attr("id");
                    var sectionNo = id.split("section");
                    var numNew = num + 1;
                    var sectionNew = "section" + numNew;
                    if (sectionNew != id) {
                        self.updateRowNumberForSection(tr, numNew, sectionNo[1]);
                    }
                    if (lineItemsSection.length == 1) {
                        lineItemsSection.find('[name="section_name' + numNew + '"]').val("");
                        lineItemsSection.addClass("hide");
                    }
                });
            });
        });
    ...
    // Added by Hoang Duc 2019-10-08 to register calculate balance
    registerCalculateBalance: function () {
        var self = this;

        app.event.on('post.grandTotalCalculated.load', function (event, grandTotal) {
            var moduleMame = app.getModuleName(),
                cash = null;

            if (moduleMame === "PurchaseOrder") {
                cash = app.unformatCurrencyToUser(jQuery('input[name="paid"]').val());
            } else {
                cash = app.unformatCurrencyToUser(jQuery('input[name="received"]').val());
            }

            self.calculateAndSetValueForBalance(grandTotal, cash);
        });

        jQuery('input[name="received"]').on('change', function () {
            var cash = app.unformatCurrencyToUser($(this).val()),
                grandTotal = app.unformatCurrencyToUser(jQuery('[name="grandTotal"]').text());

            self.calculateAndSetValueForBalance(grandTotal, cash);
        });

        jQuery('input[name="paid"]').on('change', function () {
            var cash = app.unformatCurrencyToUser($(this).val()),
                grandTotal = app.unformatCurrencyToUser(jQuery('[name="grandTotal"]').text());
    ...
    // Added by Hoang Duc 2019-10-08 calculate balance then set value for balance
    calculateAndSetValueForBalance: function(grandTotal, cash) {
        var balance = grandTotal - cash;

        jQuery('input[name="balance"]').val(app.convertCurrencyToUserFormat(balance));
    },
    // End Hoang Duc
    ...
    registerBasicEvents: function(container){ 
         ...
         * Added By Kelvin Thang
         * Date: 2018-09-17
         * Add register event For Line Items Section
         */
		this.registerEventForLineItemsSection();

        // Added by Hoang Duc 2019-10-08 to register calculate balance
        this.registerCalculateBalance();
        // End Hoang Duc
    },
});

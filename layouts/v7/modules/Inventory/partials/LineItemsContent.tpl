...
{strip}
    ...
    {assign var="sectionNum" value="sectionNum"|cat:$row_no}

	<td style="text-align:center;">
		<i class="far fa-trash-alt deleteRow cursorPointer" title="{vtranslate('LBL_DELETE',$MODULE)}"></i>
		&nbsp;<a><i class="far fa-grip-lines" border="0" title="{vtranslate('LBL_DRAG',$MODULE)}"></i></a>
	...
	{/if}
    <!--Added by Kelvin Thang -- OnlineCRM -- 2018-09-17 -->
	<input type="hidden" name="section_{$row_no}" value="{$section_no}">
		...
		<td>
			...
			{if $data.$productDeleted}
				<div class="row-fluid deletedItem redColor">
					...
					{else}
						{* Modified by Phu Vo on 2020.12.25 to fix concat string translate issue *}
						{$replaceParams = ['%product_type' => vtranslate($entityType, $entityType)]}
						{vtranslate('LBL_THIS_ITEM_IS_DELETED_FROM_THE_SYSTEM_PLEASE_REMOVE_OR_REPLACE_THIS_ITEM', $MODULE, $replaceParams)}
						{* End Phu Vo *}
					{/if}
				</div>
			{else}
				{if $COMMENT_EDITABLE}
					<div><br><textarea id="{$comment}" name="{$comment}" style=" width: 90%; " class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
				{/if}
			{/if}
		</td>
	{/if}
	...
	<td>
		<!--Added by Kelvin Thang -- OnlineCRM -- Date: 2018-07-09 -->
		<input id="{$qty}" name="{$qty}" type="text" onkeyup="formatNumber (this, 'float')" class="qty smallInputBox inputElement" style="width: 100% !important;"
			   data-rule-required=true data-rule-currency=true data-rule-greater_than_zero=true value="{if !empty($data.$qty)}{Vtiger_Currency_UIType::transformDisplayValue($data.$qty, null, true)}{else}1{/if}"
		...
		{if $MODULE neq 'PurchaseOrder'}
			...
			<span class="stockAlert redColor {if $data.$qty <= $data.$qtyInStock}hide{/if}" >
				...
				<br>
				{vtranslate('LBL_MAX_QTY_SELECT',$MODULE)}&nbsp;<span class="maxQuantity">{Vtiger_Currency_UIType::transformDisplayValue($data.$qtyInStock, null, true)}</span>
			</span>
		{/if}
	</td>
	...
	{if $LIST_PRICE_EDITABLE}
		<td>
			<div class="list-price">
				<!--Added by Kelvin Thang -- OnlineCRM -- Date: 2018-07-09 -->
				<input id="{$listPrice}" name="{$listPrice}" onkeyup="formatNumber (this, 'float')" value="{if !empty($data.$listPrice)}{Vtiger_Currency_UIType::transformDisplayValue($data.$listPrice, null, true)}{else}0{/if}" type="text"
					   data-rule-required=true data-rule-currency=true class="listPrice smallInputBox inputElement " data-is-price-changed="{if $RECORD_ID && $row_no neq 0}true{else}false{/if}" list-info='{if isset($data.$listPrice)}{Zend_Json::encode($listPriceValues)}{/if}' data-base-currency-id="{getProductBaseCurrency($productId, {$entityType})}" />
				&nbsp;
			...
			{if $ITEM_DISCOUNT_AMOUNT_EDITABLE || $ITEM_DISCOUNT_PERCENT_EDITABLE}
				<div class="item-discount-container">
					...
					<span>(-)&nbsp;
						<strong><a href="javascript:void(0)" class="inventoryLineItemEdits individualDiscount">{vtranslate('LBL_DISCOUNT',$MODULE)}
								<span class="itemDiscount">
									...
									{else if $ITEM_DISCOUNT_AMOUNT_EDITABLE && $data.$discount_type eq 'amount'}
										({Vtiger_Currency_UIType::transformDisplayValue($data.$discount_amount, null, true)})
									{else}
										(0)
									{/if}
								</span>
				...
				<div class="discountUI validCheck hide" id="discount_div{$row_no}">
					...
					<p class="popover_title hide">
						{vtranslate('LBL_SET_DISCOUNT_FOR',$MODULE)} : <span class="variable">{Vtiger_Currency_UIType::transformDisplayValue($data.$productTotal, null, true)}</span>
					</p>
							...
							<tr>
								...
								<td>
									<div class="field-value">
										...
										<input type="text" data-rule-currency=true id="discount_amount{$row_no}" name="discount_amount{$row_no}" onkeyup="formatNumber(this)" value="{Vtiger_Currency_UIType::transformDisplayValue($data.$discount_amount, null, true)}" class="span1 pull-right discount_amount discountVal {if empty($data.$checked_discount_amount)}hide{/if}"/>
									</div>
								</td>
							</tr>
						{/if}
				...
				<div class="total-after-discount" style="width:150px;">
			...
			<span class="taxDivContainer">
				<div class="taxUI hide" id="tax_div{$row_no}">
					<p class="popover_title hide">
						{vtranslate('LBL_SET_TAX_FOR',$MODULE)} : <span class="variable">{Vtiger_Currency_UIType::transformDisplayValue($data.$totalAfterDiscount, null, true)}</span>
					</p>
								...
								{foreach key=tax_row_no item=tax_data from=$data.taxes}
									...
									<tr>
										...
										<td style="text-align: right;">
											<!--Added by Klevin Thang - Date: 2018-07-17-->
											<input type="text" data-rule-positive=true data-rule-inventory_percentage=true  name="{$taxname}" id="{$taxname}" value="{Vtiger_Currency_UIType::transformDisplayValue($tax_data.percentage, null, true)}" data-compound-on="{if $tax_data.method eq 'Compound'}{Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($tax_data.compoundon))}{/if}" data-regions-list="{Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($tax_data.regionsList))}" class="span1 taxPercentage" />&nbsp;%
										</td>
										<td style="text-align: right; padding-right: 10px;">
											<input type="text" name="{$popup_tax_rowname}" class="cursorPointer span1 taxTotal taxTotal{$tax_data.taxid}" value="{Vtiger_Currency_UIType::transformDisplayValue($tax_data.amount, null, true)}" readonly />
										</td>
									</tr>
								{/foreach}
							</table>
	...
	{*{/if}*}{*-- Comment out by Kelvin Thang on 2020-29-02 --@To: Hide TOTAL HEADERS when field list price is hidden --*}
        ...
        <td>
            <div id="productTotal{$row_no}" align="right" class="productTotal">{if $data.$productTotal}{Vtiger_Currency_UIType::transformDisplayValue($data.$productTotal, null, true)}{else}0{/if}</div>
            {if $ITEM_DISCOUNT_AMOUNT_EDITABLE || $ITEM_DISCOUNT_PERCENT_EDITABLE}
                <div id="discountTotal{$row_no}" align="right" class="discountTotal">{if $data.$discountTotal}{Vtiger_Currency_UIType::transformDisplayValue($data.$discountTotal, null, true)}{else}0{/if}</div>
                <div id="totalAfterDiscount{$row_no}" align="right" class="totalAfterDiscount">{if $data.$totalAfterDiscount}{Vtiger_Currency_UIType::transformDisplayValue($data.$totalAfterDiscount, null, true)}{else}0{/if}</div>
            {/if}

            <div id="taxTotal{$row_no}" align="right" class="productTaxTotal {if $IS_GROUP_TAX_TYPE}hide{/if}">{if $data.$taxTotal}{Vtiger_Currency_UIType::transformDisplayValue($data.$taxTotal, null, true)}{else}0{/if}</div>
        </td>
    ...
    {if $LIST_PRICE_EDITABLE} {*--Begin: Added by Kelvin Thang on 2020-29-02 --@To: Hide NET PRICE HEADERS when field list price is hidden -->*}
        <td>
            <span id="netPrice{$row_no}" class="pull-right netPrice">{if $data.$netPrice}{Vtiger_Currency_UIType::transformDisplayValue($data.$netPrice, null, true)}{else}0{/if}</span>
        </td>

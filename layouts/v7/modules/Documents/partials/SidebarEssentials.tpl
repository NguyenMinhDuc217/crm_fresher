                        ...
                        <div class="list-group">   
                            ...
                            {foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS name="customView"}
                                <li style="font-size:12px;" class='listViewFilter {if $VIEWID eq $CUSTOM_VIEW->getId() && ($CURRENT_TAG eq '') && !$FOLDER_VALUE} active{/if} {if $smarty.foreach.customView.iteration gt 5} filterHidden hide{/if} '>
                                    ...
                                    <a class="filterName" href="javascript:;" data-filter-id="{$CUSTOM_VIEW->getId()}" data-toggle="tooltip" title="{$VIEWNAME}">{if {$VIEWNAME|strlen > 40} } {$VIEWNAME|substr:0:40|@escape:'html'}..{else}{$VIEWNAME|@escape:'html'}{/if}</a> 
                                        ...
                                        <span class="far fa-ellipsis-v" rel="popover" data-toggle="popover" aria-expanded="true" 
                                            {if $CUSTOM_VIEW->isMine() and $CUSTOM_VIEW->get('viewname') neq 'All'}
                                                ...
                                                {/if}
                                                toggleClass="far {if $CUSTOM_VIEW->isDefault()}fa-check-square{else}fa-square{/if}" data-filter-id="{$CUSTOM_VIEW->getId()}" 
                    ...
                    <ul id="folders-list" class="lists-menu">
                        {foreach item="FOLDER" from=$FOLDERS}
                            ...
                            <li style="font-size:12px;" class='documentFolder {if $FOLDER_VALUE eq $FOLDER->getName()} active{/if}'>
                                <a class="filterName" href="javascript:void(0);" data-filter-id="{$FOLDER->get('folderid')}" data-folder-name="{$FOLDER->get('foldername')}" data-toggle="tooltip" title="{$FOLDERNAME}">
                                    <i class="far {if $FOLDER_VALUE eq $FOLDER->getName()}fa-folderpen{else}fa-folder{/if}"></i> 
            ...
            <div class="menu-scroller scrollContainer">
                ...
                <div id="editTagContainer" class="hide">
                    ...
                    <div class="edit-tag-footer">
                        <button class="btn btn-sm btn-success saveTag">
                            <center> <i class="far fa-check"></i> </center>
                        </button>
                        <button class="btn btn-sm btn-default cancelSaveTag">

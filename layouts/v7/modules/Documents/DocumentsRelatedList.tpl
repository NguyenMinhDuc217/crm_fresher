                                ...
                                <div class="btn-group">
                                    <div class="dropdown">
                                        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                                            <span class="far fa-plus" title="{vtranslate('LBL_NEW_DOCUMENT', $MODULE)}"></span>&nbsp;&nbsp;{vtranslate('LBL_NEW_DOCUMENT', $RELATED_MODULE_NAME)}&nbsp; <i class="far fa-angle-down"></i>
                                        </button>
                                        <ul class="dropdown-menu">
                                            <li class="dropdown-header"><i class="far fa-upload"></i> {vtranslate('LBL_FILE_UPLOAD', $RELATED_MODULE_NAME)}</li>
                                            <li id="VtigerAction">
                                                <a href="javascript:Documents_Index_Js.uploadTo('Vtiger',{$PARENT_ID},'{$MODULE}')">
                                                    <img style="  margin-top: -3px;margin-right: 4%;" title="CRM" alt="CRM" src="layouts/v7/skins//images/Vtiger.png">
                                                    {vtranslate('LBL_TO_SERVICE', $RELATED_MODULE_NAME, {vtranslate('LBL_VTIGER', $RELATED_MODULE_NAME)})}
                                                </a>
                                            </li>
                                            <li role="separator" class="divider"></li>
                                            <li class="dropdown-header"><i class="far fa-link"></i> {vtranslate('LBL_LINK_EXTERNAL_DOCUMENT', $RELATED_MODULE_NAME)}</li>
                                            <li id="shareDocument"><a href="javascript:Documents_Index_Js.createDocument('E',{$PARENT_ID},'{$MODULE}')"><i class="far fa-external-link"></i>{vtranslate('LBL_FROM_SERVICE', $RELATED_MODULE_NAME, {vtranslate('LBL_FILE_URL', $RELATED_MODULE_NAME)})}</a></li>
                                            <li role="separator" class="divider"></li>
                                            <li id="createDocument"><a href="javascript:Documents_Index_Js.createDocument('W',{$PARENT_ID},'{$MODULE}')"><i class="far fa-file-text"></i>{vtranslate('LBL_CREATE_NEW', $RELATED_MODULE_NAME, {vtranslate('SINGLE_Documents', $RELATED_MODULE_NAME)})}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            {/if}
        ...
        <table id="listview-table"  class="table listview-table">
            ...
            {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
                <tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
                        ...
                        data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'
                                        {/if}>
                    <td style="width:150px">&nbsp;&nbsp;&nbsp;  {* Modified column width by Hieu Nguyen on 2020-06-26 to have enough space for 5 buttons *}
                        <span class="actionImages">
                            <a name="relationEdit" data-url="{$RELATED_RECORD->getEditViewUrl()}"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen"></i></a> &nbsp;&nbsp;
                            ...
                            {* Added by Hieu Nguyen on 2020-06-26 to display preview and download button only for uploaded file *}
                            {if $DOCUMENT_RECORD_MODEL->get('filename') && $DOCUMENT_RECORD_MODEL->get('filestatus') && $DOCUMENT_RECORD_MODEL->get('filesize') > 0}
                                <a name="viewfile" href="javascript:void(0)" data-filelocationtype="{$DOCUMENT_RECORD_MODEL->get('filelocationtype')}" data-filename="{$DOCUMENT_RECORD_MODEL->get('filename')}" onclick="Vtiger_Header_Js.previewFile(event)"><i title="{vtranslate('LBL_VIEW_FILE', $RELATED_MODULE_NAME)}" class="far fa-image alignMiddle"></i></a>&nbsp;&nbsp;
                            {/if}
                            
                            {if $DOCUMENT_RECORD_MODEL->get('filename') && $DOCUMENT_RECORD_MODEL->get('filestatus') && $DOCUMENT_RECORD_MODEL->get('filesize') > 0}
                                <a name="downloadfile" href="{$DOCUMENT_RECORD_MODEL->getDownloadFileURL()}" onclick="event.stopImmediatePropagation();"><i title="{vtranslate('LBL_DOWNLOAD_FILE', $RELATED_MODULE_NAME)}" class="far fa-download alignMiddle"></i></a>
                            {/if}
                            {* End Hieu Nguyen *}

                            {* Added by Hieu Nguyen on 2020-06-26 to allow send uploaded file as email attachment *}
                            {if $DOCUMENT_RECORD_MODEL->get('filename') && $DOCUMENT_RECORD_MODEL->get('filestatus') && $DOCUMENT_RECORD_MODEL->get('filesize') > 0}
                                &nbsp;&nbsp;
                                <a name="emailFile" href="javascript:void(0)" onclick="Documents_Index_Js.getInstance().composeEmailWithAttachment({$RELATED_RECORD->getId()});"><i title="{vtranslate('LBL_SEND_EMAIL', $RELATED_MODULE_NAME)}" class="far fa-send alignMiddle"></i></a>
                            {/if}
                            {* End Hieu Nguyen *}

                            {* Added by Hieu Nguyen on 2020-06-26 to load module custom listview row actions *}
                    ...
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                        ...
                        {* Modified by Hieu Nguyen on 2020-06-26 to re-indent code and render link for file name column *}
                        <td class="{$WIDTHTYPE} relatedListEntryValues " data-field-name="{$RELATED_HEADERNAME}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap style="width:inherit;">    {* Modified by Hieu Nguyen on 2020-01-16 to show field name attribue *}
                            {if $RELATED_MODULE->get('name') eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
                            ...
                            {else}
                                <span class= "value textOverflowEllipsis">
                                    ...
                                    {else if $HEADER_FIELD->getName() eq 'filename'}

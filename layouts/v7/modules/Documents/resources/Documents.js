...
Vtiger.Class('Documents_Index_Js', {
	...
	registerFileDragDropEvent : function(container) {
		...
		dragDropElement.on('drop', function(e) {
			...
			// Updated by Phuc on 2019.10.10 to validate file type
			if (self.fileExtCheck(container, file)) {
				...
				app.helper.showAlertNotification({
					'message' : app.vtranslate('JS_DO_NOT_ALLOW_UPLOAD_EXT')
				});
			}
			// Ended by Phuc
		});
	...
	// Added by Phuc on 2019.10.10 to validate file type
	fileExtCheck : function(container, file) {
		let fileExt = file.name.split('.').pop();
		fileExt = fileExt.toLowerCase();
		return typeof _VALIDATION_CONFIG.allowed_upload_file_exts != 'undefined' && _VALIDATION_CONFIG.allowed_upload_file_exts.indexOf(fileExt) >= 0;
	},
	// Ended by Phuc
			...
			if(!e) {
				app.helper.showModal(resp, {
					'cb' : function(modalContainer) {
						...
						// Comment out by Phu Vo on 2019.06.17 to void dubplicate register event
						// self.registerQuickCreateEvents(modalContainer);
						// End Phu Vo

						// Added by Phu Vo on 2019.06.17 trigger event document quick create popup opened
						app.event.trigger('post.documentQuickCreatePopup.load', modalContainer);
						// End Phu Vo trigger event document quick create popup openedname
					}
				});
			}
		});
    ...
    // Added by Hieu Nguyen on 2020-06-26 to enable composing email with selected document as attachment
    composeEmailWithAttachment(documentId) {
        var params = {
			'module' : 'Documents',
			'view' : 'ComposeEmail',
	...
	registerCreateDocumentModalEvents : function(container) {
		...
		if(container.find('input[name="type"]').val() === 'W') {
			...
			// Comment out by Phu Vo on 2019.06.18 => let it have default behavior

...
Vtiger.Class('Settings_LayoutEditor_Js', {
	...
	updateBlockSequence: function () {
		...
		params['action'] = jQuery('.selectedTab').val() == 'editViewTab' ? 'EditViewBlock' : 'Block';	// Modified by Hieu Nguyen on 2018-07-30
		params['mode'] = 'updateSequenceNumber';
		params['sequence'] = sequence;
		params['selectedModule'] = jQuery('#selectedModuleName').attr('value');
		params['layouteditor_tab'] = jQuery('.selectedTab').val();	// Added by Hieu Nguyen on 2021-08-23 to determine which view is editing in ajax request
	...
	saveRelatedListInfo: function () {
		...
		params['mode'] = 'update';	// Added by Hieu Nguyen on 2018-08-04
	...
	showSaveFieldSequenceButton: function () {
		var thisInstance = this;
		var layout = jQuery('.layoutEditor.active');
	...
	updateFieldSequence: function () {
		...
		params['module'] = thisInstance.getModuleName();
		params['parent'] = app.getParentModuleName();
		params['action'] = jQuery('.selectedTab').val() == 'editViewTab' ? 'EditViewField' : 'Field';	// Modified by Hieu Nguyen on 2018-07-30
		params['mode'] = 'updateFieldSequence';	// Modified by Hieu Nguyen on 2021-08-02
	...
	showFieldEditModel: function (data, blockId, fieldContainer) {
		...
		// Added by Hieu Nguyen on 2018-10-01 to trigger re-generate field name when field type is changed
		data.find('select[name="fieldType"]').on('change', function(e) {
			if(jQuery(e.currentTarget).is(':disabled')) {
				return false;
			}

			data.find('input[name="fieldLabelDisplayEn"]').trigger('keyup');
		});

		// Added by Hieu Nguyen on 2021-10-29 to generate field name form its English label
		data.find('input[name="fieldLabelDisplayEn"]').on('keyup', function(e) {
			var selectedFieldType = data.find('select[name="fieldType"]').val();
			var fieldNameInput = data.find('input[name="fieldName"]');

			if (!fieldNameInput[0].hasAttribute('disabled')) {
				var value = jQuery(e.currentTarget).val().trim();
				var fieldName = value.toLowerCase().replace(/\s/g, '_');

				// Append prefix as module name for Picklist field name
				if (selectedFieldType == 'Picklist' || selectedFieldType == 'MultiSelectCombo') {
					var sourceModule = jQuery('#selectedModuleName').val();
					var picklistNamePrefix = sourceModule.toLowerCase();
					fieldName = `${picklistNamePrefix}_${fieldName}`;
				}

				fieldNameInput.val(fieldName).data('value', fieldName).trigger('keyup');
			}
		});
		// End Hieu Nguyen

		data.find('input[name="fieldName"]').on('keyup', function(e) {
			var value = jQuery(e.currentTarget).val().trim();

			// Picklist field prefix must always be the module name
			if (data.find('select[name="fieldType"]').val() == 'Picklist') {
				if (value.indexOf(jQuery('#selectedModuleName').val().toLowerCase() + '_') !== 0) {
					jQuery(e.currentTarget).val(jQuery(e.currentTarget).data('value'));
					return;
				}
			}

			var fieldName = value.replace(/\s/g, '');
			jQuery(e.currentTarget).val(fieldName);
			jQuery(e.currentTarget).data('value', fieldName);
		});
		// End Hieu Nguyen

		// Added by Hieu Nguyen on 2018-10-01 to lower case the field name
		data.find('input[name="fieldName"]').on('keyup', function(e) {
			var value = jQuery(e.currentTarget).val().trim();
			jQuery(e.currentTarget).val(value.toLowerCase());
		});
		// End Hieu Nguyen

		// Added by Hieu Nguyen on 2018-10-01 to re-generate label key when field name is changed
		data.find('input[name="fieldName"]').on('keyup', function(e) {
			var value = jQuery(e.currentTarget).val().trim();
			var generatedLabel = 'LBL_' + value.toUpperCase().replace(/\s/g, '_');

			var fieldLabel = data.find('input[name="fieldLabel"]');
			fieldLabel.val(generatedLabel);
		});
		// End Hieu Nguyen

		// Added by Hieu Nguyen on 2018-08-13 to upper case the field label
		data.find('input[name="fieldLabel"]').on('keyup', function(e) {
			var value = jQuery(e.currentTarget).val().trim().toUpperCase().replace(/\s/g, '')
			jQuery(e.currentTarget).val(value);
		});
		// End Hieu Nguyen
			...
			submitHandler: function (form) {
				...
				if (!isEditMode) {
					...
					if (fieldTypeValue == 'Picklist' || fieldTypeValue == 'MultiSelectCombo') {
						...
						var specialChars = /[\"\,\[\]\{\}]/;	// Modified by Hieu Nguyen on 2018-08-13
						...
						for (var i = 0; i < pickListValuesArray.length; i++) {
							...
							if (specialChars.test(pickListValuesArray[i])) {
								var message = app.vtranslate('JS_SPECIAL_CHARACTERS')+" \" , [ ] { } "+app.vtranslate('JS_NOT_ALLOWED');	// Modified by Hieu Nguyen on 2018-08-13
								vtUtils.showValidationMessage(select2Element, message, showValidationParams);
								return false;
							}

							// Added by Hieu Nguyen on 2019-05-31 to validate unicode string
							if (pickListValuesArray[i].isUnicode()) {
								var errorMessage = app.vtranslate('JS_NON_UNICODE_REQUIRED');
								vtUtils.showValidationMessage(select2Element, errorMessage, showValidationParams);
								return false;
							}
							// End Hieu Nguyen
				...
				// Added by Hieu Nguyen on 2018-08-13 to validate field name and field label when creating only
				if (data.find('input[name="fieldid"]').val() == '') {
					var pattern = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
					var fieldName = form.find('input[name="fieldName"]');
					...
					if (!pattern.test(fieldNameValue)) {
						...
						vtUtils.showValidationMessage(fieldName, errorMsg, {
							position: {
								my: 'bottom left',
								at: 'top left',
								container: form
					...
					var fieldLabel = form.find('input[name="fieldLabel"]');
					var fieldLabelValue = fieldLabel.val();

					if (!pattern.test(fieldLabelValue)) {
						var errorMsg = app.vtranslate('JS_SPECIAL_CHARACTERS') + ' ' + app.vtranslate('JS_NOT_ALLOWED');

						vtUtils.showValidationMessage(fieldLabel, errorMsg, {
							position: {
								my: 'bottom left',
								at: 'top left',
								container: form
				...
				// Added by Hieu Nguyen on 2021-06-14
				if (isEditMode) {
					let fieldLabelDisplayEnInput = form.find('[name="fieldLabelDisplayEn"]');
					let fieldLabelDisplayVnInput = form.find('[name="fieldLabelDisplayVn"]');

					if (fieldLabelDisplayEnInput.val() != fieldLabelDisplayEnInput.data('curValue')) {
						form.find('[name="fieldLabelDisplayEnChanged"]').val(1);
					}

					if (fieldLabelDisplayVnInput.val() != fieldLabelDisplayVnInput.data('curValue')) {
						form.find('[name="fieldLabelDisplayVnChanged"]').val(1);
					}
				}
				// End Hieu Nguyen
				...
				if (isEditMode) {
					...
					thisInstance.saveFieldDetails(saveParams).then(function (result) {
						...
						location.reload();	// Added by Hieu Nguyen on 2018-08-17 to refresh the page after saving
					}, function () {
						app.helper.hideProgress();
					...
					thisInstance.addCustomField(blockId, form).then(
						function (data) {
							...
							if (data) {
								app.helper.hideModal();
								params['message'] = app.vtranslate('JS_CUSTOM_FIELD_ADDED', result['labelDisplay']);	// Modified by Hieu Nguyen on 2021-06-15 to display label instead of key
							...
							if (error) {
								...
								vtUtils.showValidationMessage(form.find('[name="fieldName"]'), message, {	// Modified by Hieu Nguyen on 2018-08-03
									position: {
										my: 'bottom left',
										at: 'top left',
										container: form
	...
	setFieldDetails: function (result, form) {
		...
		fieldlabelHolder.find('b').text(result['labelDisplay']);	// Modified by Hieu Nguyen on 2021-06-15 to display label instead of key
			...
			var callBackFunction = function (data) {
				...
				var params = {
					submitHandler: function (form) {
						...
						// Modified by Hieu Nguyen on 2018-08-06 to allow the underscore character
						var pattern = /^[a-zA-Z_][a-zA-Z0-9_]*$/;

						if (!pattern.test(blockLabelValue)) {
							...
							thisInstance.saveBlockDetails(form).then(
								function (data) {
									...
									if (data) {
										...
										location.reload();	// Added by Hieu Nguyen on 2018-08-06 to reload the page after saving
									} else {
	...
	registerCopyLayoutFromEditViewEvent: function () {
		var thisInstance = this;
		var contents = jQuery('#layoutEditorContainer').find('.contents');

		contents.find('.copyLayoutFromEditView').click(function (e) {
			if (!confirm(app.vtranslate('JS_COPY_LAYOUT_FROM_EDITVIEW_CONFIRM_MSG'))) {
				return;
			}
			...
			params['sourceModule'] = jQuery('#selectedModuleName').val();
			params['action'] = 'Block';
			params['mode'] = 'copyLayoutFromEditView';
			...
			.then(function (err, data) {
				...
				if (data.success == 1) {
					params['message'] = app.vtranslate('JS_COPY_LAYOUT_FROM_EDITVIEW_SUCCESS_MSG');
					app.helper.showSuccessNotification(params);
					location.reload();
					return;
				}

				params['message'] = app.vtranslate('JS_COPY_LAYOUT_FROM_EDITVIEW_ERROR_MSG');
				app.helper.showErrorNotification(params);
			});
		});
	},
	...
	displayNewCustomBlock: function (result) {
		...
		newBlockCloneCopy.attr('data-block-id', result['id']).find('.blockLabel').append(jQuery('<strong>'+result['labelDisplay']+'</strong>'));	// Modified by Hieu Nguyen on 2021-06-15 to display label instead of key
	...
	registerInactiveFieldsEvent: function () {
		...
		contents.on('click', 'button.inActiveFields', function (e) {
			...
			if (jQuery.isEmptyObject(thisInstance.inActiveFieldsList)) {
	...
	showHiddenFields: function (blockId, form) {
		...
		// Modified by Hieu Nguyen on 2018-07-31 to show up all hidden fields
		jQuery.each(Object.keys(thisInstance.inActiveFieldsList), function (i, blockId) {
			jQuery.each(thisInstance.inActiveFieldsList[blockId], function (key, value) {
				...
				var inActiveField = jQuery('<div class="col-sm-4">\n\
												<div class="checkbox">\n\
													<label><input type="checkbox" class="inActiveField" value="'+key+'" data-original-block-id="'+ blockId +'"/><span class="fieldLabel">'+value+'</span></label>\n\
												</div>\n\
											</div>');
				// End Hieu Nguyen
	...
	createReactivateFieldslist: function (blockId, form) {
		...
		form.find('.inActiveField').each(function (index, domElement) {
			...
			// Modified by Hieu Nguyen on 2018-07-31
			var originalBlockId = element.data('originalBlockId');

			if (element.is(':checked')) {
				delete thisInstance.inActiveFieldsList[originalBlockId][fieldId];
	...
	reActivateHiddenFields: function (currentBlock) {
		...
		app.request.post({'data': params}).then(
			function (err, data) {
				...
				location.reload();	// Added by Hieu Nguyen on 2018-07-31 to refresh the UI
			});
	},
	/**
	...
	registerDeleteCustomBlockEvent: function () {
		...
		contents.on('click', 'button.deleteCustomBlock', function (e) {
			...
			var message = app.vtranslate('JS_DELETE_CUSTOM_BLOCK_CONFIRM_MSG');	// Modified by Hieu Nguyen to display the right confirm message
	...
	deleteCustomField: function (fieldId) {
		...
		params['layouteditor_tab'] = jQuery('.selectedTab').val();	// Added by Hieu Nguyen on 2021-11-22 to determine which view is editing in ajax request
	...
	registerModulesChangeEvent: function () {
		...
		container.on('change', '[name="layoutEditorModules"]', function (e) {
			...
			// Modified by Hieu Nguyen on 2018-08-02 to reload layout editor when module is changed
			var mode = $('.layoutTabs').find('li.active').find('a').data('mode');
			var url = 'index.php?module=LayoutEditor&parent=Settings&view=Index&sourceModule='+ selectedModule +'&mode='+ mode;
			location.href = url;
			return;
			// End Hieu Nguyen
	...
	registerEditFieldDetailsClick: function (contents) {
		...
		contents.on('click', '.editFieldDetails', function (e) {
			...
			params['fieldid'] = fieldId;
			params['sourceModule'] = jQuery('#selectedModuleName').val();
			params['layouteditor_tab'] = jQuery('.selectedTab').val();	// Added by Hieu Nguyen on 2021-10-21 to determine which view is editing in ajax request
	...
	// Added by Hieu Nguyen on 2018-07-30 to support hide field right at field list
	registerBtnHideFieldClick: function (contents) {
		var thisInstance = this;

		contents.on('click', '.btnHideField', function (e) {
			var fieldContatiner = $(e.currentTarget).closest('li');
			var fieldPropertiesHolder = fieldContatiner.find('.fieldProperties');
			...
			var params = {
				fieldid: fieldId,
				fieldname: fieldName,
				presence: 1, 	// 0: Always display, 1: Hide, 2: Show
			}
	...
	// Added by Hieu Nguyen on 2018-07-31 to support rename block
	registerBtnEditBlockClick: function () {
		var thisInstance = this;
		var contents = jQuery('#layoutEditorContainer').find('.contents');

		contents.on('click', '.btnEditBlock', function (e) {
			var blockElement = jQuery(e.currentTarget).closest('.editFieldsTable');
			var blockId = blockElement.data('blockId');
			var blockLabelKey = blockElement.find('.translatedBlockLabel').data('labelKey');
			var blockLabelDisplayEn = blockElement.data('labelDisplayEn');
			var blockLabelDisplayVn = blockElement.data('labelDisplayVn');
			var editBlockContainer = contents.find('.editBlockModal').clone(true, true);

			var callBackFunction = function (data) {
				data.find('.editBlockModal').removeClass('hide');

				var form = data.find('.editBlockForm');
				thisInstance.setBlocksListArray(form);
				var blockLabel = form.find('[name="label"]');
				blockLabel.attr('readonly', true);
				...
				var params = {
					submitHandler: function (form) {
						...
						// Validate form
						var blockLabelValue = jQuery.trim(blockLabel.val());

						// Modified by Hieu Nguyen on 2018-08-06 to allow underscore character
						var pattern = /^[a-zA-Z_][a-zA-Z0-9_]*$/;

						if (!pattern.test(blockLabelValue)) {
							var errorInfo = app.vtranslate('JS_SPECIAL_CHARACTERS') + ' ' + app.vtranslate('JS_NOT_ALLOWED');
							vtUtils.showValidationMessage(blockLabel, errorInfo, {
								position: {
									my: 'bottom left',
									at: 'top left',
									container: form
						...
						if (jQuery.inArray(blockLabelValue, thisInstance.blockNamesList) >= 0) {
							var message = app.vtranslate('JS_BLOCK_NAME_EXISTS');

							vtUtils.showValidationMessage(blockLabel, message, {
								position: {
									my: 'bottom left',
									at: 'top left',
									container: form
						...
						// Added by Hieu Nguyen on 2021-06-14 to track label changes
						if (form.find('[name="labelDisplayEn"]').val() != blockLabelDisplayEn) {
							form.find('[name="labelDisplayEnChanged"]').val(1);
						}

						if (form.find('[name="labelDisplayVn"]').val() != blockLabelDisplayVn) {
							form.find('[name="labelDisplayVnChanged"]').val(1);
						}
						// End Hieu Nguyen
						...
						thisInstance.saveBlockDetails(form).then(
							function (data) {
								...
								if (data) {
									blockElement.find('.translatedBlockLabel').text(data['labelDisplay']);	// Modified by Hieu Nguyen on 2021-06-15 to display label instead of key

									params['message'] = app.vtranslate('JS_UPDATE_BLOCK_INFO_SUCCESS_MSG');
									app.helper.showSuccessNotification(params);
				...
				form.vtValidate(params);
				form.find('[name="blockid"]').val(blockId);
				form.find('[name="label"]').val(blockLabelKey);
				form.find('[name="labelDisplayEn"]').val(blockLabelDisplayEn);
				form.find('[name="labelDisplayVn"]').val(blockLabelDisplayVn);
			};
			...
			app.helper.showModal(editBlockContainer, modalParams);
		});
	},
	// End Hieu Nguyen
	...
	registerEventForCollapseBlock: function () {
		jQuery('#moduleBlocks').on('switchChange.bootstrapSwitch', "input[name='collapseBlock']", function (e) {
			...
			var params = {
				...
				action: jQuery('.selectedTab').val() == 'editViewTab' ? 'EditViewBlock' : 'Block',	// Modified by Hieu Nguyen on 2018-07-30
				mode: 'save',
				blockid: currentElement.data('blockId'),
				display_status: currentElement.val(),
				layouteditor_tab: jQuery('.selectedTab').val(),	// Added by Hieu Nguyen on 2021-08-26 to determine which view is editing in ajax request
			}
	...
	registerBlockEvents: function () {
		...
		thisInstance.registerBtnEditBlockClick();	// Added by Hieu Nguyen on 2018-08-01
	...
	registerFieldEvents: function (contents) {
		...
		thisInstance.registerBtnHideFieldClick(contents);	// Added by Hieu Nguyen on 2018-08-01 
	...
	triggerFieldListTabClickEvent: function () {
		...
		var activeLayoutEditorTab = contents.find('.layoutEditorTab.active');

		activeLayoutEditorTab.click(function (e) {
			var layoutEditor = contents.find(activeLayoutEditorTab.find('a').attr('href'));

			thisInstance.showFieldsListUI(layoutEditor, e).then(function (data) {
				if (!thisInstance.fieldListTabClicked) {
					...
					thisInstance.registerCopyLayoutFromEditViewEvent();	// Added by Hieu Nguyen on 2018-08-01
...
jQuery(function($) {
	// Handle navigating for reload tabs
	$('.reload-tab').click(function() {
		if (!$(this).hasClass('active')) {
			var selectedModule = $('[name="layoutEditorModules"]').val();
			var mode = $(this).find('a').data('mode');
			var url = 'index.php?module=LayoutEditor&parent=Settings&view=Index&sourceModule=' + selectedModule + '&mode=' + mode;
	...
	// Add custom edit block button for both editors
	$('.blockLabel').each(function () {
		$(this).append($('#btnEditBlockTemplate').html());
	});

	// Custom logic for detail view editor
	/*if ($('.selectedTab').val() == 'detailViewTab') {
		$('.editFieldDetails, .deleteCustomField').remove();

		$('.switch').each(function () {
			$(this).prop('title', '');
			$(this).removeClass('switch');
			$(this).addClass('cursorPointerNotAllowed');
		});
	}*/

	// Add custom hide field button for both editors
	$('.fieldLabelContainer').each(function() {
        if ($('.selectedTab').val() == 'editViewTab') {
            // Only allow user to hide field in EditView if it is not required
            if ($(this).find('.redColor')[0] == null) {
                $(this).closest('li').find('.actions').append($('#btnHideFieldTemplate').html());
            }
        }
        else {
            // Allow to hide all fields in DetailView
            $(this).closest('li').find('.actions').append($('#btnHideFieldTemplate').html());
        }
	});

	// Automatically convert english label display into label key
	$('.addBlockModal').find('input[name="labelDisplayEn"]').on('keyup', function (e) {
		var form = $(e.currentTarget).closest('form');
		var blockLabelDisplayEn = $(e.currentTarget);
		var blockLabel = form.find('input[name="label"]');

		var value = blockLabelDisplayEn.val().trim();
		var generatedLabel = 'LBL_' + value.toUpperCase().replace(/\s/g, '_');
		blockLabel.val(generatedLabel);
	});

	// Handle saving translation
	$('.btnSaveTranslation').click(function() {
		if(!confirm(app.vtranslate('JS_TRANSLATION_EDITOR_SAVE_CONFIRM_MSG'))) {
			return;
		}
		...
		params['mode'] = 'saveTranslation';
		params['lang'] = $(this).data('lang');

		// Track changed lables
		params['changed_labels'] = { 'en_us': [], 'vn_vn': [] };

		form.find('textarea').each(function () {
			var input = $(this);

			if (input.val() != input.data('curValue')) {
				params['changed_labels'][input.data('lang')].push(input.data('labelKey'));
			}
		});
		...
		.then(function (err, data) {
			...
			if (err === null) {
				app.helper.showSuccessNotification({'message': app.vtranslate('JS_TRANSLATION_EDITOR_SAVE_SUCCESS_MSG')});
			} 
			else {
				app.helper.showErrorNotification({'message': app.vtranslate('JS_TRANSLATION_EDITOR_SAVE_ERROR_MSG')});
			}

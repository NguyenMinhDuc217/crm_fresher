...
{strip}
	...
	<div id="btnEditBlockTemplate" style="display: none">
		<a href="javascript:void(0)" class="btnEditBlock" style="margin-left: 5px" title="{vtranslate('LBL_EDIT', $QUALIFIED_MODULE)}">
			<i class="far fa-pen"></i>
		</a>
	</div>

	<div id="btnHideFieldTemplate" style="display: none">
		<a href="javascript:void(0)" class="btnHideField" style="margin-left: 5px" title="{vtranslate('LBL_HIDE', $QUALIFIED_MODULE)}">
	...
	<div class="row fieldsListContainer" style="padding:1% 0">
		<div class="col-sm-6">
			...
			<button class="btn btn-primary addCustomBlock" type="button">
				<i class="far fa-plus"></i>&nbsp;
				{vtranslate('LBL_ADD_CUSTOM_BLOCK', $QUALIFIED_MODULE)}
			</button>
			&nbsp;
			{if $SELECTED_TAB eq 'detailViewTab'}
				<button class="btn btn-primary copyLayoutFromEditView" type="button">
					<i class="far fa-clone"></i>&nbsp;
					{vtranslate('LBL_COPY_LAYOUT_FROM_EDITVIEW', $QUALIFIED_MODULE)}
				</button>
			{/if}
			{* End Hieu Nguyen *}
		</div>
		...
		<div class="col-sm-12">
			<div id="moduleBlocks" style="margin-top:17px;">
				{foreach key=BLOCK_LABEL_KEY item=BLOCK_MODEL from=$BLOCKS}
					...
					{/if}
					<div id="block_{$BLOCK_ID}" class="editFieldsTable block_{$BLOCK_ID} marginBottom10px border1px {if $IS_BLOCK_SORTABLE} blockSortable{/if}" 
						data-block-id="{$BLOCK_ID}" data-sequence="{$BLOCK_MODEL->get('sequence')}" style="background: white;"
						data-label-display-en="{$BLOCK_MODEL->labelDisplayEn}" data-label-display-vn="{$BLOCK_MODEL->labelDisplayVn}"
							...
							<div class="layoutBlockHeader row">
								<div class="blockLabel col-sm-3 padding10 marginLeftZero" style="word-break: break-all;">
									{if $IS_BLOCK_SORTABLE}
										<i class="far fa-grip-lines cursorPointerMove"></i>&nbsp;&nbsp;
									{/if}

									{* Modified by Hieu Nguyen on 2018-08-06 *}
									<strong class="translatedBlockLabel" data-label-key="{$BLOCK_LABEL_KEY}">{vtranslate($BLOCK_LABEL_KEY, $SELECTED_MODULE_NAME)}</strong>
									{* End Hieu Nguyen *}
								</div>
								<div class="col-sm-9 padding10 marginLeftZero">
									<div class="blockActions" style="float:right !important;">
										{* Modified by Hieu Nguyen on 2018-07-20 *}
										{if $SELECTED_TAB eq 'detailViewTab'}
										<span>
											<i class="far fa-info-circle" title="{vtranslate('LBL_COLLAPSE_BLOCK_DETAIL_VIEW', $QUALIFIED_MODULE)}"></i>&nbsp; {vtranslate('LBL_COLLAPSE_BLOCK', $QUALIFIED_MODULE)}&nbsp;
										...
										{if $BLOCK_MODEL->isAddCustomFieldEnabled() && $SELECTED_TAB eq 'editViewTab'}
											<button class="btn btn-default addButton btn-sm addCustomField" type="button">
												<i class="far fa-plus"></i>&nbsp;&nbsp;{vtranslate('LBL_ADD_CUSTOM_FIELD', $QUALIFIED_MODULE)}
											</button>&nbsp;
										{/if}
										{* End Hieu Nguyen *}
										...
										{if $BLOCK_MODEL->isActionsAllowed()}
											...
											{if $BLOCK_MODEL->isDeletable()}	{* Modified by Hieu Nguyen on 2021-07-29 to check if block can be deleted by customer, dev or R&D *}
														...
														<div class="row">
															...
															<span class="col-sm-1">&nbsp;
																{if $IS_FIELDS_SORTABLE}
																	<i class="far fa-grip-lines cursorPointerMove" border="0" title="{vtranslate('LBL_DRAG',$QUALIFIED_MODULE)}"></i>
																{/if}
															</span>
													...
													<div class="row " style="padding: 10px 0px;">
														...
														<div class="fieldProperties col-sm-10" data-field-id="{$FIELD_MODEL->get('id')}">
															<span class="mandatory switch text-capitalize {if (!$IS_MANDATORY)}disabled{/if} {if $FIELD_MODEL->isMandatoryOptionDisabled()} cursorPointerNotAllowed {else} cursorPointer {/if}"
																...
																<i class="far fa-exclamation-circle" data-name="mandatory" 
															...
															<span class="quickCreate switch {if (!$IS_QUICK_EDIT_ENABLED)}disabled{/if} 
																...
																<i class="far fa-plus" data-name="quickcreate" 
															...
															<span class="header switch {if (!$IS_HEADER_FIELD)} disabled {/if} 
																...
																<i class="far fa-flag" data-name="headerfield" 
															...
															<span class="summary switch {if (!$IS_SUMMARY_VIEW_ENABLED)} disabled {/if} 
																...
																<i class="far fa-key" data-name="summaryfield" 
																	data-enable-value="1" data-disable-value="0"
														...
														<span class="col-sm-2 actions">
															{if $FIELD_MODEL->isEditable()}
																<a href="javascript:void(0)" class="editFieldDetails">
																	<i class="far fa-pen" title="{vtranslate('LBL_EDIT', $QUALIFIED_MODULE)}"></i>
																</a>
															{/if}
															{if $FIELD_MODEL->isDeletable()}	{* Modified by Hieu Nguyen on 2021-07-29 to check if field can be deleted by customer, dev or R&D *}
																<a href="javascript:void(0)" class="deleteCustomField pull-right" data-field-id="{$FIELD_MODEL->get('id')}"
																	...
																	{/if} >
																	<i class="far fa-trash-alt" title="{vtranslate('LBL_DELETE', $QUALIFIED_MODULE)}"></i>
																</a>
															{/if}
														</span>
													</div>
									...
									{if $BLOCK_MODEL->isAddCustomFieldEnabled() && $SELECTED_TAB eq 'editViewTab'}
										<li class="row dummyRow">
											...
											<span class="col-sm-4" style="margin-top: 7%;margin-left: -15%;">
												<button class="btn btn-default btn-sm addButton"><i class="far fa-plus"></i>&nbsp;&nbsp;{vtranslate('LBL_ADD',$QUALIFIED_MODULE)}</button>
											</span>
										</li>
									{/if}
								{/if}
			...
			<div class="col-sm-4">
				<div class="marginLeftZero" data-field-id="" data-sequence="">
					<div class="row">
						<span class="col-sm-1">&nbsp;
							{if $IS_SORTABLE}
								<i class="far fa-grip-lines dragImage" border="0" title="{vtranslate('LBL_DRAG',$QUALIFIED_MODULE)}"></i>
							{/if}
						</span>
						<div class="col-sm-9" style="word-wrap: break-word;">
							<div class="fieldLabelContainer row">
	...
	<div class="modal-dialog modal-content addBlockModal hide">
		...
		<form class="form-horizontal addCustomBlockForm">
			<div class="modal-body">
				{* Modified by Hieu Nguyen on 2018-08-06 *}
				{include file=vtemplate_path('BlockFormFields.tpl', $QUALIFIED_MODULE)}
				{* End Hieu Nguyen *}
	...
	{* Added by Hieu Nguyen on 2018-07-31 *}
	<div class="modal-dialog modal-content editBlockModal hide">
		{assign var=HEADER_TITLE value={vtranslate('LBL_EDIT_BLOCK', $QUALIFIED_MODULE)}}
		{include file="ModalHeader.tpl"|vtemplate_path:$MODULE TITLE=$HEADER_TITLE}

		<form class="form-horizontal editBlockForm padding10">
	...
	<div class="modal-dialog inactiveFieldsModal hide">
		<div class="modal-header">
			...
			<h4>{vtranslate('LBL_INACTIVE_FIELDS', $QUALIFIED_MODULE)}</h4>
		</div>

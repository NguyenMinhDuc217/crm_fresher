...
Settings_Vtiger_Edit_Js('Settings_Webforms_Edit_Js', {}, {
    ...
    // Added by Hieu Nguyen on 2020-08-25 to handle init form event
    registerFormInitEvent: function (form) {
        // Init round robbin user list
        var roundRobinUsersInput = form.find('[name="roundrobin_userid"]');

        if (form.find('[name=record]').val() == '' || form.find('.roundrobin_userid > [name="current_owner_id"]').val() == 1) {
            roundRobinUsersInput.attr('data-selected-tags', '');
        }

        CustomOwnerField.initCustomOwnerFields(roundRobinUsersInput);
    },
	...
	registerAddFileFieldEvent: function (form) {
		...
		container.find('#addFileFieldBtn').click(function (e) {
			...
			var html = '<tr>\n\
							...
							<td class="textAlignCenter" style="vertical-align: middle;">\n\
								<a class="removeFileField" style="color: black;"><i class="far fa-trash-alt icon-trash"></i></a>\n\
							</td>\n\
						</tr>';
...
//	},
	...
	registerEvents : function() {
        ...
        this.registerFormInitEvent(form);   // Added by Hieu Nguyen on 2020-08-21

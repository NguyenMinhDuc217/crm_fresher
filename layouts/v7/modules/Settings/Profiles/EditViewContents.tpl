			...
			<div class="col-lg-6 col-md-6 col-sm-6">
				<label class="control-label">
					...
					<span style="margin-left: 10px">
						<i class="far fa-info-circle" title="{vtranslate('LBL_VIEW_ALL_DESC',$QUALIFIED_MODULE)}"></i>
					</span>
				</label>
				<br>
				<label class="control-label">
					...
					<span style="margin-left: 15px">
						<i class="far fa-info-circle" title="{vtranslate('LBL_EDIT_ALL_DESC',$QUALIFIED_MODULE)}"></i>
					</span>
				</label>
				<br><br>
			</div>
									...
									{if $PROFILE_MODULE->getFields() && $PROFILE_MODULE->isEntityModule() }
										...
										<table class="table table-bordered no-border">
											...
											{foreach from=$PROFILE_MODULE->getFields() key=FIELD_NAME item=FIELD_MODEL name="fields"}
												...
												{if $FIELD_MODEL->get('uitype') != '83' && $FIELD_MODEL->get('displaytype') neq '6'}	{* Modified by Hieu Nguyen on 2021-10-21 to display field without depending on which view it belong to *}
													{if $COUNTER % 3 == 0}
														<tr>
													{/if}
													<td >
										...
										{if $MODULE_NAME eq 'Calendar'}
											...
											<table class="table table-bordered">
												{foreach from=$EVENT_MODULE->getFields() key=FIELD_NAME item=FIELD_MODEL name="fields"}
													{* Modified by Hieu Nguyen on 2021-10-21 to display field without depending on which view it belong to *}
													{assign var="FIELD_ID" value=$FIELD_MODEL->getId()}

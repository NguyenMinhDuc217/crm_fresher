								...
								<tr class="listViewEntries" data-id="{$LISTVIEW_ENTRY->getId()}"
									...
									{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
										...
										<td class="listViewEntryValue textOverflowEllipsis {$WIDTHTYPE}" width="{$WIDTH}%" nowrap>
											...
											{*--Begin: Modified by Kelvin Thang on 2020-01-10 set transform Display Value for Currency/Nunber fields *}
											{if $LISTVIEW_HEADERNAME == 'conversion_rate' && $MODULE eq 'Currency'}
												{$LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME)}
											{* Added by Hieu Nguyen on 2020-08-12 to render link to access record DetailView *}
											{elseif strpos($LISTVIEW_HEADERNAME, 'name') >= 0 && method_exists($LISTVIEW_ENTRY, 'getDetailViewUrl')}
												<a href="{$LISTVIEW_ENTRY->getDetailViewUrl()}">{$LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME)}</a>
											{* End Hieu Nguyen *}
											{else}
												{$LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME)}
											{/if}
											{*--End: Modified by Kelvin Thang on 2020-01-10 set transform Display Value for Currency/Nunber fields *}

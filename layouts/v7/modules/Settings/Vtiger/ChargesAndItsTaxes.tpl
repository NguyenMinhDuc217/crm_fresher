	...
	<div class="col-lg-6">
		<div class="marginBottom10px">
			<button type="button" class="btn btn-default addCharge addButton module-buttons" data-url="{Inventory_Charges_Model::getCreateChargeUrl()}" data-type="1">
                <i class="far fa-plus"></i>&nbsp;&nbsp;{vtranslate('LBL_ADD_NEW_CHARGE', $QUALIFIED_MODULE)}</button>
		</div>
				...
				<tr class="opacity" data-charge-id="{$CHARGE_MODEL->getId()}">
					...
					<td class="{$WIDTHTYPE}" style="border-right:none;border-left:none">
						<div class="pull-right actions">
							<a class="editCharge cursorPointer" data-url="{$CHARGE_MODEL->getEditChargeUrl()}"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen alignMiddle"></i></a>
						</div>
					</td>
				</tr>
			{/foreach}
	...
	<div class="col-lg-6">
		<div class="marginBottom10px">
			<button type="button" class="btn btn-default addChargeTax addButton module-buttons" data-url="{$CREATE_TAX_URL}" data-type="1">
                <i class="far fa-plus"></i>&nbsp;&nbsp;{vtranslate('LBL_ADD_NEW_TAX_FOR_CHARGE', $QUALIFIED_MODULE)}</button>
		</div>
		<table class="table table-bordered shippingTaxTable">
			...
			{foreach item=CHARGE_TAX_MODEL from=$CHARGE_TAXES}
				<tr class="opacity" data-taxid="{$CHARGE_TAX_MODEL->get('taxid')}" data-taxtype="{$CHARGE_TAX_MODEL->getType()}">
					...
					<td class="{$WIDTHTYPE}" style="border-right:none;border-left:none"><span class="taxType">{vtranslate($CHARGE_TAX_MODEL->getTaxType(), $QUALIFIED_MODULE)}</span></td>
					<td class="{$WIDTHTYPE}" style="border-right:none;border-left:none"><span class="taxMethod">{vtranslate($CHARGE_TAX_MODEL->getTaxMethod(), $QUALIFIED_MODULE)}</span></td>
					<td class="{$WIDTHTYPE}" style="border-right:none;border-left:none"><span class="taxPercentage">{$CHARGE_TAX_MODEL->getTax()}%</span></td>
					<td class="{$WIDTHTYPE}" style="border-right:none;border-left:none"><div class="pull-right"><input type="checkbox" class="editTaxStatus" {if !$CHARGE_TAX_MODEL->isDeleted()}checked{/if} /></div></td>
					<td style="border-left:none;border-right:none;" class="{$WIDTHTYPE}">
						<div class="pull-right actions">
							<a class="editChargeTax cursorPointer" data-url="{$CHARGE_TAX_MODEL->getEditTaxUrl()}"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen alignMiddle"></i></a>
						</div>
					</td>
				</tr>
			{/foreach}

					...
					<div class="col-lg-6">
						...
						<div class="marginBottom10px">
							<button type="button" class="btn btn-default addTax addButton btn-default module-buttons" data-url="{$CREATE_TAX_URL}" data-type="0">
								<i class="far fa-plus"></i>&nbsp;&nbsp;{vtranslate('LBL_ADD_NEW_TAX', $QUALIFIED_MODULE)}
							</button>
						</div>
						<table class="table table-bordered inventoryTaxTable">
							...
							{foreach item=PRODUCT_SERVICE_TAX_MODEL from=$PRODUCT_AND_SERVICES_TAXES}
								<tr class="opacity" data-taxid="{$PRODUCT_SERVICE_TAX_MODEL->get('taxid')}" data-taxtype="{$PRODUCT_SERVICE_TAX_MODEL->getType()}">
									...
									<td style="border-left:none;border-right:none;" class="{$WIDTHTYPE}"><span class="taxType">{vtranslate($PRODUCT_SERVICE_TAX_MODEL->getTaxType(), $QUALIFIED_MODULE)}</span></td>
									<td style="border-left:none;border-right:none;" class="{$WIDTHTYPE}"><span class="taxMethod">{vtranslate($PRODUCT_SERVICE_TAX_MODEL->getTaxMethod(), $QUALIFIED_MODULE)}</span></td>
									...
									<td style="border-left:none;border-right:none;" class="{$WIDTHTYPE}"><div class="pull-right"><input type="checkbox" class="editTaxStatus" {if !$PRODUCT_SERVICE_TAX_MODEL->isDeleted()}checked{/if} /></div></td>
									<td style="border-left:none;border-right:none;" class="{$WIDTHTYPE}">
										<div class="pull-right actions">
											<a class="editTax cursorPointer" data-url="{$PRODUCT_SERVICE_TAX_MODEL->getEditTaxUrl()}"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen alignMiddle"></i></a>&nbsp;
										</div>
									</td>
								</tr>
							{/foreach}

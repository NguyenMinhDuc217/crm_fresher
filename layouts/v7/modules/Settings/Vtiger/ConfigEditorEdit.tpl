			...
			<div class="contents">
				<form id="ConfigEditorForm" class="form-horizontal" data-detail-url="{$MODEL->getDetailViewUrl()}" method="POST">
					...
					{assign var=FIELD_VALIDATION  value=['HELPDESK_SUPPORT_EMAIL_ID'	=> 'data-rule-email="true"',
																	'upload_maxsize'	=> 'data-rule-range=[1,256] data-rule-positive="true" data-rule-wholeNumber="true"',
															...
															'minilist_widget_max_columns' => 'data-rule-range=[1,12] data-rule-positive="true" data-rule-wholeNumber="true"']} {*-- Added By Kelvin Thang on 2020-01-07 config set minilist widget max columns in dashboard*}
								...
								<div  class="{$WIDTHTYPE}  col-lg-4 input-group">
									{if $FIELD_DETAILS['fieldType'] == 'picklist'}
										...
										{if $FIELD_NAME == 'default_reply_to'}
											<div class="input-group-addon input-select-addon"><i class="far fa-info-circle" data-toggle="tooltip" data-placement="right" title="{vtranslate('LBL_DEFAULT_REPLY_TO_INFO',$QUALIFIED_MODULE)}"></i></div>
										{/if}
									...
									{else if $FIELD_NAME == 'USE_RTE'}
										...
										<div class="input-info-addon"> <i class="far fa-question-circle"  data-toggle="tooltip" data-placement="right" title="{vtranslate('LBL_PERSONAL_EMAIL_TRACKING_INFO',$QUALIFIED_MODULE)}"></i>
										</div>
									{else}

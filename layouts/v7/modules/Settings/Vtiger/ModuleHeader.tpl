		...
		<div class="module-action-content clearfix">
			<div class="col-lg-7 col-md-7">
				{if $USER_MODEL->isAdminUser()}
					...
					&nbsp;<span class="far fa-angle-right pull-left {if $VIEW eq 'Index' && $MODULE eq 'Vtiger'} hide {/if}" aria-hidden="true" style="padding-top: 12px;padding-left: 5px; padding-right: 5px;"></span>
				{/if}
					...
					{if $MODULE neq 'Vtiger'}
						...
						<span class="current-filter-name settingModuleName filter-name pull-left">
							{if $smarty.request.view eq 'Calendar'}
								...
								{else}
									{vtranslate({$PAGETITLE}, $QUALIFIED_MODULE)}&nbsp;<span class="far fa-angle-right" aria-hidden="true"></span>&nbsp;{$USER_MODEL->getName()}
								{/if}
					...
					<ul class="nav navbar-nav">
						{foreach item=BASIC_ACTION from=$MODULE_BASIC_ACTIONS}
							<li>
								...
								{else}
									<button type="button" class="btn addButton btn-primary module-buttons" 
							...
							<li>
								<div class="settingsIcon">
									<button type="button" class="btn btn-default module-buttons dropdown-toggle" data-toggle="dropdown" aria-expanded="false" title="{vtranslate('LBL_SETTINGS', $MODULE)}">
										<span class="far fa-wrench" aria-hidden="true"></span>&nbsp; <i class="far fa-angle-down"></i>
									</button>
						...
						{if $LISTVIEW_LINKS['LISTVIEWBASIC']|@count gt 0 and !in_array($MODULE, $RESTRICTED_MODULE_LIST)}
							...
							{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
								...
								<li class="{if in_array($MODULE, ['Roles', 'Profiles']) && isForbiddenFeature('CreateNewRolesAndProfiles')}hide{/if} {if $MODULE eq 'Workflows' && isForbiddenFeature('CreateNewWorkflows')}hide{/if}">	{* Modified by Hieu Nguyen on 2022-03-01 to check forbidden feature *}

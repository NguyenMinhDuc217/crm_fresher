		...
		<div class="modal-content">
			...
			<div class="modal-body form-horizontal">
				{foreach item=APP_NAME from=$APP_ARRAY}
					{* Refactored by Hieu Nguyen on 2021-8-18 *}
					{assign var=AVAILABLE_MODULES value=Settings_MenuEditor_Module_Model::getAvailableModules($APP_NAME)}
					
					<div class="row modulesContainer {if $APP_NAME neq $SELECTED_APP_NAME} hide {/if}" data-appname="{$APP_NAME}">
						<div class="col-lg-12 col-md-12 col-sm-12">
							{if count($AVAILABLE_MODULES) gt 0}
								{foreach item=MODULE_NAME from=$AVAILABLE_MODULES}
									<span class="btn-group" style="margin-bottom: 10px; margin-left: 25px; margin-right: -15px;">
										<buttton class="btn addButton btn-primary module-buttons addModule" data-module="{$MODULE_NAME}" style="text-transform: inherit;margin-right:15px">{vtranslate($MODULE_NAME, $MODULE_NAME)}&nbsp;&nbsp;

...
jQuery.Class('Settings_Menu_Editor_Js', {}, {
		...
		container.on('click', '.menuEditorAddItem', function(e) {
			...
			app.request.post({data: params}).then(function(err, data){
				...
				app.helper.showModal(data, {cb: function(data){
					...
					// Added by Phu Vo on 2021.08.31 to enable scroll bar
					app.helper.showHorizontalScroll($(data).find('.modal-body'));
					// End Phu Vo
				}});
			});
		});
	...
	registerAddModulePreSaveEvents : function(data) {
		...
		container.on('click', '.moduleSelection li a', function(){
			...
			$(this).parents('.btn-group').find('.dropdown-toggle').html(selText+'&nbsp;&nbsp; <i class="far fa-angle-down"></i>');
	...
	registerScrollbar: function() {
		app.helper.showHorizontalScroll($('.settingsPageDiv'));
	},
	...
	registerEvents : function() {
		...
		// Added by Phu 2021.08.31 to enable scroll bar

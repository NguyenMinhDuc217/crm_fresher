...
jQuery(function($) {
	...
	// Handle click event for button Reset Service
	$('.btnResetService').on('click', function () {
		if (!confirm(app.vtranslate('JS_RESET_SERVICE_CONFIRM_MSG'))) {
			return false;
		}
		...
		app.request.post({data: params}).then(function (err, res) {
			if (!err && res && res.success) {
				var successMsg = app.vtranslate('JS_RESET_SERVICE_SUCCESS_MSG');
				app.helper.showSuccessNotification({ 'message': successMsg });

				location.reload();
				return;
			}

			var errorMsg = app.vtranslate('JS_RESET_SERVICE_ERROR_MSG');
			app.helper.showErrorNotification({ 'message': errorMsg });
		});
	});

    // Handle click event for button Test Service
	$('.btnTestService').on('click', function () {
		if (!confirm(app.vtranslate('JS_TEST_SERVICE_CONFIRM_MSG'))) {
			return false;
		}
		...
		app.request.post({data: params}).then(function (err, res) {
			if (!err && res && res.success) {
				var successMsg = app.vtranslate('JS_TEST_SERVICE_SUCCESS_MSG');
				app.helper.showSuccessNotification({ 'message': successMsg });
				return;
			}

			var errorMsg = app.vtranslate('JS_TEST_SERVICE_ERROR_MSG');
			app.helper.showErrorNotification({ 'message': errorMsg });
		});
	});
});

...
Settings_Vtiger_Index_Js("Settings_PBXManager_Index_Js", {}, {
	...
	registerEditViewEvents: function () {
		...
		// Added by Hieu Nguyen on 2018-10-05
		var gatewayField = form.find('select[name="gateway"]');

		gatewayField.change(function() {
			var url = gatewayField.data('url') + '&gateway=' + gatewayField.val();
			app.helper.showProgress();

			// Reload the form to show the selected gateway params
			thisInstance.loadContents(url).then(
				function(err, data) {
					// Re-register the form event after re-loading

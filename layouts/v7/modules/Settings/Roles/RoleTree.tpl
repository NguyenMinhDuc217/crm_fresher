        ...
        <div class="toolbar-handle">
            ...
            {if $smarty.request.view != 'Popup'}
                <div class="toolbar">
                    ...
                    {if !isForbiddenFeature('CreateNewRolesAndProfiles')}
                        &nbsp;<a href="{$CHILD_ROLE->getCreateChildUrl()}" data-url="{$CHILD_ROLE->getCreateChildUrl()}" title="{vtranslate('LBL_ADD_RECORD', $QUALIFIED_MODULE)}"><span class="far fa-plus-circle"></span></a>
                    {/if}
                    {* End Hieu Nguyen *}
                    &nbsp;<a data-id="{$CHILD_ROLE->getId()}" href="javascript:;" data-url="{$CHILD_ROLE->getDeleteActionUrl()}" data-action="modal" title="{vtranslate('LBL_DELETE', $QUALIFIED_MODULE)}"><span class="far fa-trash-alt"></span></a>
                </div>
            {/if}
        </div>

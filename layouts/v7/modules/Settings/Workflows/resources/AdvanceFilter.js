...
Vtiger_AdvanceFilter_Js('Workflows_AdvanceFilter_Js',{},{
    ...
    loadConditions : function(fieldSelect) {
        ...
        // Added by Hieu Nguyen on 2020-07-30 to rebuild the condition options for user reference field
        let fieldInfo = fieldSelected.data('fieldinfo');

        if (this.getModuleName() == 'Workflows' && (fieldInfo.name == 'createdby' || fieldInfo.name == 'modifiedby')) {
            conditionList = ['has changed', 'is', 'is not'];
        }
        // End Hieu Nguyen

        // Added by Hieu Nguyen on 2022-05-06 to rebuild the condition options for tags field (NOTE: Currently for tags field in MAIN MODULE ONLY!)
        if (fieldInfo.name == 'tags') {
            conditionList = ['contains'];  // Support only option: contains
        }
        // End Hieu Nguyen
    ...
    // Added by Hieu Nguyen on 2020-07-31 to handle condition change event
    registerConditionChange : function() {
		...
		filterContainer.on('change', 'select[name="comparator"]', function (e) {
			...
			var conditionFieldInput = conditionRow.find('select[name="columnname"]');
            var conditionComparatorInput = $(this);
            var conditionValueInputContainer = conditionRow.find('.fieldUiHolder');

			// Load field UI and init validation
			thisInstance.loadFieldSpecificUi(conditionFieldInput);
			thisInstance.addValidationToFieldIfNeeded(conditionFieldInput);

            // Hide value input if the comparator is in list [has changed, is empty, is not empty]
            if ($.inArray(conditionComparatorInput.val(), ['has changed', 'is empty', 'is not empty']) !== -1) {
                conditionValueInputContainer.removeClass('visible').addClass('invisible');
                conditionValueInputContainer.find(':input, select').val('');
            }
            else {
                conditionValueInputContainer.removeClass('invisible').addClass('visible');
            }
		});
	},
    // End Hieu Nguyen
				...
				if (fieldType == 'picklist' || fieldType == 'multipicklist') {
                    for(var key in fieldList) {
                        ...
                        if(field == 'value' && valueSelectElement.is('input')) {
                            // Modified by Hieu Nguyen on 2021-06-18 to get values from select2 input
                            var selectedValues = valueSelectElement.val();
                            rowValues[field] = selectedValues;
                            // End Hieu Nguyen
                    ...
                    for(var key in fieldList) {
                        ...
                        if(field == 'value'){
                            ...
                            else if (fieldType == 'integer' || field == 'decimal' || fieldType == 'currency' || fieldType == 'double') { // Modified by Phu Vo on 2021.06.14 to add type double
                                rowValues[field] = app.unformatCurrencyToUser(valueSelectElement.val());
                            }
                            // End Hieu Nguyen
                            else {
                                rowValues[field] = valueSelectElement.val();
...
// Modified by Hieu Nguyen on 2021-06-18 to fix bug label translate error when edit workflow
Vtiger_Picklist_Field_Js('Workflows_Picklist_Field_Js', {}, {
    ...
    getUi: function () {
        ...
        jQuery.map(pickListValues, function(label, key) {
            tags.push({ id: key, text: label });
        });
        ...
        selectContainer.data('tags', tags).data('picklistvalues', pickListValues).data('maximumSelectionSize', 1);
	...
	getUi: function () {
		...
		jQuery.map(pickListValues, function (label, key) {
			if (jQuery.inArray(key, selectedOptions) !== -1) {
				selectedOptionsValue += key + ',';
			}

			tags.push({ id: key, text: label });
		})

		selectedOptionsValue = selectedOptionsValue.substring(0, selectedOptionsValue.lastIndexOf(','));
		var html = '<input type="hidden" class="col-lg-12 select2" name="'+ this.getName() +'[]" id="'+ this.getName() +'" data-fieldtype="multipicklist">';
		var selectContainer = jQuery(html).val(selectedOptionsValue);
		selectContainer.data('tags', tags).data('picklistvalues', pickListValues);
		selectContainer.data('placeholder', app.vtranslate('JS_PLEASE_SELECT_ATLEAST_ONE_OPTION')).data('closeOnSelect', true);
		this.addValidationToElement(selectContainer);
		return selectContainer;
	}

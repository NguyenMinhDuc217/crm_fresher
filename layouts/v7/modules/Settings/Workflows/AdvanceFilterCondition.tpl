...
<div class="row conditionRow" style="margin-bottom: 10px;">
		...
		<select class="{if empty($NOCHOSEN)}select2{/if} col-lg-12" name="columnname" data-placeholder="{vtranslate('LBL_SELECT_FIELD',$QUALIFIED_MODULE)}">
			...
			{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
					...
					{* Added by Hieu Nguyen on 2022-05-06 to support tags field in workflow (NOTE: Currently for tags field in MAIN MODULE ONLY!) *}
					{if $FIELD_NAME eq 'tags'}
						{$FIELD_INFO['tag_list'] = Vtiger_Tag_Model::getAllPublicTags()}
						{$FIELD_INFO['value'] = join(', ', $CONDITION_INFO['value'])}
					{/if}
					{* End Hieu Nguyen *}
    ...
    <span class="col-lg-1" style="line-height: 30px;">
		<i class="deleteCondition far fa-trash-alt cursorPointer" title="{vtranslate('LBL_DELETE', $MODULE)}"></i>
	</span>
</div>
{/strip}
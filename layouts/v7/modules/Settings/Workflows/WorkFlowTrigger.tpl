        ...
        <div class="col-sm-6 controls">
			...
			<span><input type="radio" name="workflow_trigger" value="6" {if $EXECUTION_CONDITION eq '6'} checked="" {else if $SCHEDULED_WORKFLOW_COUNT >= $MAX_ALLOWED_SCHEDULED_WORKFLOWS} disabled="disabled" {/if}> {vtranslate('LBL_TIME_INTERVAL', $QUALIFIED_MODULE)}
				{if $SCHEDULED_WORKFLOW_COUNT >= $MAX_ALLOWED_SCHEDULED_WORKFLOWS}
					&nbsp;&nbsp;<span class="alert-info textAlignCenter"><i class="far fa-info-circle"></i>&nbsp;&nbsp;({vtranslate('LBL_MAX_SCHEDULED_WORKFLOWS_EXCEEDED', $QUALIFIED_MODULE, $MAX_ALLOWED_SCHEDULED_WORKFLOWS)})</span>
				{/if}
			</span>
        </div>
    </div>
    
    <div class="form-group workflowMassActionBlock {if !in_array($EXECUTION_CONDITION, array(1, 2 , 3))} hide {/if}">
        <label for="name" class="col-sm-3 control-label">
            {vtranslate('LBL_WORKFLOW_UPDATE_IMPORT_BLOCK', $QUALIFIED_MODULE)}
        </label>
        <div class="col-sm-5 controls">
            <span class="flex-center-center" style="justify-content: start;">
                <input type="checkbox" name="workflow_mass_action" class="bootstrap-switch hide"  {if $WORKFLOW_MODEL_OBJ->workflow_mass_action eq '1'}checked{/if} />
                <span style="margin-right: 4px;"></span>
                {vtranslate('LBL_WORKFLOW_MASS_ACTION', $QUALIFIED_MODULE)}
            </span>
        </div>
    </div>

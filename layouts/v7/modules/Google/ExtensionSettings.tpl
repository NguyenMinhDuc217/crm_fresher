		...
		<div class="row">
			<div class="col-sm-12 col-xs-12">
				<table class="listview-table table-bordered" align="center">
					<thead>
					<th> 
						{assign 'lblData' value=['%system_name' => $MODULE]}
						{vtranslate('LBL_SYSTEM_DATA', $MODULE, $lblData)} 
					</th>
					<th>
						{assign 'lblData' value=['%system_name' => {vtranslate('APPTITLE', 'vTiger')}]}
						{vtranslate('LBL_SYSTEM_DATA', $MODULE, $lblData)} 
					</th>
		...
		{if !$IS_SYNC_READY}
			...
			<div class="row">
				<div class="col-sm-3 col-xs-3">
					<a id="authorizeButton" class="btn btn-block btn-social btn-lg btn-google-plus" data-url='index.php?module={$MODULE}&view=List&operation=sync&sourcemodule={$SOURCEMODULE}'><i class="fab fa-google-plus"></i>{vtranslate('LBL_SIGN_IN_WITH_GOOGLE', $MODULE)}</a>
				</div>
			</div>
		{else}
			<div class="row">
			...
			<div class="row">
				<div class="col-sm-3 col-xs-3">
					<a id="authorizeButton" class="btn btn-block btn-social btn-lg btn-google-plus" data-url='index.php?module={$MODULE}&view=List&operation=changeUser&sourcemodule={$SOURCEMODULE}'><i class="fab fa-google-plus"></i> {vtranslate('LBL_CHANGE_USER', $MODULE)} </a>
				</div>
			</div>
		{/if}
		<br>
		<div class="row">
			<div class="col-sm-12 col-xs-12">
				<div class="vt-default-callout vt-info-callout"> 
					<h4 class="vt-callout-header"><span class="far fa-info-circle"></span>&nbsp; {vtranslate('LBL_INFO')} </h4><br>
					<div>
						{vtranslate('LBL_REDIRECT_URL_MESSAGE', $MODULE)}<br><br>
						{vtranslate('LBL_REDIRECT_URL', $MODULE)} : <span style="color: #15c !important">{Google_Utils_Helper::getRedirectUrl()}</span> {* Modified by Hieu Nguyen on 2020-02-18 to standardize coding *}
					</div>
					<br>

...
{strip}
	...
	<div class="modal-dialog modal-md QuickCreate">
		<div class="modal-content">
			<form class="form-horizontal recordEditView Calendar" id="QuickCreate" name="QuickCreate" method="post" action="index.php">
					...
					<div class="quickCreateContent calendarQuickCreateContent" style="padding-top:2%;margin-top:5px;">
						...
						<div>
							...
							<div>
								...
								{* Modified by Hieu Nguyen on 2021-10-22 to escape special characters from rendering HTML to avoid error *}
								<input type="text"
									id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->get('name')}"
									...
									{if $FIELD_MODEL->get('uitype') eq '3' || $FIELD_MODEL->get('uitype') eq '4'|| $FIELD_MODEL->isReadOnly()} readonly {/if}
						...
						<table class="massEditTable table no-border">
							<tr>
								...
								{if $FIELD_MODEL->get('uitype') eq "19"}
									...
									<td class='fieldLabel {$FIELD_NAME} col-lg-3'> {* Edit by Phu Vo on 2019.06.14 to add field name *}
									...
									<td class="fieldValue {$FIELD_NAME} col-lg-9" {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}> {* Edit by Phu Vo on 2019.06.14 to add field name *}
										{* Modified by Hieu Nguyen on 2019-11-21 to load custom code *}

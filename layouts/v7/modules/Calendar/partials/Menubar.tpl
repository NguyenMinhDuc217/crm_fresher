...
<div id="modules-menu" class="modules-menu">
	<ul>
		{foreach item=SIDE_BAR_LINK from=$QUICK_LINKS['SIDEBARLINK']}
			...
			{assign var=VIEW_ICON_CLASS value="fa-calendar"}
			{if $SIDE_BAR_LINK->get('linklabel') eq 'LBL_CALENDAR_VIEW'}
				{assign var=CURRENT_LINK_NAME value="Calendar"}
				{assign var=VIEW_ICON_CLASS value="fa-calendar-week"}
			{else if $SIDE_BAR_LINK->get('linklabel') eq 'LBL_SHARED_CALENDAR'}
				{assign var=CURRENT_LINK_NAME value="SharedCalendar"}
				{assign var=VIEW_ICON_CLASS value="fa-calendar-alt"}
			{/if}

			{* Added by Hieu Nguyen on 2022-09-05 to check access forbidden feature *}
			{if $CURRENT_LINK_NAME == 'List' && isForbiddenFeature('CalendarActivityList')}{continue}{/if}
			{* End Hieu Nguyen *}

			<li data-toggle="tooltip" data-placement="right" class="{if $CURRENT_LINK_NAME eq $CURRENT_VIEW}active{/if}" title="{vtranslate($SIDE_BAR_LINK->get('linklabel'),'Calendar')}">
				<a href="{$SIDE_BAR_LINK->get('linkurl')}">
					<i class="fad {$VIEW_ICON_CLASS}"></i>

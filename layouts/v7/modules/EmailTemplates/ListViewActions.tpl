    ...
    <div id="listview-actions" class="listview-actions-container">
        ...
        <div class = "row">
            <div class="btn-toolbar col-md-3" role="group" aria-label="...">
                {* Commented out by Hieu Nguyen on 2021-08-06 to hide button switch ListView mode as this feature is not useful but cause more bugs to maintain *}
                {* <div class="btn-group" role="group" aria-label="...">
                    <button type="button" class="btn btn-default viewType" title="{vtranslate('LBL_LIST_VIEW',$MODULE)}" data-mode="list" {if $VIEWTYPE eq 'list'} disabled="disabled" {/if}><i class="far fa-th-list"></i></button>
                    <button type="button" class="btn btn-default viewType" title="{vtranslate('LBL_THUMBNAIL_VIEW',$MODULE)}" data-mode="grid" {if $VIEWTYPE eq 'grid'} disabled="disabled" {/if}><i class="far fa-th-large"></i></button>
                </div> *}
                <div class="btn-group">
                {if $editAction}
                ...
                {/if}
                {if $deleteAction}
                    <button type="button" class="btn btn-outline-danger" id={$MODULE}_listView_massAction_{$deleteAction->getLabel()}   {* Modified by Hieu Nguyen on 2021-08-06 to remove red background *}
            ...
            <div class='col-md-6'>
                {if $LISTVIEW_ENTRIES_COUNT eq '0' and $REQUEST_INSTANCE->isAjax()}
                    ...
                    {if $VIEWID}
                        ...
                        {/if}
                        {if $CVNAME neq 'All'}
                            <div>{vtranslate('LBL_DISPLAYING_RESULTS',$MODULE)} {vtranslate('LBL_FROM',$MODULE)} <b>{$CVNAME}</b>. <a href='{$DEFAULT_FILTER_URL}'>{vtranslate('LBL_SEARCH_IN',$MODULE)} {vtranslate('ALL',$MODULE)} {vtranslate($MODULE, $MODULE)}</a> </div>
                        {/if}
                    {/if}
                {/if}

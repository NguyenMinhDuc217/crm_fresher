	...
	{foreach item=FIELD_MODEL key=FIELD_NAME from=$SUMMARY_RECORD_STRUCTURE['SUMMARY_FIELDS']}
		...
		{if $FIELD_MODEL->get('name') neq 'modifiedtime' && $FIELD_MODEL->get('name') neq 'createdtime'}
			<tr class="summaryViewEntries">
                {* Modified by Phu Vo on 2019.05.28 to add field name to td class at summary screen *}
				<td class="fieldLabel {$FIELD_MODEL->getName()}" >
                {* End Phu Vo *}
                ...
                {* Modified by Phu Vo on 2019.05.28 to add field name to td class at summary screen *}
				<td class="fieldValue {$FIELD_MODEL->getName()}">
                {* End Phu Vo *}
                    <div class="row">
                        ...
                        {if $FIELD_MODEL->isEditable() eq 'true' && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true' && $FIELD_MODEL->get('uitype') neq 69}
                            ...
                            <span class="action"><a href="#" onclick="return false;" class="editAction far fa-pen"></a></span>
                        {/if}
                    </div>
				</td>
			</tr>

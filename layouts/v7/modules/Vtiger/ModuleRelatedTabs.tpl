	...
	<div class='related-tabs row'>
		<ul class="nav nav-tabs">
			...
			{for $i = 0 to $COUNT1-1}
				...
				{* Modidifed by Hieu Nguyen on 2018-09-06 *}
				{assign var="DETAILVIEWRELATEDLINKLBL" value= vtranslate($RELATED_LINK->getLabel(), $MODULE_NAME)}
				{* End Hieu Nguyen *}
				...
				<li class="tab-item {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
					data-module="{$RELATEDMODULENAME}" data-relation-id="{$RELATED_LINK->getId()}" {if $RELATEDFIELDNAME}data-relatedfield ="{$RELATEDFIELDNAME}"{/if}>
					<a href="index.php?{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" class="textOverflowEllipsis" displaylabel="{$DETAILVIEWRELATEDLINKLBL}" recordsCount="" >
						{if $RELATEDMODULENAME eq "ModComments"}
							<span class="tab-icon"><i class="far fa-comment"></i><span> &nbsp;<span class="tab-label"><strong>{$DETAILVIEWRELATEDLINKLBL}</strong></span>
						{else}
						<!-- Modified by Kelvin Thang -- Date: 2018-07-02-->
							<span class="tab-icon" >
			...
			{if $COUNT gt $LIMIT}
				<li class="dropdown related-tab-more-element">
					<a href="javascript:void(0)" data-toggle="dropdown" class="dropdown-toggle">
						<span class="tab-label"> 
							<strong>{vtranslate("LBL_MORE",$MODULE_NAME)}</strong> &nbsp; <b class="far fa-angle-down"></b>
						</span>
					</a>
					<ul class="dropdown-menu pull-right fancyScrollbar"> {* Added by Vu Mai on 2022-10-21 to fix bug scrollbar in related tab more dropdown menu *}
						{for $j = $COUNT1 to $COUNT-1}
							...
							<li class="more-tab {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
								...
								<a href="index.php?{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" displaylabel="{$DETAILVIEWRELATEDLINKLBL}" recordsCount="">
									{if $RELATEDMODULENAME eq "ModComments"}
										{* Modified by Phu Vo on 2021.05.21 to fix related module lable spacing issue *}
										<span class="tab-icon textOverflowEllipsis">
											...
											<span class="tab-label" style=" display: none; white-space: nowrap;">
												&nbsp;<strong>{$DETAILVIEWRELATEDLINKLBL}</strong>
											</span>
										</span>
										{* End Phu Vo *}
									...
									{/if}
									<span class="numberCircle hide">0</span> {* Modified by Phu Vo on 2019.10.30 Use css to handle spacing *}
								</a>
							</li>
						{/for}
					</ul>

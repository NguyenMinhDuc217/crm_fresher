...
{strip}
	<div class="relatedHeader">
		<div class="btn-toolbar row">
					...
					<div class="btn-group">
						...
						{else}
							...
							{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}
								{* Modified by Hieu Nguyen on 2022-09-06 to display icon based on preset linkicon attribute *}
								<button type="button" module="{$RELATED_MODULE_NAME}"
									class="btn btn-default {if $IS_SELECT_BUTTON eq true} selectRelation{else} addButton" name="addButton{/if}"
									...
									{/if}
								>
									{if $IS_SELECT_BUTTON eq false}<i class="far {if $RELATED_LINK->getIcon()}{$RELATED_LINK->getIcon()}{else}fa-plus{/if}"></i>&nbsp;{/if}&nbsp;{$RELATED_LINK->getLabel()}
								</button>
								{* End Hieu Nguyen *}
							{/if}
						{/if}
            ...
            {* Added by Hieu Nguyen on 2019-07-24 for custom filters related list header *}
            {assign var="CUSTOM_HEADER_FILTERS" value="modules/$MODULE/tpls/RelatedListCustomHeaderFilters.tpl"}

            {if file_exists($CUSTOM_HEADER_FILTERS)}
                {include file=$CUSTOM_HEADER_FILTERS}
            {/if}
            {* End Hieu Nguyen *}

        ...
        <div class="sidebar-container lists-menu-container">
            ...
            <div class="menu-scroller scrollContainer" style="position:relative; top:0; left:0;">
				<div class="list-menu-content">
                    {* Formated indent by Hieu Nguyen on 2019-11-27 *}
                    ...
                    {if $CUSTOM_VIEWS && count($CUSTOM_VIEWS) > 0}
                        {foreach key=GROUP_LABEL item=GROUP_CUSTOM_VIEWS from=$CUSTOM_VIEWS}
                            ...
                            {* Added by Hieu Nguyen on 2020-06-26 to determine group type *}
                            {assign var="GROUP_TYPE" value=$GROUP_LABEL}
                            {* End Hieu Nguyen *}
                                ...
                                <ul class="lists-menu">
                                    ...
                                    {foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS name="customView"}
                                        ...
                                        <li style="font-size:12px;" class='listViewFilter {if $VIEWID eq $CUSTOM_VIEW->getId() && ($CURRENT_TAG eq '')} active {if $smarty.foreach.customView.iteration gt 10} {assign var=count value=1} {/if} {else if $smarty.foreach.customView.iteration gt 10} filterHidden hide{/if} '> 
                                            ...
                                            {* Modified by Hieu Nguyen on 2019-11-27 to display owner name on tooltip *}
                                            {assign var="FILTER_NAME" value=$CUSTOM_VIEW->get('viewname')}
                                            {assign var="IS_SYSTEM_FILTER" value=($FILTER_NAME == 'All' || $LIST_STATUS == 3)}
                                            {assign var="CAN_DELETE" value=($GROUP_TYPE == 'Mine' || ($GROUP_TYPE == 'Shared' && $FILTER_NAME != 'All' && $LIST_STATUS != 3 && $USER_MODEL->isAdminUser()))}

                                            <a class="filterName listViewFilterElipsis" 
                                                href="{$LISTVIEW_URL|cat:'&viewname='|cat:$CUSTOM_VIEW->getId()|cat:'&app='|cat:$SELECTED_MENU_CATEGORY}" 
                                                oncontextmenu="return false;" 
                                                data-filter-id="{$CUSTOM_VIEW->getId()}" 
                                                data-toggle="tooltip"
                                                title="{$VIEWNAME|@escape:'html'}"
                                                data-system-filter="{$IS_SYSTEM_FILTER}"
                                                data-can-delete="{$CAN_DELETE}"
                                            >
                                                {if !$CUSTOM_VIEW->isMine() && $FILTER_NAME != 'All'}
                                                    <span title="{trim($CUSTOM_VIEW->getOwnerName())}"><img src="{getUserAvatar($CUSTOM_VIEW->get('userid'))}" class="user-avatar"/></span>
                                                {/if}
                                            ...
                                            <div class="pull-right">
                                                <span class="js-popover-container" style="cursor:pointer;">
                                                    {* Modified by Hieu Nguyen on 2020-06-26 to allow removing shared list *}
                                                    <span class="far fa-ellipsis-v" rel="popover" data-toggle="popover" aria-expanded="true" 
                                                        {if $CUSTOM_VIEW->get('viewname') neq 'All'}
                                                            ...
                                                            {if $CUSTOM_VIEW->isDeletable()}data-deleteurl="{$CUSTOM_VIEW->getDeleteUrl()}"{/if}
                                                        {/if}
                                                        {if $CUSTOM_VIEW->get('shared') eq '1'}data-shared="1"{/if}
                                                        data-filter-id="{$CUSTOM_VIEW->getId()}" 
                        ...
                        <div id="filterActionPopoverHtml">
                            <ul class="listmenu hide" role="menu">
                                ...
                                {* Modified by Hieu Nguyen on 2020-06-26 to allow removing a shared filter *}
                                <li role="presentation" class="removeFilter">
                                    <a role="menuitem"><i class="far fa-close"></i>&nbsp;{vtranslate('LBL_REMOVE_SHARED_FILTER', 'CustomView')}</a>
                                </li>
                                ...
                                <li role="presentation" class="duplicateFilter">
                                    <a role="menuitem" ><i class="far fa-files"></i>&nbsp;{vtranslate('LBL_DUPLICATE',$MODULE)}</a>
                                </li>
                                <li role="presentation" class="toggleDefault">
                                    <a role="menuitem" >
                                        <i data-check-icon="fa-check-square" data-uncheck-icon="fa-square"></i>&nbsp;{vtranslate('LBL_DEFAULT',$MODULE)}
                                    </a>
                                </li>
                            </ul>
                        </div>
                    ...
                    {* End indent format by Hieu Nguyen *}
                </div>
            </div>
        </div>
    </div>
                ...
                <div>
                    <div class="editTagContainer hide">
                        ...
                        <div class="edit-tag-footer">
                            <button class="btn btn-sm btn-success saveTag" type="button">
                                <center> <i class="far fa-check"></i> </center>
                            </button>
                            <button class="btn btn-sm btn-default cancelSaveTag" type="button">
                                <center> <i class="far fa-close"></i> </center>
                            </button>
                        </div>
                    </div>
                </div>

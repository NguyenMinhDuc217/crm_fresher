...
<div class="input-group">
    {*-- Added By Kelvin Thang on Date 2019-12-10 added formatNumber and maxlength --*}
	<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="inputElement currencyField" onkeyup="formatNumber (this, 'float')" maxlength="22" data-field-type="{$FIELD_INFO['type']}" name="{$FIELD_NAME}"
...
{else if ($FIELD_MODEL->get('uitype') eq '72') && ($FIELD_NAME eq 'unit_price')}
	<div class="input-group" style="float:none;">
        {*-- Added By Kelvin Thang on Date 2019-12-10 added formatNumber and maxlength --*}
        <input id="{$MODULE}-editview-fieldname-{$FIELD_NAME}"  type="text" class="inputElement unitPrice currencyField" onkeyup="formatNumber (this, 'float')" maxlength="22" data-field-type="{$FIELD_INFO['type']}" name="{$FIELD_NAME}"
...
<div class="input-group">
    {*-- Added By Kelvin Thang on Date 2019-12-10 added formatNumber and maxlength --*}
    <input type="text" class="input-lg currencyField" onkeyup="formatNumber (this, 'float')" maxlength="22" data-field-type="{$FIELD_INFO['type']}" name="{$FIELD_NAME}"
        value="{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}" {if !empty($SPECIAL_VALIDATOR)}data-validator={Zend_Json::encode($SPECIAL_VALIDATOR)}{/if}
        {if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if} data-rule-currency='true'
        {if count($FIELD_INFO['validator'])}
            data-specific-rules='{ZEND_JSON::encode($FIELD_INFO["validator"])}'

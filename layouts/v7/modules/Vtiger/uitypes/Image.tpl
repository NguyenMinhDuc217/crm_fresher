	...
	{else}
		...
		<div class="fileUploadContainer text-left">
			<div class="fileUploadBtn btn btn-primary">
				<span><i class="far fa-laptop"></i> {vtranslate('LBL_UPLOAD', $MODULE)}</span>
		...
		{* Modified by Phu Vo on 2019.08.28 to style image edit view *}
				...
				<div class="swipe-image-holder-lg" style="margin-top:5px;" title="{$IMAGE_INFO.name}">
					{if !empty($IMAGE_INFO.path) && !empty({$IMAGE_INFO.orgname})}
						...
						<span class="image-label">
							...
							<span class="image-delete-wraper"><input type="button" id="file_{$ITER}" value="{vtranslate('LBL_DELETE','Vtiger')}" class="imageDelete"></span>
						</span>
					{/if}
				</div>
			{/foreach}

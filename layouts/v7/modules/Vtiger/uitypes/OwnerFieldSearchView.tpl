	...
	<select class="select2 listSearchContributor {$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}" multiple data-fieldinfo='{$FIELD_INFO|escape}' style="display:none">
		<optgroup label="{vtranslate('LBL_USERS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
                    <option value="{$OWNER_ID}" data-picklistvalue= '{$OWNER_ID}' {if in_array(trim(decode_html($OWNER_ID)),$SEARCH_VALUES)} selected {/if} {* Modified by Hieu Nguyen on 2019-08-29 to search owner by id instead of name *}
		...
		<optgroup label="{vtranslate('LBL_GROUPS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEGROUP_LIST}
				<option value="{$OWNER_ID}" data-picklistvalue= '{$OWNER_ID}' {if in_array(trim($OWNER_ID),$SEARCH_VALUES)} selected {/if}  {* Modified by Hieu Nguyen on 2019-08-29 to search owner by id instead of name *}

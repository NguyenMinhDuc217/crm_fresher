                                ...
                                <li>
                                    ...
                                    {if $IMAGE_DETAILS neq '' && $IMAGE_DETAILS[0] neq '' && $IMAGE_DETAILS[0].path eq ''}
                                        <div class="update_icon bg-info">
                                            <i class='update_image fad fa-user-alt'></i>
                                        </div>
                                    {else}
                                        ...
                                        {foreach item=FIELDMODEL from=$RECENT_ACTIVITY->getFieldInstances()}
                                            {if $FIELDMODEL && $FIELDMODEL->getFieldInstance() && $FIELDMODEL->getFieldInstance()->isViewable() && $FIELDMODEL->getFieldInstance()->getDisplayType() neq '5'}
                                                <div class='font-x-small updateInfoContainer textOverflowEllipsis'>
                                                    ...
                                                    &nbsp;
                                                        {* Begin: Comment out By Kelvin Thang on 2020.06.26 [History] Updated revision for meaningful words *}
                                                            {*{vtranslate('LBL_UPDATED')}*}
                                                        {* End: Comment out By Kelvin Thang on 2020.06.26 *}
                                                    </div>
                                                {else}
                                            ...
                                            {if $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
                                                 {* Begin: Modified By Kelvin Thang on 2020.06.26 [History] Updated revision for meaningful words *} 
                                                <div class="update-to">
                                                    <span class="field-name">
                                                        {*{vtranslate('LBL_TO')}*}
                                                        {vtranslate('LBL_TO_UPDATE_ACTIVITIES')}
                                                    </span>&nbsp;
                                                ...
                                                {* End: Modified By Kelvin Thang on 2020.06.26 *}
                                            {/if}
                                            </div>
                                        {/if}
                                    {/foreach}
                                ...
                                <li>
									...
									<div class="update_icon bg-info-{$RELATED_MODULE|strtolower}">
										{if {$RELATED_MODULE|strtolower eq 'modcomments'}}
											{assign var="VICON_MODULES" value="fad fa-comments-alt"}
                                        ...
                                        <h5>
                                            ...
                                            <span>
                                                {if $RECENT_ACTIVITY->isRelationLink()}
                                                    {if in_array($RELATED_MODULE, ['Emails', 'SMS']) }
                                                        {assign var="LBL_RELATION" value="LBL_SENT"}
                                                    {* Added by Hieu Nguyen on 2021-05-20 to translate specific label for Comment *}
                                                    {elseif $RELATED_MODULE == 'ModComments'}
                                                        {assign var="LBL_RELATION" value="LBL_COMMENTED"}
                                                    {* End Hieu Nguyen *}
                                                    {else}
                                                        {assign var="LBL_RELATION" value="LBL_LINKED"}
                                                    {/if}

                                                    {vtranslate($LBL_RELATION, $MODULE_NAME)}
                                            ...
                                            <span>
                                                ...
                                                {if $PERMITTED}
                                                    {if $RELATED_MODULE eq 'ModComments'}
                                                        {renderCommentWithMentions($RELATION->getLinkedRecord()->getName())} {* Modified by Hieu Nguyen on 2021-05-20 to render comment with mentions *}
                                                    {else}

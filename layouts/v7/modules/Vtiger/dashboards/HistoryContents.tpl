	...
	{if $HISTORIES neq false}
		{foreach key=$index item=HISTORY from=$HISTORIES}
					...
					<div class="row entry clearfix">
						...
						<div class="col-lg-10 pull-left">
							...
							{else if $HISTORY->isCreate()}
								<div>
									<b>{$USER->getName()}</b> {vtranslate('LBL_ADDED')} aaa
							...
							{else if ($HISTORY->isRelationLink() || $HISTORY->isRelationUnLink())}
								...
								<div>
									...
									{if $HISTORY->isRelationLink()}
                                        {if in_array($RELATION->getLinkedRecord()->getModuleName(), ['Emails', 'SMS']) }
                                            {vtranslate('LBL_SENT', $MODULE_NAME)}
                                        {else}
                                    ...
                                    {* End: Modified By Kelvin Thang on 2020.06.26 [History] Updated revision for meaningful words *}
							...
							{/if}{vtranslate('LBL_FOR')} <a class="cursorPointer" {if stripos($PARENT_DETAIL_URL, 'javascript:')===0}
							   onclick='{$PARENT_DETAIL_URL|substr:strlen("javascript:")}' {else} onclick='window.location.href="{$PARENT_DETAIL_URL}"' {/if}>
                                    ...
                                    {* Begin: Modified By Kelvin Thang on 2020.06.26 [History] Updated revision for meaningful words *}
                                    {vtranslate($RELATION->getParent()->getParent()->getModuleName(), $RELATION->getParent()->getParent()->getModuleName())}
                                    {* End: Modified By Kelvin Thang on 2020.06.26 [History] Updated revision for meaningful words *}
			...
			{else if $MODELNAME == 'ModComments_Record_Model'}
				<div class="row">
					<div class="col-lg-1 pull-left">
						<span><i class="far fa-comment entryIcon" title={$TRANSLATED_MODULE_NAME}></i></span>
					</div>
					<div class="col-lg-10 pull-left" style="margin-top:5px;">
						...
						<div><i>"{renderCommentWithMentions($HISTORY->get('commentcontent'))}"</i></div>    {* Modified by Hieu Nguyen on 2021-05-20 to render comment with mentions *}
					</div>

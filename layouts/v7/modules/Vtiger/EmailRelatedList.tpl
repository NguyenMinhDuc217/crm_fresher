                    ...
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                        <div class="btn-group">
                            ...
                            <button onclick="javascript:Vtiger_Detail_Js.triggerSendEmail('index.php?module={$MODULE}&view=MassActionAjax&mode=showComposeEmailForm&step=step1&relatedLoad=true','Emails');" type="button" class="btn addButton btn-default
                                    ...
                                    {/if}
                                    {if $IS_SELECT_BUTTON neq true}name="composeEmail"{/if}>{if $IS_SELECT_BUTTON eq false}<i class="far fa-plus"></i>&nbsp;{/if}&nbsp;{$RELATED_LINK->getLabel()}
                            </button>
                        </div>
                    {/foreach}
                    &nbsp;
                            ...
                            <td class="{$WIDTHTYPE}">
                                ...
                                <div class="pull-right actions">
                                    <span class="actionImages">
                                            <a name="emailsDetailView" data-id='{$RELATED_RECORD->getId()}'><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="far fa-bars"></i></a>&nbsp;&nbsp;
                                            {if $RELATED_RECORD->getEmailFlag() eq 'SAVED'}
                                            <a name="emailsEditView"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="far fa-pen"></i></a>	&nbsp;&nbsp;
                                            {/if}

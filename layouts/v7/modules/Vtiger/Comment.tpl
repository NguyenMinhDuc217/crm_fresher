					...
					<div class="col-lg-12">
						<div class="media">
							<div class="media-left title" id="{$COMMENT->getId()}">
                                {*  Modified by Hieu Nguyen on 2021-03-16 to get creator info *}
                                {assign var=CREATOR value=$COMMENT->getCommentedByModel()}
							...
							<div class="media-body">
								<div class="comment" style="line-height:1;">
                                    {*  Modified by Hieu Nguyen on 2021-03-16 to get creator info *}
									<span class="creatorName" {if $CREATOR}data-creator-id="{$CREATOR->getModule()->getName()}:{$CREATOR->get('id')}"{/if}>
                                        {$CREATOR_NAME}
                                    </span>
                                    &nbsp;&nbsp;
                                    {* End Hieu Nguyen *}
									...
									<div class="commentInfoContentBlock">
                                        {*  Modified by Hieu Nguyen on 2021-03-16 to render comment with @mention *}
										<span class="commentInfoContent" data-comment-db-format="{$COMMENT->get('commentcontent')|escape:'html'}" data-comment-display-format="{renderCommentWithMentions($COMMENT->get('commentcontent'))|escape:'html'}">
											{renderCommentWithMentions($COMMENT->get('commentcontent'))}
										</span>
                                        {* End Hieu Nguyen *}
									</div>
									<br>
													...
													<div class="span11 commentAttachmentName">
														<span class="filePreview">
															<a name="downloadfile" href="{$FILE_DETAIL['url']}">
																<i title="{vtranslate('LBL_DOWNLOAD_FILE',$MODULE_NAME)}" class="far fa-download alignMiddle"></i>
															</a>

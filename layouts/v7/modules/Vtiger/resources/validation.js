...
jQuery.validator.addMethod(
	...
	function(params, element) {
		return app.vtranslate('JS_VALIDATE_OPTIONAL_MIN_LENGTH_MESSAGE', { 'min': params });
	}
);

// Implemented by Hieu Nguyen on 2019-03-15
jQuery.validator.addMethod(
...
// Implemented by Hieu Nguyen on 2019-08-21
jQuery.validator.addMethod(
    ...
    function (value, element, params) {
        try {
            if (value) {
                ...
                var dateValue = app.helper.getDateInstance(value, dateFormat);
                var compareDate = app.helper.getDateInstance(params, dateFormat);
    ...
    jQuery.validator.format(app.vtranslate('JS_VALIDATION_RULE_LESS_THAN_OR_EQUAL_TO_DATE_ERROR_MSG'))
);
...
jQuery.validator.addMethod(
    ...
    jQuery.validator.format(app.vtranslate('JS_VALIDATION_RULE_GREATER_THAN_OR_EQUAL_TO_DATE_ERROR_MSG'))
);
...
jQuery.validator.addMethod(
    ...
    function (value, element, params) {
        try {
            if (value) {
                var dateFormat = app.getDateFormat();
                var dateValue = app.helper.getDateInstance(value, dateFormat);
                var compareDateBegin = app.helper.getDateInstance(params[0], dateFormat);
                var compareDateEnd = app.helper.getDateInstance(params[1], dateFormat);

                if (dateValue < compareDateBegin || dateValue > compareDateEnd) {
                    return false;
                }
            }
    ...
    jQuery.validator.format(app.vtranslate('JS_VALIDATION_RULE_BETWEEN_DATE_RANGE_ERROR_MSG'))
);
// End Hieu Nguyen

// Added by Phu Vo on 2018.12.14
jQuery.validator.addMethod("asciiOnly", function(value, element, params) {
	try{
...
}, jQuery.validator.format(app.vtranslate('JS_VALIDATE_ASCII_ONLY')));
// End by Phu Vo

// Added by Phu Vo on 2019.06.10 [CustomOwnerField] validate assigned user field
jQuery.validator.addMethod('main-owner', function(value, element, params) {
	try {
		// Not validate if there is no user selected
...
}, jQuery.validator.format(app.vtranslate('JS_MAIN_OWNER_VALIDATION_MSG')));
// End Phu Vo [CustomOwnerField]

// Added by Vu Mai on 2022-07-27 to check duplicate value inside the dynamic table
jQuery.validator.addMethod(
    'dynamic-table-duplicate-check', 
    function (value, element, params) {
        try {
            if (params == true && value) {
				...
				let inputs = table.find('tbody').find('tr:visible').find(`[name="${inputName}"]`);
				let isDuplicated = false;

				inputs.each(function (index, input) {
					if (inputs.index(element) != index) {
						if (value == $(input).val()) {
							isDuplicated = true;
    ...
    jQuery.validator.format(app.vtranslate('JS_VALIDATION_RULE_DYNAMIC_TABLE_DUPLICATE_VALUE_ERROR_MSG'))
);
// End Vu Mai
...
// Refactor by Phu Vo on 2021.01.12 to unformat value using user number config before check for valid
jQuery.validator.addMethod("WholeNumber", function(value, element, params) {
	let regex= /[^+\-0-9.]+/; // Not number?

	value = app.unformatCurrencyToUserNoParse(value);
...
jQuery.validator.addMethod("reference_required", function (value, element, params) {
    ...
    // Added by Hieu Nguyen on 2020-01-06 to handle reference field validation in QuickEdit form
    if (isNaN(referenceValue)) {
		referenceValue = jQuery(element).parent().parent().find('input.referenceId').attr('value');
	}
    // End Hieu Nguyen
...
$.extend($.validator.messages, {
	required: app.vtranslate('JS_FIELD_REQUIRED_FIELD'),
	remote: 'Please fix this field.',
	email: app.vtranslate('JS_PLEASE_ENTER_VALID_EMAIL_ADDRESS'),
	url: app.vtranslate('JS_PLEASE_ENTER_VALID_URL'),
	date: app.vtranslate('JS_PLEASE_ENTER_VALID_DATE'),
	dateISO: app.vtranslate('JS_PLEASE_ENTER_VALID_DATE_ISO'),
	number: app.vtranslate('INVALID_NUMBER'),
	digits: app.vtranslate('JS_PLEASE_ENTER_ONLY_DIGITS'),
	creditcard: app.vtranslate('JS_PLEASE_ENTER_VALID_CREDIT_CARD_NUMBER'),
	equalTo: app.vtranslate('JS_PLEASE_ENTER_SAME_VALUE_AGAIN'),
	accept: app.vtranslate('JS_PLEASE_ENTER_ACCEPTED_EXT'),
	maxlength: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_NO_MORE_THAN')),
	minlength: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_AT_LEAST')),
	rangelength: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_IN_RANGE_LENGTH')),
	range: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_IN_RANGE')),
	max: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_LESS_THAN_OR_EQUAL')),
	min: jQuery.validator.format(app.vtranslate('JS_PLEASE_ENTER_GREATER_THAN_OR_EQUAL'))
});

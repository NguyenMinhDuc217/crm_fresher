...
Vtiger.Class('Vtiger_Index_Js', {
	 ...
	 * @returns {undefined}
	 */
	// Modified by Hieu Nguyen on 2022-05-15
	// Example: <a href="javascript:Vtiger_Index_Js.showMap(this)" data-module="Contacts" data-record="123">Show Map</a>
	showMap: function (element) {
		app.helper.showProgress();
		element = jQuery(element);
		let moduleName = element.data('module');
		let recordId = element.data('record');
		...
		.then(function (err, res) {
			...
			let address = res.address.trim();

			if (!address) {
				app.helper.showErrorNotification({ message: app.vtranslate('JS_GOOGLE_MAPS_EMPTY_ADDRESS_ERROR_MSG') });
				return false;
			}
				
			GoogleMaps.showMaps(address);
		});
	...
	requestReminder : function() {
        return; // Added by Hieu Nguyen on 2019-03-28 to prevent loading default reminders
	...
	showReminderPopup : function(record) {
		var notifyParams = {
			'title' : record.activitytype + ' - ' +
					...
					'<i id="reminder-postpone-'+record.id+'" title="'+app.vtranslate('JS_POSTPONE')+'" class="cursorPointer far fa-clock"></i>',
	...
	registerEvents: function() {
		...
		// Added by Phu Vo on 2021.05.21 to support menu dropdown position
			...
			app.request.post({"data":params}).then(function(err,data){
				if(err === null){
					app.helper.loadPageOverlay(data,{'ignoreScroll' : true,'backdrop': 'static'}).then(function(){
						...
						// Added by Phu Vo on 2019.06.13 to trigger popup overlay opened
						app.event.trigger('post.taskManagementOverlay.load', jQuery('#taskManagementContainer'));
						// End Init custom owner field for taskmanagement popup overlay
	...
	registerQuickCreateSubMenus : function() {
		jQuery("#quickCreateModules").on("click",".quickCreateModuleSubmenu",function(e){
			...
			jQuery(e.currentTarget).closest('#quickCreateModules').find('.dropdown.open').removeClass('open');	// Added by Hieu Nguyen on 2022-03-15 to hide all other dropdown before showing current dropdown
	...
	registerQuickCreateEvent : function (){
		...
		jQuery("#quickCreateModules").on("click",".quickCreateModule",function(e,params){
			// Added by Hieu Nguyen on 2022-03-15 to support open dropdown menu by clicking the toggle button
			if ($(e.target).data('toggle') == 'dropdown') {
				e.preventDefault();
				e.stopImmediatePropagation();
				jQuery(e.currentTarget).closest('#quickCreateModules').find('.dropdown.open').removeClass('open');
				jQuery(e.currentTarget).closest('.dropdown').toggleClass('open');
				return;
			}
			// End Hieu Nguyen
			...
			app.helper.showProgress();
			thisInstance.getQuickCreateForm(quickCreateUrl,quickCreateModuleName,params).then(function (data, scripts) {    // Modified by Hieu Nguyen on 2020-10-22 to add param 'scripts'
				app.helper.hideProgress();
				var callbackparams = {
					'cb' : function (container){
                        // Added by Hieu Nguyen on 2020-10-22 to load scripts after the quick create model is opened
                        scripts.forEach((file) => {
                            jQuery.getScript(file, () => { console.log('Script ' + file + ' loaded') });
                        });
                        // End Hieu Nguyen
		...
		// Added by Hieu Nguyen on 2022-03-15 to hide Quick Create more dropdown when one of its options is already clicked
		jQuery('#quickCreateModules').on('click', '.quickcreateMoreDropdown li', function () {
			$(this).closest('.dropdown').removeClass('open');
		});
		// End Hieu Nguyen

		// Added by Hieu Nguyen on 2022-03-15 to hide Quick Create more dropdown when user clicked outside Quick Create menu
		jQuery('html').on('click', function (e) {
			if (jQuery(e.target).closest('#quickCreateModules')[0] == null) {
				jQuery('#quickCreateModules').find('.dropdown.open').removeClass('open');
			}
		});
		// End Hieu Nguyen
	},
	...
	getQuickCreateForm: function(url, moduleName, params) {
        ...
        // Modified by Hieu Nguyen on 2020-10-22 to remove script from HTML to prevent multiple script VM running at the same time
		app.request.post({ data:requestParams }).then(function (err, data) {
            var container = jQuery('<div>'+ data +'</div>');
            var script = container.find('script[src*="QuickCreate.js"]');
            var scriptSrc = script.attr('src');
            script.remove();
			aDeferred.resolve(container.html(), [scriptSrc]);
		});
        // End Hieu Nguyen
	...
	registerAppTriggerEvent : function() {
		...
		jQuery('.app-trigger, .app-icon, .app-navigator').on('click',function(e){
			e.stopImmediatePropagation();   // Modified by Hieu Nguyen on 2021-05-21 to fix bug menu auto hide after click edit button at Opportunities subpanel
			toggleAppMenu();
		});

		// Modified by Hieu Nguyen on 2023-01-28 to support fixed menu
		jQuery('html').on('click', function() {
			if (!jQuery('#page').hasClass('fixed-menu')) {
				toggleAppMenu('hide');
			}
		});

		jQuery(document).keyup(function (e) {
			if (!jQuery('#page').hasClass('fixed-menu') && e.keyCode == 27) {
		...
		// Modified By Vu Mai on 2022-08-03 to update 3-level menu
		jQuery('.app-modules-dropdown-container').hover(
			...
			function (e) {
				...
				setTimeout(function () {
					if (dropdownContainer.find('.app-modules-dropdown').length && !dropdownContainer.find('.app-modules-dropdown:hover') !== 0 && !dropdownContainer.is(':hover')) {
		...
		jQuery('.app-item, .submenu-app-item').on('click', function () {
		...
		// [CustomMenu] Added by Hieu Nguyen on 2023-01-28
		// [CustomMenu] Modified by Vu Mai on 2023-02-02 to save pin_menu state to user preferences
		jQuery('#app-menu').find('#btn-pin-menu').on('click', function (e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			
			let state = true;

			if (jQuery('#app-menu').find('[name="pin_menu"]').val() == 'true') {
				state = false;
			}

			jQuery('#app-menu').find('[name="pin_menu"]').val(state);
			...
			.then(function (err, res) {
				...
				jQuery('#page').toggleClass('fixed-menu');
				jQuery('#app-menu').find('#btn-pin-menu').toggleClass('active');
				jQuery('.app-footer').toggleClass('fixed-menu');
			...
			// Refresh page to reload dashlet style if module is Home
		...
		jQuery('#app-menu').find('.actions .btn-expand').on('click', function (e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			$(this).closest('.menu-group').find('ul.menu-items').toggleClass('expanded');
			$(this).hide();
			jQuery('#app-menu').find('.actions .btn-collapse').show();
			$(this).closest('.dropdown').trigger('mouseenter');	// [CustomMenu] Modified by Vu Mai on 2023-02-02 to update position after click expand
		});

		jQuery('#app-menu').find('.actions .btn-collapse').on('click', function (e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			$(this).closest('.menu-group').find('ul.menu-items').toggleClass('expanded');
			$(this).hide();
			jQuery('#app-menu').find('.actions .btn-expand').show();
		});
		// End Hieu Nguyen
...
//			});
	...
	openPopUp : function(e) {
		...
		var sourceFieldElement = parentElem.find('input.sourceField');  // Modified by Hieu Nguyen on 2020-01-06 to fix bug source field can not be found when it has more than 1 css classes
...
//					
	...
	registerHoverEventOnAttachment: function () {
		return // Added by Phu Vo on 2021.09.27 to disable show hide download button
	...
	getPopUpParams : function(container) {
		...
		// [Core] Added by Phu Vo on 2021.06.21 to support parent module on container
		if (!params.related_parent_module && container.data('related_parent_module')) {
			params.related_parent_module = container.data('related_parent_module');
			params.related_parent_id = container.data('related_parent_id');

			// Added by Tin Bui on 2021.09.17 to support related label
			if (!params.tab_label && container.data('tab_label')) {
				params.tab_label = container.data('tab_label');
			}
			// End Tin Bui
		}
		// End Phu Vo
	...
	*/
	...
	getParentElement : function(element) {
		...
		// Added by Hieu Nguyen on 2018-11-07 to support flex relate field in custom form
		if(parent.length === 0) {
			parent = element.closest('.fieldLabel');
		}
		// End Hieu Nguyen
	...
	/** Implemented by Phu Vo on 2021.05.21 to support menu dropdown position */
	handleMenuPosition: function () {
		$('.app-modules-dropdown-container').on('hover', function(e) {
			let dropdownContainer = jQuery(e.currentTarget);
			let dropdownList = dropdownContainer.find('.app-modules-dropdown');
			let totalHeight = dropdownList.innerHeight() + dropdownContainer.position().top;
			let windowInnerHeight = window.innerHeight;
			let gap = parseInt(Number(totalHeight - windowInnerHeight));

			// Modified by Vu Mai on 2023-02-20 to support menu vertical scrollable
			if(dropdownContainer.length) {
				if (gap > 0) {
					dropdownList.css('top', dropdownContainer.position().top - gap - 50);
				}
				else {
					dropdownList.css('top', dropdownContainer.position().top);
				}
			}
			// End Vu Mai
		});
...
jQuery(function ($) {
	// Added by Hieu Nguyen on 2022-04-08 to check for valid session every 30 seconds
	if (location.href.indexOf('index.php') >= 0 && !(app.getModuleName() == 'Users' && app.getViewName() == 'Login')) {
		setInterval(() => {
			vtUtils.checkSession();	
		}, 30000);
	}

    // Added by Hieu Nguyen on 2019-10-22 to prevent redundant spaces
    if (typeof _VALIDATION_CONFIG != 'undefined' && _VALIDATION_CONFIG.prevent_redundant_spaces == true) {
        $(document).on('keydown', 'input[type="text"]', function(e) {
            if (e.keyCode == 32) {
                var text = $(this).val();
                var cursorPosition = $(this).prop('selectionStart');

                // Prevent when user enter the begining space or double spaces in text input fields
                if (cursorPosition == 0 || text.substr(cursorPosition - 1, 1) == ' ') {
                    e.preventDefault();
                }
            }
        });
    ...
    // [DataSecurity] Added by Hieu Nguyen on 2021-02-22 to hide forbidden features for System Admin
    if (typeof _CURRENT_USER_META != 'undefined' && _CURRENT_USER_META.username == 'sysadmin') {
        $('#appnavigator').remove();    // Modules menu
        $('.search-links-container').remove();  // Global search
        $('#navbar').find('ul.nav > li:last').addClass('sysadmin'); // Global buttons
        $('#navbar').find('ul.nav > li:last').prev('li').addClass('sysadmin'); // Global buttons
        $('#navbar').find('ul.nav > li:not(.sysadmin)').remove(); // Global buttons
    }
    // End Hieu Nguyen
});

// Util function to check for valid session. Added by Hieu Nguyen on 2022-04-08
vtUtils.checkSession = function () {
	$.ajax({
		...
		success: function (data) {
			...
			if (data.session_alive == false) {
				window.onbeforeunload = null;
                location.href = 'index.php?module=Users&view=Login&error=sessionTimeoutOrLoggedOut';
			}
		}
	});
}

// Util function to run action function when condition function return true. Added by Hieu Nguyen on 2018-10-15
vtUtils.doWhen = function(condition, action) {
	try {
		var doWhenInterval = setInterval(function() {
			if(condition()) {
				action();
				clearInterval(doWhenInterval);
			}
		}, 100);
	}
	catch(err) {
...
// Util function to init the custom date picker fields. Added by Hieu Nguyen on 2018-11-07
vtUtils.initDatePickerFields = function(form) {
	form.find('.datePicker').datepicker({
		format: $('body').data('userDateformat'),
		autoclose: true,
		todayBtn: 'linked',
		clearBtn: true,
		todayHighlight: true, // Added by Phu Vo on 2020.12.11 to highlight today button
	});
...
// Util function to open Quick Create Modal for any module. Added by Hieu Nguyen on 2018-10-15
// @params : 
// {
//    data: {[field]: [value]}
//    parentModule: [module] //ex
//    parentId: [crmid]
//	  preShowCb: [Function]
//    postShowCb: [Function]
//    postSaveCb: [Function]
// }
vtUtils.openQuickCreateModal = function(moduleName, params = {}) { // Modified by Phu Vo on 2018.02.25 => add params to quickCreateForm
	var thisInstance = Vtiger_Index_Js.getInstance();
	var url = 'index.php?module='+ moduleName +'&view=QuickCreateAjax';
    // var relatedField = params.relatedField;
    var parentModule = params.parentModule || app.getModuleName();
    var parentId = params.parentId || app.getRecordId() || '';
    var eliminatedKeys = ['view', 'module', 'mode', 'action'];

	params = params || {};
	params.data = params.data || {};

	// Common case auto assign. Modified by Phu Vo on 2019.08.28 to add common case for Modcomments
	if (moduleName === 'ModComments') {
		params.data.related_to = parentId;
	}

	// Modified by Phu Vo on 2022.14.01 from update calendar (v3 maybe) account, lead, contact have separate related field
	if (moduleName == 'Calendar' || moduleName == 'Events') {
		if (parentModule == 'Accounts') {
			params.data.related_account = parentId;
		}
		else if (parentModule == 'Contacts') {
			params.data.contact_id = parentId;
		}
		else if (parentModule == 'Leads') {
			params.data.related_lead = parentId;
		}
		else {
			params.data.parent_id = parentId;
		}
	}
	// Added by Hieu Nguyen on 2022-06-07 to fill default value for new Ticket
		...
		else if (parentModule == 'CPTarget') {
			params.data.related_cptarget = parentId;
		}
	}
	// End Hieu Nguyen
    else if (parentModule && parentId) {
        params.data.parent_id = parentId;
	}
	// End Phu Vo

    params.callbackFunction = params.postSaveCb;
    // delete params.postSaveCb;

	if(typeof params.callbackFunction === 'undefined') {
		params.callbackFunction = function(data, err) {
			var viewname = app.view();

			if((moduleName == parentModule) && (viewname == 'List')) {
				var listinstance = app.controller();
				listinstance.loadListViewRecords();
			}
		};
	...
	thisInstance.getQuickCreateForm(url, moduleName, params).then(function (data, scripts) {
		app.helper.hideProgress();

		var callbackparams = {
			'preShowCb': params.preShowCb,
			'cb': function (container) {
                // Added by Hieu Nguyen on 2020-10-22 to load scripts after the quick create model is opened
                scripts.forEach((file) => {
                    jQuery.getScript(file, () => { console.log('Script ' + file + ' loaded') });
                ...
                if(parentModule && parentId) {
                    jQuery('<input type="hidden" name="sourceModule" value="'+parentModule+'" />').appendTo(form);
                    jQuery('<input type="hidden" name="sourceRecord" value="'+parentId+'" />').appendTo(form);
					
					// Modified by Phu vo to fix dubplicate relation between Parent module and ModComment (ModComment add relation in EventHandler)
					if(moduleName !== 'ModComments') jQuery('<input type="hidden" name="relationOperation" value="true" />').appendTo(form);
					// End warkaround duplicate ModComments relation
                }
                
                // Update related fields
                for(let fieldName in params.data) {
                    if(!eliminatedKeys.includes(fieldName)) {
                        let field = form.find(`[name="${fieldName}"]`);
                        if(field.length === 0) {
                            jQuery('<input type="hidden" name="'+fieldName+'" value="'+params.data[fieldName]+'" />').appendTo(form);
                        }
                    }
                }
				...
				app.helper.registerLeavePageWithoutSubmit(form);
				app.helper.registerModalDismissWithoutSubmit(form);
                if(params.postShowCb && params.postShowCb instanceof Function) params.postShowCb(form);

                // Event registers
                form.one('pre.QuickCreateForm.save', () => {
                    if(params.preSaveCb && params.preSaveCb instanceof Function) params.preSaveCb(form);
                });
                container.one('hidden.bs.modal', () => {
                    if(params.postHideCb && params.postHideCb instanceof Function) params.postHideCb(form);
                });
			},
			backdrop: 'static',
			keyboard: false
...
// Implement by Phu Vo to process quick view related list modal
// params: { 
//     data: {
//         module: [ModuleName],
...
//     },
//     preReloadCb: [funcion],
//     postReloadCb: [function],
//     preShowCb: [function],
//     postShowCb: [function]
// }
vtUtils.openRelatedListModal = function (params) {
    params = params || {};

	// Validate input
    if (!params.data) return;
    if (!params.data.module || !params.data.relatedModule || !params.data.record) return;
	...
	let initialSearchParams = params.data.search_params || [];

    let getRelatedListSearchParams = function (container) {
        let relatedListTable = container.find('.searchRow');
        let searchParams = [];
        let currentSearchParams = [];

        if ($('#currentSearchParams').val()) {
            currentSearchParams = JSON.parse($('#currentSearchParams').val());
        }
        ...
        relatedListTable.find('.listSearchContributor').each(function (index, domElement) {
            ...
            let fieldInfo = related_uimeta.field.get(fieldName);

            if (fieldName in currentSearchParams) delete currentSearchParams[fieldName];
            ...
            let storedOperator = searchContributorElement.parent().parent().find('.operatorValue').val();
        ...
        for (let i in currentSearchParams) {
            let fieldName = currentSearchParams[i]['fieldName'];
            let searchValue = currentSearchParams[i]['searchValue'];
            let searchOperator = currentSearchParams[i]['comparator'];
    ...
    let registerEvents = function (container) {
        // event listener
        container.find('#PreviousPageButton').on('click', e => prev(container, e.target));
        container.find('#NextPageButton').on('click', e => next(container, e.target));
        container.find('#pageToJumpSubmit').on('click', e => jump(container, e.target));
        container.find('.listViewContentHeaderValues').on('click', e => sort(container, e.target));
        container.find('.removeSorting').on('click', e => clearSort(container, e.target));
        container.find('[data-trigger="relatedListSearch"]').on('click', e => {
            search(container, e.target);
            return false;
        });

		// Register clear filter button
		container.find('.clearFilters').attr('onclick', null)
		container.find('.clearFilters').on('click', e => {
			params.data.search_params = initialSearchParams
			loadRelatedList(container, params);
		})

        // Prevent modal form submit
        container.find('form').on('submit', () => false);

		updateTotalRecords(container)
    }

    let updateTotalRecords = function (container) {
        let aDeferred = jQuery.Deferred();
        let initParams = {
            action: 'RelationAjax',
			module: params.data.module,
            mode: 'getRelatedListPageCount',
			relatedModule: params.data.relatedModule,
			record: params.data.record,
			search_params: params.data.search_params
        }

        app.request.post({ data: initParams }).then((err, res) => {
            if (err === null) {
                if (typeof res !== 'object') res = JSON.parse(res);

                container.find('#totalCount').val(res.numberOfRecords);
                container.find('.showTotalCountIcon').replaceWith($(`<span class="showTotalCountIcon">${res.numberOfRecords}</span>`));
                container.find('#totalPageCount').text(res.page);
				container.find('.totalRecords').text(res.numberOfRecords);
	...
	let preShowUi = function (container) {
        container.find('.addButton, .relationDelete').remove();
		container.find('.bottomscroll-div').height(500);
		
        // Prevent reload current page
        container.find('.listViewEntries a').each((index, target) => {
            const href = $(target).attr('href');
            if (href && href.search('.php') > -1) $(target).attr('target', '_blank');
        });

		// Prevent page jump dropdown hide on click inside dropdown menu
		container.find('#PageJumpDropDown').on('click', function (event) {
            event.stopImmediatePropagation();
        });
	}

    let applyUi = function (container) {
		app.helper.showBothAxisScroll(container.find('.bottomscroll-div'));
        app.event.trigger('post.relatedListPopup.load', container);
		vtUtils.applyFieldElementsView(container);
		vtUtils.enableTooltips();
    }

    let loadRelatedList = function (container, params) {
        if (!params.data) return;
        if (!params.data.module || !params.data.relatedModule || !params.data.record) return;

        app.helper.showProgress();
        app.request.post({ data: { ...initParams, ...params.data } }).then((err, res) => {
            app.helper.hideProgress();

            if (err === null) {
                if (!(res instanceof jQuery)) res = jQuery(res);

                if (typeof params.preReloadCb === 'function') params.preReloadCb(res);

				preShowUi(res);
                container.find('.modal-body').html(res);
                registerEvents(container);
                applyUi(container);

                if (typeof params.postReloadCb === 'function') params.postReloadCb(container);
            }
        });
    }

    let prev = function (container, target) {
        if ($(target).is(':disabled')) return;
        params.data.page = Number(container.find('[name="currentPageNum"]').val()) - 1;

        loadRelatedList(container, params);
    };

    let next = function (container, target) {
        if ($(target).is(':disabled')) return;
        params.data.page = Number(container.find('[name="currentPageNum"]').val()) + 1;
    ...
    let jump = function (container, target) {
		let negativeRegex= /(^[-]+\d+)$/;
        let totalPages = Number(container.find('#totalPageCount').text());
        let newPageNumber = Number(container.find('#pageToJump').val());
        let currentPageNumber = Number(container.find('#pageNumber').val());
		let pageToJump = container.find('#pageToJump');

		vtUtils.hideValidationMessage(pageToJump);

		if (newPageNumber == 0) {
			let errorInfo = app.vtranslate('JS_VALUE_SHOULD_BE_GREATER_THAN_ZERO');

			vtUtils.showValidationMessage(pageToJump, errorInfo, {
				position : {
					my: 'top left',
					at: 'bottom left',
					container: pageToJump.closest('.listview-pagejump')
				}
		   	});

		   	return;
		}
		else if (isNaN(newPageNumber) || newPageNumber < 0 || String(newPageNumber).match(negativeRegex)) {
			let errorInfo = app.vtranslate('JS_ACCEPT_POSITIVE_NUMBER');
		...
		else if (newPageNumber === currentPageNumber){
            app.helper.showAlertNotification({ 'message': app.vtranslate('JS_YOU_ARE_IN_PAGE_NUMBER') + " " + newPageNumber });
            return;
        }
        
        params.data.page = newPageNumber;

        loadRelatedList(container, params);
    }

    let sort = function (container, target) {
        params.data.orderby = $(target).data('fieldname');
        params.data.sortorder = $(target).data('nextsortorderval');

        loadRelatedList(container, params);
    }

    let clearSort = function (container, target) {
        delete params.data.orderby;
        delete params.data.sortorder;

        loadRelatedList(container, params);
    }

    let search = function (container, target) {
        params.data.search_params = getRelatedListSearchParams(container);
        loadRelatedList(container, params);
    }
	...
	let requestParams = { data: { ...initParams, ...params.data } }; // Merge input params with initial params

    app.request.post(requestParams).then((err, res) => {
        app.helper.hideProgress();

        if (err === null) {
            let modal = $('.modal-template-xl').clone().addClass('relatedListModal');
            modal.find('.modal-body').append(res);
			modal.find('.modal-footer').remove();

			// Process modal title
			if (params.title) modal.find('.modal-header .pull-left').html(params.title);
            ...
            let options = {
                preShowCb: modal => {
                    ...
                    if (typeof params.preShowCb === 'function') params.preShowCb(modal);
                },
                cb: modal => {
                    registerEvents(modal);
					applyUi(modal);

                    if (typeof params.postShowCb === 'function') params.postShowCb(modal);
                }
            };

            // process
            app.helper.showModal(modal, options);
        }
    });
}
// End Phu Vo

                ...
                var registerShowAllTagEvents = function(modalContainer) {
                   ...
                   app.helper.showVerticalScroll(modalContainer.find('.dropdown-menu .scrollable'));
                    ...
                    modalContainer.find('.currentTagMenu').off('click', 'li > a').on('click', 'li > a',function(e){   
                        ...
                        selectedTag.prepend('<i class="deleteTag far fa-close"></i>');
                        selectedTag.append('<div class="tag-actions-wrapper"><i class="editTag far fa-pen"></i></div>');
                        currentTagsSelected.push(selectedTag.data('id'));

                        let deletedTagIndex = deletedTags.findIndex(id => id == selectedTag.data('id'));

                        // Remove seleted id from deleted tags
                        if (deletedTagIndex > -1) {
                            deletedTagIndex.splice(deletedTagIndex, 1);
                        }
                        // End Phu Vo
                        element.remove();
...
//                    vtUtils.showSelect2ElementView(tagInputEle, params);
                    ...
                    // Modified by Phu Vo on 2021.08.29 to use form validate mechanism
                    form.off('submit').vtValidate({
                        submitHandler: function (form) {
                            form = jQuery(form);
                            var modalContainerClone = modalContainer.clone(true);
                            // app.helper.hideModal();  // Commented out by Hieu Nguyen on 2021-12-10 to prevent hidding popup before ajax return
                            ...
                            // Modified by Hieu Nguyen on 2021-12-10 to display error message when assigning tags
                            self.saveTag(saveParams)
                            .then(
                                function (data) {
                                    app.helper.hideModal(); // Only hide model when the action is success
    ...
    registerEditTagEvents : function(){
        ...
        jQuery(document).on('click', '.editTagContainer .saveTag', function(e){
            ...
            self.updateTag(valueParams).then(function(data){
                ...
                app.helper.showErrorNotification({ 'message': error.message }); // Modified by Hieu Nguyen on 2021-12-10 to display error message
            });
        });

...
jQuery.Class("Vtiger_Popup_Js",{
	 ...
	 */
	// Modified by Hieu Nguyen on 2022-11-10 to skip disabled inputs
	clearFilters: function() {
		$('.searchRow input:not(:disabled), .searchRow select:not(:disabled)').val('').trigger('change');
		$('[data-trigger="PopupListSearch"]').trigger('click');
	},
    ...
    getPopupListSearchParams : function(){
            ...
            if(jQuery('#currentSearchParams').val())
                ...
                listViewTable.find('.listSearchContributor:input').each(function(index,domElement){ // Modified by Hieu Nguyen on 2019-06-21 to collect inputs only
	...
	getPageRecords : function(params){
		...
		Vtiger_BaseList_Js.getPageRecords(params).then(
            function(data){
                ...
                // Added by Hieu Nguyen on 2019-06-21 to trigger event post.Popup.reload
                app.event.trigger('post.Popup.reload', jQuery('#popupContents'));
                // End Hieu Nguyen
            }
        );
	...
	totalNumOfRecords: function (clickedElement) {
		let thisInstance = this;
		let popupContainer = thisInstance.getPopupPageContainer();
		let totalCountHiddenInput = popupContainer.find('#totalCount');
		var totalCount = totalCountHiddenInput.val();
		...
		if (totalCount === '') {
			thisInstance.getPageCount().then(function (data) {
				...
				totalCountHiddenInput.val(totalCount);
				thisInstance.showPagingInfo(totalCount);
			});
		}
		else {
			thisInstance.showPagingInfo(totalCount);
	...
	showPagingInfo: function (totalCount) {
		...
		let pagingInfoElement = popupContainer.find('.pagingInfo');

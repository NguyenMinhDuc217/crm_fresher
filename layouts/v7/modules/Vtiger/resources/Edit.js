    ...
    */
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			...
			var imageId = element.closest('div').find('.image').data().imageId; // Modified by Phu Vo on 2019.09.17 use swipebox instead of ImageGraphic
    ...
    registerCurrencyChangeEvent : function() {
        var formElement = this.getForm();
        formElement.find('.currency').on('change',function() {
    ...
    /* Added By Kelvin Thang on 2020.10.27 fixed issue #488 Click cancel in the edit view, when you right-click --  */
    registerCancel: function () {
        jQuery('.cancelLink').click(function () {
            // Modified by Phu Vo to fix bug reload when cancel on popup overlay
            let modal = $(this).closest('.modal');
        	if (modal[0] != undefined && modal.find('.close')[0] != undefined) {
                window.onbeforeunload = null;
                return true;
            }
            // End Phu Vo
            ...
            if (history.length < 2) {
                window.onbeforeunload = null;
                return window.location.href = 'index.php?module=' + app.getModuleName() + '&view=List&app=' + app.getAppName();
            }
            else {
                return true;
            }

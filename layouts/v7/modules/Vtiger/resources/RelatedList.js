...
jQuery.Class("Vtiger_RelatedList_Js",{
	...
	clearFilters: function() {
		...
		$('[data-trigger="relatedListSearch"]').trigger('click');
	},
},{
    ...
    // Refactored code by Hieu Nguyen on 2021-06-30
	registerScrollForRollupComments: function () {
        jQuery(document).scroll(function () {
            if (
                $(window).scrollTop() + $(window).height() >= $(document).height() - 30
                && jQuery('.related-tabs').find('li.active').data('module') == 'ModComments'
                && jQuery('#rollup-comments').is(':checked')
            ) {
                if (Vtiger_RelatedList_Js.loaded && jQuery('#rollup-comments').data('hasComments') == 1) {
                    Vtiger_RelatedList_Js.loaded = false;
					app.helper.showProgress();
                    
                    var btnToggleRollUp = jQuery('#rollup-comments');
                    var moduleName = btnToggleRollUp.data('moduleName');
                    var recordId = btnToggleRollUp.data('recordId');
                    var rollUpId = btnToggleRollUp.data('rollupId');
                    var rollUpState = btnToggleRollUp.is(':checked') ? 1 : 0;
                    var startIndex = parseInt(btnToggleRollUp.data('startIndex'));

                    var url = 'index.php?module=Vtiger&view=ModCommentsDetailAjax&mode=getNextGroupOfRollupComments' +
                        '&parent='+ moduleName +'&parentId='+ recordId +'&rollupid='+ rollUpId +'&rollup_status='+ rollUpState +'&startindex='+ startIndex;
                    ...
                    app.request.get({ 'url': url }).then(function (err, data) {
                        ...
                        if (data) {
							btnToggleRollUp.data('startIndex', startIndex + 10);
							jQuery('.commentsBody ul.unstyled:first').append(jQuery(data).children());
						}
                        else {
							btnToggleRollUp.data('hasComments', 0);
						}
                    });
                }
            }
    ...
    // Modified this function by Hieu Nguyen on 2020-06-12 to fix bug related list show total of all records after filter
    getPageJumpParams: function() {
        ...
        var params = {
			...
			'data': {
                ...
                'search_params': JSON.stringify(detailInstance.getRelatedListSearchParams())
			}
		};
		...
		if(totalPageNumber === ""){
			...
			if(totalRecordCount !== '') {
				...
				if(curEle.attr('id') !== 'PageJump') {
					thisInstance.showPagingInfo(totalRecordCount);	// Modified by Hieu Nguyen on 2021-08-13
				}
				return;
			}
			...
			thisInstance.pageJump().then(function(data){
				...
				totalCountElem.val();
				if(curEle.attr('id') !== 'PageJump') {
					thisInstance.showPagingInfo(numOfrecords);	// Modified by Hieu Nguyen on 2021-08-13
				}
			});
		}
	},
	...
	showPagingInfo: function (totalCount) {
		...
		let pagingInfoElement = container.find('.pagingInfo');

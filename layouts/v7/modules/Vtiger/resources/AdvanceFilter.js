...
jQuery.Class("Vtiger_AdvanceFilter_Js",{
	...
	loadConditions : function(fieldSelect) {
        ...
        if (fieldInfo.name == 'assigned_user_id' || fieldInfo.name == 'main_owner_id') {
        ...
        if (fieldInfo.name == 'createdby' || fieldInfo.name == 'modifiedby' || fieldInfo.column == 'inventorymanager') {
            conditionList = ['c', 'k'];     // Support only 2 options: contains and not contains
        }
        // End Hieu Nguyen
	...
	loadFieldSpecificUi : function(fieldSelect) {
        ...
        if (fieldModel.getName() == 'assigned_user_id' || fieldModel.getName() == 'main_owner_id') {
            var selectedTags = ''
            ...
            fieldSpecificUi = '<input type="text" name="'+ fieldModel.getName() +'" value="'+ fieldModel.getValue() +'" data-selected-tags=\''+ selectedTags +'\' data-value="value" class="form-control">';
            fieldUiHolder.html(fieldSpecificUi);

            // Init select2
            var input = fieldUiHolder.find('input[name="'+ fieldModel.getName() +'"]');
        ...
        if (fieldModel.getName() == 'createdby' || fieldModel.getName() == 'modifiedby' || fieldInfo.column == 'inventorymanager') {
            var selectedTags = ''
	...
	getValues : function() {
		...
		conditionGroups.each(function(index,domElement){
			...
			conditions.each(function(i, conditionDomElement){
				...
				var rowValues = {};
				if(fieldType == 'owner' || fieldType == 'ownergroup' || jQuery.inArray(fieldDataInfo.name, ['createdby', 'modifiedby']) >= 0 || fieldDataInfo.column == 'inventorymanager'){
                ...
                // End Hieu Nguyen
					...
					for(var key in fieldList) {
						...
						if (field == 'value') {
                            ...
                            if (fieldType == 'integer' || field == 'decimal' || fieldType == 'currency' || fieldType == 'double') { // Modified by Phu Vo on 2021.06.14 to add type double
...
Vtiger_Picklist_Field_Js('AdvanceFilter_Picklist_Field_Js',{},{
    ...
    // Modified by Hieu Nguyen on 2020-12-29 to fix issue option language toggle when switch between conditions
	getUi: function () {
        var pickListValues = this.getPickListValues();
        var selectedOption = app.htmlDecode(this.getValue());
        var selectedOptionsArray = selectedOption.split(',')

        var html = '<select class="select2 inputElement" multiple name="'+ this.getName() +'[]" data-value="value">';
        ...
        for (var option in pickListValues) {
            ...
            html += '<option value="'+ option +'" '+ selected +'>'+ pickListValues[option] +'</option>';
        }
...
// Added by Hieu Nguyen on 2021-01-26 to support tags field in saved search
Vtiger_Tags_Field_Js('AdvanceFilter_Tags_Field_Js', {}, {

...
//			this.positionNoDataMsg();
	...
	refreshWidget : function() {
		...
		// Modified by Hieu Nguyen on 2020-03-10 to show the loading right at selected widget
        contentContainer.closest('.dashboardWidget').find('.loading').show();
        // End Hieu Nguyen

		app.request.post(params).then(
			function(err,data){
                // Modified by Hieu Nguyen on 2020-03-10 to hide the loading at selected widget
                contentContainer.closest('.dashboardWidget').find('.loading').hide();
                // End Hieu Nguyen
	...
	registerWidgetPostLoadEvent : function(container) {
		var thisInstance = this;
		container.one(Vtiger_Widget_Js.widgetPostLoadEvent, function(e) { // Update by Phuc on 2020.04.13
			thisInstance.postLoadWidget();

			// Added by Phu Vo on 2019.06.18 to trigger post widget event
			app.event.trigger('post.widget.load', container);
			// End trigger post widget event (Phu Vo)
		})
	},

	registerWidgetPostRefreshEvent : function(container) {
		var thisInstance = this;
		// Bug #6: Modified by Phu Vo on 2020.02.28
		// Make sure widgetPostRefereshEvent will just init only one time
		container.one(Vtiger_Widget_Js.widgetPostRefereshEvent, function(e) {
...
Vtiger_Widget_Js('Vtiger_KeyMetrics_Widget_Js', {}, {
	// Modified by Phu Vo on 2021.05.21 to handle widget sizing and scrollbar
    postLoadWidget: function() {
		this._super();
		...
		if (container.find('.dashboardWidgetHeader')[0] != null) {
			adjustedHeight = container.innerHeight() - container.find('.dashboardWidgetHeader').innerHeight();
		}
		else {
			adjustedHeight = this.getContainer().height() - 50;
		}
...
Vtiger_Widget_Js('Vtiger_TopPotentials_Widget_Js', {}, {
    ...
    // Added by Hieu Nguyen on 2020-03-16 to fix bug widget content invisible after some resize actions
	// Modified by Phu Vo on 2021.05.21 to handle widget sizing and scrollbar
    postResizeWidget: function () {
		var widgetContent = jQuery('.dashboardWidgetContent', this.getContainer());
		let container = this.getContainer();
...
Vtiger_Widget_Js('Vtiger_MiniList_Widget_Js', {
	...
	// Modified by Phu Vo on 2021.05.21 to handle widget sizing and scrollbar
	postLoadWidget: function() {
        // app.helper.hideModal(); // Comment out by Phu Vo on 2020.10.29 to prevent unpredictable hide modal
        this.restrictContentDrag();
...
Vtiger_History_Widget_Js('Vtiger_OverdueActivities_Widget_Js', {}, {
	...
	registerLoadMore: function() {
        ...
        parent.on('click','a[name="history_more"]', function(e) {
            ...
            // Modified by Hieu Nguyen on 2021-01-04 to fix bug link load more does not work with the new filter UI
            var user = parent.find('[name="assigned_user_id"]').val();
			var url = element.data('url');
			var params = url + '&content=true&assigned_user_id=' + user;
            // End Hieu Nguyen
...
// CustomWidget. Implemented by Hieu Nguyen on 2019-09-25
Vtiger_Widget_Js('Vtiger_Custom_Widget_Js', 
    {},
    {
		// Modified by Phu Vo on 2021.05.21 to handle widget sizing and scrollbar
        postLoadWidget: function() {
            // app.helper.hideModal(); // Comment out by Phu Vo on 2020.10.29 to prevent unpredictable hide modal
            this.restrictContentDrag();
            this.registerFilterChangeEvent();   // Important

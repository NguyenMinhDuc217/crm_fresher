...
Vtiger.Class("Vtiger_List_Js", {
	...
	triggerSendSMSOTT: function (massActionUrl, channel, targetButton) {
		var listInstance = app.controller();
		var invalid = listInstance.checkListRecordSelected();
		...
		var callback = function (container) {
			...
			var title = $(targetButton).text().trim();
		...
		massActionUrl += '&channel=' + channel;
		Vtiger_List_Js.triggerMassAction(massActionUrl, callback);
	},
    // End Hieu Nguyen
					...
					function (error, data) {
						...
						if (data) {
							var callback = function (data) {
								...
								// Added by Phu Vo on 2019.06.20 => Trigger event to peform hook action in needed
								app.event.trigger('post.transferOwnershipPopup.load', data);
								// Trigger event to peform hook action in needed
							}
							var params = {};
							params.cb = callback
	...
	triggerMassAction: function (massActionUrl, callbackFunction) { // Added param callbackFunction by Hieu Nguyen on 2020-11-10
						...
						if (data) {
							app.helper.showModal(data, {'cb': function (modal) {
									if (postData.mode === "showAddCommentForm") {
                                        ...
                                        mentionHandler.attach(modal.find('#addCommentTextArea'));

                                        modal.find('#addCommentTextArea').on('input', function () {
                                    ...
                                    // Added by Hieu Nguyen on 2020-11-10 to handle callback
	...
	/**
	 * Function to trigger clear list view filter action
	 * @author Phu Vo (2019.08.28)
	 * @update By Phu Vo on 2020.03.03 fix clear filters didn't work with field current search params
	 */
	clearFilters: function() {
		$('.searchRow input, .searchRow select').val('').trigger('change');
		$('[name="currentSearchParams"]').val('').trigger('change');
		$('[data-trigger="listSearch"]').trigger('click');
	},
}, {
	...
	performMassDeleteRecords: function (url) {
		...
		if (listSelectParams) {
			...
			app.helper.showConfirmationBox({'message': message}).then(function (e) {
                ...
                // Commented out this line by Hieu Nguyen on 2019-05-28 to avoid orveriding the search and tag params
				...
				app.request.post({data: listSelectParams}).then(
						function (error, result) {
							...
							// Added by Phuc on 2019.11.28 to show notification
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
			...
			// Updated by Phuc on 2020.02.24 to apply search params from link
			if (jQuery(event.target).data('trigger') != 'listSearch' && typeof currentSearchParams[i][0] != 'undefined' && typeof event.keyCode == 'undefined') {
				for (var j in currentSearchParams[i]) {
					var fieldName = currentSearchParams[i][j][0];
					var searchValue = currentSearchParams[i][j][2];
					var searchOperator = currentSearchParams[i][j][1];
					var exist = false;
	...
	registerInlineEdit: function (currentTrElement) {
		...
		for (var i = 0; i < tdElements.length; i++) {
			...
			var customHandlingFields = ['boolean', 'owner', 'ownergroup', 'picklist', 'multipicklist', 'reference', 'string', 'url', 'currency', 'text', 'email']; // Modified by Hoang Duc 24-05-2019 to fix bug check box always not checked
            ...
            // Added by Hieu Nguyen on 2020-03-23 to set the right reference name for reference field
            if (fieldType == 'reference') {
                fieldData['value'] = valueElement.text().trim();
            }
            // End Hieu Nguyen
			...
			if (fieldType === 'reference') {
				if (value !== 0) {
                    // Updated by Hieu Nguyen on 2020-03-23 to set reference id for reference field
					jQuery('input[name="' + fieldName + '"]', editElement).prop('value', tdElement.data('rawvalue'));
                    // End Hieu Nguyen
				}
			}
				...
				app.request.post({data: params}).then(function (err, result) {
					if (result) {
						...
						thisInstance.loadListViewRecords(params).then(function (data) {
							thisInstance.hideInlineEdit(currentTrElement);	// Refactored by Hieu Nguyen on 2021-09-14
							app.helper.hideProgress();
							app.helper.showSuccessNotification({"message": app.vtranslate('JS_SAVE_SUCCESSFULLY')}); // Updated by Phuc on 2019.11.18 to show message for action
	...
	getInlineEditedFieldValue: function (fieldElement, inputElement) {
		...
		if (jQuery.inArray(fieldType, picklistTypes) !== -1) {
			...
			// [Core] Bug #146: Modified by Phu Vo on 2020.03.17 to fix checkbook uneditable alway submit with value 0
			if (fieldElement.find('input:checkbox')[0]) {
				value = fieldElement.find('input:checkbox').is(':checked') ? 1 : 0;
			}
			else {
				value = fieldElement.data('rawvalue');
			}
			// End Phu Vo
		} else {
	...
	hideInlineEdit: function (record) {
		jQuery('div.inline-save', record).addClass('hide');
		jQuery('a.action', record).removeClass('hide');
		jQuery('span.edit', record).empty();
		jQuery('span.edit', record).addClass('hide');
		jQuery('span.fieldValue', record).removeClass('hide');
	...
	registerRowClickEvent: function () {
        ...
        // Added by Hieu Nguyen on 2020-03-24 to handle click event for edit button
        // listViewContentDiv.on('click', '.listViewEntries a[name="editlink"]', function (e) {
        //     var href = $(this).attr('href');
        // });

        /* Commented out this line by Hieu Nguyen on 2020-03-14 to disable row click to detail at listview
	...
	registerRowDoubleClickEvent: function () {
		...
		listViewContentDiv.on('dblclick', '.listViewEntries', function (e) {
			if (listViewContentDiv.find('#isListViewQuickEditSupported').val() == 'no') {   // Refactored function name by Hieu Nguyen on 2020-01-16
				return;
			}

			var currentTrElement = jQuery(e.currentTarget);

			// Added by Hieu Nguyen on 2022-10-11 to prevent trigger quick edit when no field is supported
			if (currentTrElement.find('span.edit').length == 0) {
				return;
			}
			// End Hieu Nguyen
	...
	registerListViewBasicActions: function () {
		...
		app.event.on('post.listViewMassEdit.loaded', function (e, container) {
			...
			// Modified by Hieu Nguyen on 2022-07-26
			let form = $('form#massEdit');
			var form_original_data = form.serializeFormData();

			form.on('submit', function (event) {
				thisInstance.saveMassedit(form, event, form_original_data, isOwnerChanged);
		...
		// Refactored by Hieu Nguyen on 2022-06-22
		app.event.on('post.listViewMassAction.loaded', function (e, container) {
			let form = container.find('form#massSave');
			if (form[0] == null) return;	// Do nothing when the form is not loaded

			// Added by Phu Vo to register sms notify validate
			if (form.find('[name=module]').val() === 'SMSNotifier') {
			...
			// Validate form
			form.vtValidate({
				ignore: 'input[type="file"].multi',	// JQuery Validator is not working with multi file upload fields
				submitHandler: function (form) {
					app.helper.showProgress();
					...
					.then(function (err, data) {
                        ...
                        // Modified by Hieu Nguyen on 2020-11-18 to handle result message
						if (form.find('[name="module"]').val() == 'SMSNotifier') {
							if (data && data[0].status == 'failed') {
								...
								// Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
								var successMsg = app.vtranslate('JS_SENT_SMS_OTT_SUCCESS_MSG');
								app.helper.showSuccessNotification({ title: app.vtranslate('JS_SUCCESS'), message: successMsg });
							}
						}
                        // End Hieu Nguyen
	...
	// Refactored by Hieu Nguyen on 2022-07-26
	saveMassedit: function (form, event, originalData, isOwnerChanged) {
		event.preventDefault();
		let newData = form.serializeFormData();
		app.helper.showProgress();

		if (newData !== originalData || isOwnerChanged) {
			// Remove data of select and checkbox tags that is not changed
			for (var key in originalData) {
				let input = form.find('[name="' + key + '"]');

				if ((input.is('select') || input.is(':checkbox')) && (originalData[key] == newData[key])) {
					delete newData[key];
				}
			}

			// Re-assign assigned user
			if (!newData['assigned_user_id'] && isOwnerChanged) {
				newData['assigned_user_id'] = originalData['assigned_user_id'];
			}

			// Call ajax to save updated data
			app.request.post({ data: newData })
			.then(function (err, data) {
				app.helper.hideProgress();
	...
	performMergeRecords: function () {
		...
		// Modified by Phu Vo on 2019.05.29 to process in all case
		let length = $('input.listViewEntriesCheckBox:checked').length;
	...
	getListSelectAllParams: function (jsonDecode) {
        ...
        // Added by Hieu Nguyen on 2019-05-28 to fix bug incorrect filter with tag
        var searchParams = self.getListSearchParams() || [];
        var tagParams = self.getListTagParams() || [];
        params.search_params = JSON.stringify(searchParams.concat(tagParams));
        // End Hieu Nguyen
	...
	registerCheckBoxClickEvent: function () {
		...
		listViewPageDiv.on('click', '.listViewEntriesCheckBox', function (e) {
			...
			self.showSelectAll(); // Added by Phu Vo on 2019.08.29 to trigger select all behavior
		});
	...
	// Method modified by Phu Vo on 2019.08.29 to show clearer selectd row message
	showSelectAll: function () {
		let totalCount = $('#totalCount').val();
		
		if (totalCount) {
			this.updateSelectAllMsg();
		}
		else {
			app.helper.showProgress();
	...
	/**
	 * Update select all message or hide it depend on selected count
	 * @author Phu Vo (2019.08.29)
	 * Trigger within showSelectAll
	 */
	updateSelectAllMsg: function() {
		let listViewContainer = this.getListViewContainer();
		let totalCount = $('#totalCount').val();
		let selectedCount = listViewContainer.find('.listViewEntriesCheckBox:checked').length;
		let pageLimit = listViewContainer.find('.listViewEntriesCheckBox').length;
		let currentPage = $('#pageNumber').val();

		let updateMsg = (count) => {
			let message = $('#selectAllMsgDiv').data('message');

			message = message
				.replace('%selected_count', count)
				.replace('%current_page', currentPage)
				.replace('%total_count', totalCount);

			$('#selectAllMsgDiv').html(message);
			this.showSelectAllMsgDiv();
		}

		if (selectedCount === 0) {
			this.deSelectAllWithNoMessage();
		}
		else if (selectedCount < pageLimit) {
			updateMsg(selectedCount);
		}
		else if (selectedCount === pageLimit){
			updateMsg(app.vtranslate('JS_ALL'));
		}
		else {
			this.deSelectAllWithNoMessage();
		}
	...
	registerSelectAllClickEvent: function () {
		...
		listViewPageDiv.on('click', '#selectAllMsgDiv', function (e) {
			...
			// Added by Phu Vo to disable all check box when select all record clicked
			listViewPageDiv.find('.listViewEntriesMainCheckBox').prop({'checked': true, 'disabled': true});
			listViewPageDiv.find('.listViewEntriesCheckBox').prop({'checked': true, 'disabled': true});
			// End Phu Vo
		});
		self.markSelectedIdsCheckboxes();
	},
	registerDeSelectAllClickEvent: function () {
		...
		listViewPageDiv.on('click', '#deSelectAllMsgDiv', function (e) {
			...
			// Added by Phu Vo to enable all check box when select all record clicked
			listViewPageDiv.find('.listViewEntriesMainCheckBox').prop('disabled', false);
			listViewPageDiv.find('.listViewEntriesCheckBox').prop('disabled', false);
			// End Phu Vo
		});
	},
	...
	getUnStarRecordRows: function () {
		var listViewContainer = this.getListViewContainer();
		return listViewContainer.find('tr.listViewEntries .markStar .fa-star').closest('tr');
	},
...
//			e.stopPropagation();
	...
	totalNumOfRecords: function (currentEle) {
		...
		if (totalNumberOfRecords === '') {
			...
			thisInstance.getPageCount().then(function (data) {
				...
				thisInstance.showPagingInfo(totalNumberOfRecords);	// Modified by Hieu Nguyen on 2021-08-13
	...
	// Refactored this function by Hieu Nguyen on 2021-08-13
	showPagingInfo: function (totalCount) {
		let listViewContainer = this.getListViewContainer();
		let pagingInfoElement = listViewContainer.find('.pagingInfo');
		
		if (totalCount > 0) {
			pagingInfoElement.find('.totalRecords').text(totalCount);
		}
		else {
			pagingInfoElement.hide();
		}
	},
	// End Hieu Nguyen
    ...
    /* Commented out this function by Hieu Nguyen on 2020-03-24 as we don't need it anymore
			...
			var callback = function (container) {
					...
					onFilterComplete: function (macthedItems) {
						if (macthedItems.length > 0) {
							jQuery.each(macthedItems, function (i, ele) {
								...
								if (availFieldBlock.find('i').hasClass('fa-chevron-right')) {
				...
				availFieldsListContainer.on('click', '.availFieldBlock a[data-parent="#accordion"]', function (e) {
					...
					if (closestItag.hasClass('fa-chevron-right')) {
						closestItag.removeClass('fa-chevron-right').addClass('fa-angle-down');
					} else {
						closestItag.removeClass('fa-angle-down').addClass('fa-chevron-right');
					}
	...
	registerEvents: function () {
		...
		// this.registerEditLink(); // Comment out by Phu Vo on 2020.03.25 "We don't need it any more" as the comment at its declaration said
...
//      
		...
		app.event.on('Vtiger.Post.MenuToggle', function () {
            ...
            //-- Added by Kelvin Thang on 2020-08-07 - fixed issue #1140 [Listview] Clicking on the search box will result in column deviation
            jQuery('#listview-table').floatThead('reflow');
		});

		this.registerDynamicDropdownPosition();
		//	this.registerHeaderReflowOnListSearchSelections();	Comment out by Vu Mai on 2022-12-28 to prevent header reflow when change dropdown value on listview according to QA feedback
	...
	//	Comment out by Vu Mai on 2022-12-28 to prevent header reflow when change dropdown value on listview according to QA feedback
	...
	registerDropdownPosition: function (container) {
		...
		jQuery('.table-actions').on('click', '.dropdown', function (e) {
			...
			// Added by Phu Vo on 2021.09.27 to fix multiple dropdown show at a time
			$('.listview-row-dropdown:visible').remove();	// Modified by Vu Mai on 2022-08-11 to change class remove
			// End Phu Vo
			...
			fixed_dropdown_menu.addClass('listview-row-dropdown');	// Added by Vu Mai on 2022-08-11 to add identification class for dropdown in row on Listview
		...
		jQuery('.listViewEntries').mouseleave(function (e) {
			...
			setTimeout(function () {
				if (jQuery('.listview-row-dropdown:hover').length == 0) {	// Modified by Vu Mai on 2022-08-11 to change class handle hover event
	...
	/** Modified by Phu Vo on 2021.05.21 to fix list view table sizing */

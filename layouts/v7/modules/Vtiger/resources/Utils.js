...
var vtUtils = {
    ...
    registerEventForDateFields : function(parent, params) {
            ...
            jQuery(element).each(function(index, Elem){
                element = jQuery(Elem);
                 if(calendarType == "range"){
					...
					// Added by Hieu Nguyen on 2021-12-21 to trigger change on target element
					element.on('datepicker-change', function (event, data) {
		...
		// [Core] Added by Phu Vo on 2019.11.06 to handle date range picker close behavior
		$('body').on('click', event => {
			let dateRageInput = $('.dateRange input').not($(event.target));

			dateRageInput.each((index, target) => {
				$(target).data('datepicker') && $(target).data('datepicker').hide();
			});
		});
		// End Phu Vo
    },
    ...
    applyFieldElementsView : function(container){
		...
		// [Core] Added by Phu Vo on 2021.01.29 to support tooltip on every form view
		this.enableTooltips();
		// End Phu Vo
    },
	...
	/** Modified by Phu Vo on 2021.05.21 to use */
    enableTooltips : function(options) {
		const defaultOptions = {
			allowHTML: true,
		};

		generalOptions = Object.assign(defaultOptions, options);

        jQuery(function () {
			jQuery('[data-toggle="tooltip"]').each(function(index, element) {
				let options = Object.assign({}, generalOptions);
				let target = $(element);
				let title = target.attr('data-tippy-content') || target.attr('data-title') || target.attr('title');

				if (!title) return;

				// if (target.is(':input') && !title) {
				// 	title = target.val();
				// 	target.on('change', function () {
				// 		target.attr('data-tippy-content', target.val());
				// 	});
				// }
				...
				target.attr('data-tippy-content', title);

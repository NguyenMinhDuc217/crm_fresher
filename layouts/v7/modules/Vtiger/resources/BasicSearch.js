...
Vtiger.Class('Vtiger_BasicSearch_Js',{},{
	...
	addSearchListener : function () {
		jQuery('.search-link .keyword-input').on('VT_SEARCH_INTIATED',function(e,args){
            ...
            // Added by Hieu Nguyen on 2020-07-03 to validate global search keyword
            jQuery('.error-notify').remove();
            
            if (val.length < _VALIDATION_CONFIG.global_search_keyword_min_length) {
                var message = app.vtranslate('Vtiger.JS_GLOBAL_SEARCH_KEYWORD_MIN_LENGHT_ERROR_MSG', { 'min_length': _VALIDATION_CONFIG.global_search_keyword_min_length });
			...
			app.request.get({'url': url}).then(function (error, data) {
				if (error == null) {
					...
					app.helper.loadPageOverlay(data).then(function (modal) {
						...
						// [Core] Added by Phu Vo on 2020.07.08 to trigger custom event
						app.event.trigger('post.globalSearchResult.load', modal);
						// End Phu Vo

						// Added by Phu Vo on 2021.12.27 to fix global search list don't have scroll bar
						app.helper.showHorizontalScroll(modal.find('.listViewPageDiv .table-container'));
						// End Phu Vo
					});
				}
			});
	...
	registerScrollbar: function () {
		app.helper.showHorizontalScroll($('.settingsgroup .panel-group'));
	},

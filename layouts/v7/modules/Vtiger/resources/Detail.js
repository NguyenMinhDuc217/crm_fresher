...
Vtiger.Class("Vtiger_Detail_Js",{
	...
	getInstance: function(){
		if( Vtiger_Detail_Js.detailInstance == false ){
			...
			var view = app.view(); // Fix can't get particular module detail view js
    ...
    // Modified by Hieu Nguyen on 2020-11-10 to send both SMS and OTT messages
	triggerSendSMSOTT: function (detailActionUrl, channel, targetButton) {
		this.showSendSMSOTTModal(detailActionUrl, channel, targetButton);
	},

	// Refactored from function sendSMS() and sendSMSAction() by Hieu Nguyen on 2020-11-10 to send both SMS and OTT messages
	showSendSMSOTTModal: function (detailActionUrl, channel, targetButton) {
		let thisInstance = this;
		detailActionUrl += '&channel=' + channel;

		let callbackFunction = function (container) {
			container = $('#sendSmsContainer');

			let title = $(targetButton).text().trim();
			container.find('.modal-header').find('h4.pull-left').text(title);
			container.find('[name="channel"]').val(channel);
			container.find('#phoneFormatWarningPop').popover();

			// Added by Phu Vo on 2019.04.19
			...
			if (channel == 'SMS' && typeof _PROVIDER_INFO != 'undefined' && _PROVIDER_INFO.unicode_sms_supported) {
				container.find('#message').removeAttr('data-rule-asciiOnly');
			}
			// End Hieu Nguyen
			
			// Added by Phu Vo on 2020.12.03 to ignore ascii validation on zalo channel
			if (channel != 'SMS') container.find('#message').removeAttr('data-rule-asciiOnly');
			// End Phu Vo

			// Added by Hieu Nguyen on 2022-10-05 to trigger event when send message modal is loaded
			app.event.trigger('post.sendMessageModal.load', container);
			// End Hieu Nguyen
		}

		// Call ajax to get modal content
		app.helper.showProgress();
		let selectedIds = [app.getRecordId()];
		let data = { 'selected_ids': JSON.stringify(selectedIds) };

		app.request.post({ 'url': detailActionUrl, 'data': data, 'dataType': 'html' })
	 ...
	 deleteRelatedActivity : function(e) {
		...
		if(recurringEnabled) {
			app.helper.showConfirmationForRepeatEvents({ mode: 'delete' }).then(function(params) {    // Modified by Hieu Nguyen on 2020-03-17 to specify the desired confirm popup view
		...
		app.request.post({url: url}).then(
				function(err, data) {
					...
					if(status == 'Failed'){
						...
						// Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
						app.helper.showErrorNotification({ title: app.vtranslate('JS_ERROR'), message: app.vtranslate('JS_FAILED_TO_CHECK_SMS_STATUS')});
					}
					else {
						var message = data['message'];
						app.helper.showErrorNotification({title:status, message:message});
	...
	triggerTransferOwnership : function(massActionUrl){
		...
		app.request.post({data:params}).then(
			function(error, data) {
				...
				// Modified by Phu Vo to add call back function and trigger event
				let params = {
					cb: model => {
						app.event.trigger('post.transferOwnershipPopup.load', model);
					}
				}

				app.helper.showModal(data, params);
				// End add call back function and trigger event
	...
	referenceFieldNames : {
		// Modified by Hieu Nguyen on 2022-01-06 to set the right related field name for modules Accounts, Contacts and Leads
		'Accounts' : 'related_account',
		'Contacts' : 'contact_id',
		'Leads' : 'related_lead',
		// End Hieu Nguyen
	...
	getRelatedListSearchParams : function() {
		...
		relatedListTable.find('.listSearchContributor').each(function(index,domElement){
            ...
            if(typeof fieldInfo == 'undefined'){
                ...
                /** Added By Kelvin Thang on 2020-07-17 Fixed related fields wrong Search Operator information */
                if (typeof fieldInfo != 'undefined' && typeof fieldInfo != 'object' && fieldInfo != '') {
                    fieldInfo = JSON.parse(fieldInfo.replace(/&quot;/g, '"'));
                }
            }
	...
	registerSendSmsSubmitEvent: function() {
		...
		jQuery('body').on('submit', '#massSave', function(e) {
			...
			const smsMaxLength = window._SMS_CONFIG ? window._SMS_CONFIG.max_characters || 0 : 0;

			if (smsTextLength > smsMaxLength) {
	...
	SendSmsSave: function(form) {
		...
		app.request.post({data: formData}).then(
            // Modified by Hieu Nguyen on 2020-11-18
            function (err, data) {
                app.helper.hideProgress();
                app.helper.hideModal();
                ...
                if (err){
                    // Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
                    app.helper.showErrorNotification({message:app.vtranslate('JS_SENT_SMS_OTT_ERROR_MSG')});
                    return;
                }
                
                if (data && data[0].status == 'failed') {
                    // Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
                    var errorMsg = app.vtranslate('JS_SENT_SMS_OTT_ERROR_MSG') + '<br/>' + data[0].statusmessage;
                    app.helper.showErrorNotification({ 'title': app.vtranslate('JS_FAILED'), 'message': errorMsg });
                } else {
                    // Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
                    var msg = app.vtranslate('JS_SENT_SMS_OTT_SUCCESS_MSG');
                    app.helper.showSuccessNotification({ 'title': app.vtranslate('JS_SUCCESS'), 'message': msg });
                }

                // Added by Phuc on 2020.03.17 to reload SMS history if exist
                if (jQuery('#SMSTableDetail').length) {
                    jQuery('#SMSTableDetail').DataTable().ajax.reload();
                }
                // Ended by Phuc
            }
		);
	...
	registerEventForRelatedTabClick : function(){
		...
		jQuery('.related-tabs', detailViewContainer).on('click', 'li.more-tab', function(e,urlAttributes){
			...
			/*--Modified By Kelvin Thang -- Date: 2018-07-12--*/
			/*moreTabElement.find('.content').text('');*/
			moreTabElement.find('.content').hide();
			moreTabElement.find('.tab-label').show();
			...
			// self.registerQtipevent(moreTabElement); // Remove tooltip by Phu Vo on 2022.17.01
		});
	...
	loadSelectedTabContents: function(tabElement, urlAttributes){
			...
			// Added by Hieu Nguyen on 2018-07-17 to force reload the full detail page for re-rendering the layout 
			...
			self.loadContents(url,urlAttributes).then(function(data){
				...
				// Added by Phu Vo on 2019.10.25 to update related record counter after change tab
	...
	registerEventsForRelatedList : function(){
		...
		this.registerRelatedRecordDetail(); // Added by Hieu Nguyen on 2021-01-21 to show related record detail
		...
		app.event.on("post.relatedListLoad.click",function(event, container){
            ...
            if ($('.related-tabs').find('li.active').data('module') == 'ModComments') {
                mentionHandler.attach($('#newCommentTextArea'));
                self.initBtnToggleRollupComments($('#recent-comments'));
            }
            // End Hieu Nguyen
		});
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			...
			var key = 'LBL_UNLINK_CONFIRMATION'; // Updated by Phuc on 2019.10.28 to dispay correct unlink label
			...
			if(relatedModuleName == 'Calendar' && row.data('recurringEnabled')) {
				app.helper.showConfirmationForRepeatEvents({ mode: 'delete' }).then(function(customParams) {    // Modified by Hieu Nguyen on 2020-03-17 to specify the desired confirm popup view
	...
	*/
				...
				function(err,data){
					if(err === null){
						if(typeof data !== 'object'){
							...
							// Added by Phuc on 2019.11.18 to alert if delete record successfully
						...
						// Added by Phuc on 2019.11.18 to alert if delete record error
	...
	saveFieldValues : function (fieldDetailList) {
        ...
        // Added by Hieu Nguyen on 2019-05-24 to set additional data for saving custom owner field
        if(data['field'] == 'assigned_user_id') {
            jQuery('#owner-quickedit-container').find(':input').each(function() {
	...
	ajaxEditHandling : function(currentTdElement){
		...
		// Added by Phu Vo on 2019.09.22 to assign value for boolean field
		if (fieldType == 'boolean') value = rawValue === 1 ? true : false;
		// End assign value for boolean field
		...
		if(jQuery('.editElement',editElement).length === 0){
			...
			var actionButtons = '<span class="pointerCursorOnHover input-group-addon input-group-addon-save inlineAjaxSave"><i class="far fa-check"></i></span>';
			actionButtons += '<span class="pointerCursorOnHover input-group-addon input-group-addon-cancel inlineAjaxCancel"><i class="far fa-close"></i></span>';
		...
		if(fieldType === 'reference'){
            // Modified by Hieu Nguyen on 2020-01-03 to fix bug relate field updated to empty when click QuickEdit save button without any change right at page first load
            var referenceIdInput = editElement.find('input[name="'+ fieldName +'"]');
            referenceIdInput.data('value', rawValue);

            var referenceNameInput = editElement.find('input[name="'+ fieldName +'_display"]');
            referenceNameInput.val(detailViewValue.text().trim());
            
            if (value && !referenceNameInput.attr('disabled')) {
                referenceNameInput.attr('disabled', 'disabled');
                editElement.find('.clearReferenceSelection').removeClass('hide');
            }
            // End Hieu Nguyen

            // Added by Hieu Nguyen on 2019-01-06 to fix bug required reference field can not pass the validation check
            referenceNameInput.removeClass('input-error');
            
            referenceNameInput.on(Vtiger_Edit_Js.postReferenceSelectionEvent, function(e, res) {
                var data = res.data;
                var id = (data.id) ? data.id : Object.keys(data)[0];

                referenceIdInput.val(id);
                referenceIdInput.data('value', id);
            });

            editElement.on('click', '.clearReferenceSelection', function(e) {
                referenceIdInput.val('');
                referenceIdInput.data('value', '');
            });
            // End Hieu Nguyen
		}
	...
	registerAjaxEditSaveEvent : function(contentHolder){
		...
		contentHolder.on('click','.inlineAjaxSave',function(e){
			...
			var fieldName = fieldBasicData.data('name');
			var fieldType = fieldBasicData.data("type");
			var previousValue = jQuery.trim(fieldBasicData.data('value'));  // Modified by Hieu Nguyen on 2019-01-02 to get correct previous value
			...
			if(previousValue == ajaxEditNewValue) {
				...
				var params = {
					// Modified on 2019.07.09 by Phu Vo, Fix js error can not get defaultView of undefined by ignore owner template input validation
					'ignore' : 'span.hide .inputElement,input[type="hidden"], #owner-quickedit-template input',
					// End Fix js error can not get defaultView of undefined by ignore owner template input validation
					...
					submitHandler : function(form){
						...
						thisInstance.saveFieldValues(fieldNameValueMap).then(function(err, response) {
							...
							if(fieldBasicData.data('type') == 'picklist' && app.getModuleName() != 'Users') {   // Modified by Hieu Nguyen on 2019-12-20 to make this the first condition
								...
								for(var i=0; i< rawPicklistValues.length; i++) {
									...
									// Refactor code convention by Phu Vo on 2019.09.27
									if (color) {
										var contrast = app.helper.getColorContrast(color);
										var textColor = (contrast === 'dark') ? 'white' : 'black';
                                ...
                                // Modified by Hieu Nguyen on 2019-01-02 to fixed bug module icon updated by name field
								contentHolder.closest('.detailViewContainer').find('.recordBasicInfo')
								...
								if(contentHolder.hasClass('overlayDetail')) {
									contentHolder.find('.overlayDetailHeader').find('.'+fieldName)
									    .html(postSaveRecordDetails[fieldName].display_value); // Refactor code convention by Phu Vo on 2019.09.27
								} else {
									contentHolder.closest('.detailViewContainer').find('.recordBasicInfo')  // Modified by Hieu Nguyen on 2019-01-02 to fixed bug module icon updated by name field
									    .find('.'+fieldName).html(postSaveRecordDetails[fieldName].display_value); // Refactor code convention by Phu Vo on 2019.09.27
							    }
							}
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
            ...
            // Added by Hieu Nguyen on 2020-07-14 to set default value
            var urlParams = app.convertUrlToDataParams(element.data('url'));
            customParams['activitytype'] = urlParams.activitytype;
            customParams['eventstatus'] = urlParams.eventstatus;
            customParams['taskstatus'] = urlParams.taskstatus;
			// End Hieu Nguyen
			
			// Added by Phu Vo to mark event trigger from activity widget (work-around)
			var fromWidget = true;
			// End Phu Vo
			...
			app.event.on("post.QuickCreateForm.show",function(event,form){
				...
				// Added by Hieu Nguyen on 2020-07-14 to make these fields readonly
				// Modified by Phu Vo on 2020.12.25
				// Workaround readonly logic trigger every time from create activity button on activity widget was clicked
				if (fromWidget) {
					fromWidget = false;

					form.find('[name="activitytype"]').attr('readonly', true);
					form.find('[name="eventstatus"]').attr('readonly', true);
					form.find('[name="taskstatus"]').attr('readonly', true);
				};
				// End Hieu Nguyen & Phu Vo
	...
	registerSummaryViewContainerEvents : function(summaryViewContainer){
		...
		summaryViewContainer.on('click', '.editStatus', function(e){
			...
			// Added by Phu Vo on 2021.07.12 to prevent duplicate event registration
			currentDiv.off('clickoutside');
			// End Phu Vo
	...
	removeCommentBlockIfExists : function() {
        ...
        vtUtils.hideValidationMessage(Commentswidget.find('.commentcontent'));  // Added by Hieu Nguyen on 2021-03-17 to hide validation error when closing the comment form
    ...
    // Added by Hieu Nguyen on 2021-06-29
    initBtnToggleRollupComments: function (container) {
        var self = this;
        container.find('#rollup-comments').bootstrapSwitch();

        container.off('switchChange.bootstrapSwitch').on('switchChange.bootstrapSwitch', '#rollup-comments', function(e) {
            self.toggleRollupComments(e);
        });
    },

    // Refactored code and fixed bug toggle rollup error by Hieu Nguyen on 2021-06-29
	toggleRollupComments: function (e) {
		e.stopPropagation();
		e.preventDefault();
		var self = this;
		var currentTarget = jQuery(e.currentTarget);
		var moduleName = currentTarget.data('moduleName');
		var recordId = currentTarget.data('recordId');
		var rollUpId = currentTarget.data('rollupId');
		var rollUpState = currentTarget.is(':checked');

		var viewType = jQuery('.related-tabs').find('li.active').data('module') == 'ModComments' ? 'RelatedList' : 'SummaryView';
        app.helper.showProgress();

        if (viewType == 'SummaryView') {
			url = 'index.php?module='+ moduleName +'&relatedModule=ModComments&view=Detail' +
                '&mode=showRecentComments&parent='+ moduleName +'&record='+ recordId +
                '&rollupid='+ rollUpId +'&rollup_status='+ (rollUpState ? 1 : 0) +'&rollup-toggle=1&limit=5';
            
			app.request.get({ 'url': url }).then(function (err, data) {
                app.helper.hideProgress();

                // Restore to last state if saving error
                if (err) {
                    jQuery('#rollup-comments').attr('checked', !rollUpState).trigger('change');
                    return;
                }

                // Show new comments
                let widgetContainer = jQuery('div[data-name="ModComments"] div.widget_contents');
				widgetContainer.html(data);
                vtUtils.enableTooltips();
                self.initBtnToggleRollupComments(widgetContainer);
			});
		}
		else if (viewType == 'RelatedList') {
            var params = {
                'module': 'Vtiger',
                'view': 'ModCommentsDetailAjax',
                'mode': 'saveRollupSettings',
                'parent': moduleName,
                'parentId': recordId,
                'rollupid': rollUpId,
                'rollup_status': rollUpState ? 1 : 0
            };
			...
			app.request.post({ 'data': params }).then(function (err, data) {
                ...
                // Reload comment list
				jQuery('div.related-tabs li[data-module="ModComments"]').trigger('click');
			});
		}
	},
	 ...
	 */
    // Modified by Hieu Nguyen on 2020-01-16 to refactor code and modified the logic to show overlay detail on name field only
		...
		detailContentsHolder.on('click', '.listViewEntries', function (e) {
			...
			if (selection.length == 0) { 
                ...
                if (targetElement.is('a')) {
                    ...
                    var recordUrl = selectedRow.data('recordurl');
                    var linkUrl = targetElement.attr('href');
                    
                    // This is actually a url
                    if (linkUrl != '#' && linkUrl.indexOf('javascript') < 0) {
                        // User clicked on trigger field, show overlay detailview        
                        if (linkUrl == recordUrl) {
                            self.showOverlayDetailView(recordUrl);
                        }
                        // User clicked on external link, open it in new tab
                        else {
    ...
    showOverlayDetailView: function(recordUrl) {
        app.helper.showProgress();
        ...
        app.request.get({ data: params }).then(function (err, response) {
            ...
            app.helper.loadPageContentOverlay(response, overlayParams).then(function(container) {
                // Init overlay form
                var detailViewController = Vtiger_Detail_Js.getInstanceByModuleName(params.module);
                detailViewController.showScroll(jQuery('.overlayDetail .modal-body'));
                detailViewController.setModuleName(params.module);
                detailViewController.setOverlayDetailMode(true);
                detailViewController.setContentHolder(container.find('.overlayDetail'));
                detailViewController.setDetailViewContainer(container.find('.overlayDetail'));
                detailViewController.registerOverlayEditEvent();
                detailViewController.registerBasicEvents();
                detailViewController.registerClickEvent();
                detailViewController.registerHeaderAjaxEditEvents(container.find('.overlayDetailHeader'));
                detailViewController.registerEventToReloadRelatedListOnCloseOverlay(parentRecordId);
                app.event.trigger('post.overlay.load', parentRecordId, params); 

                // Prevent overlay form submit
    ...
    // Implemented by Hieu Nguyen on 2021-01-21 to show related record detail
    registerRelatedRecordDetail: function(){
		var self = this;
		var detailViewContainer = this.getContentHolder();

		detailViewContainer.on('click', 'a[name="relationDetail"]', function(e) {
			e.stopImmediatePropagation();
			var detailUrl = $(this).data('url');
			self.showOverlayDetailView(detailUrl);
		});
	},
	...
	registerEvents : function() {
        ...
        // Added by Hieu Nguyen on 2021-03-16 to support @mention in comment
        CKEDITOR.disableAutoInline = true;
        mentionHandler.attach($('#newCommentTextArea'));

        $('.details').on('input', '.commentTextArea', function () {
            var container = $(this).parent('div');
            container.find('[name="commentcontent"]').val(mentionHandler.getDbFormat($(this)));
        });
        // End Hieu Nguyen
		...
		detailContentsHolder.on('click','.saveComment', function(e){
			...
			if(!element.is(":disabled")) {
				...
				dataObj.then(function(data){
                    ...
                    // Modified by Hieu Nguyen on 2021-03-17 to clear comment form after saving comment
					var commentInfoBlock = currentTarget.closest('.singleComment');
					var commentTextAreaElement = closestAddCommentBlock.find('.commentTextArea');
					var commentHiddenInput = closestAddCommentBlock.find('.commentcontent');
					commentTextAreaElement.html('');
					commentHiddenInput.val('')
                    // End Hieu Nguyen

					closestAddCommentBlock.find('#is_private').removeAttr('checked');
					if(mode == "add"){
						var commentId = data['id'];
                        app.helper.showProgress();  // Added by Hieu Nguyen on 2021-03-17 to show loading when reload the comment list
						var commentHtml = self.getCommentUI(commentId);
						commentHtml.then(function(data){
                            app.helper.hideProgress();  // Added by Hieu Nguyen on 2021-03-17 to hide loading after the comment list is loaded
							var html;
							...
							if(commentBlock.length > 0){
								...
								// commentTextAreaElement.css({height : '71px'});   // Commented out this line by Hieu Nguyen on 2021-03-17
							}
                        ...
                        // Modified by Hieu Nguyen on 2021-03-16 to display comment with @mention
						commentInfoContent.data('commentDbFormat', data.commentcontent);
						commentInfoContent.data('commentDisplayFormat', data.commentcontent_rendered);
						commentInfoContent.html(data.commentcontent_rendered);
                        // End Hieu Nguyen
		...
		detailContentsHolder.on('click','.editComment', function(e){
            ...
            // Modified by Hieu Nguyen on 2021-03-16 to support @mention in comment
			var commentDbFormat = commentInfoContent.data('commentDbFormat');
			var commentDisplayFormat = commentInfoContent.data('commentDisplayFormat');

			editCommentBlock.find('.commentcontent').text(app.helper.getDecodedValue(commentDbFormat));
			editCommentBlock.find('.commentTextArea').html(commentDisplayFormat);
            ...
            mentionHandler.attach(editCommentBlock.find('#editCommentTextArea'));
            // End Hieu Nguyen
		});
		...
		detailContentsHolder.on('click','.replyComment', function(e){
            ...
            // Modified by Hieu Nguyen on 2021-03-16 to support @mention in comment
            var creatorElm = commentInfoBlock.find('.creatorName');
            var creatorName = creatorElm.text().trim();
            var creatorId = creatorElm.data('creatorId').trim();

            mentionHandler.attach(addCommentBlock.find('#replyCommentTextArea'));
            mentionHandler.insertNewMention(addCommentBlock.find('#replyCommentTextArea'), creatorId, creatorName);
            // End Hieu Nguyen
		});
		...
		app.event.on('post.summarywidget.load',function(event,widgetContainer){
            ...
            // Added by Hieu Nguyen on 2021-03-16 to support @mention in comment
            if (widgetContainer.hasClass('widgetContainer_comments')) {
                mentionHandler.attach($('#newCommentTextArea'));

                widgetContainer.find('.commentTextArea').on('input', function () {
                    var container = $(this).parent('div');
                    container.find('[name="commentcontent"]').val(mentionHandler.getDbFormat($(this)));
                });
            }
			...
			// Modified by Hieu Nguyen on 2021-06-29 to init rollup comment button at Summary View
			if (widgetContainer.data('name') == 'ModComments' && jQuery('#rollup-comments')[0] != null) {
				self.initBtnToggleRollupComments(widgetContainer);
			}
			// End Hieu Nguyen
		});		
		
        // Modified by Hieu Nguyen on 2021-06-29 to init rollup comment button at Comments subpanel
        if (jQuery('.related-tabs').find('li.active').data('module') == 'ModComments' && jQuery('#rollup-comments')[0] != null) {
            self.initBtnToggleRollupComments(jQuery('#recent-comments'));
        }
        // End Hieu Nguyen

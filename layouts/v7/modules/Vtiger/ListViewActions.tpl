    ...
    <div id="listview-actions" class="listview-actions-container">
        ...
        <div class = "row">
            ...
            <div class="btn-group listViewActionsContainer" role="group" aria-label="...">
                ...
                {/if}
                {if $deleteAction}
                    <button type="button" class="btn btn-outline-danger" id={$MODULE}_listView_massAction_{$deleteAction->getLabel()} 
            ...
            <div class='col-md-7'>
                ...
                {* [Core] Modified by Phu Vo on 2019.08.28 to display clearer current filter message *}
                ...
                {if $VIEWID}
                    ...
                    {if $CVNAME eq 'All' && $CURRENT_TAG neq ''}
                        {$replaceParams = ['%module_name' => vtranslate($MODULE, $MODULE), '%tag_name' => $TAGS[$CURRENT_TAG]->getName()]}
                        <div class="current-filter-message" data-toggle="tooltip" title="{vtranslate('LBL_DISPLAYING_RESULTS_TAG', $MODULE, $replaceParams)}">
                            {vtranslate('LBL_DISPLAYING_RESULTS_TAG', $MODULE, $replaceParams)}
                        </div>
                    {else if $CVNAME eq 'All'}
                    ...
                    {else}
                        {$replaceParams = ['%module_name' => vtranslate($MODULE, $MODULE), '%filter_name' => vtranslate($CVNAME, $MODULE)]}
                        <div class="current-filter-message" data-toggle="tooltip" title="{vtranslate('LBL_DISPLAYING_RESULTS_FILTER', $MODULE, $replaceParams)}">
                            {vtranslate('LBL_DISPLAYING_RESULTS_FILTER', $MODULE, $replaceParams)}
                        </div>
                    {/if}
                {/if}

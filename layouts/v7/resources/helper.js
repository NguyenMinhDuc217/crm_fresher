...
jQuery.Class("Vtiger_Helper_Js",{
    ...
    /** Modified by Phu Vo on 2021.05.21 to change scrollbar library to perfect scrollbar */
    ...
    showModal : function(content,params) {
        ...
        // Modified by Tung Nguyen on 2022.12.06 to register event preShowCb onetime
        container.off('show.bs.modal').one('show.bs.modal', () => {
            if (typeof params.preShowCb === 'function') params.preShowCb(container);
        });
        
        // Modified by Phuc on 2019.10.10 to off container before trigger new one
        ...
        // Added by Hieu Nguyen on 2020-03-19 to do work arround for static backdrop
        if (params.backdrop == 'static') {
            jQuery(container).data('bs.modal').options.backdrop = 'static';
        }
        // End Hieu Nguyen
    ...
    */ 
    ...
    _initNotificationDefaults : function() {
        if(jQuery('.module-action-bar').length) {
            ...
            // Modified by Vu Mai on 2022-12-15 to fix bug notification is not showing
            let offsetTop = jQuery('.module-action-bar').offset().top;
            let actionBarHeight = jQuery('.module-action-bar').height();

            // Set offset top is module action bar to prevent the location of the notification is off-screen
            if (offsetTop > actionBarHeight) {
                offsetTop = actionBarHeight;
            }
            // End Vu Mai
            ...
            var defaultSettings = {
                    ...
                    'offset' : {
                        'x' : 50,
                        'y' : 92 + offsetTop    // Modified by Vu Mai on 2022-12-15 to fix bug notification is not showing
                    },
                    'z_index' : 10003,
                    // Modified by Phu Vo on 2019.04.19 => get template dynamic
                    'template' : this._getNotificationTemplate(),
                    // End Phu Vo
                    'delay' : 600,
                    'animate' : 'null'
                }
    ...
    _getNotificationTemplate : function(options) {
        options = options || {};
        
        let templateClass = `col-xs-11 col-sm-3 vt-notification vt-notification-{0} ${options.type}`.trim();

        let template = `
            <div data-notify="container" class="${templateClass}" role="alert">
                <div class="notificationHeader">
                    <span data-notify="icon"></span> 
                </div>
                <div class="notificationBody">
                    <button type="button" aria-hidden="true" class="close pull-right" data-notify="dismiss"><i class="far fa-times"></i></button>
    ...
    showAlertNotification : function(options,settings) {
        // Modified by Phu Vo on 2019.04.19
        var defaultOptions = {
        ...
        var defaultSettings = {
            template: this._getNotificationTemplate({type: 'alert-notify'}),
        }
    ...
    showErrorNotification : function (options, settings) {
        ...
        var defaultSettings = {
            ...
            template: this._getNotificationTemplate({type: 'error-notify'}),
        }
        // End Phu Vo
    ...
    showSuccessNotification : function(options, settings) {
        ...
        var defaultSettings = {
            template: this._getNotificationTemplate({type: 'success-notify'}),
        }
	...
	showConfirmationForRepeatEvents : function(customParams) {
		...
		var params = {
			module : 'Calendar',
			view : 'RecurringUpdateConfirmationAjax',   // Modified by Hieu Nguyen on 2020-03-17 to support loading both edit and delete event confirm popup
		};
		...
		app.request.get({'data' : params}).then(function(e,data) {
			...
			var callback = function(modalContainer) {
                ...
                if (customParams.mode == 'edit') {
                    modalContainer.find('button.close').remove();
                }
                // End Hieu Nguyen
	...
	registerModalDismissWithoutSubmit: function(form) {
		var initialFormData = form.serialize();
		form.find('.modal-footer .close, .modal-footer .cancelLink').click(function(e) { // Modified by Phuc on 2020.07.31 to register event for form only, not for all links -- Modified by Kelvin Thang on 2020.11.03 register event for form modal only
			if (initialFormData != form.serialize() && form.data('submit') != "true") {
        ...
        // Added by Hieu Nguyen on 2019-09-19 to prevent modal dismiss when clicking on the backdrop

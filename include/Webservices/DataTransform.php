 ...
 *************************************************************************************/
	...
	class DataTransform{
		...
		function sanitizeForInsert($row,$meta){
            ...
            // Modifed by Hieu Nguyen on 2019-07-08 to fix bug can not save owner fields when import and convert Lead
            $ownerFields = $meta->getOwnerFields();

			foreach ($ownerFields as $field) {
				if (!empty($row[$field]) && strpos($row[$field], 'x') !== false) {
					$row[$field] = end(explode('x', $row[$field])); // Needs normal owner id
				}
			}
            
            //$row = DataTransform::sanitizeOwnerFields($row, $meta);
            // End Hieu Nguyen
			...
			foreach($ownerFields as $index=>$field){
				if(isset($row[$field]) && $row[$field]!=null && $row[$field] != 0){
					...
					if ($ownerType) {
                        ...
                        // Modified by Hieu Nguyen on 2019-07-26 to fix bug generate wrong webservice owner id (1x1x111)
                        if (strpos($row[$field], 'x') == false) {
						    $row[$field] = vtws_getId($webserviceObject->getEntityId(), $row[$field]);
                        }
                        // End Hieu Nguyen
					...
					if($fieldObj->getFieldDataType()=="currency") {
						if($fieldObj->getUIType() == '71') {
							...
							if (!empty($currencyConversionRate)) {
								$rawBaseCurrencyValue = CurrencyField::convertToBaseCurrency($row[$fieldName], $currencyConversionRate);

...
class VtigerCRMObject{
	...
	public function create($element, $ownerPopulated = false) { // Modified by Hieu Nguyen on 2020-08-04 to support param $ownerPopulated
		global $adb;
        ...
        // Added by Hieu Nguyen on 2020-08-04 to prevent preset main owner id to be overridden while saving from Workflow
        $this->instance->column_fields['owner_populated'] = $ownerPopulated;
        // End Hieu Nguyen
		
        // Modified by Hieu Nguyen on 2021-05-10 to fix bug transaction hang forever when there is an exception throws by $this->instance->Save()
        try {
            $adb->startTransaction();
		    $this->instance->Save($this->getTabName());
            $error = $adb->hasFailedTransaction();	// Function hasFailedTransaction() must be called before calling function completeTransaction() to avoid lossing error counter
			$adb->completeTransaction();
            return !$error;
        }
        catch(Exception $ex) {

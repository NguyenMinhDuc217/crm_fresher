...
function vtws_isRoleBasedPicklist($name) {
	// Added by Phu Vo on 2020.06.05
	global $nonRoleBasedPicklists;
	if (in_array($name, $nonRoleBasedPicklists)) return false; 
	// End Phu Vo
 ...
 */
function vtws_transferRelatedActivities($leadId,$accountId,$contactId,$relatedId) { // Rename by Phu Vo on 2021.01.03
	...
	for($i=0;$i<$rowCount;++$i) {
		...
		// Refactor code by Phu Vo on 2021.01.01 to improve readaliblty and make sure to transer activities to only one target
		// Edited by Phu Vo on 2022.01.18 to revert how relation with contact is saved
		if ($type != 'Emails') {
			if (!empty($accountId) && $relatedId == $accountId) {
				$sql = 'INSERT INTO vtiger_seactivityrel(crmid, activityid) VALUES (?, ?)';
				$resultNew = $adb->pquery($sql, [$accountId, $activityId]);

				if ($resultNew == true) {
					$sql = 'UPDATE vtiger_activity SET related_account = ? WHERE activityid = ?';
					$adb->pquery($sql, [$accountId, $activityId]);
				}

				if ($resultNew === false) return false;
			}
			if (!empty($contactId) && $relatedId == $contactId) {
			...
			// Remove old relation value with lead
			$sql = 'UPDATE vtiger_activity SET related_lead = NULL WHERE activityid = ?';
			$adb->pquery($sql, [$activityId]);
		}
		else {
			$sql = 'INSERT INTO vtiger_seactivityrel(crmid, activityid) VALUES (?, ?)';
	...
	foreach ($mandatoryFields as $field) {
		if (empty($entity[$field])) {
			...
			if (($fieldInfo['type']['name'] == 'picklist' || $fieldInfo['type']['name'] == 'multipicklist' || $fieldInfo['type']['name'] == 'date' || $fieldInfo['type']['name'] == 'datetime') && ($fieldInfo['editable'] == true)) {
				...
				$entity[$field] = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
			}
		}
	}
	return $entity;

...
function vtws_convertlead($entityvalues, $user) {
	checkRecordLimitWhenConvertData('Leads');	// Added by Hieu Nguyen on 2022-04-07 to check record limit before convert Lead
	...
	$activeAdminUser = Users::getRootAdminUser();
	...
	try {
		...
		vtws_transferRelatedActivities($leadIdComponents[1], $accountId, $contactId, $relatedIdComponents[1]); // Modified by Phu Vo on 2021.01.03
...
function vtws_updateConvertLeadStatus($entityIds, $leadId, $user) {
	...
	if ($entityIds['Accounts'] != '' || $entityIds['Contacts'] != '') {
         ...
         * Date: 2018-09-28
         * set status Converted for Lead
         */
		//$sql = "UPDATE vtiger_leaddetails SET converted = 1 where leadid=?";
		$sql = "UPDATE vtiger_leaddetails SET converted = 1, leadstatus = 'Converted' where leadid=?";
		$result = $adb->pquery($sql, array($leadIdComponents[1]));
		if ($result === false) {

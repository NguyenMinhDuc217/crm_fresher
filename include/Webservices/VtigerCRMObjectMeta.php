...
class VtigerCRMObjectMeta extends EntityMeta {
	...
	function hasAssignPrivilege($webserviceId){
		...
		// Modified by Phu Vo on 2019.07.09 to process with custom owner field logic
		if ( // Convert to webservice format if it is an owner value from request
			Vtiger_CustomOwnerField_Helper::isOwnerUserId($webserviceId) 
			|| Vtiger_CustomOwnerField_Helper::isOwnerGroupId($webserviceId)
		) {
			$ownerId = Vtiger_CustomOwnerField_Helper::findOwnerIdFromRequestString($webserviceId);
			$webserviceId = vtws_getWebserviceEntityId(vtws_getOwnerType($ownerId), $ownerId);
		}
		// End process with custom owner field logic
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from vtiger_users where id=? and deleted < 2"; // Modified by Hieu Nguyen on 2021-07-13 to load only user has not been deleted permanently
	...
	function exists($recordId){
		...
		if($this->objectName == 'Users'){
			if (array_key_exists($recordId, $user_exists_cache)) {
				...
				$sql = "select 1 from vtiger_users where id=? and deleted < 2"; // Modified by Hieu Nguyen on 2021-07-13 to load only user has not been deleted permanently
			}

...
function vtws_create($elementType, $element, $user) {
	...
	if ($meta->hasMandatoryFields($element)) {
        ...
        // Modified by Hieu Nguyen on 2020-08-4 to prevent overriding preset owner field values
        if (!empty($element['main_owner_id'])) {
            $entity = $handler->create($elementType, $element, true);
        }
        else {

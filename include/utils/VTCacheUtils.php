...
class VTCacheUtils {
	...
	// Modified $presencein param by Hieu Nguyen on 2018-12-29
	static function lookupFieldInfo_Module($module, $presencein = array('0', '1', '2')) {
	...
	// Refactored by Hieu Nguyen on 2022-03-12 to store cache dynamically
	static function updateReport_Info($userId, $reportId, array $cacheData) {
		if (!isset(self::$_reportmodule_infoperuser_cache[$userId])) {
			self::$_reportmodule_infoperuser_cache[$userId] = [];
		}
		
		self::$_reportmodule_infoperuser_cache[$userId][$reportId] = $cacheData;
	}

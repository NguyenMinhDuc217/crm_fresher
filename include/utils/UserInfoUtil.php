...
/** Function to check if the currently logged in user is permitted to perform the specified action
  ...
  *
 */
function isPermittedByModule($module, $actionname, $record_id = '') // Rename function by Hieu Nguyen on 2021-05-31 to apply caching
{
	global $log;
    ...
    // Added by Hieu Nguyen on 2020-12-24 to fix permission check for Calendar
    if ($module == 'Events') $module = 'Calendar';
    // End Hieu Nguyen

    // Addded by Hieu Nguyen on 2020-12-22 to allow admin, manager and all assigned users to access Task record
    if ($module == 'Calendar' && !empty($record_id) && vtws_getCalendarEntityType($record_id) == 'Calendar' && isCalendarTaskPermittedBySharing($record_id, $actionname) == 'yes') {
        return 'yes';
    }
    // End Hieu Nguyen
	...
	if(vtlib_isModuleActive($checkModule)){
        ...
        // Modified by Hieu Nguyen on 2020-06-16 to check permission with new owner field logic
		$ownerArr = getRecordOwnerId($record_id, true);
        $recordOwnerArr = [];

        if (empty($ownerArr)) {
            $groupId = getRecordGroupId($record_id);
			return isPermittedByOwner($module, $tabid, $actionid, $record_id, 'Groups', $groupId);
		}

        if ($ownerArr['main_owner_id'] == $ownerArr['smownerid']) {
            return isPermittedByOwner($module, $tabid, $actionid, $record_id, 'Users', $ownerArr['main_owner_id']);
        }
        else {
            $permission = 'no';
            $ownerArr = Vtiger_Owner_UIType::getCurrentOwners($ownerArr['smownerid'], false);
            
            foreach ($ownerArr as $owner) {
                list($recOwnType, $recOwnId) = explode(':', $owner['id']);

                $permission = isPermittedByOwner($module, $tabid, $actionid, $record_id, $recOwnType, $recOwnId);
                if ($permission == 'yes') return $permission;
            }
...
// Added by Hieu Nguyen on 2020-06-16. This function was splitted from line 459-575 for function isPermitted() above so that it can be called multiple times
function isPermittedByOwner($module, $tabid, $actionid, $record_id, $recOwnType, $recOwnId) {
    ...
    if($recOwnType == 'Users')
    {
        ...
        foreach($subordinate_roles_users as $roleid=>$userids)
        {
            if(in_array($recOwnId,$userids))
            {
                ...
                // Modified by Hieu Nguyen on 2020-12-30 to check sharing rule for Calendar Events only
                if ($module == 'Calendar' && vtws_getCalendarEntityType($record_id) == 'Events') {
                    $permission = isCalendarEventPermittedBySharing($record_id, $action);
                }
                // End Hieu Nguyen
    ...
    // Added by Hieu Nguyen on 2020-08-31 to check permission for private activities
    if ($module == 'Calendar') {
        if (in_array($action, ['index']) || $current_user->is_admin == 'on') {
            return 'yes';
        }

        $recordData = getFullRecordData($record_id, $module);

        if ($recordData['main_owner_id'] == $current_user->id) {
            return 'yes';
        }

        // Check record busy
        $recordData['activityid'] = $record_id;
        $recordBusy = Calendar_Data_Model::isRecordBusy($recordData, 'SharedCalendar', $recOwnId, $current_user->id);
        $permission = 'no';

        if ($recordBusy) {
            $relatedActivitiesDisplayConfig = Calendar_Data_Model::getDisplayConfigForActivitiesRelatedList();
            
            if ($relatedActivitiesDisplayConfig['main_owner_full_access'] == '1' && in_array($action, ['DetailView'])) {
                $permission = 'yes';
            }
        }
        ...
        // Pass difficult cases for specific functions
        if (vtws_getCalendarEntityType($record_id) == 'Events') {
            return isCalendarEventPermittedBySharing($record_id, $action);
        }
        else {
            return isCalendarTaskPermittedBySharing($record_id, $action);
        } 
    }
    // End Hieu Nguyen
    ...
    if($others_permission_id == 0)
    {
        ...
        {
            // Modified by Hieu Nguyen on 2020-12-29 to remove condition for Calendar as this case is handled above
...
/** Function to check if the currently logged in user has Read Access due to Sharing for the specified record
  ...
  * @returns yes or no. If Yes means this action is allowed for the currently logged in user. If no means this action is not allowed for the currently logged in user
 */
// Modified by Hieu Nguyen on 2022-09-07 to use common function hasReadWritePermissionBySharing()
function isReadPermittedBySharing($module, $tabid, $actionid, $record_id) {
	...
	$permission = hasReadWritePermissionBySharing($module, $tabid, $actionid, $record_id, 'read');
	$log->debug("Exiting isReadPermittedBySharing method. Permission = {$permission}");
	return $permission;
}
...
function isReadWritePermittedBySharing($module, $tabid, $actionid, $record_id) {
	...
	$permission = hasReadWritePermissionBySharing($module, $tabid, $actionid, $record_id, 'write');
	$log->debug("Exiting isReadWritePermittedBySharing method. Permission = {$permission}");
	return $permission;
}

// Added by Hieu Nguyen on 2022-09-07. This function was extracted from the original function isReadWritePermittedBySharing() for common usage and to be easier to maintain
function hasReadWritePermissionBySharing($module, $moduleId, $actionId, $recordId, $permission) {
	global $log, $current_user;

	if (!in_array($permission, ['read', 'write'])) {
		throw new AppException("Invalid value for $permission param at function hasReadWritePermittedBySharing()", 500);
	}

	require("user_privileges/sharing_privileges_{$current_user->id}.php");
	$sharingModuleList = getSharingModuleList();

	// This module is not in sharing list
	if (!in_array($module, $sharingModuleList)) {
		return 'no';
	}

	// Check sharing permission on given record
	$log->debug('Checking sharing permission on given record');
	$recordOwner = getRecordOwnerId($recordId, true);
	$sharingPermissionsVar = "{$module}_share_{$permission}_permission";
	$sharingPermissions = ${$sharingPermissionsVar};
	$permission = hasSharingPermissionOnRecord($recordOwner['smownerid'], $sharingPermissions);
	if ($permission == 'yes') return 'yes';

	// Check sharing permission on parent records
	$relatedModules = $related_module_share[$moduleId];

	if (is_array($relatedModules)) {
		$log->debug('Checking sharing permission on parent records');

		foreach ($relatedModules as $parentModuleId) {
			$parRecordOwner = getParentRecordOwner($moduleId, $parentModuleId, $recordId, true);
			
			if (sizeof($parRecordOwner) > 0) {
				$parentModule = getTabname($parentModuleId);
				$sharingPermissionsVar = "{$parentModule}_{$module}_share_{$permission}_permission";
				$sharingPermissions = ${$sharingPermissionsVar};
				$permission = hasSharingPermissionOnRecord($recordOwner['smownerid'], $sharingPermissions);
				if ($permission == 'yes') return 'yes';
			}
...
// Added by Hieu Nguyen on 2022-09-07 to check if current user has permission to access record with the givent record's assigned user id and an array of permission to check
function hasSharingPermissionOnRecord($recordAssignedUserId, $recordModuleSharingPermissions) {
	$recordOwners = Vtiger_Owner_UIType::getCurrentOwners($recordAssignedUserId);

	// Check sharing permission on all owners of the record
	foreach ($recordOwners as $ownerInfo) {
		...
		if (strpos($ownerId, Vtiger_CustomOwnerField_Helper::USER_ID_PREFIX) !== false) {
			...
			foreach ($recordModuleSharingPermissions['ROLE'] as $roleId => $userIds) {
				if (in_array($ownerId, $userIds)) {
					return 'yes';
				}
			}

			foreach ($recordModuleSharingPermissions['GROUP'] as $groupId => $userIds) {
				if (in_array($ownerId, $userIds)) {
					return 'yes';
				}
			}
		...
		// Owner is a group
		if (strpos($ownerId, Vtiger_CustomOwnerField_Helper::GROUP_ID_PREFIX) !== false) {
			$ownerId = str_replace(Vtiger_CustomOwnerField_Helper::GROUP_ID_PREFIX, '', $ownerId);

			if (array_key_exists($ownerId, $recordModuleSharingPermissions['GROUP'])) {
				return 'yes';
			}
		}
	}
...
{
	// Added by Hieu Nguyen on 2023-01-03 to apply caching
	static $cache = [];
...
// Modified by Hieu Nguyen on 2020-12-29 to check permission for Calendar's Event record by sharing rules
function isCalendarEventPermittedBySharing($recordId, $action) {
	global $adb, $current_user;
    $recordData = getFullRecordData($recordId, 'Calendar');
	$permission = 'no';

    // User accepted activity invitation can access the record detail
    if (
        in_array($action, ['index', 'DetailView'])
    ...
    // User can see public events and access that events detail
    if (in_array($action, ['index', 'DetailView']) && $recordData['visibility'] == 'Public') {
        return 'yes';
    }

    // Only main owner can see edit and unlink button at subpanel
    if (Calendar_Logic_Model::isAtActivitiesRelatedList() && in_array($action, ['EditView', 'Delete'])) {
        if ($recordData['main_owner_id'] != $current_user->id) {
            return 'no';
        }
    }
...
// Modified by Hieu Nguyen on 2020-12-29 to check permission for Calendar's Task record by sharing rules
function isCalendarTaskPermittedBySharing($recordId, $action) {
	global $current_user;
	require('user_privileges/user_privileges_'. $current_user->id .'.php');
    if ($is_admin) return 'yes';

	$permission = 'no';
    $recordOwners = getRecordOwnerId($recordId, true);

    // Owner is custom group
    if (Vtiger_CustomOwnerField_Helper::isCustomGroup($recordOwners['smownerid'])) {
        $groupUserIds = getGroupMemberIds($recordOwners['smownerid']);

		// User can access event if he is one of the event owners
        if (in_array($current_user->id, $groupUserIds)) {
            $permission = 'yes';
        }
		else {
			// User can access event if he is not the event owner but is the manager of event owner
			foreach ($groupUserIds as $groupUserId) {
				if (isCurrentUserRoleLevelGreaterThan($groupUserId)) {
					$permission = 'yes';
					break;
				}
			}
    ...
    // Owner is a single normal group
    else if (vtws_getOwnerType($recordOwners['smownerid']) == 'Groups') {
        $groupUserIds = getGroupMemberIds($recordOwners['smownerid']);
    ...
    // Owner is a single user
        ...
        $mainOwnerId = $recordOwners['main_owner_id'];

        // User can access event if he is the event owner
        if ($current_user->id == $mainOwnerId) {
            $permission = 'yes';
        }
        else {
			// User can access event if he is not the event owner but is the manager of event owner
			if (isCurrentUserRoleLevelGreaterThan($mainOwnerId)) {
				$permission = 'yes';
			}
        }
    }
...
// Added by Hieu Nguyen on 2022-11-18 to check if current user role level is greater than the specific user's role
function isCurrentUserRoleLevelGreaterThan($userId) {
	static $cache = [];
	if ($cache[$userId] != null) return $cache[$userId];

	// Get record owner role
	$recordOwnerModel = Users_Record_Model::getInstanceFromPreferenceFile($userId);
	$recordOwnerRole = $recordOwnerModel->getRole();
	$recordOwnerParentRoles = getParentRole($recordOwnerRole);	// Do not use function getParentRoleSequence() as it will return null when user is admin
	...
	if ($currentUserRole != $recordOwnerRole && in_array($currentUserRole, $recordOwnerParentRoles)) {
		$result = true;
	}

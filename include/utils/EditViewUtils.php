...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	elseif(in_array($module, $lineItemSupportedModules))
	{
		$query = '(SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.product_no AS productcode, vtiger_products.purchase_cost,
					...
					WHERE vtiger_seproductsrel.crmid=? AND vtiger_crmentity.deleted=0 AND vtiger_products.active=1)
					UNION
					...
					WHERE vtiger_crmentityrel.crmid=? AND vtiger_crmentity.deleted=0 AND vtiger_service.active=1)';
	...
	elseif ($module == 'Vendors') {
		$query = 'SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.product_no AS productcode, vtiger_products.purchase_cost,
					...
					INNER JOIN vtiger_productcf ON vtiger_products.productid = vtiger_productcf.productid
					WHERE vtiger_vendor.vendorid=? AND vtiger_crmentity.deleted=0 AND vtiger_products.active=1';
			$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
         ...
         * get section data
        */
        $sectionName = $adb->query_result($result,$i-1,'section_name');
        $sectionNum = $adb->query_result($result,$i-1,'section_num');

        if ($sectionNum) {
            $product_Detail[$i]['sectionName'.$i] = $sectionName;
            $product_Detail[$i]['sectionNum'.$i] = $sectionNum;
            $product_Detail[$i]['sectionNumTabID'] = $sectionNum;
            $product_Detail[$i]['sectionNumTabName'] = $sectionName;
        }
		...
		if (in_array($module, $lineItemSupportedModules) || $module === 'Vendors' || (!$focus->mode && $seid)) {
			$subProductsQuery = 'SELECT vtiger_seproductsrel.crmid AS prod_id, quantity FROM vtiger_seproductsrel
								 ...
								 WHERE vtiger_seproductsrel.productid=? AND vtiger_seproductsrel.setype=? AND vtiger_products.active=1';

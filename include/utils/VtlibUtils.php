...
function vtlib_purify($input, $ignore=false) {
	...
	//-- Removed By Kelvin Thang -- on 2019-08-01 -- Reverted back "Fix bug set currency user and Merge Record" changed path "Fix in inventory convert data before insert to db"
    /*if(!empty($input) &&
        $_POST['module'] != 'Users'
        && is_numeric(Vtiger_Currency_UIType::convertToDBFormat($input, null, true))
        && $_REQUEST['view'] != 'MergeRecord'
    ){
	    $input = Vtiger_Currency_UIType::convertToDBFormat($input, null, true);
    }*/
	...
	if(!$ignore) {
		...
		if($__htmlpurifier_instance == false) {
			...
			include_once ('libraries/htmlpurifier/library/HTMLPurifier.safe-includes.php');	// Modified by Hieu Nguyen on 2022-08-03 to prevent error 'HTML Purifier autoloader registrar is not compatible with non-static object methods due to PHP Bug #44144'

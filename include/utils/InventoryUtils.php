...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		//--Begin: Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
		$qty = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['qty'.$i], null, true));	
		$listprice = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['listPrice'.$i], null, true));	
		//--End: Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
		
		$comment = vtlib_purify($_REQUEST['comment'.$i]);
		
		//--Begin: Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
		$purchaseCost = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['purchaseCost'.$i], null, true));		
		$margin = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['margin'.$i], null, true));
		//--End: Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
         ...
         * Add by Kelvin Thang
         * Date: 2018-09-07
         * Desc: insert section for item
         */
        $sectionNum = vtlib_purify($_REQUEST['section_'.$i]);
        $sectionName = vtlib_purify($_REQUEST['section_name'.$sectionNum]);
         ...
         * Date: 2018-09-10
         * insert section data for item
         */

		$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, section_num, section_name)
					VALUES(?,?,?,?,?,?,?,?,?,?,?)';
		$qparams = array($focus->id,$prod_id,$prod_seq,$qty,$listprice,$comment,$description, $purchaseCost, $margin, $sectionNum, $sectionName);
		$adb->pquery($query,$qparams);
		...
		elseif($_REQUEST['discount_type'.$i] == 'amount')
		{
			...
			//--Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
			$discount_amount = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['discount_amount'.$i], null, true));			
		...
		if($_REQUEST['taxtype'] == 'group')
		{
			for($tax_count=0;$tax_count<count($all_available_taxes);$tax_count++)
			{
				...
				if(isset($_REQUEST[$request_tax_name])) {
					//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
					$tax_val = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST[$request_tax_name], null, true));				
				}
		...
		else
		{
			...
			for($tax_count=0;$tax_count<count($taxes_for_product);$tax_count++)
			{
				...
				$updatequery .= " $tax_name = ?,";
				//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
				array_push($updateparams, vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST[$request_tax_name], null, true)) );
			}
		}
		...
		for($taxCount=0; $taxCount<count($all_available_taxes); $taxCount++) {
			if ($all_available_taxes[$taxCount]['method'] === 'Deducted') {
				...
				if(isset($_REQUEST[$requestTaxName])) {
					//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
					$taxValue = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST[$requestTaxName], null, true)) ;					
				}
	...
	//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
	$subtotal = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['subtotal'], null, true));

	$updatequery .= " subtotal=?,";
	array_push($updateparams, $subtotal);

	//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
	$pretaxTotal = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['pre_tax_total'], null, true));
	...
	{
		//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
		$discount_amount_final = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['discount_amount_final'], null, true));		
	...
	//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
	$shipping_handling_charge = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['shipping_handling_charge'], null, true));
	...
	if($_REQUEST['adjustmentType'] == '-')
		//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
		$adjustmentType = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['adjustmentType'], null, true));	
	
	//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
	$adjustment = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['adjustment'], null, true));	
	$updatequery .= " adjustment=?,";
	array_push($updateparams, $adjustmentType.$adjustment);
	
	//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
	$total = vtlib_purify(Vtiger_Currency_UIType::convertToDBFormat($_REQUEST['total'], null, true));		
	$updatequery .= " total=?,";
	...
	if (isset($_REQUEST['charges'])) {
		 ...
		 * Date: 2018-07-23
		 * Convert data format curency to data db
		 */
		foreach($chargesInfo as $keyCharge => $valCharge ){
			if(!empty($chargesInfo[$keyCharge]['value'])){
				//-- Modified By Kelvin Thang -- on 2019-08-01 -- convert To DB Format for field munber
				$chargesInfo[$keyCharge]['value'] = $input = Vtiger_Currency_UIType::convertToDBFormat($chargesInfo[$keyCharge]['value'], null, true);
			}
		}
	}
...
// Added by Vu Mai on 2022-09-30 to search inventory item by keyword (Copied from ChatbotIframe by Phu Vo)
function searchInventoryItemByKeyword($itemType, $keyword = '', $ignoreIds = []) {
	global $adb;
	if (!in_array($itemType, ['Products', 'Services'])) return;
	$keyword = $adb->sql_escape_string($keyword);
	$queryParams = [];

	if ($itemType === 'Products') {
		$aclQuery = CRMEntity::getListViewSecurityParameter('Products');
		$sql = "SELECT p.productid AS id,
			...
			INNER JOIN vtiger_crmentity ON (vtiger_crmentity.setype = 'Products' AND p.productid = vtiger_crmentity.crmid AND vtiger_crmentity.deleted = 0)
			WHERE 1 = 1 {$aclQuery} ";
	}

	if ($itemType === 'Services') {
		$aclQuery = CRMEntity::getListViewSecurityParameter('Services');
		$sql = "SELECT s.serviceid AS id,
				s.service_no AS product_no,
				s.servicename AS label,
				s.unit_price AS price,
				s.purchase_cost AS purchase_cost
			FROM vtiger_service AS s
			INNER JOIN vtiger_crmentity ON (vtiger_crmentity.setype = 'Services' AND s.serviceid = vtiger_crmentity.crmid AND vtiger_crmentity.deleted = 0)
			WHERE 1 = 1 {$aclQuery} ";
	}

	if (!empty($keyword)) {
		$sql .= "AND vtiger_crmentity.label LIKE '%{$keyword}%' ";
	}
	
	if (!empty($ignoreIds)) {
		$ignoreIdsString = "(" . generateQuestionMarks($ignoreIds) .")";
		$sql .= "AND vtiger_crmentity.crmid NOT IN {$ignoreIdsString} ";
		$queryParams[] = $ignoreIds;
	}
...
// Added by Vu Mai on 2022-11-14 to convert Lead when save inventory for Lead in QuickCreate form
function convertLead($recordModel, $extraValues = []) {
	...
	if (!empty($extraValues['Accounts']['accountname'])) {
		$modules[] = 'Accounts';
	}
	else if (!empty($recordModel->get('company'))) {
		$modules[] = 'Accounts';
		$extraValues['Accounts']['accountname'] = $recordModel->get('company');
	}
	...
	foreach ($modules as $moduleName) {
		if (vtlib_isModuleActive($moduleName)) {
			...
			$entityValues['entities'][$moduleName]['assigned_user_id'] = $recordModel->get('assigned_user_id');
			$entityValues['entities'][$moduleName]['main_owner_id'] = $recordModel->get('main_owner_id');
			$entityValues['entities'][$moduleName]['owner_populated'] = true;
			...
			if (!empty($extraValues[$moduleName])) {
...
// Added by Vu Mai on 2022-11-14 to get all tags linked to target to sync to new record (Copied from SocialChatbox Popup by Phu Vo)
function getCustomerTags($customerId, $customerType) {
	...
	$query = "SELECT *
		...
		AND vtiger_freetagged_objects.object_id = ? ";
	$params = [$userId, $customerId];
	
	if(!empty($customerType)) {
		$query .=  ' AND vtiger_freetagged_objects.module = ?';
		array_push($params, $customerType);
	}

	$query .= " ORDER BY vtiger_freetagged_objects.object_id, FIELD(vtiger_freetags.visibility, 'private', 'public'), tag_id ";
	$result = $adb->pquery($query, $params);
	...
	while ($row = $adb->fetchByAssoc($result)) {
		...
		$tag['text'] = $row['tag'];
		$tag['name'] = $row['tag'];
		$tag['type'] = $row['visibility'];
		if (!isset($tagsList[$row['object_id']])) $tagsList[$row['object_id']] = [];
		$customerTags[] = decodeUTF8($tag);
	}

	return $customerTags;
}

// Added by Vu Mai on 2022-11-17 to transfer activity from target to contact when converting from target to contacts without going through leads
function transferTargetRelatedActivitiesToContacts($targetId, $relatedId) {
	global $adb;
...
// Added by Vu Mai on 2022-11-22 to transfer all comments from target to contact when converting from target to contacts without going through leads
function transferTargetRelatedCommentsToContact($targetId, $relatedId) {
	global $adb;
	
	$sql = "UPDATE vtiger_modcomments SET related_to = ? WHERE related_to = ?";
	$adb->pquery($sql, [$relatedId, $targetId]);

	return true;
}

// Added by Vu Mai on 2022-11-22 to notify msg to client after converted customer
function notifyMsgToClientAfterConverted($oldCustomerId, $newCustomerId, $type) {
	global $current_user;
	$data = [];
...
// Added by Vu Mai on 2023-01-07 to get status field name by module name
function getStatusFieldName($moduleName) {

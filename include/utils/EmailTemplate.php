...
s								 */
						...
						// Modified by Hieu Nguyen on 2021-08-10 to display line break in email
						else if ($fieldModel->getFieldDataType() == 'text') {
							$values[$field] = nl2br($value);
						}
						// End Hieu Nguyen
						else {
							if ($referenceColumn == 'contactid' && $this->module == 'Events') {
							...
							// Modified by Phu Vo on 2019.11.25 to add google map link to email location field (wPRY9cU5)
							else if ($field === 'location' && !empty($value)) {
								$googleMappLink = "https://www.google.com/maps/place/" . urlencode($value);
								$values[$field] = "<a href='{$googleMappLink}'>{$value}</a>";
							}
							// End Phu Vo
							else {
				...
				if (!$this->isProcessingReferenceField($params)) {
					foreach ($columnList as $column) {
						...
						if($this->removeTags){
						    // Modified by Hoang Duc 2019-10-04 to fix json_encode unicode
							$encodedValue = json_encode($replaceValue, JSON_UNESCAPED_UNICODE);
							// End Modified by Hoang Duc 2019-10-04 to fix json_encode unicode

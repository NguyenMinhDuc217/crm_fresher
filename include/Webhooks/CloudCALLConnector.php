...
/*
	Webhook CloudCALLConnector
	Author: Loc Huynh
	Date: 2023-03-08
	Purpose: to handle request from CloudCALL webhook and forward that request into real-time service
*/

require_once('include/utils/CallCenterUtils.php');

class CloudCALLConnector extends Vtiger_EntryPoint {
	...
	function process(Vtiger_Request $request) {
		...
		saveLog('CALLCENTER', '[CloudCALL] Webhook data', $data);
		...
		if (PBXManager_CloudCALL_Connector::isExists($data['call_id'], $data['state'])) {
			exit;
		}
		...
		if (!empty($receiverId)) {
			...
			PBXManager_CloudCALL_Connector::forwardToCallCenterBridge($msg);
			CallCenterUtils::saveDebugLog('[CloudCALL] Data sent to call popup for ' . $agentExtNumber, null, $msg);
		}

		// Save call history
		PBXManager_CloudCALL_Connector::handleCallEvent($data);
	}
}
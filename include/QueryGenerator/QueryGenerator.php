...
class QueryGenerator {
    ...
    // Implemented by Hieu Nguyen on 2019-06-14 to generate condition sql for owner fields
    protected function getConditionForOwnerFields($valueIndex, $fieldName, $fieldInfo, $conditionInfo, $fieldGlue) {
        $conditionSql = '';
        $ownerIdStr = is_array($conditionInfo['value']) ? $conditionInfo['value'][$valueIndex] : trim(explode(',', $conditionInfo['value'])[$valueIndex]);
    ...
    protected function getConditionForUserReferenceFields($valueIndex, $fieldName, $fieldInfo, $conditionInfo, $fieldGlue) {
        $conditionSql = '';
        $userIdStr = is_array($conditionInfo['value']) ? $conditionInfo['value'][$valueIndex] : trim(explode(',', $conditionInfo['value'])[$valueIndex]);
        ...
        $conditionSql .= "{$fieldGlue} {$fieldInfo->getTableName()}.{$fieldInfo->getColumnName()} {$valueCompareOperator} {$userId}";
	...
	public function getWhereClause() {
		...
		foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			if ($field->getFieldDataType() == "multipicklist" && $conditionInfo['operator'] == 'c') {
				$valueSqlList = $this->getConditionValueFieldMultipicklist($conditionInfo['value'], $field, $field->getTableName());
			}
			else { // Default case
				$valueSqlList = $this->getConditionValue($conditionInfo['value'], $conditionInfo['operator'], $field);
			}
			// End By Tùng Nguyễn To fix search with multipicklist
			...
			foreach ($valueSqlList as $i => $valueSql) {    // Modified by Hieu Nguyen on 2019-06-14 to get the index
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    // Modified by Hieu Nguyen on 2019-06-14 to support filtering custom owner field
    ...
    // Modified by Hieu Nguyen on 2020-12-18 to fix bug query is cached when conditions is cleared
	...
	// Add By Tùng Nguyễn To fix search with multipicklist
	public function getConditionValueFieldMultipicklist($value, $field, $tableName) {
		$valueArray = explode(',', $value);

		foreach ($valueArray as $key => $value) {
			$fieldName = $field->getColumnName();
			$valueArray[$key] = " FIND_IN_SET('{$value}', REPLACE({$tableName}.{$fieldName}, ' |##| ', ',')) > 0 ";
		}

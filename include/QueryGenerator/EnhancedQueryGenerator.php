...
class EnhancedQueryGenerator extends QueryGenerator {
	...
	// Added by Hieu Nguyen on 2021-09-24 to get field type for all columns in advanced filter
	function getAdvFilterFieldTypes($advFilterList) {
		global $adb;
		$queryCondition = '';
		$queryParams = [];
		...
		foreach ($advFilterList as $groupColumns) {
			...
			if (count($filterColumns) > 0) {
				foreach ($filterColumns as $filterInfo) {
					$nameComponents = explode(':', $filterInfo['columnname']);

					if ($queryCondition != '') {
						$queryCondition .= ' OR ';
					}

					$queryCondition .= "(f.tablename = ? AND f.columnname = ?)";
					$queryParams[] = $nameComponents[0];	// Table name
					$queryParams[] = $nameComponents[1];	// Column name
				}
			}
		}
		...
		$sql = "SELECT f.tablename, f.columnname, ft.fieldtype
			FROM vtiger_ws_fieldtype AS ft
			INNER JOIN vtiger_field AS f ON (f.uitype = ft.uitype)
			WHERE {$queryCondition}";
		$result = $adb->pquery($sql, $queryParams);
		$fieldTypes = [];

		while ($row = $adb->fetchByAssoc($result)) {
			$fieldTypes["{$row['tablename']}_{$row['columnname']}"] = $row['fieldtype'];
		}
	...
	public function parseAdvFilterList($advFilterList, $glue = '') {
		...
		// Added by Hieu Nguyen on 2021-09-24 to prefetch the field types mapping
		$fieldTypesMapping = $this->getAdvFilterFieldTypes($advFilterList);
		// End Hieu Nguyen
		...
		foreach ($advFilterList as $groupindex => $groupcolumns) {
			...
			if (count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index => $filter) {
					// Modified by Hieu Nguyen on 2021-09-24 to only escape string for LIKE query when the field type is not picklist/mutipicklist and the comparator is not e/n
					$nameComponents = explode(':', $filter['columnname']);
					$fieldType = $fieldTypesMapping["{$nameComponents[0]}_{$nameComponents[1]}"];
					$isPicklistField = in_array($fieldType, ['picklist', 'multipicklist']);

					if (!$isPicklistField && !in_array($filter['comparator'], ['e', 'n'])) {
						$filter['value'] = Vtiger_Util_Helper::escapeSqlStringForLIKEQuery($filter['value']);
					}
					// End Hieu Nguyen
                    ...
                    // Added by Hieu Nguyen on 2019-12-10 to fix bug filter datetime field in between mode
                    else if ($nameComponents[4] == 'DT' && $filter['comparator'] == 'bw') {
                        $value = substr_replace($filter['value'], '23:59:59', 31);
                        $this->addCondition($name, $value, $filter['comparator']);
                    }
                    // End Hieu Nguyen
                    else {
	...
	public function getSelectClauseColumnSQL() {
		...
		foreach ($this->fields as $field) {
			...
			if (count($matches) != 0) {
				...
				if ($this->meta->getEntityName() == 'Calendar' && $field == 'parent_id') {
					$columns[] = "MAX(CASE WHEN vtiger_seactivityrel_seentity.setype NOT IN ('Accounts', 'Leads') THEN vtiger_seactivityrel_seentity.crmid ELSE '' END) AS parent_id";	// Use MAX to get id (if exist) instead of null value in duplicated row
				}
				else {
	...
	public function getFromClause() {
		...
		foreach ($tableList as $tableName) {
			if ($tableName == 'vtiger_users') {
				...
				$sql .= " {$tableJoinMapping[$tableName]} {$tableName} ON vtiger_crmentity.smownerid = {$tableName}.groupid";
		...
		if ($this->meta->getTabName() == 'Calendar' && in_array('parent_id', $fieldList)) {
			$tableJoinCondition['parent_id'] = ['vtiger_seactivityrel_seentity' => 'vtiger_seactivityrel.crmid = vtiger_seactivityrel_seentity.crmid'];
			$tableJoinMapping['vtiger_seactivityrel_seentity'] = 'LEFT JOIN vtiger_crmentity';
		}
		// End Hieu Nguyen
	...
	public function getWhereClause() {
		...
		foreach ($this->conditionals as $index => $conditionInfo) {
			...
			// Add By Tùng Nguyễn To fix search with multipicklist
			if ($field->getFieldDataType() == "multipicklist" && $conditionInfo['operator'] == 'c') {
				$valueSqlList = $this->getConditionValueFieldMultipicklist($conditionInfo['value'], $field, $tableName);
			}
			else {
			...
			foreach ($valueSqlList as $i => $valueSql) {    // Modified by Hieu Nguyen on 2019-05-30 to get the index
				if (in_array($baseFieldName, $this->referenceFieldList)) {
					if ($conditionInfo['operator'] == 'y') {
                        ...
                        // Modified by Hieu Nguyen on 2019-10-22 to support custom user reference field
						if ($field->getUIType() == 52 || $field->getUIType() == 77) {
                            $fieldSql .= $this->getConditionForUserReferenceFields($i, $fieldName, $field, $conditionInfo, $fieldGlue);
						}
                        else {
                            // Default case
					...
					if ($conditionInfo['operator'] == 'y') {
                        ...
                        // Modified by Hieu Nguyen on 2019-05-30 to support filtering custom owner field
                        if ($fieldName == 'assigned_user_id' || $fieldName == 'main_owner_id') {
                            $fieldSql .= $this->getConditionForOwnerFields($i, $fieldName, $field, $conditionInfo, $fieldGlue);
                        }
                        else {
                            // This is default case, not tested which case will fallback here
                            $fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or vtiger_groups$ownerTableName.groupname $valueSql)";
                        }
                        // End Hieu Nguyen
					}
				}
                // Commented out these line to fix bug can not filter date_start and due_date at Calendar ListView
                ...
                elseif ($field->getFieldDataType() == 'datetime') {
					...
					if ($fieldName == 'birthday' && !$this->isRelativeSearchOperators($conditionInfo['operator'])) {
						...
						// Add By Tùng Nguyễn To fix search with multipicklist
						elseif ($field->getFieldDataType() == 'multipicklist' && $conditionInfo['operator'] == 'c') {

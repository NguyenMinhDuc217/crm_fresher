...
$log = LoggerManager::getLogger('ADODB');
...
class PearDatabase{
	...
	function convert2Sql($ps, $vals) {
   	...
   	*/
	// Modified by Hieu Nguyen on 2022-05-10
	function pquery($sql, $params = array(), $dieOnError = false, $msg = '') {
		...
		$log->debug('[PearDatabase:pquery] Query: ' . $this->convert2Sql($sql, $params));
		...
		$this->logSqlTiming($startTime, $endTime, $sql, $params);

		$this->lastmysqlrow = -1;

		if (!$result) {
			$sql = $this->convert2Sql($sql, $params);
			$this->checkError("[PearDatabase:pquery] {$msg}\nQuery Failed: {$sql}\n", $dieOnError);
		}
	...
	// Modified by Hieu Nguyen on 2018-10-23 to allow passing params
    function getOne($sql, $params = [], $dieOnError = false, $msg = '') {
		global $log;
		$this->checkConnection();
		$this->executeSetNamesUTF8SQL();

		$log->debug('[PearDatabase:getOne] Query: ' . $this->convert2Sql($sql, $params));
		$startTime = microtime(true);
		...
		$endTime = microtime(true);
		$this->logSqlTiming($sql_start_time, $endTime, $sql);

		if ($result === false) {
			$sql = $this->convert2Sql($sql, $params);
			$this->checkError("[PearDatabase::getOne] {$msg}\nQuery Failed: {$sql}\n", $dieOnError);
		}
    ...
    // Modified by Hieu Nguyen on 2020-12-01 to retry and log error
    function getUniqueID($tableName) {
		$this->checkConnection();
		$id = $this->database->GenID("{$tableName}_seq", 1);
        ...
        if (empty($id)) {
            ...
            saveLog('ADODB', '[PearDatabase::getUniqueID] Error getting new id for table ' . $tableName, $trace);
            throw new Exception('Error generating new record id. Please try again!');
        }

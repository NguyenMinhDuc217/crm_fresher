...
class ListViewController {
	...
	public function fetchNameList($field, $module, $result) {	// Modified by Hieu Nguyen on 2022-01-19 to pass module name as second param
		...
		// Added by Hieu Nguyen on 2022-10-19 to get the right value for parent_id field of activity
		...
		if($referenceFieldList) {
			foreach ($referenceFieldList as $fieldName) {
				if (in_array($fieldName, $listViewFields)) {
					...
					$this->fetchNameList($field, $baseModule, $result);	// Modified by Hieu Nguyen on 2022-01-19 to pass module name as second param
				}
			}
		}
		...
		for ($i = 0; $i < $rowCount; ++$i) {
			...
			foreach ($listViewFields as $fieldName) {
				...
				if(count($matches) > 0) {
					...
					// Modified by Hieu Nguyen on 2022-10-19 to get the right value for parent_id field of activity
					if ($baseModule == 'Calendar' && $fieldName == 'parent_id') {
...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
				...
				elseif ($fieldDataType == 'integer') { /*Added Kelvin Thang -- Date:2018-07-26 -- format for value type int*/
                    $value = Vtiger_Integer_UIType::getDisplayValue($value);
				}
				elseif ($fieldDataType == 'double') { /*Added Kelvin Thang -- Date:2018-07-26 -- format for value type double*/
					$value = Vtiger_Double_UIType::getDisplayValue($value);

...
class CurrencyField {
    ...
    public function getDisplayValue($user=null, $skipConversion=false, $skipFormatting=false) {
		...
		if($skipConversion == false) {
			$value = self::convertFromBaseCurrency($value,$this->conversionRate);
		}
    ...
    public function getDBInsertedValue($user=null, $skipConversion=false) {
		...
		if($skipConversion == false) {
			$value = self::convertToBaseCurrency($value,$this->conversionRate);
		}
	...
	public static function convertToBaseCurrency($amount, $conversionRate) {
		if ($conversionRate == 0) return 0;
		return $amount / $conversionRate;
	}
	
	public static function convertFromBaseCurrency($amount, $conversionRate) {
     ...
     * @return mixed|string
     */
    function formatNumberToUser($value, $type='int', $user = null,  $skipConversion=false, $skipFormatting=false){
        $value = self::convertToUserFormat($value, $user, $skipConversion, $skipFormatting);

        global $current_user;
        if (!$user) {
            $user = $current_user;
        }
		if($user->truncate_trailing_zeros == true || $type == 'int') {

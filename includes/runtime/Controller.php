...
abstract class Vtiger_View_Controller extends Vtiger_Action_Controller {
	...
	function getViewer(Vtiger_Request $request) {
		if(!$this->viewer) {
			...
			// [Core] Added by Phu Vo on 2020.03.18 to assign current view itself
	...
	// Added by Hieu Nguyen on 2021-09-21 to display header warning
	function displayHeaderWarning(Vtiger_Request $request, $htmlContent) {
		$viewer = $this->getViewer($request);
		$viewer->assign('HEADER_WARNING', $htmlContent);
	}
	 ...
	 * @param Vtiger_Request $request
	 */
    // Modified by Hieu Nguyen on 2010-11-21 to load language string from all modules
	function getJSLanguageStrings(Vtiger_Request $request) {
		$currentModuleName = $request->getModule(false);

		if ($currentModuleName === 'Settings:Users') {
			$currentModuleName = 'Users';
		}

		$currentModuleJsLang = Vtiger_Language_Handler::export($currentModuleName, 'jsLanguageStrings');
        
        // For back-work compatible
        $result = $currentModuleJsLang;

        // New format for multiple modules support
        $result["_MODULE_{$currentModuleName}"] = $currentModuleJsLang;

        // Load other modules lang
        $pattern = 'languages/en_us/*.php';
        $langFiles = [];
            
        foreach (glob($pattern) as $langFilePath) {
            list($moduleName, $ext) = explode('.', basename($langFilePath));

            if ($moduleName != $currentModuleName) {
                $result["_MODULE_{$moduleName}"] = Vtiger_Language_Handler::export($moduleName, 'jsLanguageStrings', false);
            }
        }

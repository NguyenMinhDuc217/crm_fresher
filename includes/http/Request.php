...
class Vtiger_Request {
	...
	function __construct($values, $rawvalues = array(), $stripifgpc=true) {
		$this->valuemap = removeHtml($values);
	 ...
	 */
	// Modified by Hieu Nguyen on 2020-11-05 to support saving content without purified
	function getRaw($key, $defvalue = '', $skipPurify = false) {
		if (isset($this->rawvaluemap[$key])) {
            if ($skipPurify) {
                return $this->rawvaluemap[$key];
            }

			return vtlib_purify($this->rawvaluemap[$key]);  // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
		}
	...
	protected function validateReferer() {
		...
		if (isset($_SERVER['HTTP_REFERER']) && $user) {//Check for user post authentication.
            // Modified condition by Hieu Nguyen on 2020-02-14 to allow oauth callback redirect to extension modules
			global $site_URL, $oauthCallbackAllowedModules;

			if (stripos($_SERVER['HTTP_REFERER'], $site_URL) !== 0) {
                $moduleName = $this->get('module');

                if ($moduleName != 'Install' && !in_array($moduleName, $oauthCallbackAllowedModules)) {

<?php
    ...
    */
    class SocketIO
    {
        ...
        * @return bool
        */
        public function send($ssl, $host = null, $port = null, $data = null, $address = "/socket.io/?EIO=2", $transport = 'websocket')
        {

            $wsProtocol = $ssl ? 'ssl://' : '';

            $fd = fsockopen($wsProtocol . $host, $port, $errno, $errstr);
            if (!$fd) {
                return false;
            } //Can't connect tot server
			
			// Modified by Hieu Nguyen on 2016-12-16
			$action = '';
			...
			// Hieu Nguyen: the lines below is not important anymore. Only to help the connection disconnect properly

...
*/
	...
	*/
	...
	function Execute($sql,$inputarr=false) {
		...
		// Added by Hieu Nguyen on 2018-12-07 to track for db changes
		//$this->trackDbChanges($sql);  // Uncomment this line when you need to debug
		// End Hieu Nguyen

		return $ret;
	}

    // Implemented by Hieu Nguyen on 2018-12-07
    function trackDbChanges($sql) {
        if(empty($sql)){
            return;
        }else{
            $sqlTrack = $sql;
        ...
        // Track danger keywords that change db structure
        if(preg_match('/CREATE TABLE|ALTER TABLE|DROP TABLE/', strtoupper($sqlTrack), $matches, PREG_OFFSET_CAPTURE)) {
            $track = true;
        }

        // Track important tables
        if(!$track && preg_match('/INSERT |UPDATE |DELETE FROM/', strtoupper($sqlTrack), $matches, PREG_OFFSET_CAPTURE)) {
            $trackTables = [
                'vtiger_field',
                'vtiger_field_seq',
                'vtiger_blocks',
                'vtiger_blocks_seq',
                'vtiger_editview_blocks',
                'vtiger_editview_blocks_seq',
            ...
            foreach($trackTables as $tableName) {
                $trackPattern .= "insert into {$tableName}|update {$tableName}|delete from {$tableName}|";
            }

            if(preg_match('/'. substr($trackPattern, 0, -1) .'/', strtolower($sqlTrack), $matches, PREG_OFFSET_CAPTURE)) {
                $track = true;
            }

            // Testing only. Don't forget to comment out this line in normal case!
            //$track = true;
        }
        ...
        if($track) {
            global $current_user;
            // Convert LAST_INSERT_ID statement into runable sql
            if(strpos($sqlTrack, 'LAST_INSERT_ID(id+1)') !== false) {
                global $dbconfig;

                list($t, $seqTable) = explode(' ', $sqlTrack);
                $mainTable = str_replace('_seq', '', $seqTable);

                $sqlGetKeyField = "SELECT COLUMN_NAME
                ...
                //-- Modified By Kelvin Thang - on 2018-12-14 - fix issue set id is 0
                $keyField = $this->GetOne($sqlGetKeyField,false);
                $sqlTrack = "update {$seqTable} set id = (select (max({$keyField})) from {$mainTable})";
                //-- Set data last insert id
                @$this->_query("update {$seqTable} set id=LAST_INSERT_ID(id)",false);
                //-- End By Kelvin Thang
            }
            ...
            $log = "\r\n/* Time: {$microTime}::{$time} - User: {$current_user->id}::{$current_user->user_name} */\r\n";
            $log .= $sqlTrack . ";\r\n";

            file_put_contents('logs/dbtracking.log', $log, FILE_APPEND);
        }
    }

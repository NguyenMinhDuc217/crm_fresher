...
function csrf_ob_handler($buffer, $flags) {
    ...
    if ($GLOBALS['csrf']['frame-breaker'] && !$is_partial) {
        // Modified condition by Hieu Nguyen on 2019-11-07 to allow loading current page inside an iframe of other page if they are in the same origin
        $buffer = preg_replace('/<\/head>/', '<script type="text/javascript">if (top != self && top.location.origin != self.location.origin) {top.location.href = self.location.href;}</script></head>', $buffer,$count);
    }

...
class Activity extends CRMEntity {
	...
	function save_module($module)
	{
		...
		// Modified by Hieu Nguyen on 2022-01-19 to save parent record into table vtiger_cntactivityrel
		$parentId = $this->column_fields['parent_id'];

		if (!empty($parentId)) {
			// Remove the old record
			$adb->pquery('DELETE FROM vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['parent_id']]);

			// Insert new record
			$adb->pquery('INSERT INTO vtiger_seactivityrel VALUES(?, ?)', [$parentId, $this->id]);
		}
		elseif (empty($parentId) && $this->mode == 'edit' && !empty($this->fetchedRow['parent_id'])) {
			$adb->pquery('DELETE FROM vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['parent_id']]);
		}
		// End Hieu Nguyen

		// Modified by Hieu Nguyen on 2022-01-05 to save related Contact into table vtiger_cntactivityrel
		$contactId = $this->column_fields['contact_id'];

		if (!empty($contactId)) {	// User set related Contact
			// Remove the old record
			$adb->pquery('DELETE from vtiger_cntactivityrel WHERE activityid = ?', [$this->id]);

			// Insert new record
			$adb->pquery('INSERT INTO vtiger_cntactivityrel VALUES(?, ?)', [$contactId, $this->id]);
		}
		elseif (empty($contactId) && $this->mode == 'edit') {	// User remove related Contact
			$adb->pquery('DELETE FROM vtiger_cntactivityrel WHERE activityid = ?', [$this->id]);
		}
		// End Hieu Nguyen

		// Modified by Hieu Nguyen on 2022-01-10 to save related Account into table vtiger_seactivityrel
		$accountId = $this->column_fields['related_account'];

		if (!empty($accountId)) {	// User set related Account
			// Remove the old record
			$adb->pquery('DELETE FROM vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['related_account']]);

			// Insert new record
			$adb->pquery('INSERT INTO vtiger_seactivityrel VALUES(?, ?)', [$accountId, $this->id]);
		}
		elseif (empty($accountId) && $this->mode == 'edit' && !empty($this->fetchedRow['related_account'])) {	// User remove related Account
			$adb->pquery('DELETE FROM vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['related_account']]);
		}
		// End Hieu Nguyen

		// Modified by Hieu Nguyen on 2022-01-10 to save related Lead into table vtiger_seactivityrel
		$leadId = $this->column_fields['related_lead'];

		if (!empty($leadId)) {	// User set related Lead
			// Remove the old record
			$adb->pquery('DELETE from vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['related_lead']]);

			// Insert new record
			$adb->pquery('INSERT INTO vtiger_seactivityrel VALUES(?, ?)', [$leadId, $this->id]);
		}
		elseif (empty($leadId) && $this->mode == 'edit' && !empty($this->fetchedRow['related_lead'])) {	// User remove related Lead
			$adb->pquery('DELETE from vtiger_seactivityrel WHERE activityid = ? AND crmid = ?', [$this->id, $this->fetchedRow['related_lead']]);
		}
		// End Hieu Nguyen
		...
		//Handling for invitees
			...
			// $this->insertIntoInviteeTable($module,$invitees_array);  // Commented out by Hieu Nguyen on 2019-11-27 to use saveInvitations() instead

		//Inserting into sales man activity rel
		$this->insertIntoSmActivityRel($module);

		// Disabled by Phu Vo on 2019.04.04 => Use realtime notify instead
...
function insertIntoRecurringTable(& $recurObj)
{
	...
	function insertIntoInviteeTable($module,$invitees_array)
	{
		...
		// Modified by Phu Vo on 2019.04.04
		if($this->mode == 'edit'){
			// Delete only invitee not in new array
			$sql = "DELETE FROM vtiger_invitees WHERE activityid = ? AND inviteeid NOT IN ('" . join("', '", $invitees_array) . "')";

			$adb->pquery($sql, array($this->id));
		}

		// Find exist invitee, change status from 'sent' to ''
		$sql = "SELECT GROUP_CONCAT(inviteeid) FROM vtiger_invitees WHERE activityid = ?";
		$existedInvitees = explode(",", $adb->getOne($sql, [$this->id]));
		
		foreach($invitees_array as $inviteeid) {
			if($inviteeid != '') {
				if(!in_array($inviteeid, $existedInvitees)) {
...
//calendarsync
	...
	*/
	...
	// Modified by Hieu Nguyen on 2022-01-14
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		// Clear related id for custom relation fields
		if (in_array($return_module, ['Accounts', 'Leads'])) {
			$relateFieldMapping = [
				'Accounts' => 'related_account',
				'Leads' => 'related_lead'
			];

			$relateFieldName = $relateFieldMapping[$return_module];
			$sql = "UPDATE vtiger_activity SET {$relateFieldName} = '' WHERE activityid = ?";
	...
	public function getNonAdminAccessControlQuery($module, $user,$scope='') {
        ...
        // Modified condition by Hieu Nguyen on 2020-08-28 to ignore these logic when at Activities subpanel
		if ($is_admin == false && !Calendar_Logic_Model::isAtActivitiesRelatedList() && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1) {
			$sharedTabId = null;

			// For Events
			$tableName = 'vt_tmp_u'. $user->id .'_t'. $tabId .'_events';
			$this->setupTemporaryTableForEvents($tableName, $sharedTabId, $user, $current_user_parent_role_seq, $current_user_groups);

			// Modified join condition by Hieu Nguyen on 2019-11-13 to get the right condition with new owner field logic
			$query = " LEFT JOIN $tableName $tableName$scope ON (
					($tableName$scope.id = vtiger_crmentity$scope.smownerid OR $tableName$scope.id = vtiger_crmentity$scope.main_owner_id) 
			...
			$this->setupTemporaryTableForTask($task_tableName, $tabId, $user, $current_user_parent_role_seq, $current_user_groups, $defaultOrgSharingPermission[$tabId]);

			// Modified join condition by Hieu Nguyen on 2019-11-13 to get the right condition with new owner field logic
			$query .= " LEFT JOIN $task_tableName $task_tableName$scope ON (
					($task_tableName$scope.id = vtiger_crmentity$scope.smownerid OR $task_tableName$scope.id = vtiger_crmentity$scope.main_owner_id) 
	...
	protected function setupTemporaryTableForEvents($tableName, $tabId, $user, $parentRole, $userGroups) {
		...
		if(is_object($result)) {
            // Modified by Hieu Nguyen on 2020-03-05 to load events from all users for My Calendar & Shared Calendar (private user's events display as busy)
            ...
            if ($_POST['calendar_view'] == 'MyCalendar' || $_POST['calendar_view'] == 'SharedCalendar') {
                $query = "REPLACE INTO $tableName (id) SELECT id FROM vtiger_users WHERE deleted = 0";
	...
	protected function setupTemporaryTableForTask($tableName, $tabId, $user, $parentRole, $userGroups, $sharingPermission) {
		...
		// Modified by Hieu Nguyen on 2022-01-10 to insert all subusers, subgroups and parent groups of that subgroups into the temp table so that current user can see activities assigned to himself and to his sublevel employees
		$userModel = Users_Record_Model::getCurrentUserModel();
		$subUserIds = array_keys($userModel->getSubordinateUsers());
		$userIds = array_merge($subUserIds, [$userModel->getId()]);
		$groupIds = getGroupsByUsers($userIds);

		$selectQuery = "(SELECT id FROM vtiger_users WHERE deleted = 0 AND id IN (". implode(', ', $userIds) .")) ";

		if (!empty($groupIds)) {	// We need to double check here as some user do not belong to any group
			$selectQuery .= "UNION 
				(SELECT groupid FROM vtiger_groups WHERE groupid IN (". implode(', ', $groupIds) ."))
				UNION
				(SELECT groupid from vtiger_group2grouprel where containsgroupid IN (". implode(', ', $groupIds) .")) ";
		}

		$query = "CREATE TEMPORARY TABLE IF NOT EXISTS {$tableName} (id INT(11) PRIMARY KEY, shared INT(1) DEFAULT 0) IGNORE ". $selectQuery;
		$db = PearDatabase::getInstance();
		$db->pquery($query, []);
		// End Hieu Nguyen
	}
	...
	public function buildWhereClauseConditionForCalendar($scope = '') {
		...
		// Modified condition by Hieu Nguyen on 2020-08-28 to ignore these logic when at Activities subpanel
		if (!$userModel->isAdminUser() && !Calendar_Logic_Model::isAtActivitiesRelatedList() && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1) {

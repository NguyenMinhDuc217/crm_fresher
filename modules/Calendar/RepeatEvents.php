...
class Calendar_RepeatEvents {
			...
			foreach ($recurObj->recurringdates as $index => $startDate) {
				...
				if(!empty($recordId) && !empty($startDate)) {
					...
					if($focus->column_fields['recurringEditMode'] == 'future' && $recordModel->get('date_start') >= $eventStartDate) {
                        ...
                        // Added by Hieu Nguyen on 2019-12-10 to handle invitation for edited recurring event
                        Events_Invitation_Helper::saveInvitations($recordId, $focus->column_fields['invitee_ids'], $recordModel->getData(), true);
                        // End Hieu Nguyen
	...
	static function createRecurringEvents($focus, $recurObj, $recurringDates, $parentId = false) {
		...
		foreach ($recurringDates as $index => $startDate) {
            ...
            // Added by Hieu Nguyen on 2019-12-10 to handle invitation for new recurring event
            Events_Invitation_Helper::saveInvitations($new_focus->id, $new_focus->column_fields['invitee_ids'], $new_focus->column_fields, true);
		    // End Hieu Nguyen

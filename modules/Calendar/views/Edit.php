...
Class Calendar_Edit_View extends Vtiger_Edit_View {
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		// Added by Hieu Nguyen on 2022-09-06 to check access forbidden feature
		$mode = $request->getMode();

		if ((empty($mode) || $mode == 'Calendar') && empty($record)) {
			checkAccessForbiddenFeature('CreateTask');
		}
		...
		// [CallCenter] Added by Phu Vo on 2020.07.27 Prevent edit on auto call record
		if (!empty($record) && PBXManager_Logic_Helper::isAutoCall($record)) {
			throw new AppException(vtranslate('LBL_PERMISSION_DENIED'));
		}
		// End Phu Vo
	}
	...
	function Events($request, $moduleName) {
        ...
        // Added by Hieu Nguyen on 2019-10-29 to load custom code for overlay edit view
		...
		// Modified Hieu Nguyen 2022-01-14 to auto set related account when related contact is set
		...
		// $viewer->assign('INVITIES_SELECTED', $recordModel->getInvities());   // Commented out this line by Hieu Nguyen on 2019-11-29 as it's not used anymore

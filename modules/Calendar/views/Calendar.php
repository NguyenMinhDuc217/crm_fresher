...
class Calendar_Calendar_View extends Vtiger_Index_View {
	...
	public function preProcess(Vtiger_Request $request, $display = true) {
		...
		$viewer->assign('MODULE_MODEL', $moduleModel);	// Added by Hieu Nguyen on 2022-09-06
	...
	public function getCalendarSettings(Vtiger_Request $request){
		...
		// $allUsers = Users_Record_Model::getAll(true);    // Commented out by Hieu Nguyen on 2020-02-26 to boost performance
        ...
        // Modified by Hieu Nguyen on 2020-02-26 to convert shared users into select2 tagging format
        $sharedUserIds = [];

        foreach ($sharedUsers as $sharedUserId => $calendarHostId) {
            $sharedUserIds[] = 'Users:' . $sharedUserId;
        }

		$viewer->assign('SELECTED_USERS_STRING', join(',', $sharedUserIds));
        // End Hieu Nguyen

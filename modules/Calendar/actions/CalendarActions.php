...
class Calendar_CalendarActions_Action extends Vtiger_BasicAjax_Action {
	...
	public function fetchAgendaViewEventDetails(Vtiger_Request $request) {
		...
		if ($recordModel->get('location')) {
			// Modified by Phu Vo on 2020.02.24 to genereate google link with event location
			$locationLink = "<a href='#' onclick='GoogleMaps.showMaps(\"{$recordModel->get('location')}\")'>{$recordModel->get('location')}</a>";
			$result[vtranslate('Location', $moduleName)] = $locationLink;
			// End Phu Vo
		}

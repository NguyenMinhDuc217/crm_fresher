...
class Calendar_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		// Modified by Hieu Nguyen on 2020-02-24 to set default reminder time for Event and Task
        if ($recordModel->get('mode') != 'edit') {
            self::setDefaultReminderTime($activityType);
        }
        // End Hieu Nguyen
    ...
    // Added by Hieu Nguyen on 2020-02-24 to handle set default reminder time for both Events and Calendar modules
    static function setDefaultReminderTime($activityType) {
        $setReminder = $_POST['set_reminder'];

		if ($setReminder == '1') { // [Calendar] Request #251: Modified by Phu Vo fix bug could not save remider
			$_REQUEST['set_reminder'] = 'Yes';

            // The field set_reminder is not display in QuickCreate form
            if (empty($setReminder)) {                
                $calendarUserSettings = Calendar_Settings_Model::getUserSettings();

                if ($activityType == 'Call') {
                    $defaultTime = $calendarUserSettings['default_call_reminder_time'];
                }
                else if ($activityType == 'Meeting') {
                    $defaultTime = $calendarUserSettings['default_meeting_reminder_time'];
                }
                else {
                    $defaultTime = $calendarUserSettings['default_task_reminder_time'];
                }

                // Set default value for this field as calendar user settings
                $_REQUEST['remdays'] = $defaultTime['days'];
                $_REQUEST['remhrs'] = $defaultTime['hours'];
                $_REQUEST['remmin'] = $defaultTime['mins'];
            }
		}
        else {

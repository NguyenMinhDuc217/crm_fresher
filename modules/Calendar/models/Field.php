...
class Calendar_Field_Model extends Vtiger_Field_Model {
	...
	*/
	...
	public function getDisplayValue($value, $record = false, $recordInstance = false) {
		if ($recordInstance) {
			if ($this->getName() == 'date_start') {
				$dateTimeValue = explode('', $value)[0] . ' '. $recordInstance->get('time_start');	// Modified by Hieu Nguyen on 2022-10-14 to fix bug duplicate time component that cause parsing error
				...
				$dateTimeValue = explode('', $value)[0] . ' '. $recordInstance->get('time_end');	// Modified by Hieu Nguyen on 2022-10-14 to fix bug duplicate time component that cause parsing error
				$value = $this->getUITypeModel()->getDisplayValue($dateTimeValue);
				list($startDate, $startTime) = explode(' ', $value);
		...
		// Added by Phu Vo on 2020.02.24 to genereate google link with event location
		if ($this->getName() === 'location' && !empty($value)) {
			return "<a href='#' onclick='GoogleMaps.showMaps(\"{$value}\")'>{$value}</a>";
		}
		// End Phu Vo

		// Added by Phu Vo on 2020.07.13 to customize how duration field display
		if ($this->getName() == 'duration' && !empty($value)) {
	...
	// Implemented By Hieu Nguyen on 2022-01-05
	public function getReferenceList($hideDisabledModules = true, $presenceZero = true) {
		$referenceList = parent::getReferenceList($hideDisabledModules, $presenceZero);

		// Remove Leads, Contacts and Accounts from parent_id field as these related modules has it own relation field now
		if ($this->getName() == 'parent_id') {
			$excludeModules = ['Leads', 'Contacts', 'Accounts'];
			$referenceList = array_diff($referenceList, $excludeModules);
			$referenceList = array_values($referenceList);	// To reset the index
		}

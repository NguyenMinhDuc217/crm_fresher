	...
	public function getStructure() {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			...
			if (!empty($recordModelFieldValue)) {
				...
				$fieldModel->set('fieldvalue', $fieldValue);
			// [Calendar] Request #251: Added by Phu Vo to fetch visibility default value base on Calendar settings
			}
			else if ($fieldName == 'visibility' && empty($recordModelFieldValue)) {
				$currentUserModel = Users_Record_Model::getCurrentUserModel();
				$sharedType = $currentUserModel->get('calendarsharedtype');

				if ($sharedType == 'public' || $sharedType == 'selectedusers') {
					$fieldModel->set('fieldvalue', 'Public');
				}
				else {
					$fieldModel->set('fieldvalue', 'Private');
				}
			// End Request #251
			} else {

...
class Calendar_ListView_Model extends Vtiger_ListView_Model {
    ...
    // Removed function getBasicLinks by Hieu Nguyen on 2019-12-10 as it has no affect to the Calendar's header buttons. These buttons will be handled by the Module Model
	...
	public function getAdvancedLinks(){
        ...
        // Commented out by Hieu Nguyen on 2019-12-10 as Import button is not a part of ListView's advanced links!
	...
	public function getListViewEntries($pagingModel) {
		...
		foreach($listViewEntries as $recordId => $record) {
            ...
            handleProcessRecords($moduleName, $listViewRecordModels[$recordId]);
			// End Hieu Nguyen

            // Added by Hieu Nguyen on 2019-11-28 to hide unaccepted invited events
            if (
				in_array($rawData['activitytype'], ['Call', 'Meeting'])	// To skip this logic for Task records
                && !$currentUser->isAdminUser() 
                && Events_Invitation_Helper::isInvitee($currentUser->getId(), 'Users', $recordId) 
                && !Events_Invitation_Helper::isInvitationAccepted($currentUser->getId(), 'Users', $recordId)
            ) {
                $recordsToUnset[] = $recordId;
            }
            // End Hieu Nguyen
			
            if (!$currentUser->isAdminUser() && $rawData['activitytype'] == 'Task' && isCalendarTaskPermittedBySharing($recordId, 'index') == 'no') {
				$recordsToUnset[] = $recordId;
			}
		}

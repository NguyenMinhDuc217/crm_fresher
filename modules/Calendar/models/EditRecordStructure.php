				...
				foreach($fieldModelList as $fieldName=>$fieldModel) {
					if($fieldModel->isEditable()) {
						if($recordExists) {
							...
							// Added by Vu Mai on 2023-02-23 to remove option Campaign in reference list if this activity was linked to Telesales Campaign 
							else if ($fieldName == 'parent_id') {
								$referenceList = $fieldModel->getReferenceList();

								if (!empty($recordModel->get('related_campaign'))) {
									$excludeModules = ['Campaigns'];

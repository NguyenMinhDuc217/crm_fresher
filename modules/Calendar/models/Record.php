...
class Calendar_Record_Model extends Vtiger_Record_Model {
	...
	// Imlemented by Phu Vo on 2018.12.15
	function getPBXLogData() {
		global $adb;

		$sql = "SELECT * FROM vtiger_pbxmanager WHERE sourceuuid = ?";
		$params = [$this->get('pbx_call_id')];
...
/**
	...
	public function delete() {
		// Added by Phu Vo to prevent delete record in some case
	...
	public function getRecurringDetails() {
		...
		if ($recurringObject) {
			...
			$recurringInfoDisplayData['recurringcheck'] = 'No'; // Bug #371: Modified by Phu Vo on 2020.03.18 to use recurringcheck raw value
    ...
    /**
     * Implement by Phu Vo on 2020.01.20
    */
    public function isEditable() {
        // Added by Hieu Nguyen on 2020-08-31 to hide edit button at Activities related list based on user permission
        global $current_user;
        $recordData = getFullRecordData($this->getId(), 'Calendar');

        if (Calendar_Logic_Model::isAtActivitiesRelatedList() && $recordData['main_owner_id'] != $current_user->id) {
            if ($current_user->is_admin == 'on' || isPermitted('Calendar', 'EditView', $this->getId()) == 'yes') {
                return true;
            }
        ...
        if ($this->get('missed_call') == 1) return false;
		if (PBXManager_Logic_Helper::isAutoCall($this->getId())) return false;
    ...
    */
    public function isDeletable() {
        // Added by Hieu Nguyen on 2020-08-31 to hide unlink button at Activities related list based on user permission
        global $current_user;
        $recordData = getFullRecordData($this->getId(), 'Calendar');
        
        if (Calendar_Logic_Model::isAtActivitiesRelatedList() && $recordData['main_owner_id'] != $current_user->id) {
            if ($current_user->is_admin == 'on' || isPermitted('Calendar', 'Delete', $this->getId()) == 'yes') {
                return true;
            }

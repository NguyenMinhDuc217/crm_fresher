...
class Calendar_Module_Model extends Vtiger_Module_Model {
	...
	// Added by Hieu Nguyen on 2022-09-06 to check if user can create activity or not
	public static function canCreateActivity($activityType) {
		if (!in_array($activityType, ['Call', 'Meeting', 'Task'])) return false;
		$moduleModel = Vtiger_Module_Model::getInstance('Calendar');
		if (!$moduleModel->isPermitted('CreateView')) return false;

		if ($activityType == 'Task' && isForbiddenFeature('CreateTask')) {
			return false;
		}
	 ...
	 * @return <String>
	 */
	// Modified by Hieu Nguyen on 2022-09-05 to auto switch to My Calendar when Calendar Listview is not supported
	public function getDefaultViewName() {
		$defaultView = $this->getCalendarViewName();

		if (isForbiddenFeature(('CalendarActivityList')) && $defaultView == 'List') {
	 ...
	 * @return <String>
	 */
    // Modified by Hieu Nguyen on 2019-12-10 to specify the event type
	public function getCreateEventRecordUrl($eventType = 'Call') {
		return 'index.php?module='.$this->get('name').'&view='.$this->getEditViewName().'&mode=Events&activitytype=' . $eventType;
	}
	...
	public function getConfigureRelatedListFields() {
		return parent::getConfigureRelatedListFields();	// Modified by Hieu Nguyen on 2019-01-04
	}
	...
	public function getSummaryViewFieldsList() {
		return parent::getSummaryViewFieldsList();	// Modified by Hieu Nguyen on 2019-01-04
	}
	/**
	...
	public function getSideBarLinks($linkParams) {
		...
		if ($linkParams['ACTION'] == 'SharedCalendar') {
			$quickWidgets[] = array(
				...
				'linkurl' => 'module='.$this->get('name').'&view=UserCalendarViews&mode=getSavedUserFeedList',   // Modified by Hieu Nguyen on 2019-10-30 
				'linkicon' => ''
			);
		}
    ...
    // Added by Hieu Nguyen on 2019-12-10 to display module basic links
    public function getModuleBasicLinks($forCalendarView = false) {
		$createPermission = $this->isPermitted('CreateView');
		$basicLinks = [];

		if ($createPermission) {
            $basicLinks = [];

			if (self::canCreateActivity('Call')) {
				$btnCreateCall = [
                    'linktype' => 'BASIC',
                    'linklabel' => 'LBL_ADD_CALL',
                    'linkurl' => $this->getCreateEventRecordUrl('Call'),
                    'linkicon' => 'fa-phone-plus'
                ];

				if ($forCalendarView) $btnCreateCall['linkurl'] = 'Calendar_Calendar_Js.showCreateEventModal({ activityType: "Call" });';
				$basicLinks[] = $btnCreateCall;
			}

			if (self::canCreateActivity('Meeting')) {
				$btnCreateMeeting = [
                    'linktype' => 'BASIC',
                    'linklabel' => 'LBL_ADD_MEETING',
                    'linkurl' => $this->getCreateEventRecordUrl('Meeting'),
                    'linkicon' => 'fa-screen-users'
                ];

				if ($forCalendarView) $btnCreateMeeting['linkurl'] = 'Calendar_Calendar_Js.showCreateEventModal({ activityType: "Meeting" });';
				$basicLinks[] = $btnCreateMeeting;
			}

			if (self::canCreateActivity('Task')) {
				$btnCreateTask = [
                    'linktype' => 'BASIC',
				...
				if ($forCalendarView) $btnCreateTask['linkurl'] = 'Calendar_Calendar_Js.showCreateTaskModal();';
				$basicLinks[] = $btnCreateTask;
			}
			
			$importPermission = $this->isPermitted('Import');

			if (!$forCalendarView && $createPermission && $importPermission && self::canCreateActivity('Task')) {
				$basicLinks[] = [
					'linktype' => 'BASIC',
	...
	*/
	...
	// Moved functions getCalendarViewTypes into My Calendar Model by Hieu Nguyen on 2019-12-19
    ...
    // Moved functions getCalendarViewTypesToAdd, getVisibleCalendarViewTypes, checkDuplicateView, updateCalendarViewVisibility, deleteCalendarView, addCalendarView, getCalendarViewConditions into My Calendar Model by Hieu Nguyen on 2019-12-19
	...
	*/
	...
	public function getModuleIcon($activityType = null) {
		global $moduleIcons; // Added by Phu Vo on 2021.04.21 to support get module icon from config

		$moduleName = $this->getName();
		$title = vtranslate($moduleName, $moduleName);

		$keyIcon = !empty($activityType) ? $moduleName . '-' . $activityType : $moduleName;

		if (!empty($moduleIcons[$keyIcon])) {
			$cssClass = $moduleIcons[$keyIcon];

	...
	public function process(Vtiger_Request $request) {
		...
		if($userSetupStatus) {
			...
			if($isFirstUser) {
				...
				$viewer->assign('BASE_CURRENCIES', $currenciesList);    // Modified the variable name by Hieu Nguyen on 2019-11-08 to differentiate with user currency
			}

            // Added by Hieu Nguyen on 2019-11-07 to assign currencies and other enum list into the layout
            global $combo_strings;
            require_once('include/ComboStrings.php');
            $viewer->assign('ENUM_LIST', $combo_strings);

            $userCurrencies = $userModuleModel->getUserCurrencies();
            $viewer->assign('USER_CURRENCIES', $userCurrencies);
            // End Hieu Nguyen

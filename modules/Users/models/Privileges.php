...
class Users_Privileges_Model extends Users_Record_Model {
	...
	public static function getInstanceById($userId) {
        ...
        static $cache = [];
        if (!empty($cache[$userId])) return $cache[$userId];
        // End Hieu Nguyen
	 ...
	 * @return boolean
	 */
	// Refactored by Hieu Nguyen on 2021-07-16 to make this function easy to maintain
	public static function isPermittedToChangeUsername($targetUserId) {
		$targetUserModel = parent::getInstanceFromPreferenceFile($targetUserId);
		$currentUserModel = parent::getCurrentUserModel();
		
        // [DataSecurity] Added by Hieu Nguyen on 2021-02-22 to prevent editing username of System Admin user
        if ($targetUserModel->get('user_name') == 'sysadmin') {
            return false;
        }
        // End Hieu Nguyen
		...
		$editingSelfUser = ($targetUserId === $currentUserModel->getId());
		...
		// Admin can change other normal user's username if that user is not the Account Owner
		if ($currentUserModel->isAdminUser() && !$targetUserModel->isAccountOwner()) {
			if (!$targetUserModel->isAdminUser()) {
				return true;
			}
		}

		// System Admin can change any user's username except himself
		if ($currentUserModel->get('user_name') == 'sysadmin' && !$editingSelfUser) {
			return true;
		}

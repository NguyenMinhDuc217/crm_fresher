...
class Users_Record_Model extends Vtiger_Record_Model {
    ...
    // Implemented by Hieu Nguyen on 2019-03-13
    static function isDuplicated($userId, $checkField, $checkValue) {
        global $adb;
        $checkField = $adb->sql_escape_string($checkField); // For safe query

        $sql = "SELECT COUNT(id) FROM vtiger_users WHERE id != ? AND {$checkField} = ? AND deleted = 0";
        $params = [$userId, $checkValue];
	...
	public function getChangeUsernameUrl(){
        // [DataSecurity] Added param parent=Settings to support check action for System Admin user
		return 'index.php?module='.$this->getModuleName().'&parent=Settings&view=EditAjax&mode=changeUsername&record='.$this->getId();
	}

	public function getChangePwdUrl(){
        // [DataSecurity] Added param parent=Settings to support check action for System Admin user
		return 'index.php?module='.$this->getModuleName().'&parent=Settings&view=EditAjax&mode=changePassword&recordId='.$this->getId();
	}
	...
	public function save() {
		...
		parent::save();
		$this->saveTagCloud();
        // $this->addDashboardTabs();   // Commented out this line by Hieu Nguyen on 2021-05-27 to disable this logic as user dashboard is already controlled by Dashboard Manager rules
	}
	...
	public static function getInstanceFromUserObject($userObject) {
        ...
        static $cache = [];
        $cacheKey = $userObject->id;
        if (isset($cache[$cacheKey])) $cache[$cacheKey];
        // End Hieu Nguyen
        ...
        // Modifed by Hieu Nguyen on 2021-05-31 to apply caching
        $userModel->setData($userObject->column_fields)->setModule('Users')->setEntity($userObject);
        $cache[$cacheKey] = $userModel;
        return $userModel;
        // End Hieu Nguyen
	}
	...
	public static function getInstanceByName($userName) {
        ...
        static $cache = [];
        if (!empty($cache[$userName])) return $cache[$userName];
        // End Hieu Nguyen
		...
		if ($db->num_rows($result)) {
            ...
            $instance = Users_Record_Model::getInstanceById($db->query_result($result, 0, 'id'), 'Users');
            $cache[$userName] = $instance;
            return $instance;
            // End Hieu Nguyen
		}
		return false;
	...
	public static function getActiveAdminUsers() {	// Converted this function to static by Hieu Nguyen on 2021-07-30 to be called without creating new instance
		$db = PearDatabase::getInstance();
		...
		if($noOfUsers > 0) {
			...
			for($i=0; $i<$noOfUsers; ++$i) {
				...
				// Modified by Hieu Nguyen on 2018-10-11 to fix bug all record have the same data
				/*$focus->id = $userId;
				$focus->retrieve_entity_info($userId, 'Users');
				...
				$users[$userId] = self::getInstanceById($userId, 'Users');
				// End Hieu Nguyen
			}
		}
		return $users;
	...
	public function deleteUserPermanently($userId, $newOwnerId) {
		...
		$sql = "UPDATE vtiger_users SET deleted = 2 WHERE id = ?";  // Modified by Hieu Nguyen on 2021-07-13 to mark user as permanently deleted instead of delete the record from database
		$db->pquery($sql, array($userId));

        // Added by Hieu Nguyen on 2021-07-07 to delete user user_privileges and sharing_privileges files when user is permanently deleted

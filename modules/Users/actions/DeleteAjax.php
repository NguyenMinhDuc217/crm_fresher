...
class Users_DeleteAjax_Action extends Vtiger_Delete_Action {
	...
	public function process(Vtiger_Request $request) {
        ...
        $result['message'] = vtranslate('LBL_USER_DEACTIVATED_SUCCESSFULLY', $moduleName);  // Modified by Hieu Nguyen on 2021-07-13

		if($mode == 'permanent'){
            Users_Record_Model::deleteUserPermanently($ownerId, $newOwnerId);
            $result['message'] = vtranslate('LBL_USER_DELETED_SUCCESSFULLY', $moduleName);  // Added by Hieu Nguyen on 2021-07-13
        } else {

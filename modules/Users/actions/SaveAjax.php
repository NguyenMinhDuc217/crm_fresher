...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	function __construct() {
		...
		$this->exposeMethod('togglePinMenu');	// [CustomMenu] Added by Vu Mai on 2023-02-02
	}
	...
	public function process(Vtiger_Request $request) {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
            // Modified by Hieu Nguyen on 2020-01-09 to save signature in HTML format
            if ($request->get('field') == $fieldName) {
                if ($fieldName == 'signature') {
                    $value = $request->getRaw('value', null);
                }
                else {
                ...
                $value = $fieldModel->get($fieldName);
            }
            
            $fieldValue = $displayValue = Vtiger_Util_Helper::toSafeHTML($value);
            // End Hieu Nguyen
	...
	public function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		// Added by Phuc on 2020.02.19 to change value to on from 1
		if ($fieldName === 'is_admin' && $currentUserModel->isAdminUser() && $request->get('value') == '1') {
	...
	public function togglePinMenu(Vtiger_Request $request) {
		$state = $request->get('state');
		$currentUserModel = Users_Record_Model::getCurrentUserModel();

		// Do saving preferences
		Users_Preferences_Model::savePreferences($currentUserModel->getId(), 'pin_menu', $state);

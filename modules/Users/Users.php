...
class Users extends CRMEntity {
	...
	var $error_string;
	var $is_admin;
	var $deleted;   // 0: Not Deleted, 1: Deactivated, 2: Permanently Deleted
	...
	var $list_fields = Array(
			...
			'User Name'=>Array('vtiger_users'=>'user_name'),    // Moved user name right before role name by Hieu Nguyen on 2021-01-19
			'Role Name'=>Array('vtiger_user2role'=>'roleid'),
			'LBL_IS_ONLINE' => Array('vtiger_users'=>'is_online'), // Added by Phu Vo on 2021.08.13
			'Status'=>Array('vtiger_users'=>'status'),
			'Email'=>Array('vtiger_users'=>'email1'),
			// 'Email2'=>Array('vtiger_users'=>'email2'),   // Removed by Hieu Nguyen on 2020-01-15
			'Admin'=>Array('vtiger_users'=>'is_admin'),
			'User Type' => ['vtiger_users' => 'users_type'],	// Added by Hieu Nguyen on 2021-09-13
			'Phone'=>Array('vtiger_users'=>'phone_work'),
			'CRM Phone Extension' => ['vtiger_users' => 'phone_crm_extension'],	// Added by Hieu Nguyen on 2019-03-15
	);
	var $list_fields_name = Array(
			'Last Name'=>'last_name',
			'First Name'=>'first_name',
			'User Name'=>'user_name',   // Moved user name right before role name by Hieu Nguyen on 2021-01-19
			'Role Name'=>'roleid',
			'LBL_IS_ONLINE' => 'is_online', // Added by Phu Vo on 2021.08.13
			'Status'=>'status',
			'Email'=>'email1',
			// 'Email2'=>'email2',  // Removed by Hieu Nguyen on 2021-01-15
			'Admin'=>'is_admin',
			'User Type' => 'users_type',	// Added by Hieu Nguyen on 2021-09-13
			'Phone'=>'phone_work',
            'CRM Phone Extension' => 'phone_crm_extension',    // Added by Hieu Nguyen on 2019-03-15
	);
	...
	function retrieve_entity_info($record, $module, $allowDeleted = false) {
		...
		for($i=0; $i<$noofrows; $i++) {
			...
			$fld_value = decodeUTF8($fld_value);	// Added by Hieu Nguyen on 2022-10-13 to display correct unicode values
	...
	function transformOwnerShipAndDelete($userId,$transformToUserId){
		...
		// Modified query by Hieu Nguyen on 2022-01-17 to auto clear call center phone ext of inactive user so that admin can assign that ext for other user
		$sql = "UPDATE vtiger_users SET phone_crm_extension = '', status = ?, deleted = ? where id = ?";
		$adb->pquery($sql, ['Inactive', 1, $userId]);
		// End Hieu Nguyen
	 ...
	 */
	// Renamed and modified this function by Hieu Nguyen on 2021-07-30 to get root admin id
	public static function getRootAdminId() {
		return 1;	// Default is 1, can be changed to any number to be more secured
	}
	 ...
	 */
	// Renamed and modified this function by Hieu Nguyen on 2021-07-30 to get root admin user
	public static function getRootAdminUser() {
		$adminId = self::getRootAdminId();
		$user = new Users();
		$user->retrieveCurrentUserInfoFromFile($adminId);
		return $user;
	}

	// Keep this function to allow backward compatible for other extensions
	public static function getActiveAdminUser() {
		return self::getRootAdminUser();
	}
	...
	* @return user id
	*/
	// Modified by Hieu Nguyen on 2021-07-30 to get exactly the active account owner user id
	public static function getAccountOwnerId() {
		global $adb;
		$sql = "SELECT id FROM vtiger_users WHERE is_owner = 1 AND deleted = 0 AND status = 'Active'";
		$ownerId = $adb->getOne($sql, []);
		return $ownerId;
	}
	...
	*/
    // Modified by Hieu Nguyen on 2021-07-13 to set only 1 selected user as account owner
    function transferOwnership($recordId) {
		if ($recordId) {
			global $adb;
            $sqlGetAllOwners = 'SELECT id FROM vtiger_users WHERE deleted < 2 AND is_owner = 1';
            $result = $adb->pquery($sqlGetAllOwners, []);

            // Unset all old owners
            while ($row = $adb->fetchByAssoc($result)) {
                $oldOwnerId = $row['id'];
                $adb->pquery('UPDATE vtiger_users SET is_owner = ? WHERE id = ?', [0, $oldOwnerId]);
                require_once('modules/Users/CreateUserPrivilegeFile.php');
                createUserPrivilegesfile($oldOwnerId);
            }

            // Then set selected user as owner
            $adb->pquery('UPDATE vtiger_users SET is_owner = ? WHERE id = ?', [1, $recordId]);
   ...
   //END
	...
	function createRecords($obj) {
		...
		for($i = 0; $i < $numberOfRecords; ++$i) {
			$errors = [];	# Added by Hieu Nguyen on 2022-09-19 to collect errors
			$mandatoryFlag = true;
			...
			if($status) {
				$mandatoryFlag = false;
				$errors[] = 'Username is duplicated!';	# Added by Hieu Nguyen on 2022-09-19 to collect errors
			}
				...
				foreach($blockInstance as $fieldInstance) {
					...
					if($fieldName == 'status') {
						...
						if($fieldValue != $data['user_password']) {
							$mandatoryFlag = false;
							$errors[] = 'Confirm password not matched!';	# Added by Hieu Nguyen on 2022-09-19 to collect errors
						}
						...
						if(empty($roleId)) {
							$mandatoryFlag = false;
							$errors[] = 'Role not matched!';	# Added by Hieu Nguyen on 2022-09-19 to collect errors
						}
						$fieldValue = $roleId;
						unset($roleId);
						...
						if(!preg_match($pattern, $fieldValue)) {
							$mandatoryFlag = false;
							$errors[] = 'Email not valid!';	# Added by Hieu Nguyen on 2022-09-19 to collect errors
						}
					}
			...
			foreach($mandatoryFields as $mandatoryField) {
				if(empty($record[$mandatoryField])) {
					$mandatoryFlag = false;
					$errors[] = "Field '{$mandatoryField}' is required!";	# Added by Hieu Nguyen on 2022-09-19 to collect errors
					break;
				}
			}
			...
			$obj->updateImportStatus($rowId, $entityInfo, $errors);	# Added by Hieu Nguyen on 2022-09-19 to fill errors into error reason column
		}

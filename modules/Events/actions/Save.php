...
class Events_Save_Action extends Calendar_Save_Action {
	...
	protected function getRecordModelFromRequest(Vtiger_Request $request) {
        ...
        // Modified by Hieu Nguyen on 2019-11-22 to handle invitation
        $contactIdsValue = $request->get('contact_invitees');
        $userIdsValue = $request->get('user_invitees');

		if(!empty($contactIdsValue) || !empty($userIdsValue)) {
            $contactIds = [];
            $userIds = [];
            
            if (!empty($contactIdsValue)) {
                $contactIds = explode(',', $contactIdsValue);

                // Convert into the same format as users
                if (!empty($contactIds)) {
                    for ($i = 0; $i < count($contactIds); $i++) {
                        $contactIds[$i] = 'Contacts:' . $contactIds[$i];
                    }
                }
            }
            ...
            $inviteeIds = array_merge($contactIds, $userIds);
			$recordModel->set('invitee_ids', $inviteeIds);
		}
        // End Hieu Nguyen

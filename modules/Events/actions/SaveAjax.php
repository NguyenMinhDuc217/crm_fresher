...
class Events_SaveAjax_Action extends Events_Save_Action {
	...
	public function getRecordModelFromRequest(Vtiger_Request $request) {
        ...
        // Modified by Hieu Nguyen on 2020-02-19 to fill visibility to prevent failure when pushing event to Google
		if (empty($visibility) && $recordModel->get('mode') != 'edit') {
			$assignedUserIdString = $recordModel->get('assigned_user_id');  // Assigned To in Events will be a single User in new logic
            $mainOwnerId = Vtiger_CustomOwnerField_Helper::getOwnerIdFromRequest($assignedUserIdString);
			$sharedType = Calendar_Module_Model::getSharedType($mainOwnerId);

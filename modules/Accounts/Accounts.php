...
class Accounts extends CRMEntity {
		...
		foreach($accounts_list as $account_id => $account_info) {
			...
			foreach ($this->list_fields_name as $fieldname=>$colname) {
				...
				if(!$hasRecordViewAccess && $colname != 'accountname') {
					...
					// Added by Phu Vo on 2019.06.20 => process custom owner list content
					else if ($colname == 'assigned_user_id') {
						echo ('');
					}
					// End Phu Vo
	...
	*/
	function __getParentAccounts($id, &$parent_accounts, &$encountered_accounts) {
		...
		// Comment out by Phu Vo on 2019.06.21 to apply Custom Owner Field
		/*$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
							'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
		...
		// [CustomOwnerField] Edited by Phu Vo on 2019.06.21
		$query = "SELECT 
				vtiger_account.*, 
		...
		foreach($this->list_fields_name as $fieldname=>$columnname) {
			if ($columnname == 'assigned_user_id') {
				// Edit by Phu Vo on 2019.06.21 to process custom owner user display
				// $parent_account_info[$columnname] = $adb->query_result($res, 0, 'user_name');
				$displayValue = Vtiger_Owner_UIType::getCurrentOwnersForDisplay($adb->query_result($res, 0, 'ownerid'), false, true);
				$parent_account_info[$columnname] = $displayValue;
				// End Phu Vo
			} else {
	...
	*/
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$temporaryTableCrmentityrel = self::setupTemporaryTableCrmentityrel($entityIds);

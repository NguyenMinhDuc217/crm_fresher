...
{strip}
    <label>
        ...
        &nbsp;
        <span>{vtranslate("LBL_BUSINESS_TYPE_B2B", $QUALIFIED_MODULE)}</span>
    </label>
    &nbsp;&nbsp;
    <label>
        {* <input type="radio" name="accounts_business_type" value="B2C" {if $RECORD && $RECORD->get('accounts_business_type') == "B2C"}checked{/if}> *}
        <input type="radio" name="accounts_business_type" value="B2C" {if $RECORD && $RECORD->get('accounts_business_type') == "B2C"}checked{/if} {if $RECORD == '1'}checked{/if}>
        &nbsp;
        <span>{vtranslate("LBL_BUSINESS_TYPE_B2C", $QUALIFIED_MODULE)}</span>
    </label>
{/strip}
{* End Minh Duc *}
...
class Leads_Record_Model extends Vtiger_Record_Model {
	 ...
	 * @return Array
	 */
	// Modified by Tùng Nguyễn on 2022-08-24 to Rearrange fields in popup Convert Leads
	function getContactFieldsForLeadConvert() {
		...
		if ($moduleModel->isActive()) {
			$fieldModels = $moduleModel->getFields();
            $complusoryFields = array('lastname', 'firstname', 'email'); // Modified by Hieu Nguyen on 2020-03-19 to show both firstname and lastname fields in the convert lead form

			// Add required field
            foreach ($fieldModels as $fieldName => $fieldModel) {
                if ($fieldModel->isMandatory() && !in_array($fieldName, ['assigned_user_id', 'account_id'])) {
					if (!in_array($fieldName, $complusoryFields)) {
						$complusoryFields[] = $fieldName;
					}
                }
            }

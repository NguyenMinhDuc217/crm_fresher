...
class MailManager_Message_Model extends Vtiger_MailRecord  {
	...
	public function __getpart($imap, $messageid, $p, $partno) {
    	...
    	elseif ($p->type==0 && $data) {
    		...
    		$data = self::__convert_encoding($data, 'UTF-8');   // Modified by Hieu Nguyen on 2021-04-29 to prevent incorrect charset decoding (function __convert_encoding is already auto detect the source encoding, no need to specify)
	...
	public function getAttachmentIcon($fileName) {
		...
		switch(strtolower($ext)) {
			...
			case 'wma' :
			case 'wav' :
			case 'ogg' : $icon = 'fa-file-audio';
				break;
			case 'xls' :
			case 'xlsx' : $icon = 'fa-file-excel';
				break;
			case 'webm' :
			case 'mkv' :
			case 'flv' :

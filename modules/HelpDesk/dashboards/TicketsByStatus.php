...
class HelpDesk_TicketsByStatus_Dashboard extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ($user == 'all') $user = ''; // Added by Phuc on 2019.10.24 to show all tickets

		$data = $moduleModel->getTicketsByStatus($user, $dates);
		// End Phu Vo

        $listViewUrl = $moduleModel->getListViewUrlWithAllFilter();
        for($i = 0;$i<count($data);$i++){
            $data[$i]["links"] = $listViewUrl.$this->getSearchParams($data[$i][2],$request->get('assigned_user_id'), $request->get('dateFilter')).'&nolistcache=1'; // Updated by Phuc on 2020.02.19
        }
		...
		$ownerFieldModel = Vtiger_Field_Model::getInstance('assigned_user_id', $moduleModel);

...
Class Inventory_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {$viewer = $this->getViewer($request);
		...
		// Added by Kelvin Thang on 2020-02-29 to load custom code
		...
		if ($request->has('totalProductCount')) {
			...
			} elseif ($sourceRecord && in_array($sourceModule, array('Accounts', 'Contacts', 'Potentials', 'Vendors', 'PurchaseOrder', 'SalesOrder'))) { // Modified by Tung Nguyen on 2022.08.18 to add SalesOrder support case create order from related list of other module
		...
		foreach($requestFieldList as $fieldName=>$fieldValue) {
            ...
            // Modified by Hieu Nguyen on 2020-12-08
			if ($fieldModel->isEditable() || in_array($fieldName, $this->allowPrefillFields)) {
         ...
         * Date: 2018-09-14
         * convert construct data for product item
         */
        $relatedProdCount = count($relatedProducts);
        $relatedProdSection = array();
        for($relProdCount=1; $relProdCount <= $relatedProdCount; $relProdCount++){
            $numSection = $relatedProducts[$relProdCount]['sectionNum'.$relProdCount];
            $numSection = (!empty($numSection))? $numSection : 1;
            $relatedProdSection[$numSection][$relProdCount] = $relatedProducts[$relProdCount];
        }

		//-- Added by Kelvin Thang on 2020-02-29 -@Assign list of Combo Products
		$viewer->assign('COMBO_PRODUCTS', CPComboProducts_Data_Model::initComboProducts($moduleName));
		//-- End by Kelvin Thang
		...
		$viewer->assign('RELATED_PRODUCTS', $relatedProdSection);
	...
	function getHeaderScripts(Vtiger_Request $request) {
		...
		//-- Added by Kelvin Thang on 2020-02-29 -@Assign list of PS Template 
		if (Vtiger_Module_Model::getInstance('CPComboProducts')->isActive()) {
			$jsFileNames[] = 'modules.CPComboProducts.resources.ComboProducts';
		}
		//-- End by Kelvin Thang on 2020-02-29 -@Assign list of PS Template 

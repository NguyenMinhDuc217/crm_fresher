...
class Inventory_Record_Model extends Vtiger_Record_Model {
		...
		for ($i=1;$i<=$productsCount; $i++) {
			...
			if ($taxtype == 'individual') {
				...
				foreach ($productTaxes as $taxId => $taxInfo) {
					...
					//$relatedProducts[$i]['taxTotal'.$i]	= Vtiger_Currency_UIType::transformDisplayValue($taxTotal, null, true);
				}
				$netPrice = $totalAfterDiscount + $taxTotal;
				$relatedProducts[$i]['netPrice'.$i] = number_format($netPrice, $numOfCurrencyDecimalPlaces, '.', '');
				//$relatedProducts[$i]['netPrice'.$i] = Vtiger_Currency_UIType::transformDisplayValue($netPrice, null, true);
			}
	...
	function setRecordFieldValues($parentRecordModel) {
        ...
        Vtiger_CustomOwnerField_Helper::removeUnassignableUsers($this);
        // End Hieu Nguyen

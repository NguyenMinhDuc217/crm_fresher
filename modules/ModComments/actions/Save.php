...
class ModComments_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		$result['commentcontent_rendered'] = renderCommentWithMentions($result['commentcontent']);  // Added by Hieu Nguyen on 2021-03-17 to return rendered comment with mention
		$result['success'] = true;
	...
	protected function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		$recordModel->set('commentcontent', $request->get('commentcontent'));   // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
		$recordModel->set('reasontoedit', $request->get('reasontoedit'));       // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS

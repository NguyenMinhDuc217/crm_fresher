...
class EmailTemplates_Record_Model extends Vtiger_Record_Model {
	...
	public static function getInstanceById($templateId, $module=null) {
        ...
        static $cache = [];
        if (!empty($cache[$templateId])) return $cache[$templateId];
        // End Hieu Nguyen
	...
	// Added by Phuc on 2019.11.26
	static function isDuplicated($templateId, $checkField, $checkValue) {
		global $adb;
		$checkField = $adb->sql_escape_string($checkField); // For safe query

		$sql = "SELECT COUNT(templateid) FROM vtiger_emailtemplates WHERE templateid != ? AND {$checkField} = ?";
		$params = [$templateId, $checkValue];

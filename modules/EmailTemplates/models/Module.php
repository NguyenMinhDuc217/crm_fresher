...
class EmailTemplates_Module_Model extends Vtiger_Module_Model {
	...
	*/
	...
	public function saveRecord(EmailTemplates_Record_Model $recordModel) {
		...
		if (empty($systemtemplate)) $systemtemplate = '0';

		if (empty($templateid)) {
			$templateid = $db->getUniqueID('vtiger_emailtemplates');
			$sql = "INSERT INTO vtiger_emailtemplates(templatename, subject, description, module, body, deleted, systemtemplate, safe_content, templateid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		}
		else {
			$sql = "UPDATE vtiger_emailtemplates SET templatename = ?, subject = ?, description = ?, module = ?, body = ?, deleted = ?, systemtemplate = ?, safe_content = ? WHERE templateid = ?";
		}
		...
		$params = [
			...
			$recordModel->get('module'),
			$recordModel->get('body'),
			0, 
			$systemtemplate,
			$recordModel->get('safe_content'),
			$templateid
		];

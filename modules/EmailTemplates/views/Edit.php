...
Class EmailTemplates_Edit_View extends Vtiger_Index_View {
	...
	function getHeaderScripts(Vtiger_Request $request) {
		...
		$jsFileNames = array(
			...
			"~modules/EmailTemplates/resources/EditView.js" // Added by Phuc on 2019.11.27 to validate UI
		);
	...
	public function initializeContents(Vtiger_Request $request, Vtiger_Viewer $viewer){
		...
		if (!empty($record) && $request->get('isDuplicate') == true) {
            ...
            if ($recordModel->get('safe_content') == 1 && $current_user->is_admin != 'on') {
                die('<center>'. vtranslate('LBL_SAFE_CONTENT_TEMPLATE_NON_ADMIN_EDIT_ERROR_MSG', $moduleName) .'</center>');
            }
            // End Hieu Nguyen

...
class Vtiger_Module_Model extends Vtiger_Module {
    ...
    protected $entitity = null; // Added by Hieu Nguyen on 2021-05-31 to boost performance
    ...
    // Added by Hieu Nguyen on 2021-05-31 to boost performance
    public function isReadonlyModule() {
        return !empty($this->entitity) && $this->entitity->isReadonly;
    }
	...
	public function isQuickCreateSupported() {
		// Added by Phu Vo on 2020.07.27 to disable create on readonly module
		if ($this->isReadonlyModule()) {    // Modified by Hieu Nguyen on 2021-05-31 to boost performance
			return false;
		}
		// End Phu Vo
	 ...
	 */
	public function getFields($requireBlock = true) {
		// Modified by Hieu Nguyen on 2021-10-20 to load all fields for Roles and Profile permission setting form
		if ($_REQUEST['module'] == 'Profiles') {
			static $fieldModels = [];
			if (!empty($fieldModels)) return $fieldModels;
			$moduleFields = Vtiger_Field::getAllForModule($this);

			foreach ($moduleFields as $fieldObject) {
				$fieldModel = Vtiger_Field_Model::getInstanceFromFieldObject($fieldObject);
	 ...
	 * @return <Array of Vtiger_Field_Model> - list of field models
	 */
	public function getFieldsByType($type, $requireBlock = true) {
		if (!is_array($type)) {
			$type = [$type];
		}

		$fields = $this->getFields($requireBlock);
		$fieldList = [];
	...
	public function getConfigureRelatedListFields(){
        // Modified by Hieu Nguyen on 2019-10-09 to support relation list layout editor
	...
	public static function getInstanceFromModuleObject(Vtiger_Module $moduleObj){
        ...
        // Added by Hieu Nguyen on 2021-05-31 to boost performance
        if ($objectProperties['isentitytype'] == '1') {
            $moduleModel->entitity = CRMEntity::getInstance($objectProperties['name']);
        }
        // End Hieu Nguyen

        $cache[$cacheKey] = $moduleModel;   // Added by Hieu Nguyen on 2021-05-31 to apply caching
		return $moduleModel;
	}
	...
	public static function getAll($presence = array(), $restrictedModulesList = array()) {
		...
		if(!$moduleModels){
			...
			for($i=0; $i<$noOfModules; ++$i) {
                ...
                // Added by Hieu Nguyen on 2019-12-18 to hide forbidden modules from Module Manager
                if (isForbiddenFeature($row['name']) || isHiddenModule($row['name'])) {
                    continue;
                }
                // End Hieu Nguyen
	...
	function getCalendarActivities($mode, $pagingModel, $user, $recordId = false) {
        ...
        // Modified by Hieu Nguyen on 2020-12-23 to add keyword DISTINCT, field main_owner_id, and replace INNER JOIN with LEFT JOIN to display both related and invited activities
		$query = "SELECT DISTINCT vtiger_crmentity.crmid, crmentity2.crmid AS parent_id, vtiger_crmentity.smownerid, vtiger_crmentity.main_owner_id, vtiger_crmentity.setype, vtiger_activity.* 
            FROM vtiger_activity
            INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_activity.activityid
            LEFT JOIN vtiger_seactivityrel ON vtiger_seactivityrel.activityid = vtiger_activity.activityid
            LEFT JOIN vtiger_crmentity AS crmentity2 ON vtiger_seactivityrel.crmid = crmentity2.crmid AND crmentity2.deleted = 0 AND crmentity2.setype = ?
        ...
        if ($recordId) {
            $parentRecordType = Vtiger_Functions::getCRMRecordType($recordId);
            $parentRecordModel = Vtiger_Record_Model::getInstanceById($recordId, $parentRecordType);
        }
    ...
    // Extracted this function from original function getRelatedActivities by Hieu Nguyen on 2020-12-25 to make this block of code reuseable
    function getRelatedActivityRecordsForWidget($db, $query, $params, $currentUser, $pagingModel, $parentRecordModel = null) {
        // Added by Hieu Nguyen on 2020-12-25
        $result = $db->pquery($query, $params);
		...
		for($i=0; $i<$numOfRows; $i++) {
            ...
            // Modified by Hieu Nguyen on 2020-12-25 to display Activities related list with the same rule as Shared Calendar
            $newRow['assigned_user_id'] = $newRow['smownerid'];    // To show the value of "Assigned to"

            if ($parentRecordModel && ($parentRecordModel->get('main_owner_id') != $currentUser->getId() || $relatedActivitiesDisplayConfig['main_owner_full_access'] != '1')) {    // For subpanel and summary widget
                if ($newRow['activitytype'] != 'Task') {
                    $recordVisible = Calendar_Data_Model::isEventVisible($newRow, 'SharedCalendar', $newRow['main_owner_id'], $currentUser->getId());
                    ...
                    $recordBusy = Calendar_Logic_Model::isRelatedActivityBusy($newRow['crmid'], $parentRecordModel->getId());
            
                    if ($recordBusy) {
                        $newRow['subject'] = Calendar_Data_Model::getBusyTitle($newRow['main_owner_id']);
                    }
                }
            }

            if (!$parentRecordModel && $newRow['main_owner_id'] != $currentUser->getId() && $newRow['activitytype'] != 'Task') {   // For Upcoming Activities and Overdue Activities dashlet
                $recordVisible = Calendar_Data_Model::isEventVisible($newRow, 'SharedCalendar', $newRow['main_owner_id'], $currentUser->getId());
                ...
                $recordBusy = Calendar_Data_Model::isRecordBusy($newRow, 'SharedCalendar', $newRow['main_owner_id'], $currentUser->getId());
            
                if ($recordBusy) {
                    // Do not show busy title for event that is a invitation and accepted by current user
                    if (
                        !(Events_Invitation_Helper::isInvitee($currentUser->getId(), 'Users', $newRow['activityid']) 
                        && Events_Invitation_Helper::isInvitationAccepted($currentUser->getId(), 'Users', $newRow['activityid']))
                    ) {
                        $newRow['subject'] = Calendar_Data_Model::getBusyTitle($newRow['main_owner_id']);
                    }
                }
            ...
            // Modified condition by Hieu Nguyen on 2020-12-30 to hide task for user that is not the task owner
			if (!$currentUser->isAdminUser() && $newRow['activitytype'] == 'Task' && isCalendarTaskPermittedBySharing($newRow['crmid'], 'index') == 'no') { 
				$recordsToUnset[] = $newRow['crmid'];
			}
		}
	...
	public function isPermitted($actionName) {
        static $cache = [];
        if (isset($cache[$actionName])) return $cache[$actionName];

		// Added by Phu Vo on 2020.07.27 to disable create on readonly module
		if ($this->isReadonlyModule()) {
            if (in_array($actionName, ['CreateView', 'EditView', 'Delete'])) {
                $result = false;
                $cache[$actionName] = $result;
                return $result;
            }
		}
		// End Phu Vo

		$result = ($this->isActive() && Users_Privileges_Model::isPermitted($this->getName(), $actionName));
        $cache[$actionName] = $result;
        return $result;
	}
	...
	public function getOwnerWhereConditionForDashBoards ($owner) {
		// [CustomOwnerField] Bug #20: Modified by Phu Vo on 2020.02.06 to process case get all users
		if ($owner === 'all') return '';
		// End Bug #20
		...
		// Modified by Phuc on 2019.10.24 to get owner for dashboard
		$ownerId = '';
		$ownerSql = '';
		...
		if (is_array($ownerId)) {
			...
			$ownerSql = " (vtiger_crmentity.smownerid IN ('{$ownerId}')
		...
		elseif (!empty($ownerId)) {
			$ownerSql = " (vtiger_crmentity.smownerid = '{$ownerId}'
				-- if user are belong to a custom group
				OR '{$ownerId}' IN (
	...
	public function getModuleBasicLinks(){
		...
		// Added by Phu Vo to handle readonly module basic links
		if (isReadonlyModule($moduleName)) {
			$basicLinks = removeButtons($basicLinks, ['LBL_ADD_RECORD', 'LBL_IMPORT']);
		}
		// End Phu Vo

		// Added by Hieu Nguyen on 2021-09-21 to hide button import for Inventory modules
		global $inventoryModules, $developerTeam;

		if (in_array($moduleName, $inventoryModules) && !in_array($developerTeam, ['DEV', 'R&D'])) {
			$basicLinks = removeButtons($basicLinks, ['LBL_IMPORT']);
		}
		// End Hieu Nguyen
	 ...
	 * @return <String> - query
	 */
    // Modified by Hieu Nguyen on 2019-11-22
	public function getSearchRecordsQuery($searchValue,$searchFields, $parentId=false, $parentModule=false) {
		$query = "SELECT ".implode(',',$searchFields)." FROM vtiger_crmentity WHERE label LIKE '%$searchValue%' AND vtiger_crmentity.deleted = 0";

        // Fix bug loading data from all modules when this module is also selected as parent module in parent_id field
        if ($parentModule == $this->get('name')) {
            $query .= " AND vtiger_crmentity.setype = '{$this->get('name')}'";
        }
	...
	public function getRelationQuery($recordId, $functionName, $relatedModule, $relationId) {
		...
		if(count($relatedListFields) > 0) {
			...
			// Edited by Phuc Lu on 2019.08.16 to fix preg_split with case sensitive
			$newQuery = preg_split('/\sFROM\s/i', $query);
			// Ended by Phuc
			
			//--Begin: Fix Bug By Kelvin Thang on 2019-07-24 decs: Column 'crmid' in field list is ambiguous when Load relation Activity
            if($relatedModuleName == 'Calendar'
                && in_array('parent_id', $relatedListFields)
                && preg_match('/vtiger_seactivityrel.crmid/', $selectColumnSql, $matches, PREG_OFFSET_CAPTURE)
            ){
                $selectColumnSql = str_replace("vtiger_seactivityrel.crmid","vtiger_seactivityrel.crmid AS parent_id",$selectColumnSql);
            }
            //--End: Fix Bug By Kelvin Thang on 2019-07-24
		...
		foreach($relatedModuleRecordIds as $recordId) {
			if(Users_Privileges_Model::isPermitted($moduleName, 'Save', $recordId)) {
				try {
					...
					return $e->getMessage(); // Added by Phu Vo on 2019.08.26 to return missing error message
				}
			}
		}

		return true; // Added by Phu Vo on 2019.08.26 to default return true
	}
	...
	*/
	...
	public function getPopupViewFieldsList(){
        // Modified by Hieu Nguyen on 2019-10-09 to support popup layout editor
        $configuredFields = Settings_LayoutEditor_Module_Model::getModuleLayoutFields($this->getName(), 'Popup');
        if (!empty($configuredFields)) return $configuredFields;
        // End Hieu Nguyen
...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
	...
	public function isListViewQuickEditAllowed() {  // Refactored function name by Hieu Nguyen on 2020-01-16
		// Added by Phu Vo on 2020.07.27 to disable create on readonly module
		if ($this->isReadonlyModule()) return false;    // Modified by Hieu Nguyen on 2021-05-31 to boost performance
		// End Phu Vo
		
		return $this->isPermitted('EditView');
	}

	/** Modified by Phu Vo on 2021.04.21 support get module icon from config */
	public function getModuleIcon() {
		...
		$cssClass = $moduleIcons['Default'];

		if (!empty($moduleIcons[$moduleName])) {
			$cssClass = $moduleIcons[$moduleName];
		}

		$moduleIcon = "<i class='{$cssClass}' title='$title' aria-hidden='true'></i>";

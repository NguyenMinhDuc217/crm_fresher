...
class Vtiger_Tag_Model extends Vtiger_Base_Model {
    ...
    // Implemented by Hieu Nguyen on 2021-01-26 to get all accessible tags of current user 
    public static function getAllUserAccessibleTags() {
        global $adb, $current_user;
        $sql = "SELECT * FROM vtiger_freetags WHERE owner = ? OR visibility = 'public'";
...
//        }
	...
	public static function saveForRecord($recordId , $tagList , $userId='', $module='') {
		...
		if (!$GLOBALS['save_tags_event_handled_globally']) {
			try {
				$handlerName = getEventHandlerClass($module);

				if (method_exists($handlerName, 'tagsChanged')) {
					$handlerName::tagsChanged($recordId, 'Link', ['new_tags' => $tagList]);
				}
			}
			catch (Exception $e) {
				saveLog('PLATFORM', '[Vtiger_Tag_Model::saveForRecord] Custom handler error: '. $e->getMessage(), $e->getTrace());
			}
		}
		// End Hieu Nguyen
	}
	...
	public static function deleteForRecord($recordId, $tagList, $userId ='', $module='') {
		...
		if (!$GLOBALS['save_tags_event_handled_globally']) {
			try {
				...
				if (method_exists($handlerName, 'tagsChanged')) {
					$handlerName::tagsChanged($recordId, 'Unlink', ['unlinked_tags' => $tagList]);
				}
			}
			catch (Exception $e) {
				saveLog('PLATFORM', '[Vtiger_Tag_Model::deleteForRecord] Custom handler error: '. $e->getMessage(), $e->getTrace());
			}
		}
		// End Hieu Nguyen
	}
	...
	// Added by Hieu Nguyen on 2021-11-29 to get all public tags
	public static function getAllPublicTags() {
		global $adb;
		$sql = "SELECT * FROM vtiger_freetags WHERE visibility = ?";
		$result = $adb->pquery($sql, [self::PUBLIC_TYPE]);
		$publicTags = [];
	...
	// Added by Hieu Nguyen on 2021-12-13 to check for duplicated tag
	public function isDuplicated() {
		global $adb;
		$tagName = $this->getName();
		$tagId = $this->getId();

		$sql = "SELECT 1 FROM vtiger_freetags WHERE (tag = ? OR raw_tag = ?)";

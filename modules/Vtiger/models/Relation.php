...
class Vtiger_Relation_Model extends Vtiger_Base_Model{
	...
	public function addRelation($sourcerecordId, $destinationRecordId) {
		...
		// Added by Hieu Nguyen on 2021-11-23 to trigger custom even handler when a related record linked
		try {
			// Trigger global event
			$globalHandlerName = getGlobalEventHandlerClass();
			$globalHandlerName::relationLinked($this, $sourceModuleName, $sourcerecordId, $destinationModuleName, $destinationRecordId);

			// Trigger module event
			$moduleHandlerName = getEventHandlerClass($sourceModuleName);

			if (method_exists($moduleHandlerName, 'relationLinked')) {
				$moduleHandlerName::relationLinked($this, $sourcerecordId, $destinationRecordId, $destinationModuleName);
			}
		}
		catch (Exception $e) {
			saveLog('PLATFORM', '[Vtiger_Relation_Model::addRelation] Custom handler error: ' . $e->getMessage(), $e->getTrace());
		}
		// End Hieu Nguyen
	}
	...
	public function deleteRelation($sourceRecordId, $relatedRecordId){
		...
		// Added by Hieu Nguyen on 2021-11-23 to trigger custom even handler when a related record unlinked
		try {
			// Trigger global event
			$globalHandlerName = getGlobalEventHandlerClass();
			$globalHandlerName::relationUnlinked($this, $sourceModuleName, $sourceRecordId, $destinationModuleName, $relatedRecordId);

			// Trigger module event
			$moduleHandlerName = getEventHandlerClass($sourceModuleName);

			if (method_exists($moduleHandlerName, 'relationUnlinked')) {
				$moduleHandlerName::relationUnlinked($this, $sourceRecordId, $relatedRecordId, $destinationModuleName);
			}
		}
		catch (Exception $e) {
			saveLog('PLATFORM', '[Vtiger_Relation_Model::deleteRelation] Custom handler error: ' . $e->getMessage(), $e->getTrace());
		}
		// End Hieu Nguyen
	...
	// Implemented by Hieu Nguyen on 2022-12-26 to check if we are at the related list or not
	public static function isAtRelatedList() {
		$request = getRequest();

		if ($request->get('view') == 'Detail' && $request->get('mode') == 'showRelatedList') {
			return true;
		}
	...
	// Implemented by Hieu Nguyen on 2022-12-26 to get parent record model when we are at the related list
	public static function getParentRecordModelAtRelatedList() {
		if (!self::isAtRelatedList()) return null;
		$request = getRequest();
	...
	public static function getAllRelations($parentModuleModel, $selected = true, $onlyActive = true) {
		...
		for($i=0; $i<$db->num_rows($result); $i++) {
			...
			// Added by Hieu Nguyen on 2021-08-18 to hide forbidden modules in this package and hidden modules from Related List
			if (isForbiddenFeature($row['modulename']) || isHiddenModule($row['modulename'])) {
				continue;
			}
			// End Hieu Nguyen

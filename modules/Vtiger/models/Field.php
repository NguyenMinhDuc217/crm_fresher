...
class Vtiger_Field_Model extends Vtiger_Field {
    ...
    // Implemented by Hieu Nguyen on 2021-07-01 to get unique fields of the specific module
    static function getUniqueFields($moduleName, $returnIdsOnly = false) {
        global $adb;
        $moduleId = getTabid($moduleName);
        $sql = "SELECT fieldid, fieldname FROM vtiger_field WHERE tabid = ? AND isunique = 1";
	...
	public function getFieldDataType() {
		if(!$this->fieldDataType) {
            ...
            // Added by Hieu Nguyen on 2019-06-18 to determine owner field
            } else if($uiType == '53') {
                $fieldDataType = 'owner';
            // End Hieu Nguyen
			} else if($uiType == '55' && stripos($this->getName(), 'salutationtype') !== false) {
				$fieldDataType = 'picklist';
            // [FullNameConfig] Added by Hieu Nguyen on 2020-03-04 to determine firstname and lastname fields
			} else if($uiType == '255') {
                $fieldDataType = 'personName';
            // End Hieu Nguyen
            // Added by Hieu Nguyen on 2021-01-26 to determine tags field
            } else if(stripos($this->getName(), 'tags') !== false) {
                $fieldDataType = 'tags';
            // End Hieu Nguyen
	...
	public function getReferenceList($hideDisabledModules = true, $presenceZero = true) {
		...
		if($presenceZero && is_array($referenceList) && count($referenceList) > 0) {
			foreach($referenceList as $key => $referenceModule) {
				...
				if($moduleModel && $moduleModel->get('presence') != 0 || isForbiddenFeature("Module{$referenceModule}")) {	// Modified by Hieu Nguyen on 2022-09-19 to hide unsupported modules
	...
	public function isViewEnabled() {
		// Added by Hieu Nguyen on 2021-10-20 to display field in Roles and Profiles settings without depending on which view it belong to
	...
	public function isEditEnabled() {
        ...
        // Modified by Hieu Nguyen on 2019-06-11 to load nonEditable fields from config
        global $nonEditableFields;
		$restrictedFields = $nonEditableFields;
        // End Hieu Nguyen
	...
	public function isAjaxEditable() {
		// Added by Phu Vo to block edit on read only module fields
		$moduleModel = $this->getModule();
		if ($moduleModel && $moduleModel->isReadonlyModule()) return false; // Modified by Hieu Nguyen on 2021-05-31 to boost performance
		// End Phu Vo
	...
	public function getCustomViewColumnName() {
		...
		$escapedFieldLabel = html_entity_decode(str_replace(' ', '_', $fieldLabel));    // Modified by Hieu Nguyen on 2020-03-16 to fix bug selected field name for custom view can not be mapped when loading selected fields for edit
		$moduleFieldLabel = $moduleName.'_'.$escapedFieldLabel;
	...
	public function getFieldInfo() {
		...
		if($this->getFieldDataType() == 'owner') {
            // Comment out by Hieu Nguyen on 2019-05-24 to boost performance for custom owner field
		...
		if($this->getFieldDataType() == 'reference') {
			...
			// Added by Hieu Nguyen on 2022-07-26 to hide relation fields linked to forbidden modules
			if (count($this->fieldInfo['referencemodules']) == 1) {
				$forbiddenModules = getForbiddenFeatures('module');

				if (in_array($this->fieldInfo['referencemodules'][0], $forbiddenModules)) {
					$this->set('presence', 1);
				}
			}
			// End Hieu Nguyen
	...
	*/
	 ...
	 * @return <Array>
	 */
    // Refactored code and modified by Hieu Nguyen on 2021-05-31 to apply caching
    // Modified by Hieu Nguyen on 2021-06-10 to check field visibility based on current view
	public static function preFetchModuleFieldPermission($tabId, $accessMode = 'readonly') {
        static $cache = [];
        $cacheKey = "{$tabId}_{$accessMode}";
        ...
        if ($GLOBALS['current_view'] == 'edit') $presenceField = 'editview_presence';
        ...
        if (count($profileList) > 0) {
            if ($accessMode == 'readonly') {
                $query = "SELECT vtiger_profile2field.visible, vtiger_field.fieldid 
                    ...
                    WHERE vtiger_field.tabid = ? AND vtiger_profile2field.visible = 0 
                        ...
                        AND vtiger_field.{$presenceField} IN (0, 2) 
                ...
                $query = "SELECT vtiger_profile2field.visible, vtiger_field.fieldid 
                    ...
                    INNER JOIN vtiger_def_org_field ON vtiger_def_org_field.fieldid = vtiger_field.fieldid 
                    WHERE vtiger_field.tabid=? AND vtiger_profile2field.visible = 0 
                        AND vtiger_profile2field.readonly = 0 AND vtiger_def_org_field.visible = 0  
                        AND vtiger_profile2field.profileid IN (". generateQuestionMarks($profileList) .") 
                        AND vtiger_field.{$presenceField} IN (0, 2) 
                    GROUP BY vtiger_field.fieldid";
            }
            ...
            if ($accessMode == 'readonly') {
                $query = "SELECT vtiger_profile2field.visible, vtiger_field.fieldid 
                    ...
                    INNER JOIN vtiger_def_org_field ON vtiger_def_org_field.fieldid = vtiger_field.fieldid 
                    WHERE vtiger_field.tabid = ? AND vtiger_profile2field.visible = 0 
                        AND vtiger_def_org_field.visible = 0 AND vtiger_field.{$presenceField} IN (0, 2) 
                    GROUP BY vtiger_field.fieldid";
            }
            else {
                $query = "SELECT vtiger_profile2field.visible, vtiger_field.fieldid 
        ...
        $cache[$cacheKey] = $modulePermission;
	...
	public function __update() {
		...
		// Modifed by Hieu Nguyen on 2018-07-31
		$presenceField = $_REQUEST['layouteditor_tab'] == 'editViewTab' ? 'editview_presence' : 'presence';

		$query = "UPDATE vtiger_field SET typeofdata=?, {$presenceField}=?, quickcreate=?, masseditable=?, defaultvalue=?, summaryfield=?, headerfield=?";

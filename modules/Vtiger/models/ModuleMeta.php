...
class Vtiger_ModuleMeta_Model extends Vtiger_Base_Model {
	 ...
	 * @return <Array of Vtiger_Field>
	 */
    // Modified this function by Hieu Nguyen on 2021-05-11 to show unique fields as default field for check duplicate while import
	public function getDefaultFieldsForDuplicateCheck() {
		$moduleFields = $this->getAccessibleFields($this->moduleName);
		$nameFields = vtws_getEntityNameFields($this->moduleName);
        $nameFieldInstances = [];
		$uniqueFieldInstances = [];

		foreach ($moduleFields as $fieldName => $fieldInstance) {
			$columnName = $fieldInstance->getColumnName();
			
            if (in_array($columnName, $nameFields)) {
				$nameFieldInstances[$fieldName] = $fieldInstance;
			}

            if ($fieldInstance->isUnique()) {
				$uniqueFieldInstances[$fieldName] = $fieldInstance;
			}
		}

		$duplicateCheckFields = (!empty($uniqueFieldInstances)) ? $uniqueFieldInstances : $nameFieldInstances;

...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	// Added by Hieu Nguyen on 2022-10-11 to check if $this->rawData or raw data of specific field is available or not
	public function hasRawData($fieldName = null) {
		if (empty($fieldName)) return !empty($this->rawData);
		return isset($this->rawData[$fieldName]);
	}

	// Added by Hieu Nguyen on 2022-10-11 to fetch full record data in case some fields are not fetched yet (ex: at ListView, Popup List and Subpanel)
	public function fetchFullRecordData() {
		if (!$this->getId()) return false;
		$fullRecordModel = self::getInstanceById($this->getId(), $this->getModuleName());
		$this->fetchedRow = $fullRecordModel->fetchedRow;
		$this->setData($fullRecordModel->getData());
		$this->setRawData($fullRecordModel->getRawData());
	}
	...
	public function getFullDetailViewUrl() {
        ...
        if ($moduleName == 'Events') $moduleName = 'Calendar';  // Added by Hieu Nguyen on 2020-10-20 to show full events details in Calendar module
	...
	public static function getInstanceById($recordId, $module=null) {
        ...
        // Added by Hieu Nguyen on 2019-03-11 to store the raw fetched row so that we can compare with the modified data later 
        $instance->fetchedRow = array_merge([], $focus->column_fields->getColumnFields());
        // End Hieu Nguyen

        // Modified by Hieu Nguyen on 2021-04-06 to support caching
		$instance->setData($focus->column_fields)->setRawData($instance->fetchedRow);	# Fix bug getRawData() and getRaw() sometimes return null value
		$instance->set('id', $recordId)->setModuleFromInstance($module)->setEntity($focus);
	...
	// This function will search and return the first record that matching the given conditions. Implemented by Hieu Nguyen on 2019-07-23
	public static function getInstanceByConditions($moduleName, array $conditions = []) {
		global $adb;

		// Added by Hieu Nguyen on 2021-04-06 to support caching
		if (empty($moduleName) || empty($conditions)) return null;
		static $cache = [];
		$cacheKey = $moduleName .'-'. md5(serialize($conditions));
		if (!empty($cache[$cacheKey])) return $cache[$cacheKey];
		// End Hieu Nguyen

		$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
		$sql = "SELECT {$moduleModel->basetableid}
			FROM {$moduleModel->basetable}
			INNER JOIN vtiger_crmentity ON (crmid = {$moduleModel->basetableid} AND deleted = 0)
			WHERE 1 = 1 ";

		foreach ($conditions as $fieldName => $value) {
			$fieldName = $adb->sql_escape_string($fieldName);	// To prevent SQL Injection
			$value = $adb->sql_escape_string($value);			// To prevent SQL Injection
			$sql .= "AND {$fieldName} = '{$value}' ";
		}

		$sql .= 'LIMIT 1';
		$matchedId = $adb->getOne($sql, []);

		if (!empty($matchedId)) {
			// Modified by Hieu Nguyen on 2021-04-06 to support caching
			$instance = self::getInstanceById($matchedId, $moduleName);
			$cache[$cacheKey] = $instance;
			return $instance;
			// End Hieu Nguyen
		}
	...
	public function isEditable() {
		...
		if (isReadonlyModule($this->getModuleName())) return false;
		// End Phu Vo

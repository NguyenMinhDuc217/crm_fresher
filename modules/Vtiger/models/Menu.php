...
class Vtiger_Menu_Model extends Vtiger_Module_Model {
	 ...
	 * @return <Array> - List of Vtiger_Menu_Model instances
	 */
	// Refactored by Hieu Nguyen on 2021-12-27 to set unsupported modules in config_override.php file
	public static function getAllForQuickCreate() {
		global $hiddenModulesForQuickCreate;
		$userPrivModel = Users_Privileges_Model::getCurrentUserPrivilegesModel();
		...
		foreach ($allModules as $moduleModel) {
			...
			if (isForbiddenFeature('Module' . $moduleName)) {
				continue;
			}

			if (in_array($moduleName, $hiddenModulesForQuickCreate)) {
				continue;
			}

			if (($userPrivModel->isAdminUser() || $userPrivModel->hasGlobalReadPermission() || $userPrivModel->hasModulePermission($moduleModel->getId())) && $moduleModel->get('parent') != '') {
				$menuModels[$moduleName] = $moduleModel;
			}
		}

...
class Vtiger_MiniList_Model extends Vtiger_Widget_Model {
	...
	public function getRecords() {
		...
		if (!$this->listviewRecords) {
			...
			// Edit by Tung Bui - 07.04.2020 - Fix bug Minilist in homepage because Vtiger replace 2 times
			$query = preg_replace('/ FROM /', ',vtiger_crmentity.crmid as id FROM ', $query, 1); 
			// End
			
            if($this->getTargetModule() == 'Calendar') {
                $query = str_replace(" WHERE ", " WHERE vtiger_crmentity.setype = 'Calendar' AND ", $query);
                $query = str_replace(" ORDER BY ", " GROUP BY vtiger_activity.activityid ORDER BY ", $query);	// Added by Hieu Nguyen on 2022-10-13 to make function MAX() in SELECT statement working
            }
			...
			foreach ($entries as $id => $record) {
                ...
                handleProcessRecords($targetModuleName, $this->listviewRecords[$id]);
                // End Hieu Nguyen
			}
		}

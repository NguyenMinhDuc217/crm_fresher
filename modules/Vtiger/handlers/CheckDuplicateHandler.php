...
class CheckDuplicateHandler extends VTEventHandler {
	...
	public function triggerCheckDuplicateHandler($entityData) {
		...
		if (
            ($GLOBALS['importing'] == true && !$skipDuplicateCheck) ||  // Check for import action
            (!$moduleModel->allowDuplicates && !$skipDuplicateCheck)    // Check for other actions
        ) {
			...
			if (count($uniqueFields) > 0) {
				...
				if ($checkDuplicates) {
					...
					if ($moduleName == 'Events') {
						...
						// Commented by Hoang Duc 28-12-2018

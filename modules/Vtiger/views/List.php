...
class Vtiger_List_View extends Vtiger_Index_View {
	...
	function process (Vtiger_Request $request) {
		...
		// Added by Phu Vo on 2020.12.16 to refresh on not permitted custom view
		CustomView::checkCustomViewListPermitted($viewName, $moduleName);
		// End Phu Vo
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
		...
		// [Core] Added by Phu Vo on 2020.06.16 to fix error on list view when current tag is removed
		...
		// Added by Phuc on 2019.12.19 to check task role
		if (!empty($tag)) {
			global $adb;
			$currentUserModel = Users_Privileges_Model::getCurrentUserModel();

			$tagRow = $adb->pquery("SELECT *, IF(visibility = 'public' OR owner = ?, 1, 0) AS allow_access FROM vtiger_freetags WHERE id = ?", [$currentUserModel->getId(), $tag]);
			$tagRow = $adb->fetchByAssoc($tagRow);

			if (empty($tagRow)) {
				throw new Exception(vtranslate('LBL_TAG_NOT_EXIST' , 'Vtiger'));
			}

			if (!$tagRow['allow_access']) {
				$params = ['%tag_name' => $tagRow['tag']];
				throw new Exception(vtranslate('LBL_CANNOT_ACCESS_TAG' , 'Vtiger', $params));
			}
		}
		// Ended by Phuc
		...
		if(!$this->listViewHeaders){
            ...
            // Added by Hieu Nguyen on 2021-01-27 to hide column starred from listview headers if it is not selected in column list
            $customView = new CustomView($this->module);
		    $customViewColumns = $customView->getColumnsListByCvid($this->viewName);
            $customViewColumnsJson = json_encode($customViewColumns);

            if (strpos($customViewColumnsJson, 'starred') === false) {
                unset($this->listViewHeaders['starred']);
            }
            // End Hieu Nguyen
		}

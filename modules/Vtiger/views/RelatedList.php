...
class Vtiger_RelatedList_View extends Vtiger_Index_View {
	function process(Vtiger_Request $request) {
        ...
        // Implemetned by Hieu Nguyen on 2020-06-12 to make this logic reuseable and easy to maintain
        $whereCondition = self::getWhereCondition($searchParams, $relatedModuleModel, $moduleFields);
        // End Hieu Nguyen
        ...
        if (empty($orderBy)) {
            $sortingConfig = Settings_LayoutEditor_Module_Model::getModuleLayoutSorting($relatedModuleName, 'RelationList');
    ...
    // Implemetned by Hieu Nguyen on 2020-06-12 to make this logic reuseable and easy to maintain
    static function getWhereCondition(&$searchParams, $relatedModuleModel, $relatedModuleFields = []) {
        if (empty($searchParams)) $searchParams = [];

        if (empty($relatedModuleFields)) {
            $relatedModuleFields = $relatedModuleModel->getFields();
        }
        ...
        foreach ($searchParams as $fieldListGroup){
            foreach ($fieldListGroup as $fieldSearchInfo) {
                ...
                $comparator = $fieldSearchInfo[1];
                $searchValue = $fieldSearchInfo[2];
                $fieldType = $fieldSearchInfo[2];
                $fieldModel = $relatedModuleFields[$fieldName];
                $tableName = $fieldModel->get('table');
                $columnName = $fieldModel->get('column');

                // Build where condition
                $whereCondition[$fieldName] = ["{$tableName}.{$columnName}", $comparator,  $searchValue, $fieldType];
                
                // Modifiy search params array

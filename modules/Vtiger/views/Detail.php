...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		//-- Added by Kelvin Thang on 2018-12-13 to get custom status on detail header
        if (file_exists('modules/' . $moduleName . '/custom/Navigation.php')) {
            $navigationData = Array();
            require_once('modules/' . $moduleName . '/custom/Navigation.php');
            $viewer->assign('NAVIGATION_DATA', $navigationData);
        }
        //-- End Kelvin Thang

		// Added by Hieu Nguyen on 2018-07-16 to load custom code
		if(file_exists('modules/'. $moduleName .'/custom/DetailView.php')) {
			require('modules/'. $moduleName .'/custom/DetailView.php');
	...
	function process(Vtiger_Request $request) {
		...
		// Added by Phuc on 2020.06.29 to hide summary tab if module not supported
		$moduleName = $request->getModule();
		$classModuleName = "{$moduleName}_Module_Model";
		$isSummaryViewSupported = false;

		if (class_exists($classModuleName)) {
			$moduleModel = new $classModuleName();
		}
		else {
		...
		if ($currentUserModel->get('default_record_view') === 'Summary' && $isSummaryViewSupported) {
	...
	function showModuleDetailView(Vtiger_Request $request) {
        ...
        // Added by Hieu Nguyen on 2019-08-21 to load custom code for overlay detail view
		if(file_exists('modules/'. $moduleName .'/custom/DetailView.php')) {
			require('modules/'. $moduleName .'/custom/DetailView.php');
            $viewer->assign('DISPLAY_PARAMS', $displayParams);
		}
	...
	function showModuleBasicView($request) {
		...
		// Added by Phu Vo on 2019-07-23 to load custom code in case request ajax detail view (not go to preprocess)
		if(file_exists('modules/'. $moduleName .'/custom/DetailView.php')) {
			require('modules/'. $moduleName .'/custom/DetailView.php');
            $viewer->assign('DISPLAY_PARAMS', $displayParams);
		}
	...
	function showRecentComments(Vtiger_Request $request) {
		...
		$viewer->assign('MODULE_RECORD', $parentId);    // Modified by Hieu Nguyen on 2021-06-30 to fix bug record not found when toggle rollup comments at SummaryView
	...
	function showRelatedList(Vtiger_Request $request) {
		require_once('modules/Calendar/CalendarCommon.php');	// Added by Hieu Nguyen on 2022-06-16 to temporarily fix issue RelatedListCustomRowActions.tpl break the subpanel rendering
		$moduleName = $request->getModule();
	...
	public function getHeaderCss(Vtiger_Request $request) {
		...
		$cssFileNames = array(
			'~/libraries/jquery/bootstrapswitch/css/bootstrap3/bootstrap-switch.min.css',	// Modified by Vu Mai on 2022-09-06 to change version of bootstrap switch in Summary View
		);

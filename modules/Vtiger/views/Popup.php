...
class Vtiger_Popup_View extends Vtiger_Footer_View {
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			// Added by Tin Bui on 2021.09.17 to pass label to list view model
        ...
        if (empty($orderBy)) {
            $sortingConfig = Settings_LayoutEditor_Module_Model::getModuleLayoutSorting($moduleName, 'Popup');
            
            if (!empty($sortingConfig)) {
                $orderBy = $sortingConfig['sort_field'];    // Popup uses field name for sorting
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			...
			foreach ($models as $recordId => $recordModel) {
                ...
                handleProcessRecords($moduleName, $models[$recordId]);
                // End Hieu Nguyen
			}
        ...
        // Commented out by Hieu Nguyen on 2020-04-27 a this logic is not reasonable

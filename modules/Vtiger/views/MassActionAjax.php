...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
	function __construct() {
		...
		$this->exposeMethod('showSendSMSOTTModal');	// Refactored function name by Hieu Nguyen on 2022-06-22
	 ...
	 * @param Vtiger_Request $request
	 */
	function showSendSMSOTTModal(Vtiger_Request $request) {	// Refactored function name by Hieu Nguyen on 2022-06-22
		...
		// Modified by Hieu Nguyen on 2022-06-22 to speprate logic for SMS and OTT
		...
		if ($channel == 'SMS') {
			...
			if (!$activeGateway) {
				...
				$viewer->assign('BODY', vtranslate('LBL_NO_ACTIVE_GATEWAY_ERROR_MSG', $moduleName));	// Modified by Phu Vo on 2019.07.29 to change alert message
			...
			if (!$activeGateway) {
				$viewer->assign('TITLE', vtranslate('LBL_SEND_SMS', $moduleName));
				$viewer->assign('BODY', vtranslate('LBL_NO_ACTIVE_GATEWAY_ERROR_MSG', 'CPOTTIntegration'));
				echo $viewer->view('NotAccessible.tpl', $moduleName, true);
				exit;
			}
		}
		...
		// Added by Phu Vo on 2018.12.21
		$smsTemplatesCache = SMSNotifier_Record_Model::getSMSTemplateList();
		$smsTemplates = array('' => '');
		foreach($smsTemplatesCache as $template) {
			$smsTemplates[$template['description']] = $template['name'];
		}
		// End Phu vo
		...
		$viewer->assign('SMS_TEMPLATES', $smsTemplates); // Added by Phu Vo on 2018.12.21
        ...
        // Added by Hieu Nguyen on 2020-10-19 to allow inserting variable
        $viewer->assign('VARIABLE_OPTIONS', generateTemplateVariableOptions($sourceModule));
        // End Hieu Nguyen
        
		// Added by Hieu Nguyen on 2021-11-16 to provide provider info in the send SMS popup
		if ($channel == 'SMS') {
			$viewer->assign('PROVIDER_INFO', $activeGateway->getInfo());
		}
		// End Hieu Nguyen

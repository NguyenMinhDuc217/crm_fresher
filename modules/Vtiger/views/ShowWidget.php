		...
		if(!empty($componentName)) {
			...
			if(!empty($className)) {
				...
				if(!empty($linkId)) {
                    ...
                    // Added by Hieu Nguyen on 2020-10-13 to bypass saving new widget
					$widget->set('name_en', '-');
					$widget->set('name_vn', '-');
                    // End Hieu Nguyen
				...
				if (!empty($request->get('reportid'))) {
					...
					Home_DashboardLogic_Helper::addReportWidget($reportInfo, $request->get('tab', $defaultTab['id']));
				}
				// End Phu Vo

...
class Vtiger_Import_View extends Vtiger_Index_View {
	...
	function __construct() {
		...
		$this->exposeMethod('finishImport');    // Added by Hieu Nguyen on 2020-09-10 to mark import process finished
	}
	...
	function importBasicStep(Vtiger_Request $request) {
        global $current_language, $upload_dir;
		...
		if (!$fileFormat || !in_array($fileFormat, $supportedFileTypes)) {
			$fileFormat = 'xls,xlsx';   // Modified by Hieu Nguyen on 2021-05-11 to support both XLS and XLSX files
		} else {
			$fileFormat = strtolower($fileFormat);
		}

		// Modified by Hieu Nguyen on 2021-09-07 to display link download template file
        $templateFileName = 'Import_'. $moduleName .'_'. $current_language .'.xlsx';
        $templateFile = $upload_dir. 'templates/'. $templateFileName;

        if (file_exists($templateFile)) {
			$viewer->assign('TEMPLATE_FILE', $templateFile);
        }
		// End Hieu Nguyen

		$viewer->assign('AVAILABLE_FIELDS', $moduleMeta->getMergableFields());
		$viewer->assign('DUPLICATE_CHECK_FIELDS', $moduleMeta->getDefaultFieldsForDuplicateCheck());    // Refactored variable name and function name by Hieu Nguyen on 2021-05-11
	...
	function initializeMappingParameters(Vtiger_Request $request) {
		...
		if ($request->get('type') == 'xlsx' || $request->get('type') =='xls') {
			self::convertExcelToCSV($request);	// Modified by Hieu Nguyen on 2022-04-01 to call function in static style
		}
	 ...
	 * Desc: Convert xlsx/xls to csv
	 */
	static function convertExcelToCSV(Vtiger_Request $request) {	// Convert to static function by Hieu Nguyen on 2022-04-01 so that it can be called anywhere
		$current_user = Users_Record_Model::getCurrentUserModel();
		$temporaryFileName = Import_Utils_Helper::getImportFilePath($current_user);

		require_once("libraries/PHPExcel/PHPExcel.php");
		$objPHPExcel = new PHPExcel();
		$inputFileType = PHPExcel_IOFactory::identify($_FILES['import_file']['tmp_name']);
		$objReader = PHPExcel_IOFactory::createReader($inputFileType);
		$objReader->setReadDataOnly(true);
		$objPHPExcel = $objReader->load($_FILES['import_file']['tmp_name']);
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'CSV');
		$objWriter->setUseBOM(true);
		$objWriter->save($temporaryFileName);
	...
	function undoImport(Vtiger_Request $request) {
		...
		// Added by Hieu Nguyen on 2021-08-24 to mark this import as finished to clear temp tables
		$this->finishImport($request);
		// End Hieu Nguyen
	...
	function cancelImport(Vtiger_Request $request) {
		...
		if($importInfo != null) {
			if($importInfo['user_id'] == $user->id || $user->isAdminUser()) {
                ...
                $importDataController->cancelImport();
                // End Hieu Nguyen
    ...
    // Implemented by Hieu Nguyen on 2020-09-10
    function finishImport(Vtiger_Request $request) {
        $user = Users_Record_Model::getCurrentUserModel();
        $importId = $request->get('import_id');
        $importInfo = Import_Queue_Action::getImportInfoById($importId);
	...
	function checkImportStatus(Vtiger_Request $request) {
        ...
        // Modified by Hieu Nguyen on 2020-09-10 to display import result after the import process is cancelled
        if (Import_Utils_Helper::isUserImportCancelled($user)) {
            $importInfo = Import_Queue_Action::getUserCurrentImportInfo($user);

...
class Vtiger_Util_Helper {
	...
	public static function toVtiger6SafeHTML($input) {
		$allowableTags = '<a><br><img><ul><li><div><h5><span>';  // Modified by Phuc Lu on 2019.10.01 to allow more safe html tag
...
/**
	...
	public static function convertDateTimeIntoUsersDisplayFormat($dateTime, $userObject = null) {
		...
		// Added by Hieu Nguyen on 2021-10-04 to override date time format for Zalo message (Required format is HH:mm dd/MM/yyyy or dd/MM/yyyy)
		if ($GLOBALS['sending_zns']) {
		...
		// Modified by Hieu Nguyen on 2021-10-06 to return date only if the input value does not contains time
		if (strlen($dateTime) > 10) {
			return $date .' '. $time;
		}
		else {
	...
	public static function convertTimeIntoUsersDisplayFormat($time, $userObject = null) {
		...
		// Added by Hieu Nguyen on 2021-04-10 to override time format for Zalo message
		if ($GLOBALS['sending_zns']) {
			return $time;	// Required format for Zalo is HH:mm 
		}
		// End Hieu Nguyen
   ...
   function getDefaultMandatoryValue($dataType) {
	   ...
	   switch ($dataType) {
		   ...
		   case 'boolean':
			   ...
			   break;
		   case 'email':
			   $value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
			   break;
		   case 'url':
			   $value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
			   break;
   ...
   public static function checkDbUTF8Support($conn) {
	 ...
	 */
	// Renamed function name by Hieu Nguyen on 2021-09-24 to avoid confusion
	public static function escapeSqlStringForLIKEQuery($string) {
	...
	static function fillMandatoryFields($fieldName, $module, $source = '') {
		...
		switch ($fieldDataType) {
			...
			case 'time'			:	$value = '00:00:00';
									...
									break;
			case 'text'			:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			case 'phone'		:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			case 'boolean'		:	$value = false;
									...
									break;
			case 'email'		:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			case 'string'		:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			case 'url'			:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			...
			case 'currency'		:	$value = 0.00;
									...
									break;
			case 'skype'		:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									if (!empty($defaultValue)) {
										$value = $defaultValue;
									}
									break;
			...
			case 'reference'	:	$referenceFieldModule = $fieldModel->getReferenceList(true);
									...
									break;
			default				:	$value = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
									break;
		}
		return $value;
	}

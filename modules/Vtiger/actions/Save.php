...
class Vtiger_Save_Action extends Vtiger_Action_Controller {
    ...
    // Added by Hieu Nguyen on 2019-02-22
    function __construct() {
		parent::__construct();

        $GLOBALS['files'] = $_FILES;    // Prevent file lost causes by line 116 in CRMEntity.php
	}
    // End Hieu Nguyen
	...
	protected function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			...
			if($fieldValue !== null) {
                 ...
                 * Date: 2018-07-13
                 */
                if(!is_array($fieldValue) && in_array($fieldDataType,['currency', 'integer', 'double', 'number', 'boolean'])){
                    $fieldValue = Vtiger_Currency_UIType::convertToDBFormat($fieldValue, null, true);
                }else if(!is_array($fieldValue) && $fieldDataType != 'currency') {

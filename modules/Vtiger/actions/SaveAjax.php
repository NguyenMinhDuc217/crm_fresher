	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				// Added by Hieu Nguyen to return phone field's rendered value
                if ($fieldModel->getFieldDataType() == 'phone') {
					$displayValue = $fieldModel->getUITypeModel()->renderDetailViewValue($recordModel);
				}
                // End Hieu Nguyen
			...
			// Commented out these lines by Hieu Nguyen on 2021-05-20 as it't not needed anymore
	...
	public function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		if(!empty($recordId)) {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				$recordModel->set($fieldName, $fieldValue);
				if ($fieldName === 'contact_id' && $fieldValue != 0 && isRecordExists($fieldValue)) { // Modified by Phu Vo on 2020.11.19 to work around issue crmid 0

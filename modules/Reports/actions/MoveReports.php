...
class Reports_MoveReports_Action extends Vtiger_Mass_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		if (!empty ($reportIdsList)) {
			foreach ($reportIdsList as $reportId) {
				...
				if (!$reportModel->isDefault() && $reportModel->isEditable() && $reportModel->isEditableBySharing()) {
					...
					$reportsMoveDenied[] = $reportModel->getName(); // Updated by Phuc on 2019.10.25 to get name only
				}
			}
		}
		...
		if($sameTargetFolder){
                    ...
                    $result = array('success'=>false, 'message'=>vtranslate('LBL_DENIED_MOVING_REPORTS_BY_ROLE_MSG', $parentModule),'denied'=>$reportsMoveDenied); // Update label by Phuc on 2019.10.25
		}

{*
    SalesAndRevenueByEmployeeReportChart.tpl
    Author: Phuc Lu
    Date: 2020.04.28
*}
...
{strip}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/SalesAndRevenueByEmployeeReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new SalesAndRevenueByEmployeeReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new SalesAndRevenueByEmployeeReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

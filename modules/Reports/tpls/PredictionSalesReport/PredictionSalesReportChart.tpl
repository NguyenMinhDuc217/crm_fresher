{*
    PredictionSalesReportChart.tpl
    Author: Phuc Lu
    Date: 2020.05.20
*}
...
{strip}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/PredictionSalesReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new PredictionSalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new PredictionSalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

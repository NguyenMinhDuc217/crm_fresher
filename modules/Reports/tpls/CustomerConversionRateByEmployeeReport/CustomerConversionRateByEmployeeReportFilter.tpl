{*
    CustomerConversionRateByEmployeeReportFilter.tpl
    Author: Phu Vo
    Date: 2020.08.19
*}
...
{strip}
    ...
    <form id="form-filter" name="filter" action="" method="GET" class="filter-container recordEditView">
        ...
        {if $FILTER_META.report_object == 'DEPARTMENT' || $FILTER_META.report_object == 'EMPLOYEE'}
        ...
        {if $FILTER_META.report_object == 'SOURCE'}
            <div class="filter-group">
                ...
                <div class="control-label col-sm-8">
                    <select name="sources[]" multiple id="sources"  data-rule-required="true" class="filter dislayed-filter select2 width-340">
                        {html_options options=$FILTER_META.sources selected=$PARAMS.sources}
                    </select>
                </div>
            </div>
        {/if}

{*
    CustomerConversionRateByEmployeeReportChart.tpl
    Author: Phuc Lu
    Date: 2020.05.12
*}
...
{strip}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/CustomerConversionRateByEmployeeReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new CustomerConversionRateByEmployeeReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new CustomerConversionRateByEmployeeReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

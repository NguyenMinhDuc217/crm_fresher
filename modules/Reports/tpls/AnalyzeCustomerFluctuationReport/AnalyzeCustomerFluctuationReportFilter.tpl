...
{strip}
    <form id="form-filter" name="filter" action="" method="GET" class="filter-container recordEditView">
        ...
        <div class="filter-group">
            <div class="control-label fieldLabel col-sm-4">
                {vtranslate('LBL_REPORT_CUSTOMER_TYPE', 'Reports')}:
            </div>
            <div class="control-label col-sm-8">
                <select name="customer_type" id="customer_type" class="filter dislayed-filter width-340">
                    <option value="Account" {if isset($PARAMS['customer_type']) && $PARAMS['customer_type'] == 'Account'}selected{/if}>{vtranslate('LBL_REPORT_CUSTOMER_COMPANY', 'Reports')}</option>
                    <option value="Contact" {if isset($PARAMS['customer_type']) && $PARAMS['customer_type'] == 'Contact'}selected{/if}>{vtranslate('LBL_REPORT_CUSTOMER_CONTACT', 'Reports')}</option>
                    <option value="All" {if isset($PARAMS['customer_type']) && $PARAMS['customer_type'] == 'All'}selected{/if}>{vtranslate('LBL_ALL', 'Reports')}</option>
                </select> 
            </div>
        </div>

...
{strip}
    ...
    <form id="form-filter" name="filter" action="" method="GET" class="filter-container recordEditView">
        ...
        <div class="filter-group">
            ...
            <div class="control-label col-sm-8">
                <select name="report_module" data-rule-required="true" id="report_module" class="filter dislayed-filter select2 width-340">
                    {html_options options=$FILTER_META.report_modules selected=$PARAMS.report_module}
                </select>
            </div>
        </div>

        <div class="filter-group {if !isset($PARAMS.report_module) || $PARAMS.report_module != "Accounts"}hide{/if}">
            <div class="control-label fieldLabel col-sm-4">
            </div>
            <div class="control-label col-sm-8">
                <input type="checkbox" style="margin-left: 10px;" name="chx_is_real_customer" id="chx_is_real_customer" {if $PARAMS.is_real_customer == 1}checked="true"{/if}/>
                <input type="hidden" name="is_real_customer" id="is_real_customer" value="{$PARAMS.is_real_customer}"/>
                &nbsp;{vtranslate('LBL_REPORT_IS_REAL_CUSTOMER', 'Reports')}
            </div>
        </div>

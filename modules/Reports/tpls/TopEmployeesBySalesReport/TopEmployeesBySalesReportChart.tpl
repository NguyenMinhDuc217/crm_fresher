{*
    TopEmployeesBySalesReportChart.tpl
    Author: Phuc Lu
    Date: 2020.04.20
*}
...
{strip}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/TopEmployeesBySalesReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new TopEmployeesBySalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new TopEmployeesBySalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

...
{*
    StatisticCustomerBuyProductsReport.tpl
    Author: Phuc Lu
    Date: 2020.06.16
*}
...
{strip}
                ...
                <table celspacing=0 celpading=0>
                    ...
                    <tbody>
                        {foreach from=$REPORT_DATA item=ROW key=NO}
                            <tr>
                            {foreach from=$ROW item=DATA key=COLUMN}
                                {if $COLUMN != 'record_id'}
                                    ...
                                    {else}
                                        {if $COLUMN == 'email'}
                                            <td class="text-center">
                                                <a class="emailField" data-rawvalue="{$DATA}" onclick="Vtiger_Helper_Js.getInternalMailer({$ROW.record_id},'email','{$PARAMS.target}s');">{$DATA}</a>
                                            </td>
                                        {else}
                                            {if $COLUMN == 'salesorders'}
                                                <td>
                                                    {foreach from=$DATA item=SONO key=SOID name=SOI}
                                                        <a target="_blank" onclick="window.open('index.php?module=SalesOrder&view=Detail&record={$SOID}')">{$SONO}</a>{if !$smarty.foreach.SOI.last},&nbsp;{/if}
                                                    {/foreach}
                                                </td>
                                            {else}
                                                <td class="text-center">
    ...
    <div id="report_modal" title="{vtranslate('LBL_REPORT_SALES_ORDER', 'Reports')}" class="modal-dialog modal-md modal-content modal-template-md hide">
        {include file='ModalHeader.tpl'|vtemplate_path:'Vtiger' TITLE=vtranslate('LBL_REPORT_SALES_ORDER', 'Reports')}
        <div class="salesorder_list"></div>
    </div>
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/StatisticCustomerBuyProductsReportDetail.js")}"></script>
{/strip}
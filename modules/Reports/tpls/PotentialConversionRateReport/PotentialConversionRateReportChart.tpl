{*
    PotentialConversionRateReportChart.tpl
    Author: Phuc Lu
    Date: 2020.08.18
*}
...
{strip}
    ...
    <script src="{vresource_url('resources/libraries/HighCharts_8.1.0/code/modules/funnel.js')}"></script>
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/PotentialConversionRateReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new PotentialConversionRateReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new PotentialConversionRateReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

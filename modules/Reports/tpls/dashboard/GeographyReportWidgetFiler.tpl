{*
    GeographyReportWidgetFiler.tpl
    Author: Phuc Lu
    Date: 2020.07.01
*}
...
{strip}
    ...
    <div class="form-group">
        ...
        <div class="controls fieldValue col-sm-7">
            {assign var="REPORT_MODULES" value=Reports_CustomReport_Helper::getGeographyReportModules()}
            <select name="filter[report_module]"  id="report_module" class="dateField form-control widgetFilter reloadOnChange">
                {html_options options=$REPORT_MODULES selected=$PARAMS.report_module}
            </select>

            <input type="hidden" name="filter[is_real_customer]" value="{$PARAMS.is_real_customer}"> 
        </div>

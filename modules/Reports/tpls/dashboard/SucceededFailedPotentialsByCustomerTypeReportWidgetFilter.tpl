...
{strip}
    ...
    <div class="form-group">
        ...
        <label class="control-label fieldLabel col-sm-5">
            <strong> {vtranslate('LBL_REPORT_CHOOSE_CUSTOMER_TYPE', 'Reports')}:</strong>
        </label>
        <div class="controls fieldValue col-sm-7">    
            {assign var="CUSTOMER_TYPES" value=Reports_CustomReport_Helper::getPotentialCustomerTypeValues(true, false, false)}
            <select name="filter[customer_type]" id="customer_type"  data-rule-required="true" class="inputElement select2 select2-offscreen form-control widgetFilter reloadOnChange">
                {html_options options=$CUSTOMER_TYPES selected=$PARAMS.customer_type}
            </select>
        </div>
        <div style="clear: both;"></div>
    </div>

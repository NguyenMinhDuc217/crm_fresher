...
{strip}
    ...
    <div class="form-group">
        ...
        <label class="control-label fieldLabel col-sm-5">
            <strong>{vtranslate('LBL_REPORT_SOURCE', 'Reports')}:</strong>
        </label>
        <div class="controls fieldValue col-sm-7">    
            {assign var="SOURCES" value=Reports_CustomReport_Helper::getSourceValues(false, true, true)}
            <select name="filter[sources][]" multiple id="sources"  data-rule-required="true" class="inputElement select2 select2-offscreen form-control widgetFilter reloadOnChange">
                {html_options options=$SOURCES selected=$PARAMS.sources}
            </select>
        </div>
        <div style="clear: both;"></div>

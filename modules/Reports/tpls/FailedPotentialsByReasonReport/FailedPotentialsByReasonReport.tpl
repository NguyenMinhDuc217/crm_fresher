...
{strip}
                    ...
                    <tbody>
                        {foreach from=$REPORT_DATA item=ROW key=NO name=INDEX}
                            <tr {if $smarty.foreach.INDEX.last} style="font-weight: bold !important;"{/if}>
                                ...
                                {else}
                                    <td class="text-center" colspan="2">{$ROW.reason}</td>
                                {/if}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/FailedPotentialsByReasonReportDetail.js")}"></script>
{/strip}
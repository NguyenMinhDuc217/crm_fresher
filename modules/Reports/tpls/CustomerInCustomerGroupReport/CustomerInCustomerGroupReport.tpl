...
{strip}
                ...
                <table celspacing=0 celpading=0>
                    ...
                    <tbody>
                        {foreach from=$REPORT_DATA item=ROW key=NO name=INDEX}
                            <tr>
                                {if $ROW|count == 1}
                                    <td colspan="{$REPORT_HEADERS|count}" style="background: #f5f5f5; font-weight: bold; font-size: 16px;">
                                       {$ROW.group}
                                    </td>
                                {else}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/CustomerInCustomerGroupReportDetail.js")}"></script>
{/strip}
{*
    CustomerInCustomerGroupReportFilter.tpl
    Author: Phu Vo
    Date: 2020.08.19
*}
        ...
        <div class="filter-group">
            ...
            <div class="control-label col-sm-8">
                <select name="target" id="target" class="filter dislayed-filter select2 width-340">
                    <option value="Account" {if $PARAMS.target == 'Account'}selected{/if}>{vtranslate('Accounts')}</option>
                    <option value="Contact" {if $PARAMS.target == 'Contact'}selected{/if}>{vtranslate('Contacts')}</option>
                </select>
            </div>
        </div>

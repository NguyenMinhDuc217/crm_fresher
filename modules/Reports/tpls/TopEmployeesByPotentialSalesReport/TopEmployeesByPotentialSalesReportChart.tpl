{*
    TopEmployeesByPotentialSalesReportChart.tpl
    Author: Phuc Lu
    Date: 2020.08.11
*}
...
{strip}
    ...
    <script type="text/javascript" src="{vresource_url("modules/Reports/resources/TopEmployeesByPotentialSalesReportChart.js")}"></script>
    <script type="text/javascript">
        {if $DATA != false}
            if (typeof google.visualization == 'undefined') {
                google.charts.load('current', { 'packages' : ['corechart'], 'language' : 'vi' });
                google.charts.setOnLoadCallback(function () {
                    new TopEmployeesByPotentialSalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)})
                });
            }
            else {
                new TopEmployeesByPotentialSalesReportChart('{$PLACE_HOLDER_ID}', {Zend_Json::encode($DATA)});

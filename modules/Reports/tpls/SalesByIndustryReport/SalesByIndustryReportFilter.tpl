{*
    SalesByIndustryReportFilter.tpl
    Author: Phu Vo
    Date: 2020.08.19
*}
...
{strip}
    ...
    <form id="form-filter" name="filter" action="" method="GET" class="filter-container recordEditView">
        ...
        {if $FILTER_META.report_object == 'PROVINCE_BY_INDUSTRIES'} 
            <div class="filter-group">
                <div class="control-label fieldLabel col-sm-4">
                    {vtranslate('LBL_REPORT_CHOOSE_PROVINCE', 'Reports')}:
                </div>
                <div class="control-label col-sm-8">
                    {assign var="PROVINCES" value=Reports_CustomReport_Helper::getAllProvinceValues(true, false, true, ['Accounts', 'Contacts'])}
        ...
        {if $FILTER_META.report_object == 'INDUSTRY' || $FILTER_META.report_object == 'PROVINCE_BY_INDUSTRIES'}
            <div class="filter-group">
                <div class="control-label fieldLabel col-sm-4">
                    {vtranslate('LBL_REPORT_CHOOSE_INDUSTRY', 'Reports')}:
                </div>
                <div class="control-label col-sm-8">
                    <select name="industries[]" multiple id="industries"  data-rule-required="true" class="filter dislayed-filter select2 width-340">
                        {html_options options=$FILTER_META.industries selected=$PARAMS.industries}
                    </select>
                </div>
            </div>
        {/if}

        {if $FILTER_META.report_object == 'INDUSTRY_BY_PROVINCES'} 
            <div class="filter-group">
                <div class="control-label fieldLabel col-sm-4">
                    {vtranslate('LBL_REPORT_CHOOSE_INDUSTRY', 'Reports')}:
                </div>
                <div class="control-label col-sm-8">
                    {assign var="INDUSTRIES" value=Reports_CustomReport_Helper::getIndustryValues(true, false, true)}
                    <select name="industry" id="industry" data-rule-required="true" class="filter dislayed-filter select2 width-340">
                        {html_options options=$INDUSTRIES selected=$PARAMS.industry}
                    </select>
                </div>
            </div>
            ...
            <div class="filter-group">
                ...
                <div class="control-label col-sm-8">
                    {assign var="PROVINCES" value=Reports_CustomReport_Helper::getProvinceValues(false, true, true)}
                    <select name="provinces[]" id="provinces" multiple data-rule-required="true" class="filter dislayed-filter select2 width-340">
                        {html_options options=$PROVINCES selected=$PARAMS.provinces}
                    </select>
                </div>
            </div>
        {/if}
        
        {if $FILTER_META.report_object == 'SOURCE_BY_PROVINCES'} 
            <div class="filter-group">
                <div class="control-label fieldLabel col-sm-4">
                    {vtranslate('LBL_REPORT_CHOOSE_SOURCE', 'Reports')}:
                </div>
                <div class="control-label col-sm-8">
                    {assign var="SOURCES" value=Reports_CustomReport_Helper::getSourceValues(true, false, true)}
                    <select name="source" id="source" data-rule-required="true" class="filter dislayed-filter select2 width-340">
                        {html_options options=$SOURCES selected=$PARAMS.source}
                    </select>
                </div>
            </div>

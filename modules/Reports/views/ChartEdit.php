...
Class Reports_ChartEdit_View extends Vtiger_Edit_View {
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		if ($record) {
			...
			// Modified by Hieu Nguyen on 2021-08-20 to fix bug can not edit shared report
		...
		// Added by Hieu Nguyen on 2021-08-20 to check if this feature can by accessed
		if (empty($record)) {
			checkAccessForbiddenFeature('CreateDynamicReports');
		}
		// End Hieu Nguyen
	}
	...
	function step1(Vtiger_Request $request) {
        ...
        // Modified by Hieu Nguyen on 2019-06-19 to display saved members
		$sharedMembers = $reportModel->getMembers();
		$viewer->assign('SELECTED_MEMBERS_GROUP', $sharedMembers);
		//$viewer->assign('MEMBER_GROUPS',  Settings_Groups_Member_Model::getAll());    // Comment out this line to boost performance
        // End Hieu Nguyen
	...
	function step3(Vtiger_request $request) {
		...
		$data = $request->getAll();
		foreach ($data as $name => $value) {
            // Modified by Hieu Nguyen on 2019-06-19 to save members from select2 autocomplete
			if($name == 'schdayoftheweek' || $name == 'schdayofthemonth' || $name == 'schannualdates' || $name == 'recipients') {

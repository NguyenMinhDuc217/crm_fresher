...
class Reports_ExportReport_View extends Vtiger_View_Controller {
	...
	function GetPrintReport(Vtiger_Request $request) {
		...
		// Modified by Hieu Nguyen on 2018-12-09 to support custom report
		$customHandlerFile = $reportModel->get('custom_handler_file');

		if(!empty($customHandlerFile) && file_exists($customHandlerFile)) {
			$viewer->view('CustomPrintReport.tpl', $moduleName);
		}
		else {

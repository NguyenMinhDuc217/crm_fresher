...
class Reports_List_View extends Vtiger_Index_View {
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		// Added by Hieu Nguyen on 2022-06-06 to check access to invalid report folder
		$folderId = $request->get('viewname');

		if (!empty($folderId) && !in_array($folderId, ['All', 'shared'])) {
			$reportFolderInstance = Reports_Folder_Model::getInstanceById($folderId);

			if (empty($reportFolderInstance->getId())) {
				throw new AppException(vtranslate('LBL_FOLDER_NOT_EXIST_ERROR_MSG'));
			}
		}
		// End Hieu Nguyen
	}
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		// Modified by Phu Vo on 2019.09.27 to fix missing smarty variable FOLDER when call list ajax
	...
	function process(Vtiger_Request $request) {
		...
		if (empty($this->folders)) {
			$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
			$this->folders = $moduleModel->getFolders();
			$viewer->assign('FOLDERS', $this->folders);
		}
		// End Phu Vo

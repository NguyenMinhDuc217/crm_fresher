...
function getReportFieldValue ($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false) {
	...
	// Added by Hieu Nguyen on 2021-12-22 to get correct field type
	if (!empty($fieldInfo)) {
		$moduleModel = Vtiger_Module_Model::getCleanInstance($module);
		$fieldModel = new Vtiger_Field_Model();
		$fieldModel->initialize($fieldInfo);
		$fieldModel->setModule($moduleModel);
		$fieldType = $fieldModel->getFieldInfo()['type'];
	}
	// End Hieu Nguyen
	...
	if(is_object($field) &&	$field->getUIType() == 401){
        ...
        // Modified by Hieu Nguyen on 2021-06-11 to translate values for multipicklist field
		...
		$fieldvalue = join(', ', $translatedValueList);
        // End Hieu Nguyen
	...
	// Added by Hieu Nguyen on 2020-12-03 to translate salutation
    if ($fieldInfo['fieldname'] === 'salutationtype') {
		$fieldvalue = vtranslate($fieldvalue);
	}
    // End Hieu Nguyen

	// Added by Phu Vo on 2019.06.22 to render custom owner field html
	if ($fieldType === 'owner') {
		if ($_REQUEST['view'] === 'ExportReport' || $operation == 'ExcelExport') {
			$fieldvalue = Vtiger_Owner_UIType::getCurrentOwnersForDisplay($fieldvalue, true, false);
		}
		else {
			$fieldvalue = Vtiger_Owner_UIType::getCurrentOwnersForDisplay($fieldvalue, true, true);
		}
		
		// Return without escape
		return ['value' => $fieldvalue, 'type' => $fieldType];	// Modified by Hieu Nguyen on 2022-01-06 to return both field value and field type
	}
	// End render custom owner field html
	...
	// Modified by Hieu Nguyen on 2021-12-22 to return both field value and field type
	/*if($operation == 'ExcelExport') {
	...
	$value = htmlentities($fieldvalue, ENT_QUOTES, $default_charset);
	return ['value' => $value, 'type' => $fieldType];
	// End Hieu Nguyen
}
		...
		foreach ($searchParams as $i => $condition) {
			...
			if ($fieldName == 'reportname' || $fieldName == 'description') {
				...
				foreach ($searchValue as $value) {
                    // Added by Hieu Nguyen on 2019-06-19 to filter owner from listview
                    if ($fieldName == 'owner') {
...
// Added by Hieu Nguyen on 2021-12-07 to check if a field is of picklist type
function isPicklistField($tableName, $columnName) {
	global $adb;
	$sql = "SELECT uitype FROM vtiger_field WHERE tablename = ? AND columnname = ?";
	$uiType = $adb->getOne($sql, [$tableName, $columnName]);
	
	if (in_array($uiType, [15, 16, 33])) {
		return true;
	}
...
// Added by Hieu Nguyen on 2022-01-20 to add extra select fields into a query
function addExtraSelectFields($query, $extraSelectFields, $isLowerCase = true) {
	$fromKeyword = ($isLowerCase) ? 'from' : 'FROM';
	$queryParts = explode($fromKeyword, $query);
	$queryParts[0] .= ", {$extraSelectFields} ";
	$newQuery = join($fromKeyword, $queryParts);
	return $newQuery;
}

// Added by Hieu Nguyen on 2021-12-07 to add extra join query into another query
function addExtraJoinQuery($query, $extraJoinQuery, $isLowerCase = true) {
	$whereKeyword = ($isLowerCase) ? 'where' : 'WHERE';
	$queryParts = explode($whereKeyword, $query);
	$queryParts[0] .= " {$extraJoinQuery} ";
	$newQuery = join($whereKeyword, $queryParts);
	return $newQuery;
}

// Added by Tung Nguyen on 2022.03.31 to get field name by column name
function getFieldName($columnName, $tableName) {
	$fieldName = $GLOBALS['adb']->getOne("SELECT fieldname FROM vtiger_field WHERE columnname = ? AND tablename = ?", [$columnName, $tableName]);

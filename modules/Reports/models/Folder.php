...
class Reports_Folder_Model extends Vtiger_Base_Model {
	...
	function getReports($pagingModel) {
		...
		if($fldrId == 'All' || $fldrId == 'shared') {
			...
			for($i=0; $i < count($reportsList); $i++) {
				...
				$reportModel->setRawData($reportsList[$i]);
				handleProcessRecords($reportModel->getModuleName(), $reportModel);
				// End Hieu Nguyen
	...
	public static function getAll() {
		...
		if (!$folders) {
			...
			// Modified by Hieu Nguyen on 2021-09-22 to hide unsupported report folder
			$unsupportedFolders = getForbiddenReportFolders();
			$sql = "SELECT * FROM vtiger_reportfolder WHERE 1 = 1 ";

			if (!empty($unsupportedFolders)) {
				$sql .= "AND code NOT IN ('". join("', '", $unsupportedFolders) ."') ";
			}
	...
	public function getAllReportModels($allReportsList, $reportModuleModel){
		...
		foreach ($allReportsList as $key => $reportsList) {
			...
			// Added by Hieu Nguyen on 2019-07-18 to support process_record event handler
			$reportModel->setRawData($reportsList);
			handleProcessRecords($reportModel->getModuleName(), $reportModel);
			// End Hieu Nguyen

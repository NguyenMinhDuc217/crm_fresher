...
abstract class Base_Chart extends Vtiger_Base_Model{
	...
	function getTranslatedLabelFromReportLabel($column) {
		...
		$label = implode(' ', array_slice($columnLabelInfo, 1));
		$label = str_replace("'", '', $label);
		$translatedLabel = vtranslate($label, $columnLabelInfo[0]);

		if (strcmp($label, $translatedLabel) == 0) {
	...
	function getQuery() {
		...
		if(is_array($groupByColumnsByFieldModel)) {
			foreach($groupByColumnsByFieldModel as $groupField) {
				...
				// Modified by Hieu Nguyen on 2022-12-27 to fix bug Chart report show no data when the group column is Task Status
				$columnAlias = "`". $groupField->get('reportlabel') ."`";	// To escape special characters
				$groupByColumns[] = $columnAlias;
				$column = $groupField->get('reportcolumn');

				if (strpos($column, $columnAlias) === false) {
					$column = explode('AS', $column)[0] . 'AS ' . $columnAlias;
				}
		...
		$sql = split(' from ', $this->reportRun->sGetSQLforReport($reportModel->getId(), $advFilterSql, 'PDF', true), 2);
		...
		// Modified by Hieu Nguyen on 2021-12-07 to sort picklist field by its picklist value order
		if ($groupByColumns && is_array($groupByColumns)) {
			...
			foreach ($groupByColumnsByFieldModel as $i => $groupField) {
				if (in_array($groupField->get('uitype'), [15, 16, 33]) && !in_array($groupField->get('name'), $GLOBALS['customPickListFields'])) {
					$tableName = $groupField->get('table');
					$columnName = $groupField->get('column');
					$fieldName = $groupField->get('name');
					$picklistTableName = "vtiger_{$fieldName}"; 
					$groupFields[] = "{$tableName}.{$columnName}";
					$sortFields[] = "{$picklistTableName}.sortorderid ASC";

					// Add extra join query for picklist table
					$extraJoinQuery = "LEFT JOIN {$picklistTableName} ON ({$picklistTableName}.{$fieldName} = {$tableName}.{$columnName})";
					$chartSQL = addExtraJoinQuery($chartSQL, $extraJoinQuery, false);
				}
				else {
					$groupFields[] = $groupByColumns[$i];
					$sortFields[] = "{$groupByColumns[$i]} ASC";
				}
			}
			// End by Tung Nguyen
			
			$chartSQL .= " GROUP BY " . join(', ', $groupFields);
			$chartSQL .= " ORDER BY " . join(', ', $sortFields);
		}
		// End Hieu Nguyen

		// Added by Hieu Nguyen on 2021-12-07 to log chart report query
		saveLog('PLATFORM', "[Reports_Chart_Model::getQuery] Chart report query [id={$this->reportRun->reportid}]: " . $chartSQL);
		// End Hieu Nguyen
	...
	function generateData(){
		...
		for($i = 0; $i < $rows; $i++) {
			...
			if ($legendField) {
                ...
                // Modified by Hieu Nguyen on 2019-09-19 to support custom owner field
                else if ($fieldDataType == 'owner') {
                    $ownerId = $row[strtolower($legend)];
                    $label = Vtiger_Owner_UIType::getCurrentOwnersForDisplay($ownerId, false);
                } 
                // End Hieu Nguyen
                else {
			...
			if(!$this->isRecordCount()) {
				if($sectorField) {
					if($sectorField->get('uitype') == '71' || $sectorField->get('uitype') == '72') {	//convert currency fields
						$value = (float) ($row[$sector]);
						$value =  CurrencyField::convertFromBaseCurrency($value, $currencyRateAndSymbol['rate']);
        ...
        // Modified by Hieu Nguyen on 2021-03-29 to display total and percent
		...
		$data = [
            ...
            'values' => $values,
            'links' => $links,
            'graph_label' => $this->getGraphLabel() . ' (Σ='. formatNumberToUser($total) .')'
        ];
        // End Hieu Nguyen
...
class VerticalbarChart extends Base_Chart {
	function generateData() {
			...
			if ($groupByColumnsByFieldModel) {
				foreach ($groupByColumnsByFieldModel as $gFieldModel) {
                    ...
                    // Modified by Hieu Nguyen on 2019-09-19 to support custom owner field
                        ...
                        $ownerId = $row[strtolower($gFieldModel->get('reportlabel'))];
                        $label = Vtiger_Owner_UIType::getCurrentOwnersForDisplay($ownerId, false);
                    }
                    // End Hieu Nguyen
                    else {
					...
					if($queryColumnsByFieldModel) {
						foreach($queryColumnsByFieldModel as $fieldModel) {
							if($fieldModel->get('uitype') == '71' || $fieldModel->get('uitype') == '72') {
								...
								$values[$j][] = CurrencyField::convertFromBaseCurrency($value, $currencyRateAndSymbol['rate']);
        ...
        // Modified by Hieu Nguyen on 2021-03-29 to display total
        $dataFields = $this->getQueryColumnsByFieldModel();
        $dataLabels = $this->getDataLabels();

        foreach ($dataLabels as $i => &$dataLabel) {
            if ($dataLabel != vtranslate('LBL_RECORD_COUNT', 'Reports')) {
                $fieldIndex = count($dataFields) < count($dataLabels) ? $i - 1 : $i;

                if (strpos($dataFields[$fieldIndex]->get('reportcolumninfo'), ':SUM') === false) {
                    continue;
                }
            }
            ...
            $dataLabel .= ' (Σ='. formatNumberToUser($total) .')';
        }

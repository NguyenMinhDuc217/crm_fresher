...
class Reports_Record_Model extends Vtiger_Record_Model {
	...
	public function getMembers() {
		if($this->members == false) {
            // Modified by Hieu Nguyen on 2019-06-19 to convert members from group format into UI format
			$memberGroups = Settings_Groups_Member_Model::getAllByGroup($this, Settings_Groups_Member_Model::REPORTS_VIEW_MODE);
            $members = [];

            foreach($memberGroups as $groupName => $groupData) {
                foreach($groupData as $memberId => $memberData) {
                    $members[] = $memberId;
                }
            }

			$this->members = join(',' , $members);
            // End Hieu Nguyen
		}
		return $this->members;
	}
	...
	// Implemented by Hieu Nguyen on 2018-12-08
	function getCustomHandler() {
		$customHandlerFile = $this->get('custom_handler_file');

		if(!empty($customHandlerFile) && file_exists($customHandlerFile)) {
			require_once($customHandlerFile);
			$customHandlerClass = basename($customHandlerFile, '.php');
			$customReportHandler = new $customHandlerClass($this->getId());
			$reportRunInstance = ReportRun::getInstance($this->getId());

			// Transfer all data from the default handler into the custom handler
			foreach($reportRunInstance as $property => $value) {
				$customReportHandler->$property = $value;
			}
	...
	function getReportRelatedModules() {
		return $this->report->getRelatedModuleList();	// Modified by Hieu Nguyen on 2022-09-15 to hide unsupported modules from related module list
	}
	...
	function isRecordHasViewAccess($reportType){
		...
		// Added by Phu Vo on 2019.06.28 to allow admin user access every report
		if (is_admin($current_user)) return true;
		// End allow admin user access every report
	...
	function save() {
		...
		if(empty($reportId)) {
			...
			// Modified by Hieu Nguyen on 2022-03-10 to support limit records
			$sqlInsert = 'INSERT INTO vtiger_report(reportid, folderid, reportname, description, reporttype, queryid, state, owner, sharingtype, custom_handler_file, sort_column, sort_order, set_limit, limit_records)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
			$insertParams = [
				$reportId, $this->get('folderid'), $this->get('reportname'), $this->get('description'),
				$this->get('reporttype', 'tabular'), $reportId, 'CUSTOM', $currentUser->id, $sharingType,
				$this->get('custom_handler_file'), $this->get('set_limit'), $this->get('sort_column'), 
				$this->get('sort_order'), $this->get('limit_records')
			];
			$db->pquery($sqlInsert, $insertParams);
			// End Hieu Nguyen
			...
			// Modified by Hieu Nguyen on 2022-03-10 to support limit records
			$sqlUpdate = 'UPDATE vtiger_report SET reportname = ?, description = ?, reporttype = ?, folderid = ?,sharingtype = ?, custom_handler_file = ?, set_limit = ?, sort_column = ?, sort_order = ?, limit_records = ? WHERE reportid = ?';
			$updateParams = [
				decode_html($this->get('reportname')), decode_html($this->get('description')), $this->get('reporttype'),
				$this->get('folderid'), $sharingType, decode_html($this->get('custom_handler_file')),
				$this->get('set_limit'), $this->get('sort_column'),
				$this->get('sort_order'), $this->get('limit_records'), $reportId
			];
			$db->pquery($sqlUpdate, $updateParams);
			// End Hieu Nguyen
	...
	function saveSharingInformation() {
		...
		// Modified by Phu Vo on 2019.06.28 Move form for loop out here => delete sharing information every time report is saved
		...
		// End delete sharing information every time report is saved
	...
	function getReportCalulationData($filterQuery = false) {
		$reportRun = ReportRun::getInstance($this->getId());
		$data = $reportRun->GenerateReport('TOTAL', $filterQuery, true);	// Refactored output format name by Hieu Nguyen on 2021-12-22
		return $data;
	}
	/**
	...
	function getReportXLS($type = false) {
		...
		$reportRun = $this->getCustomHandler();
	...
	function getReportPrint() {
		...
		// Aded by Hieu Nguyen on 2018-12-08
		$customHandler = $this->getCustomHandler();

		if($customHandler) {
			$data['data'][0] = $customHandler->getPrintResult($advanceFilterSql);
			return $data;
		}
		// End Hieu Nguyen
	...
	function isEditableBySharing() {
		...
		// Added by Phu Vo on 2019.06.28 to allow admin user access every report
		if (Users_Record_Model::getCurrentUserModel()->isAdminUser()) return true;
		// End allow admin user access every report
		...
		if($currentUserId == $reportOnwer) {
			...
			// Modified by Phuc on 2019.11.25 to update sql to get sharing role
			// For action create new report, don't need to check sharing
			if (empty($this->getId())) {
				return true;
			}

			$query = "SELECT 1 FROM vtiger_report_sharegroups
				WHERE reportid = ? 
				AND ? IN (
					SELECT ug.userid
					FROM vtiger_users2group AS ug
					INNER JOIN vtiger_groups AS g ON (g.groupid = ug.groupid AND g.is_custom = 0)
					WHERE ug.groupid = vtiger_report_sharegroups.groupid
				)
				UNION SELECT 1 FROM vtiger_report_sharerole WHERE reportid = ?
				UNION SELECT 1 FROM vtiger_report_sharers WHERE reportid = ?
				UNION SELECT 1 FROM vtiger_report_shareusers WHERE reportid = ? AND userid = ?";
			$result = $db->pquery($query, [$reportId, $currentUserId, $reportId, $reportId, $reportId, $currentUserId]);
			// Ended by Phuc

			// Modified by Phuc on 2019.10.28 to return correct value. If there is/are sharing rows, return true, not false
			if ($db->num_rows($result)) {
				return true;
			} else {
				return false;

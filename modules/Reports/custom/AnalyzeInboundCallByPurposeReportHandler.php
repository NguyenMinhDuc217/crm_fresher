...
/*
    AnalyzeInboundCallByPurposeReportHandler.php
    Author: Phuc Lu
    Date: 2020.08.19
*/
...
class AnalyzeInboundCallByPurposeReportHandler extends SummarySalesByMarketReportHander {
    ...
    public function getReportHeaders() {
        return [
            ...
            vtranslate('LBL_EVENTS_INBOUND_CALL_PURPOSE', 'Events') => '32%',
    ...
    protected function getReportData($params, $forExport = false) {
        ...
        $sql = "SELECT COUNT(activityid)
            FROM  vtiger_events_inbound_call_purpose
            LEFT JOIN (
                vtiger_activity
                INNER JOIN  vtiger_crmentity ON (deleted = 0 AND crmid = activityid)
            ) ON (vtiger_activity.events_inbound_call_purpose = vtiger_events_inbound_call_purpose.events_inbound_call_purpose AND 
                activitytype = 'Call' AND events_call_direction = 'Inbound' AND createdtime BETWEEN '{$period['from_date']}' AND '{$period['to_date']}')";

        $allCallNumber = $adb->getOne($sql);

        // Get data
        $sql = "SELECT 0 AS no, vtiger_events_inbound_call_purpose.events_inbound_call_purpose AS events_call_purpose, COUNT(activityid) AS call_number
            FROM  vtiger_events_inbound_call_purpose
            LEFT JOIN (
                vtiger_activity
                INNER JOIN  vtiger_crmentity ON (deleted = 0 AND crmid = activityid)
                ...
                activitytype = 'Call' AND events_call_direction = 'Inbound' AND createdtime BETWEEN '{$period['from_date']}' AND '{$period['to_date']}')
            GROUP BY vtiger_events_inbound_call_purpose.events_inbound_call_purpose

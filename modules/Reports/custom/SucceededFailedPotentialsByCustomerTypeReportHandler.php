...
/*
    SucceededFailedPotentialsByCustomerTypeReportHandler.php
    Author: Phuc Lu
    Date: 2020.5.14
*/
...
class SucceededFailedPotentialsByCustomerTypeReportHandler extends SucceededFailedPotentialsByIndustryReportHandler {

    protected $dashboardWidgetFilterTemplate = 'modules/Reports/tpls/dashboard/SucceededFailedPotentialsByCustomerTypeReportWidgetFilter.tpl';
    protected $reportObject = 'CUSTOMER_TYPE';
    ...
    protected function getReportData($params, $forChart = false, $forExport = false) {
        ...
        foreach ($allTypes as $type => $label) {
            ...
            if (!$forExport) {
                $potentialConditions = [[
                    ['opportunity_type', 'e', $customerType],
        ...
        $sql = "SELECT type, SUM(amount) AS value, COUNT(potentialid) AS number
            FROM (
                    ...
                    WHERE potentialtype = ? AND potential_crmentity.createdtime BETWEEN '{$period['from_date']}' AND '{$period['to_date']}'
                ) AS temp
            GROUP BY type";

        $result = $adb->pquery($sql, [$customerType]);

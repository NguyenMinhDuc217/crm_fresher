...
/*
    TopCampaignsByAccountReportHandler.php
    Author: Phuc Lu
    Date: 2020.08.12
*/
...
class TopCampaignsByAccountReportHandler extends TopSourcesByLeadReportHandler {
    protected $targetModule = 'CAMPAIGN_ACCOUNT';
    ...
    protected function getChartData(array $params) {
        ...
        foreach ($reportData as $row) {
            $data[] = [vtranslate($row['campaignname']), (float)$row['account_number']];
            $links[] = '';
        }        
    ...
    protected function getReportData($params, $forExport = false) {
        ...
        // Data for sales
        $sql = "SELECT 0 AS no, vtiger_campaign.campaignid, vtiger_campaign.campaignname, COUNT(DISTINCT accountid) AS account_number
            FROM vtiger_account
            INNER JOIN vtiger_crmentity AS account_crmentity ON (account_crmentity.crmid = accountid AND account_crmentity.deleted = 0)
            INNER JOIN vtiger_campaign ON (vtiger_account.related_campaign = vtiger_campaign.campaignid)
            ...
            WHERE 
                ...
                AND campaign_crmentity.createdtime BETWEEN '{$period['from_date']}' AND '{$period['to_date']}'

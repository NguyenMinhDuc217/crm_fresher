...
/*
    AnalyzeMeetingFluctuationReportHandler.php
    Author: Phuc Lu
    Date: 2020.08.19
*/
...
class AnalyzeMeetingFluctuationReportHandler extends AnalyzeSalesFluctuationReportHandler {

    protected $formatNumber = 'Integer';

    protected function getChartData(array $params) {
        $chartData = parent::getChartData($params);
        $chartData['ylabel'] = vtranslate('Cuộc họp', 'Reports');
    ...
    protected function getReportData($params, $forExport = false) {
        ...
        // Get calls
        $sql = "SELECT sum_time, group_by, COUNT(activityid) AS call_number
            FROM (
                SELECT IF(createdtime < '{$seperateDate}', 1, 0) AS sum_time, activityid, {$groupBy}(createdtime) - 1 AS group_by
                FROM vtiger_activity
                INNER JOIN vtiger_crmentity ON (deleted = 0 AND crmid = activityid)
                WHERE activitytype = 'Meeting' AND (createdtime BETWEEN '{$fromDate}' AND '{$toDate}')
            ) AS temp
        ...
        while ($row = $adb->fetchByAssoc($result)) {
            ...
            $data[$row['sum_time']]['data'][$row['group_by']] = (int)$row['call_number'];
        }

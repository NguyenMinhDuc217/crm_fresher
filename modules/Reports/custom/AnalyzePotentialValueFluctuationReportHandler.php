...
/*
    AnalyzePotentialValueFluctuationReportHandler.php
    Author: Phuc Lu
    Date: 2020.08.19
*/
...
class AnalyzePotentialValueFluctuationReportHandler extends AnalyzeSalesFluctuationReportHandler {

    protected function getChartData(array $params) {
        $chartData = parent::getChartData($params);
        $chartData['ylabel'] = vtranslate('Giá trị cơ hội', 'Reports');
    ...
    protected function getReportData($params, $forExport = false) {
        ...
        // Get calls
        $sql = "SELECT sum_time, group_by, SUM(amount) AS sales
            FROM (
                SELECT IF(createdtime < '{$seperateDate}', 1, 0) AS sum_time, amount, {$groupBy}(createdtime) - 1 AS group_by

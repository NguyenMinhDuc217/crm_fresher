...
/*
    AnalyzeTaskFluctuationReportHandler.php
    Author: Phuc Lu
    Date: 2020.08.19
*/
...
class AnalyzeTaskFluctuationReportHandler extends AnalyzeSalesFluctuationReportHandler {

    protected $formatNumber = 'Integer';

    protected function getChartData(array $params) {
        $chartData = parent::getChartData($params);
        $chartData['ylabel'] = vtranslate('Công việc', 'Reports');
    ...
    protected function getReportData($params, $forExport = false) {
        ...
        $sql = "SELECT sum_time, group_by, COUNT(activityid) AS call_number
            FROM (
                ...
                WHERE activitytype = 'Task' AND (createdtime BETWEEN '{$fromDate}' AND '{$toDate}')
            ) AS temp

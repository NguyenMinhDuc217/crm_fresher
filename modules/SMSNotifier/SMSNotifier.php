...
class SMSNotifier extends SMSNotifierBase {
	...
	static function sendsms($message, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false) {
        ...
        // Modified by Hieu Nguyen on 2020-11-18 to return sent result
    ...
    // Modified by Hieu Nguyen on 2020-11-18 to save message log
	private function processFireSendSMSResponse($responses) {
		...
		foreach ($responses as $response) {
            ...
            $logData = [
                ...
                'sms_ott_notifier_id' => $this->id,
                'message' => $response['message'],
                'message_type' => $GLOBALS['sms_ott_channel'],
                'status' => $response['status'],
                'tracking_id' => $response['id'],
                'description' => $response['statusmessage'],
            ];

            CPSMSOTTMessageLog_Record_Model::addNew($logData);
		}
	}
    ...
    // Modified by Hieu Nguyen on 2020-10-28 to support sending multi-channel message
	static function fireSendSMS($message, $toNumbers) {
		if (!empty($GLOBALS['sms_ott_channel']) && $GLOBALS['sms_ott_channel'] != 'SMS') {
			$activeGateway = CPOTTIntegration_Gateway_Model::getActiveGateway($GLOBALS['sms_ott_channel']);
			
			if ($activeGateway) {
				$method = $activeGateway->channelMethodsMapping[$GLOBALS['sms_ott_channel']];

				if (!empty($method) && method_exists($activeGateway, $method)) {
					return $activeGateway->$method($message, $toNumbers);
				}
			}
		}
		else {
			...
			if ($activeProvider) {
				return $activeProvider->send($message, $toNumbers);
			}
		}

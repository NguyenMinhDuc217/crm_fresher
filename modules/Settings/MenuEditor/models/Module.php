...
class Settings_MenuEditor_Module_Model extends Settings_Vtiger_Module_Model {
	 ...
	 */
	public static function getAvailableModules($appName) {	// Refactored this function name by Hieu Nguyen on 2021-08-18 to avoid confusing
		$db = PearDatabase::getInstance();
		$modules = array();

		// Modified by Hieu Nguyen on 2018-07-19
		$sql = 'SELECT tabid FROM vtiger_tab
			WHERE tabid NOT IN (
				SELECT tabid FROM vtiger_app2tab WHERE appname = ? AND visible = ?
			)
		';

		$result = $db->pquery($sql, array($appName, 1));
		// End Hieu Nguyen
		...
		if ($count > 0) {
			for ($i = 0; $i < $count; $i++) {
                ...
                // Modified by Hieu Nguyen on 2020-12-10 to show only features that is available in the package and is not a hidden module
				if (!isForbiddenFeature("Module{$moduleName}") && !isHiddenModule($moduleName) && $moduleInstance->isActive()) {
	...
	public static function getAllVisibleModules() {
		...
		if ($count > 0) {
			for ($i = 0; $i < $count; $i++) {
                ...
                // Added by Hieu Nguyen on 2019-12-18 to hide forbidden modules from the Menu

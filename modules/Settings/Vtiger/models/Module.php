...
class Settings_Vtiger_Module_Model extends Vtiger_Base_Model {
	...
	static function getSettingsMenuListForNonAdmin() {
        ...
        // Modified by Hieu Nguyen on 2019-04-08 to load menus dynamically from table vtiger_settings_field
        global $adb;
		...
		$settingsMenuList = array('LBL_MY_PREFERENCES'	=> array('My Preferences'	=> '',
																 'Calendar Settings'=> '',
																 'LBL_NOTIFICATION_USER_CONFIG'=> 'index.php?module=Vtiger&parent=Settings&view=UserNotifications&block=11&fieldid=41',
        ...
        */
        ...
        $sql = "SELECT g.label AS group_label, m.name AS menu_label, m.linkto AS menu_url 
            FROM vtiger_settings_field AS m
            INNER JOIN vtiger_settings_blocks AS g ON (g.blockid = m.blockid)
            WHERE allow_non_admin = 1
        ...
        while ($row = $adb->fetchByAssoc($result)) {
            ...
            if (!isset($settingsMenuList[$groupLabel])) {
                $settingsMenuList[$groupLabel] = [];
            }

            $menuLabel = $row['group_label'];
            $menuLabel = $row['menu_label'];
            $menuUrl = $row['menu_url'];

            // Special cases
            if ($menuLabel == 'LBL_MY_TAGS') {
                $menuUrl = $currentUser->getMyTagSettingsListUrl();
            }
            
            $settingsMenuList[$groupLabel][$menuLabel] = $menuUrl;
        }
        // End Hieu Nguyen

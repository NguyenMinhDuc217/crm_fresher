...
class Settings_Profiles_Record_Model extends Settings_Vtiger_Record_Model {
	...
	// Modified by Hieu Nguyen on 2021-10-20
	private static $fieldLockedUiTypes = [4, 70];
	private static $readOnlyFields = ['main_owner_id', 'users_department', 'createdby', 'modifiedby', 'createdtime', 'modifiedtime', 'source'];
	// End Hieu Nguyen
	...
	public function getModuleFieldPermissionValue($module, $field) {
		// Added by Hieu Nguyen on 2021-10-20 to display locked fields as readony
		if (in_array($field->get('uitype'), self::$fieldLockedUiTypes)) {
	...
	public function isModuleFieldLocked($module, $field) {
		...
		if (in_array($fieldModel->getName(), self::$readOnlyFields)) {
			return true;
		}

        if ($fieldModel->isMandatory() || in_array($fieldModel->get('uitype'), self::$fieldLockedUiTypes) || $fieldModel->hasCustomLock()) {
			return true;
		}
		...
		if(!$this->module_permissions) {
			...
			foreach($allModules as $id => $moduleModel) {
				...
				foreach($moduleFields as $fieldName => $fieldModel) {
					if ($fieldModel->getName() == 'newfield1') {
						$debug = true;
					}
	...
	public function save() {
		...
		if(count($allModuleModules) > 0) {
			...
			foreach($allModuleModules as $tabId => $moduleModel) {
				if ($moduleModel->getName() == 'CPTestModule2') {
					$debug = true;
				}
	...
	public static function getInstanceById($profileId) {
        ...
        static $cache = [];
        if (!empty($cache[$profileId])) return $cache[$profileId];
        // End Hieu Nguyen
    ...
    public static function getInstanceByName($profileName , $checkOnlyDirectlyRelated=false, $excludedRecordId = array()) {
        ...
        static $cache = [];
        $cacheKey = $profileName .'-'. (int)$checkOnlyDirectlyRelated .'-'. md5(serialize($excludedRecordId));

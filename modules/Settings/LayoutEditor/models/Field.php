...
class Settings_LayoutEditor_Field_Model extends Vtiger_Field_Model {
	...
	public static function makeFieldActive($fieldIdsList=array(), $blockId, $moduleName = false) {
		...
		// Modifed by Hieu Nguyen on 2018-07-27
		...
		$maxSequenceQuery = "SELECT MAX({$sequenceField}) AS maxsequence FROM vtiger_field WHERE {$blockField} = ? AND {$presenceField} IN (0,2) ";
		$res = $db->pquery($maxSequenceQuery,array($blockId));
		$maxSequence = $db->query_result($res,0,'maxsequence');

		$query = "UPDATE vtiger_field SET {$blockField} = {$blockId}, {$presenceField} = 2, {$sequenceField} = CASE";
		foreach ($fieldIdsList as $fieldId) {
			$maxSequence = $maxSequence + 1;
			$query .= ' WHEN fieldid = ? THEN '. $maxSequence;
		}
		$query .= " ELSE {$sequenceField} END";
	...
	// Added by Hieu Nguyen on 2021-07-29 to check if field can be deleted by customer, dev or R&D
	public function isDeletable() {
		$registerFileType = Vtiger_BlockAndField_Helper::getFileTypeForSaving();
		$deletable = $this->isCustomField() && $this->isCreatedIn($registerFileType);
		return $deletable;
	}

	// Added by Hieu Nguyen on 2021-07-29 to check if field is original created in cus, dev or base register file
	public function isCreatedIn($registerFileType) {
		$moduleName = $this->getModuleName();
		$fileContent = Vtiger_BlockAndField_Helper::readRegisterFile($moduleName, $registerFileType);
		$fieldDef = $fileContent['fields'][$this->name];
		return count(array_keys($fieldDef)) >= Vtiger_BlockAndField_Helper::FULL_FIELD_DEF_ATTRIBUTES_NUM;
	}
	...
	public static function getAllForModule($module) {
		$db = PearDatabase::getInstance();

		$sequenceField = $_REQUEST['layouteditor_tab'] == 'editViewTab' ? 'editview_sequence' : 'sequence';
		$query = "SELECT * FROM vtiger_field WHERE tabid = {$module->getId()} AND displaytype IN (1, 2, 4, 5) ORDER BY {$sequenceField}";
	...
	public static function getInstanceFromBlockIdList($blockId, $moduleInstance= false) {
		...
		$blockField = $_REQUEST['layouteditor_tab'] == 'editViewTab' ? 'editview_block' : 'block';
		$sequenceField = $_REQUEST['layouteditor_tab'] == 'editViewTab' ? 'editview_sequence' : 'sequence';
		$blockIds = generateQuestionMarks($blockId);

		$query = "SELECT * FROM vtiger_field WHERE {$blockField} IN({$blockIds}) AND vtiger_field.displaytype IN (1,2,4,5) ORDER BY {$sequenceField}";
		// End Hieu Nguyen

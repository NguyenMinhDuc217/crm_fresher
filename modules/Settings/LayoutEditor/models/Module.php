...
class Settings_LayoutEditor_Module_Model extends Vtiger_Module_Model {
	...
	// Added by Hieu Nguyen on 2018-08-03
	public function getAllFields() {
		if(empty($this->fields)){
			$fieldList = Settings_LayoutEditor_Field_Model::getAllForModule($this);
    ...
    public function savePopupAndRelationListLayout(array $layout) {
        require_once('include/utils/FileUtils.php');
        $layoutFile = 'modules/'. $this->getName() .'/custom/PopupAndRelationListLayout.php';
        
        if (empty($layout)) {
            $layout = array('popupLayout' => [], 'relationListLayout' => []);
        }

        if (!file_exists($layoutFile)) {
            mkdir(dirname($layoutFile), 0777, true);
        }

		return FileUtils::writeArrayToFile($layout, $layoutFile);
    }
    // End Hieu Nguyen

    // Added by Hieu Nguyen on 2019-10-09
    public static function getModuleLayoutFields($moduleName, $viewName) {
        if ($moduleName == 'Events') $moduleName = 'Calendar';
        $layoutFile = 'modules/'. $moduleName .'/custom/PopupAndRelationListLayout.php';
        if (!file_exists($layoutFile)) return [];

        global $popupLayout, $relationListLayout;
        require($layoutFile);
        $selectedLayout = [];

        if ($viewName == 'Popup') $selectedLayout = $popupLayout;
        if ($viewName == 'RelationList') $selectedLayout = $relationListLayout;
        if (empty($selectedLayout)) return [];

        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
        $moduleFields = $moduleModel->getFields();
        $layoutFields = [];
        
        foreach ($selectedLayout['display_fields'] as $fieldName) {
            $fieldModel = $moduleFields[$fieldName];
            if ($fieldModel) $layoutFields[$fieldModel->column] = $fieldName;
        }
    ...
    // Added by Hieu Nguyen on 2020-08-19
    public static function getModuleLayoutSorting($moduleName, $viewName) {
        if ($moduleName == 'Events') $moduleName = 'Calendar';
        $layoutFile = 'modules/'. $moduleName .'/custom/PopupAndRelationListLayout.php';
        if (!file_exists($layoutFile)) return [];
        ...
        if ($viewName == 'RelationList') $selectedLayout = $relationListLayout;
        if (empty($selectedLayout)) return [];
        if (empty($selectedLayout['sort_field'])) return [];
        $sortFieldName = $selectedLayout['sort_field'];
        $sortOrder = $selectedLayout['sort_order'] ?? 'ASC';

        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
        $sortFieldModel = $moduleModel->getField($sortFieldName);
        if (empty($sortFieldModel)) return [];

        $sorting = [
            'sort_field' => $sortFieldModel->get('name'), 
            'sort_column' => $sortFieldModel->get('column'), 
	...
	public function getBlocks() {
		// Added by Hieu Nguyen on 2021-09-06 to reload blocks list if the cached blocks instance is not valid to prevent error in non-entity modules like Users
		if (!empty($this->blocks) && array_values($this->blocks)[0] instanceof Vtiger_Block_Model) {
			$this->blocks = null;
		}
		// End Hieu Nguyen
	...
	public function addField($fieldType, $blockId, $params) {
		...
		// Modified by Hieu Nguyen on 2018-08-03
		$moduleName = $this->getName();
		$tableName = $this->basetable;
		$columnName = trim($params['fieldName']);
		$fieldLabel = trim($params['fieldLabel']);

		if ($this->checkFieldExists($columnName)) {
		...
		$fieldModel->set('name', $columnName)
				   ...
				   ->set('generatedtype',2)
				   ->set('uitype', $uitype)
				   ->set('label', $fieldLabel)	// Modified by Hieu Nguyen on 2018-08-03
		...
		if(!in_array($columnName, $db->getColumnNames($tableName))) {
			throw new Exception(vtranslate('LBL_FIELD_COULD_NOT_BE_CREATED', 'Settings::LayoutEditor', $columnName), 513);	// Modified by Hieu Nguyen on 2018-08-03
		}
	...
	public static function getSupportedModules() {
		...
		// Support system modules by config
		if (!empty($layoutEditorConfig['allowed_system_modules'])) {
			foreach ($layoutEditorConfig['allowed_system_modules'] as $moduleName) {
				self::$supportedModules[$moduleName] = vtranslate($moduleName, $moduleName);
			}
		}

		// Show modules for developer only
		if (!empty($layoutEditorConfig['modules_allow_developer_only']) && !isDeveloperMode()) {
			foreach ($layoutEditorConfig['modules_allow_developer_only'] as $moduleName) {
				unset(self::$supportedModules[$moduleName]);
			}
		}
	...
	public static function getInstanceByName($moduleName) {
		...
		// Added by Hieu Nguyen on 2019-01-15
		if(in_array($moduleName, array('Calendar', 'Events'))) {
			$selfInstance->basetable = 'vtiger_activity';
		}
		// End Hieu Nguyen
	...
	public static function getEntityModulesList() {
		...
		// Added by Hieu Nguyen on 2021-08-18 to hide hidden modules from Layout Editor
		global $hiddenModules;
		$forbiddenModules = getForbiddenFeatures('module');
		$restrictedModules = array_merge($restrictedModules, $hiddenModules, $forbiddenModules ?? []);
		// End Hieu Nguyen
	...
	public function isSortableAllowed() {
		// Modified by Hieu Nguyen on 2019-01-15
		/*$moduleName = $this->getName();
		if (in_array($moduleName, array('Calendar', 'Events'))) {
			return false;
		}*/
	...
	public function isFieldsSortableAllowed($blockName) {
		...
		// Modified by Hieu Nguyen on 2018-10-03 to load external config

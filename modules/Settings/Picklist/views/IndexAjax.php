...
class Settings_Picklist_IndexAjax_View extends Settings_Vtiger_IndexAjax_View {
    ...
    public function showEditView(Vtiger_Request $request) {
        ...
        $valueToEdit = $request->get('fieldValue');   // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
	...
	//	$selectedFieldAllPickListValues =  array_map('Vtiger_Util_Helper::toSafeHTML', $selectedFieldAllPickListValues);
        ...
        // Added by Hieu Nguyen on 2019-05-29 to load label display in both english and vietnamese for editing
		require_once('include/utils/LangUtils.php');
		$modStringsEn = LangUtils::readModStrings($module, 'en_us');
		$modStringsVn = LangUtils::readModStrings($module, 'vn_vn');
    ...
    public function getPickListDetailsForModule(Vtiger_Request $request) {
        ...
        $pickListFields = $moduleModel->getAllPicklistFields();

        // Added by Hieu Nguyen on 2020-09-25 to show User's department field in Picklist Editor
        if ($sourceModule == 'Users') {
            $pickListFields = [];
            $pickListFields['users_department'] = $moduleModel->getField('users_department');
        }
        // End Hieu Nguyen
    ...
    public function getPickListValueForField(Vtiger_Request $request) {
        ...
        if (empty($sourceModule) || empty($pickFieldId)) return; // Added by Hieu Nguyen on 2021-06-18 to validate params

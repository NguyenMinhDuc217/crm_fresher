...
class Settings_Picklist_SaveAjax_Action extends Settings_Vtiger_Basic_Action {
    ...
    public function process(Vtiger_Request $request) {
        ...
        Vtiger_AdminAudit_Helper::saveLog('PicklistEditor', "Do action {$mode} on picklist {$pickListName}", $request);
        // End Hieu Nguyen
    ...
    public function rename(Vtiger_Request $request) {
        ...
        $newValue = $request->get('newValue');  // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
        ...
        $oldValue = $request->get('oldValue');  // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
		$id = $request->get('id');              // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
        ...
        try{
            ...
            // Added by Hieu Nguyen on 2021-06-15 to track labels
			require_once('include/utils/LangUtils.php');
            $itemLabelDisplayEn = trim($request->get('itemLabelDisplayEn'));
            $itemLabelDisplayVn = trim($request->get('itemLabelDisplayVn'));

			if ($request->get('itemLabelDisplayEnChanged') == '1') {
				$languageStrings = [$itemValue => $itemLabelDisplayEn];
				LangUtils::writeModStrings($languageStrings, [], $moduleName, 'en_us');
			}

			if ($request->get('itemLabelDisplayVnChanged') == '1') {
				$languageStrings = [$itemValue => $itemLabelDisplayVn];
				LangUtils::writeModStrings($languageStrings, [], $moduleName, 'vn_vn');
			}
            // End Hieu Nguyen
    ...
    public function remove(Vtiger_Request $request) {
        $moduleName = $request->get('source_module');
        $valueToDelete = $request->get('delete_value'); // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
        $replaceValue = $request->get('replace_value'); // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
        $pickListFieldName = $request->get('picklistName');
        $moduleModel = Settings_Picklist_Module_Model::getInstance($moduleName);
    ...
    public function assignValueToRole(Vtiger_Request $request) {
        $pickListFieldName = $request->get('picklistName');
        $valueToAssign = $request->get('assign_values');    // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
    ...
    public function saveOrder(Vtiger_Request $request) {
        ...
        $picklistValues = $request->get('picklistValues');  // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
    ...
    public function enableOrDisable(Vtiger_Request $request) {
        $pickListFieldName = $request->get('picklistName');
        $enabledValues = $request->get('enabled_values',array());   // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
        $disabledValues = $request->get('disabled_values',array()); // Modified by Hieu Nguyen on 2020-01-09 to avoid XSS
    ...
    public function edit(Vtiger_Request $request) {
		...
		if ($request->get('itemLabelDisplayEnChanged') == '1') {
			$languageStrings = [$newValue => $itemLabelDisplayEn];
			LangUtils::writeModStrings($languageStrings, [], $moduleName, 'en_us');
		}

		if ($request->get('itemLabelDisplayVnChanged') == '1') {
			$languageStrings = [$newValue => $itemLabelDisplayVn];
			LangUtils::writeModStrings($languageStrings, [], $moduleName, 'vn_vn');
		}
        // End Hieu Nguyen

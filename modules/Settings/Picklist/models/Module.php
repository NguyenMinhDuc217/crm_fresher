...
class Settings_Picklist_Module_Model extends Vtiger_Module_Model {
	...
	public function getAllPicklistFields() {
		$picklistFieldModels = parent::getFieldsByType(['picklist', 'multipicklist'], false);
		$picklistFields = [];

		foreach ($picklistFieldModels as $fieldName => $fieldModel) {
			if (Settings_Picklist_Field_Model::isUnsupportedField($fieldModel->getName())) continue;	// Added by Hieu Nguyen on 2021-10-22 to check if field is supported in Picklist Editor

			if (($fieldName != 'salutationtype') && $fieldModel->get('displaytype') != '1') {
				continue;
			}

			$picklistFields[$fieldName] = Settings_Picklist_Field_Model::getInstanceFromFieldObject($fieldModel);
		}
	...
	public function addPickListValues($fieldModel, $newValue, $rolesSelected = array(), $color = '') {
        ...
        // Added by Hieu Nguyen on 2020-10-16 to handle event whe a picklist value is created
        $em = new VTEventsManager($db);
		$data = array();
        $data['module'] = $fieldModel->getModuleName();
		$data['fieldname'] = $pickListFieldName;
		$data['optionid'] = $id;
		$data['optionvalue'] = $newValue;
		$em->triggerEvent('vtiger.picklist.aftercreate', $data);
        // End Hieu Nguyen
	...
	public function renamePickListValues($pickListFieldName, $oldValue, $newValue, $moduleName, $id, $rolesList = false, $color = '') {
		...
		$data['fieldname'] = $pickListFieldName;
		$data['optionid'] = $id;    // Added by Hieu Nguyen on 2020-10-16 to get the updated option id
	...
	public function remove($pickListFieldName , $valueToDeleteId, $replaceValueId , $moduleName) {
		...
		$data['fieldname'] = $pickListFieldName;
        $data['deletedids'] = $valueToDeleteId;  // Added by Hieu Nguyen on 2020-10-16 to get id of deleted picklist options
	...
	// Modified by Hieu Nguyen on 2021-08-18 to hide hidden modules from Picklist Editor
	public static function getPicklistSupportedModules() {
		global $adb, $hiddenModules;
		$unsupportedModules = ['Emails'];	// Modified by Hieu Nguyen on 2020-09-28 to show module Users in Picklist Editor
		$forbiddenModules = getForbiddenFeatures('module');
		$unsupportedModules = array_merge($unsupportedModules, $hiddenModules, $forbiddenModules ?? []);
		
		$query = "SELECT DISTINCT vtiger_tab.name, vtiger_tab.tablabel AS label
			FROM vtiger_tab
			INNER JOIN vtiger_field ON (vtiger_tab.tabid = vtiger_field.tabid)
			WHERE vtiger_tab.presence != 1
				AND vtiger_field.uitype IN (15, 16, 33, 114)
		...
		while ($row = $adb->fetchByAssoc($result)) {
			...
			$moduleModel->label = $moduleLabel;
			$modulesModels[] = $moduleModel;
		}

...
class Vtiger_MailScannerAction {
	 ...
	 * Modified by Tin Bui on 2022.03.15
	 */
	function __UpdateTicket($mailscanner, $mailrecord, $regexMatchInfo, $mailscannerrule) {
		...
		if ($this->lookup == 'SUBJECT') {
			...
			$usesubject = $regexMatchInfo ? $regexMatchInfo['matches'] : $mailrecord->_subject;
			...
			if ($linkfocus) {
				// Set the ticket status to Reopen if its Closed
				$currentTicketStatus = $linkfocus->column_fields['ticketstatus'];

				// If customer reply email after close ticket
				if ($currentTicketStatus == 'Closed') {
					if (!empty($linkfocus->column_fields['process_end_date'])) {
						$processEndDate = strtotime(date('Y-m-d', strtotime($linkfocus->column_fields['process_end_date'])));
						$today = strtotime(date('Y-m-d'));
						$days = floor(($today - $processEndDate) / 86400);

						// if overtime to reply ticket, create new ticket
						if ($days > $ticketConfigs['max_days_can_reopen']) {
							$mailrecord->_subject = $usesubject;
							$returnid = $this->__CreateTicket($mailscanner, $mailrecord, $mailscannerrule);
							return $returnid;
						}
					}

					$sql = "UPDATE vtiger_troubletickets SET status = ?, helpdesk_survey_status = ? WHERE ticketid = ?";
					$adb->pquery($sql, ['Reopen', 'customer_reopen_ticket', $linkfocus->id]);
				}

				// Handle save new email cc
				$newCCEmails = array_filter(!empty($mailrecord->_cc) ? $mailrecord->_cc : []);
				$currentCCEmails = array_filter(explode(' |##| ', strval($linkfocus->column_fields['helpdesk_related_emails'])));
				$diffEmails = array_diff($newCCEmails, $currentCCEmails); // Find emails in new that not in current
				
				if (is_array($diffEmails) && count($diffEmails) > 0) {
					$currentCCEmails = array_merge($currentCCEmails, $diffEmails); // Merge diff emails to current cc emails
					$sql = "UPDATE vtiger_troubletickets SET helpdesk_related_emails = ? WHERE ticketid = ?";
					$adb->pquery($sql, [implode(' |##| ', $currentCCEmails), $linkfocus->id]);
				}

				$returnid = $this->__CreateNewEmail($mailrecord, $this->module, $linkfocus);
				$logRecord = HelpDesk_EmailUtils_Helper::createReplyLog(
					$linkfocus->id, 
					$linkfocus->column_fields['contact_id'], 
					$fromemail,
					$mailrecord->_subject, 
					$mailrecord->_body, 
					'inbound', 
				...
				$this->__SaveAttachements($mailrecord, 'CPTicketCommunicationLog', $logRecord->entity);

				// Send notification to main owner when customer reply
				HelpDesk_GeneralUtils_Helper::sendTicketRepliedNotification($linkfocus->id);
			}
		}
	...
	function __CreateContact($mailscanner, $mailrecord, $mailscannerrule) {
		...
		$contact = CRMEntity::getInstance('Contacts'); // Modified by Phu Vo on 2020.09.22 to use system default method
	...
	function __CreateLead($mailscanner, $mailrecord, $mailscannerrule) {
		...
		$lead = CRMEntity::getInstance('Leads'); // Modified by Phu Vo on 2020.09.22 to use system default method
	...
	function __CreateAccount($mailscanner, $mailrecord, $mailscannerrule) {
		...
		$account = CRMEntity::getInstance('Accounts'); // Modified by Phu Vo on 2020.09.22 to use system default method
	...
	function __CreateTicket($mailscanner, $mailrecord, $mailscannerrule) {
		...
		// Assuming customer email is first _From email
		$fromEmail = $mailrecord->_from[0];
		...
		if ($contactLinktoid) {
			$accountId = $mailscanner->getAccountId($contactLinktoid);
		}
		if (!$accountId) {
			$accountId = $mailscanner->LookupAccount($fromEmail);
		}
		
		// Create trouble ticket record
		$ticket = CRMEntity::getInstance('HelpDesk'); // Modified by Phu Vo on 2020.09.22 to use system default method
		$this->setDefaultValue('HelpDesk', $ticket);

		$fromEmails = $mailrecord->_from;
		array_shift($fromEmails); 
		$ccEmails = $mailrecord->_cc;  // Added by Tin Bui on 2021.12.30 to save related emails

		$ccEmails = array_filter(array_merge($ccEmails, $fromEmails));
		$ticket->column_fields['helpdesk_related_emails'] = implode(' |##| ', $ccEmails);

		if (empty($ticket->column_fields['ticketstatus']) || $ticket->column_fields['ticketstatus'] == '?????') {
			$ticket->column_fields['ticketstatus'] = 'Open';
		}
		if (empty($ticket->column_fields['ticketpriorities']) || $ticket->column_fields['ticketpriorities'] == '?????') {
			$ticket->column_fields['ticketpriorities'] = 'Normal';
		}
		
		$ticket->column_fields['ticket_title'] = $usetitle;
		$ticket->column_fields['contact_email'] = $fromEmail;
		...
		if (!empty($contactLinktoid)) {
			...
			$contactRecord = Contacts_Record_Model::getInstanceById($contactLinktoid);
			$ticket->column_fields['contact_mobile'] = $contactRecord->get('mobile');
			$ticket->column_fields['helpdesk_contact_type'] = $contactRecord->get('contacts_type');
		}
		if ($accountId) {
	...
	function __CreateNewEmail($mailrecord, $module, $linkfocus) {
		...
		if(!$current_user) {
			$current_user = Users::getRootAdminUser();
		}
		$assignedToId = $linkfocus->column_fields['assigned_user_id'];
		if(vtws_getOwnerType($assignedToId) == 'Groups') {
			$assignedToId = Users::getRootAdminId();
		}

		$focus = CRMEntity::getInstance('Emails'); // Modified by Phu Vo on 2020.09.22 to use system default method
	...
	function __SaveAttachements($mailrecord, $basemodule, $basefocus) {
		...
		foreach($mailrecord->_attachments as $filename=>$filecontent) {
			...
			if($issaved) {
				...
				$document = CRMEntity::getInstance('Documents'); // Modified by Phu Vo on 2020.09.22 to use system default method

		...
		<div class="col-lg-9">
			{if !Settings_Workflows_Util_Helper::isCustomerModule($SOURCE_MODULE)}
				<div class="row form-group variable">
					<div class="col-lg-3 fieldLabel">{vtranslate('LBL_ADD_TO_MARKETING_LIST_TASK_RELATED_CUSTOMER_FIELD', $QUALIFIED_MODULE)}<span class="redColor">*</span></div>
			...
			<div class="row form-group variable">
				<div class="col-lg-3 fieldLabel">{vtranslate('LBL_ADD_TO_MARKETING_LIST_TASK_MARKETING_LIST', $QUALIFIED_MODULE)}<span class="redColor">*</span></div>
				<div class="col-lg-9 fieldValue">
					<select name="marketing_list_id" class="inputElement select2" style="width: 300px" data-rule-required="true" data-placeholder="{vtranslate('LBL_SELECT_OPTION', $QUALIFIED_MODULE)}">
						<option></option>
						{foreach key=KEY item=MKT_LIST_INFO from=Reports_TargetList_Helper::getTargetLists()}
							<option value="{$MKT_LIST_INFO.id}" {if $MKT_LIST_INFO.id == $TASK_OBJECT->marketing_list_id}selected{/if}>{$MKT_LIST_INFO.name}</option>
						{/foreach}
					</select>
				</div>
			</div>

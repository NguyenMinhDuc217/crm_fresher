...
//		}
	...
	public function save() {
		...
		$wf->workflow_mass_action = $this->get('workflow_mass_action'); // Modified by Phu Vo on 2023.03.07
		$wm->save($wf);
	...
	public static function getInstanceFromWorkflowObject($wf) {
		...
		$workflowModel->set('workflow_mass_action', $wf->workflow_mass_action); // Modified by Phu Vo on 2023.03.07
	...
	function transformToAdvancedFilterCondition() {
		...
		if(!empty($conditions)) {
			foreach($conditions as $index => $info) {
                ...
                // Added by Hieu Nguyen on 2020-07-01 to resolve owner name from id for Workflow conditions
                if (in_array($columnName, ['assigned_user_id', 'main_owner_id', 'createdby', 'modifiedby', 'inventorymanager'])) {
                    $value = json_encode(Vtiger_Owner_UIType::getSelectedOwnersFromOwnersString($value));
                }
                // End Hieu Nguyen

	...
	public function getFieldsByType($fieldTypes) {
		...
		if(!empty($structure)) {
			foreach($structure as $block => $fields) {
                // Added by Hieu Nguyen on 2020-06-30 to remove assigned_user_id's email/phone fields from email/phone fields in Send Email / SMS task as the main_owner_id is already cover
                if (in_array($fieldTypes, ['email', 'phone']) && $block == 'Assigned To') {
                    continue;
                }
                // End Hieu Nguyen

...
class Emails_Record_Model extends Vtiger_Record_Model {
	...
	// Added by Hieu Nguyen on 2021-12-08 to prevent delete or unlink an email log
	public function isDeletable() {
		if (strpos($this->get('subject'), '[LOG]') === 0) {
			return false;
		}
	 ...
	 */
	// Renamed and modified by Hieu Nguyen on 2021-11-03 to allow specify date time
	public function trackReads($parentId, $dateTime = '') {
		$db = PearDatabase::getInstance();
		$recordId = $this->getId();
		$date = !empty($dateTime) ? explode(' ', $dateTime)[0] : date('Y-m-d');
		$time = !empty($dateTime) ? $dateTime : date('Y-m-d H:i:s');

		// Check if this event exists by date and time
		if (!empty($dateTime)) {
			$params = [$parentId, $recordId, $date, $time];
			$exists = $db->getOne("SELECT 1 FROM vtiger_email_access WHERE crmid = ? AND mailid = ? AND accessdate = ? AND accesstime = ?", $params);
			
			// Exit if this event exists
			if ($exists == 1) {
				return;
			}
		}
		...
		$params = [$parentId, $recordId, $date, $time];
		$db->pquery("INSERT INTO vtiger_email_access(crmid, mailid, accessdate, accesstime) VALUES(?, ?, ?, ?)", $params);
	 ...
	 * @param <int> $parentId
	 */
	// Moved here and modified by Hieu Nguyen on 2021-11-03 to allow specify date time
	public function trackClicks($parentId, $dateTime = '') {
		$db = PearDatabase::getInstance();
		$recordId = $this->getId();
		$date = !empty($dateTime) ? explode(' ', $dateTime)[0] : date('Y-m-d');
		$time = !empty($dateTime) ? $dateTime : date('Y-m-d H:i:s');
		...
		if ($db->num_rows($result) > 0) {
			$db->pquery("UPDATE vtiger_email_track SET click_count = click_count + 1 WHERE crmid = ? AND mailid = ?", [$parentId, $recordId]);
		}
		else {
			$db->pquery("INSERT INTO vtiger_email_track(crmid, mailid, click_count) VALUES(?, ?, ?)", [$parentId, $recordId, 1]);
		}
	}

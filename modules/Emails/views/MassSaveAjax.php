...
class Emails_MassSaveAjax_View extends Vtiger_Footer_View {
	...
	public function massSave(Vtiger_Request $request) {
		...
		// Added by Phuc on 2020.07.31 to fix error if documents is empty or string
		if (empty($documentIds)) {
			$documentIds = [];
		}
		else if (!is_array($documentIds)) {
		...
		if ($recordModel->checkUploadSize($documentIds)) {
            ...
            // Added by Hieu Nguyen on 2021-02-01 to concat user signature into the email body when sending email
            if ($flag == 'SENT' && $signature == 'Yes') {
                $userSignature = $currentUserModel->get('signature');
                
                if (!empty($userSignature)) {
                    $decodedHtmlDescriptionToSend .= '<br /><br />'. decode_html($userSignature);
                }
            }
            // End Hieu Nguyen

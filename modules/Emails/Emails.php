...
class Emails extends CRMEntity {
	...
	function save_module($module) {
		...
		if ($_REQUEST['module'] == "Emails" && $_REQUEST['smodule'] != 'webmails' && (!$this->plugin_save)) {
			...
			if (isset($this->column_fields['parent_id']) && $this->column_fields['parent_id'] != '') {
				// Added by Phu Vo on 2020.05.26 to handle insert relationship with email and parent records 
				...
				foreach ($parentIds as $parentId) {
					if (strpos('@-1', $parentId) !== false) {
						$parentId = str_replace('@-1', '', $parentId);
	...
	public function getNonAdminAccessControlQuery($module, $user, $scope='') {
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2]
            ...
            // Modified join condition by Hieu Nguyen on 2020-06-16 to get the right condition with new owner field logic
			$query = " INNER JOIN $tableName $tableName$scope ON (
                    $tableName$scope.id = vtiger_crmentity$scope.smownerid 
                    OR $tableName$scope.id = vtiger_crmentity$scope.main_owner_id
                ) ";
            // End Hieu Nguyen
		}
		return $query;

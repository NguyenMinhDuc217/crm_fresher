...
class Import_Utils_Helper {
	...
	static $supportedFileExtensions = array('csv', 'xls', 'xlsx', 'vcf');
	...
	public static function showImportLockedError($lockInfo) {
        ...
        // Modified by Hieu Nguyen on 2020-09-10 to show error message in user's selected language
		$translatedModuleName = vtranslate($moduleName, 'Vtiger');
		$errorMessage = vtranslate('LBL_IMPORT_LOCKED_BY_OTHER_USER', 'Import', ['%user_name' => $userName, '%module_name' => $translatedModuleName]);
        // End Hieu Nguyen
    ...
    // Implemented by Hieu Nguyen on 2020-09-10 to check if the import process is cancelled
    public static function isUserImportCancelled($user) {
        global $adb;
        $importInfo = Import_Queue_Action::getUserCurrentImportInfo($user);

        if ($importInfo != null) {
            $tableName = self::getDbTableName($user);
            $cancelledCount = $adb->getOne("SELECT COUNT(id) FROM {$tableName} WHERE status = ?", [Import_Data_Action::$IMPORT_RECORD_CANCELLED]);

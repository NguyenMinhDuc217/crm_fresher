...
class Import_Data_Action extends Vtiger_Action_Controller {
	...
	static $IMPORT_RECORD_CANCELLED = 6;    // Added by Hieu Nguyen on 2020-09-10 to set record status cancelled
		...
		foreach ($moduleMandatoryFields as $mandatoryFieldName) {
			if (empty($defaultValues[$mandatoryFieldName])) {
				...
				if ($fieldInstance->getFieldDataType() == 'owner') {
					...
					$defaultValues[$mandatoryFieldName] = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
				}
			}
		}
	...
	public function import() {
		...
		// $this->finishImport();   // Commented out this line by Hieu Nguyen on 2020-09-10 to prevent loss statistics info
	}
    ...
    // Implemented by Hieu Nguyen on 2020-09-10 to mark import process as cancelled
    public function cancelImport() {
        global $adb;
        $sql = "UPDATE ". Import_Utils_Helper::getDbTableName($this->user) ." SET status = ? WHERE status = ?";
        $params = [self::$IMPORT_RECORD_CANCELLED, self::$IMPORT_RECORD_NONE];
	...
	// Modified by Hieu Nguyen on 2021-10-15 to save error reasons
	public function updateImportStatus($entryId, array $entityInfo, array $errors = []) {
		$adb = PearDatabase::getInstance();
		$errorReasons = '';
		$recordId = null;

		if (!empty($errors)) {
			$errorReasons = join(', ', $errors);
		}
		...
		$tempTableName = Import_Utils_Helper::getDbTableName($this->user);
		$sql = "UPDATE {$tempTableName} SET status = ?, error_reasons = ?, recordid = ? WHERE id = ?";
		$params = [$entityInfo['status'], $errorReasons, $recordId, $entryId];
		$adb->pquery($sql, $params);
	}
	...
	public function createRecords() {
		...
		// Added by Hieu Nguyen on 2021-10-15 to get mandatory fields for validating
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			// Added by Hieu Nguyen on 2021-10-15 to check for valid required fields
			$errors = [];

			foreach ($mandatoryFields as $fieldName) {
				if (in_array($fieldName, ['assigned_user_id', 'main_owner'])) continue;	// Do not check mandatory for owner fields
				$fieldInstance = $moduleFields[$fieldName];
				$fieldDataType = $fieldInstance->getFieldDataType();
				$translatedFieldName = vtranslate($fieldInstance->getFieldLabelKey(), $moduleName);
				$fieldValue = $fieldData[$fieldName];

				if (empty($fieldValue)) {
					$errors[] = vtranslate('LBL_ERROR_REASONS_FIELD_VALUE_IS_EMPTY', 'Import', ['%field_name' => $translatedFieldName]);
				}
			}
			// End Hieu Nguyen

			// Added by Hieu Nguyen on 2021-10-15 to validate field values
			foreach ($fieldData as $fieldName => $fieldValue) {
				if (empty($fieldValue))	continue;	// Skip empty fields as it is already be checked by previous block
				$fieldInstance = $moduleFields[$fieldName];
				$fieldDataType = $fieldInstance->getFieldDataType();
				$translatedFieldName = vtranslate($fieldInstance->getFieldLabelKey(), $moduleName);

				if ($fieldDataType == 'picklist') {
					$allOptions = $fieldInstance->getPicklistDetails();
					$matched = false;

					foreach ($allOptions as $picklistOption) {
						if (trim(strtolower($fieldValue)) == trim(strtolower($picklistOption['value'])) || trim(strtolower($fieldValue)) == trim(strtolower($picklistOption['label']))) {
							$matched = true;
							break;
						}
					}
					...
					if (!$matched) {
						$errors[] = vtranslate('LBL_ERROR_REASONS_PICKLIST_VALUE_NOT_MATCH', 'Import', ['%field_name' => $translatedFieldName]);
					}
				}
				...
				if ($fieldDataType == 'multipicklist') {
					...
					foreach ($selectedOptions as $selectedOption) {
						if (empty(trim($selectedOption))) continue;
						$matched = false;

						foreach ($allOptions as $picklistOption) {
							if (trim(strtolower($selectedOption)) == trim(strtolower($picklistOption['value'])) || trim(strtolower($selectedOption)) == trim(strtolower($picklistOption['label']))) {
								$matched = true;
								break;
							}
						}
						...
						if (!$matched) {
							$invalidOptions[] = $selectedOption;
						}
					}

					if (!empty($invalidOptions)) {
						$errors[] = vtranslate('LBL_ERROR_REASONS_MULTIPICKLIST_VALUE_NOT_MATCH', 'Import', ['%field_name' => $translatedFieldName, '%invalid_options' => join(', ', $invalidOptions)]);
					}
				}

				// Other field types: Maybe later
			}
			// End Hieu Nguyen

			if (empty($errors) && $createRecordExists) {	// Modified by Hieu Nguyen on 2021-10-15 to skip when record has any error
				$entityInfo = $focus->importRecord($this, $fieldData);
				if ($entityInfo) {
					$entityIdComponents = vtws_getIdComponents($entityInfo['id']);
					$createdRecords[] = $entityIdComponents[1];
				}
			} else if (empty($errors)) {					// Modified by Hieu Nguyen on 2021-10-15 to skip when record has any error
				if (!empty($mergeType) && $mergeType != Import_Utils_Helper::$AUTO_MERGE_NONE) {
					...
					$index = 0;
                    $isDuplicateConditionValid = true;   // Added by Hieu Nguyen on 2020-12-18 to check if the duplicate condition is valid (no empty value at any field)
					...
					foreach ($mergeFields as $mergeFieldName => $mergeFieldLabel) {
                        ...
                        // Modified by Hieu Nguyen on 2020-12-18 to fix bug auto delete existing records that has empty value at duplicate check field when import with mode override duplicate
						$comparisonValue = $fieldData[$mergeFieldName];

                        if (empty(trim($comparisonValue))) {
                            $isDuplicateConditionValid = false;
                            continue;
                        }
                        // End Hieu Nguyen
						...
						switch ($fieldDataType) {
							...
							/*-- Modified by Kelvin Thang on 2020-01-10 set formatNumber for Currency - Number fields */
							case 'integer':
								if (!empty($comparisonValue)) {
									$comparisonValue = CurrencyField::convertToUserFormat($comparisonValue, $this->user, TRUE);
								}
                    ...
                    if (!$isDuplicateConditionValid) {
                        ...
                        $queryGenerator->clearConditionals();   // To eliminate clash of next record values
                    }
					// End Hieu Nguyen
				...
				if ($createRecord) {
					...
					if ($fieldData == null) {
						...
						try {
                            ...
                            // Modified by Hieu Nguyen on 2021-05-10 to fix bug can not update queue status while import Account
							if ($entityInfo && !empty($entityInfo['id'])) {
								$entityIdComponents = vtws_getIdComponents($entityInfo['id']);
								$createdRecords[] = $entityIdComponents[1];
							}
                            // End Hieu Nguyen
            ...
            else if ($createRecord && !empty($entityInfo['id'])) {
				$entityInfo['status'] = self::$IMPORT_RECORD_CREATED;
			}

            if (!in_array($entityInfo['status'], [self::$IMPORT_RECORD_CREATED, self::$IMPORT_RECORD_MERGED, self::$IMPORT_RECORD_UPDATED])) {
                $this->updateImportStatus($rowId, $entityInfo, $errors);
                continue;
            }
            // End Hieu Nguyen
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault = true, $checkMandatoryFieldValues = true) {
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldDataType == 'owner') {
				...
				// Modified by Hoang Duc 2020-02-18 - fix import picklist data not get value of picklist
				$fieldValue = trim($fieldValue);
				$arrValue = $fieldValue ? explode(' |##| ',  $fieldValue) : [];

				if (!$arrValue) {
					if (isset($defaultFieldValues[$fieldName])) $fieldData[$fieldName] = $defaultFieldValues[$fieldName];
					else $fieldData[$fieldName] = '';
				}
				else {
					$multipicklistValue = [];
					$allMultiPicklistValues = $fieldInstance->getPicklistDetails();

					foreach ($arrValue as $selectedOption) {
						foreach ($allMultiPicklistValues as $picklistOption) {
							if(trim(strtolower($selectedOption)) == trim(strtolower($picklistOption['label']))) {
								$multipicklistValue[] = $picklistOption['value'];
							}
						}
					}

					$fieldData[$fieldName] = implode(' |##| ', $multipicklistValue);
				}
				// End added by Hoang Duc
				...
				foreach ($allPicklistDetails as $picklistDetails) {
					...
					// Added by Hoang Duc on 2020-02-18 - fix import picklist data not get value of picklist
					if (strtolower($fieldValue) == trim(strtolower($picklistDetails['label']))) {
                ...
                // Modified by Hoang Duc 2019-10-29 to fix import picklist create new option
                if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase) && !empty($picklistValueInLowerCase)) {
                    $notCategorizedOption = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
                    $fieldValue = $notCategorizedOption;
                    $picklistValueInLowerCase = $notCategorizedOption;
                    $fieldData[$fieldName] = $notCategorizedOption;
                }
                // End Hoang Duc
		...
		foreach ($mandatoryFields as $mandatoryField) {
			if (empty($fieldData[$mandatoryField])) {
				...
				if ($fieldInstance->getFieldDataType() == 'owner') {
					...
					$fieldData[$mandatoryField] = '-unknown-';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown
				}
			}
		}
	...
	public function getImportStatusCount() {
        ...
        // Modified by Hieu Nguyen on 2020-09-10 to add status CANCELLED
		$statusCount = array('TOTAL' => 0, 'IMPORTED' => 0, 'FAILED' => 0, 'PENDING' => 0, 'CREATED' => 0, 'SKIPPED' => 0, 'UPDATED' => 0, 'MERGED' => 0, 'CANCELLED' => 0);
        // End Hieu Nguyen
		...
		if ($result) {
			...
			for ($i = 0; $i < $noOfRows; ++$i) {
                ...
                // Added by Hieu Nguyen on 2020-09-10 to count CANCELLED records
                elseif (self::$IMPORT_RECORD_CANCELLED == $status) {
    ...
    //add by Tung Bui - 30/08/2018 - auto to import data to generate demo data
    public static function generateDemoData() {
        ...
        foreach ($scheduledImports as $scheduledId => $importDataController) {
            ...
            //remove data table
            $adb->pquery("DROP TABLE IF EXISTS vtiger_import_".$current_user->id);

            $importDataController->finishImport();
        }
    }

	// Modified by Phu Vo on 2020.06.09 to Refactor system email logic using common Mailer class
	public static function runScheduledImport() {
		global $current_user, $log;
		require_once('include/Mailer.php');

		$scheduledImports = self::getScheduledImport();
		$templateId = getSystemEmailTemplateByName('Scheduled Import Report');

		if (empty($templateId)) {
			$log->info("Email template is not set for import result");
			return;
		}
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$variables = [
				...
				'module' => $importDataController->module,
				'importedRecords' => $importStatusCount['IMPORTED'] . ' / ' . $importStatusCount['TOTAL'],
				'createdRecords' => $importStatusCount['CREATED'],
				'updatedRecords' => $importStatusCount['UPDATED'],
				'skippedRecords' => $importStatusCount['SKIPPED'],
				'mergedRecords' => $importStatusCount['MERGED'],
			];
			...
			Mailer::send(false, $mainReceivers, $templateId, $variables);
            Import_Queue_Action::updateStatus($scheduledId, Import_Queue_Action::$IMPORT_STATUS_COMPLETED); // Added by Hieu Nguyen on 2020-09-10 to mark schedule as completed
			// $importDataController->finishImport();   // Commented out this line by Hieu Nguyen on 2020-09-10 to prevent loss statistics info
		}
	...
	public static function getImportDetails($user, $moduleName) {
		...
		if ($result) {
			$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
			$headers = array_slice($adb->getColumnNames($tableName), 4);	// Modified by Hieu Nguyen on 2021-10-15 to exclude 4 log columns (id, status, error_reasons, recordid) from record header fields
			...
			for ($i = 0; $i < $noOfRows; ++$i) {
				...
				if ($row['status'] == self::$IMPORT_RECORD_SKIPPED) {
					...
					$record->set('error_reasons', $row['error_reasons']);	// Added by Hieu Nguyen on 2021-10-15 to attach error reasons along with failed record
	...
	public function importRecord($recordData, $operation) {
		...
		try {
			if ($recordData) {
				switch($operation) {
					case 'create' :
                        // Modified by Hieu Nguyen on 2021-05-13 to skip default module duplicate handling as import process has its own duplicate check condition
						global $skipDuplicateCheck;
						$skipDuplicateCheck = true;

                        $GLOBALS['importing'] = true;
                        $entityInfo = vtws_create($moduleName, $recordData, $user);
                        $entityInfo['status'] = self::$IMPORT_RECORD_CREATED;
                        break;
                        // End Hieu Nguyen

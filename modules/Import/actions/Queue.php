...
class Import_Queue_Action extends Vtiger_Action_Controller {
    ...
    // Modified by Hieu Nguyen on 2020-09-10 to fix bug can not drop user temp table
	public static function remove($importId) {
		$db = PearDatabase::getInstance();

		if (Vtiger_Utils::CheckTable('vtiger_import_queue')) {
            $importUserId = $db->getOne("SELECT userid FROM vtiger_import_queue WHERE importid = ?", [$importId]);
			$db->pquery("DROP TABLE IF EXISTS vtiger_import_{$importUserId}", []);

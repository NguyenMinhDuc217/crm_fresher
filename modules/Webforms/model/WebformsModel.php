...
class Webforms_Model {
    ...
    // Modified by Hieu Nguyen on 2020-09-24 to get the right owner id
	function getOwnerId() {
		$ownerIdString = vtlib_purify($this->data['ownerid']);
        $ownerId = end(explode(':', $ownerIdString));
        return $ownerId;
	}
    ...
    function getRoundrobinOwnerId() {
        ...
        // Modified by Hieu Nguyen on 2020-08-25 to parse roundrobin users
        $useridList = [];
        $userIdStrings = explode(',', $roundrobin_userid);

        foreach ($userIdStrings as $userIdString) {
            $userId = end(explode(':', $userIdString));
            $useridList[] = $userId;
        }
        // End Hieu Nguyen

	...
	public function getContacts($SyncState, $user = false) {
		...
		foreach ($contactRecords as $i => $contact) {
			...
			if (!$deleted) {
                ...
                Google_Utils_Helper::saveDebugLog('contacts', 'pull', '[Google->CRM] New/updated Contact', $contact);
                // End Hieu Nguyen
			} else {
				$recordModel->setType($this->getSynchronizeController()->getSourceType())->setMode(WSAPP_SyncRecordModel::WSAPP_DELETE_MODE);

                // Added by Hieu Nguyen on 2020-02-19 to write debug log
                Google_Utils_Helper::saveDebugLog('contacts', 'pull', '[Google->CRM] Deleted Contact', $contact);
                // End Hieu Nguyen
			}
		...
		foreach ($records as $record) {
			...
			try {
				if ($record->getMode() == WSAPP_SyncRecordModel::WSAPP_UPDATE_MODE) {
                    ...
                    // Added by Hieu Nguyen on 2020-02-19 to write debug log
                    Google_Utils_Helper::saveDebugLog('contacts', 'push', '[CRM->Google] Contact updated', $entity);
                    // End Hieu Nguyen
				} else if ($record->getMode() == WSAPP_SyncRecordModel::WSAPP_DELETE_MODE) {
					$this->addDeleteContactEntry($atom,$entity);

                    // Added by Hieu Nguyen on 2020-02-19 to write debug log
                    Google_Utils_Helper::saveDebugLog('contacts', 'push', '[CRM->Google] Contact deleted', $entity);
                    // End Hieu Nguyen
				} else {
					$this->addCreateContactEntry($atom,$entity,$user);

                    // Added by Hieu Nguyen on 2020-02-19 to write debug log
                    Google_Utils_Helper::saveDebugLog('contacts', 'push', '[CRM->Google] Contact created', $entity);
                    // End Hieu Nguyen
				}
			} catch (Exception $e) {
                // Added by Hieu Nguyen on 2020-02-19 to write debug log
                Google_Utils_Helper::saveDebugLog('contacts', 'push', '[CRM->Google] Contact push exception'. $e->getMessage(), $entity);
                // End Hieu Nguyen

...
{* {strip} *}
    ...
    {assign var=VALIDATOR value=Zend_Json::encode($SPECIAL_VALIDATOR)}
    ...
    <div class="reportContainer">
        ...
        <div class="reportContents">
                    ...
                    <tr>
                        ...
                        <td class="fieldValue alignMiddle">
                            <input name="starttime" 
                                class="inputElement dateField" 
                                data-rule-required="true" 
                                data-rule-date="true" 
                                placeholder="{vtranslate('LBL_DATE_START', $MODULE)}" 
                                value="{$TODAY_DATETIME->getDisplayDate()}" 
                                autocomplete="off"
                            />
                        </td>
                        ...
                        <td class="fieldValue alignMiddle">
                            <input name="endtime" 
                                class="inputElement dateField" 
                                data-rule-required="true" 
                                data-rule-date="true" data-validator="{htmlentities($VALIDATOR)}" data-specific-rules="{htmlentities($VALIDATOR)}" placeholder="{vtranslate('LBL_DATE_END', $MODULE)}" 
                                value="{$TODAY_DATETIME->getDisplayDate()}" 
                                autocomplete="off"
                            />
                        </td>
                        <td class="fieldLabel alignMiddle">{vtranslate('LBL_EXT_ID', $MODULE)}</td>
                        <td class="fieldValue alignMiddle"><input name="extid" class="inputElement" placeholder="{vtranslate('LBL_EXT_ID', $MODULE)}" value="" autocomplete="off"/></td>
                    </tr>
                    <tr>
            ...
            <table id="listViewTable" class="table table-striped table-bordered" style="width: 100%">
                <thead>
                    <tr>
                        <th style="width: 8%">{vtranslate('LBL_KEY', $MODULE)}</th>
                        <th style="width: 9%">{vtranslate('LBL_CALL_DIRECTION', $MODULE)}</th>
                        <th style="width: 9%">{vtranslate('LBL_CALLER_NUMBER', $MODULE)}</th>
                        <th style="width: 12%">{vtranslate('LBL_RECEIVER_NUMBER', $MODULE)}</th>
                        <th style="width: 12%">{vtranslate('LBL_DATE_START', $MODULE)}</th>
    ...
    <link type="text/css" rel="stylesheet" href="{vresource_url('modules/PBXManager/resources/YeaStarReport.css')}" />
    ...
    <script src="{vresource_url('modules/PBXManager/resources/YeaStarReport.js')}"></script>
{* {strip} *}
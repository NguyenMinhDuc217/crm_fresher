{*
    CallsSummaryThisMonthWidgetContents.tpl
    Author: Hieu Nguyen
    Date: 2019-10-01
*}
    ...
    {if count($DATA) > 0}
        <div class="row item">
            <div class="row main-data">
                <span class="value">{number_format($DATA['this_month']['calls_count'])}</span>
            ...
            <div class="row sub-data {if !$SHOW_COMPARE}invisible{/if}">
                ...
                {vtranslate('LBL_WIDGET_FROM_LAST_MONTH', $MODULE_NAME)} ({number_format($DATA['last_month']['calls_count'])})
            </div>
        </div>
        <div class="row item">
            <div class="row main-data">
                <span class="value">{number_format($DATA['this_month']['total_duration'], 1)}</span>
            ...
            <div class="row sub-data {if !$SHOW_COMPARE}invisible{/if}">
                ...
                {vtranslate('LBL_WIDGET_FROM_LAST_MONTH', $MODULE_NAME)} ({number_format($DATA['last_month']['total_duration'], 1)})
            </div>
        </div>
        <div class="row item">
            <div class="row main-data">
                <span class="value">{number_format($DATA['this_month']['duration_per_call'], 1)}</span>
            ...
            <div class="row sub-data {if !$SHOW_COMPARE}invisible{/if}">
                ...
                {vtranslate('LBL_WIDGET_FROM_LAST_MONTH', $MODULE_NAME)} ({number_format($DATA['last_month']['duration_per_call'], 1)})
            </div>
        </div>
    {else}

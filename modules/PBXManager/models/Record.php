...
class PBXManager_Record_Model extends Vtiger_Record_Model{
    ...
    public static function getInstanceById($phonecallsid){
        ...
        if (empty($phonecallsid)) return null;
        static $cache = [];
        if (!empty($cache[$phonecallsid])) return $cache[$phonecallsid];
        // End Hieu Nguyen
    ...
    public static function getInstanceBySourceUUID($sourceuuid){
        // Added by Hieu Nguyen on 2021-04-06 to support caching
        if (empty($sourceuuid)) return null;
        static $cache = [];
        if (!empty($cache[$sourceuuid])) return $cache[$sourceuuid];
        // End Hieu Nguyen

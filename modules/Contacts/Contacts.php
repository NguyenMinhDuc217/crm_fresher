...
class Contacts extends CRMEntity {
    ...
    // [FullNameConfig] Modified by Hieu Nguyen on 2020-03-04 to make both firstname and lastname field to disabled its mandatory option in LayoutEditor .
    // User can not control this in LayoutEditor, only system admin can configure which field is required in $fullNameConfig
...
function get_contactsforol($user_name)
{
  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
	 ...
	 */
	/*
	 * Modified $queryPlanner variable to fix wrong variable name
	 * by       Hoang Duc
	 * Date:    2019-10-25
	 * */
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			// Updated by Phuc on 2019.10.17 to update unlink function for campaign
			$sql = 'UPDATE vtiger_contactdetails SET related_campaign = "" WHERE contactid = ? AND related_campaign = ?';

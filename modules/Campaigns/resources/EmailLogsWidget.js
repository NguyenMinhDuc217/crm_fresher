/**
 * Name: EmailLogsWidget.js
 * Author: Phu Vo
 * Date: 2020.11.17
 */

$(() => {
    window.EmailLogsWidget = new class {
        constructor() {
            this.initDataTable();
        }
        ...
        initDataTable() {
            const self = this;
            const table = $('#email-logs-detail');
            const tableColumns = this.getDataTableColumns();

            this.EmailLogsDataTable = table.DataTable({
                ordering: false,
                processing: true,
                serverSide: true,
                pageLength: 10,
            ...
            // setTimeout(() => this.EmailLogsDataTable.columns.adjust().draw(), 0);
        }

        getDataTableColumns () {
            let widgetHeaders = _EMAIL_LOGS_DATA_TABLE_HEADERS;
            let tableColumns = [];

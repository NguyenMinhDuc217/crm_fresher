	...
	*/
	...
	function get_leads($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_leaddetails.*, vtiger_crmentity.crmid,vtiger_leadaddress.phone,vtiger_leadsubdetails.website,
				...
				WHERE vtiger_crmentity.deleted=0 AND vtiger_campaignleadrel.campaignid = ".$id; // Modified by Phu Vo to load converted Leads on related tab
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
		...
		foreach($with_crmids as $with_crmid) {
			// Modified by Phuc on 2019.10.25 to save relationship for module Contacts/Leads
			if ($with_module == 'Leads') {
				$params = [$crmid, $with_crmid];
				$checkResult = $adb->pquery('SELECT 1 FROM vtiger_leaddetails WHERE related_campaign = ? AND leadid = ?', $params);

				if($checkResult && $adb->num_rows($checkResult) > 0) {
					continue;
				}

				$adb->pquery('UPDATE vtiger_leaddetails SET related_campaign = ? WHERE leadid = ?', $params);
			} elseif ($with_module == 'Contacts') {
				$params = [$crmid, $with_crmid];
				$checkResult = $adb->pquery('SELECT 1 FROM vtiger_contactdetails WHERE related_campaign = ? AND contactid = ?', $params);

				if ($checkResult && $adb->num_rows($checkResult) > 0) {
					continue;
				}

				$adb->pquery('UPDATE vtiger_contactdetails SET related_campaign = ? WHERE contactid = ?', $params);
			// Ended by Phuc

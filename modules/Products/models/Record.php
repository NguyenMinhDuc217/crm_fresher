...
class Products_Record_Model extends Vtiger_Record_Model {
	...
	function getSubProducts($active = false) {
		...
		if ($active) {
			$query .= ' AND vtiger_products.active = 1';
		}
	...
	public function getActiveStatusOfRecord(){
		...
		$recordId = $this->getId();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT active FROM vtiger_products WHERE productid = ?',array($recordId));
		$activeStatus = $db->query_result($result, 'active');
		return $activeStatus;
	}

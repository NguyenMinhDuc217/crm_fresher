...
class Products_ListView_Model extends Vtiger_ListView_Model {
    ...
    // Implemented by Hieu Nguyen on 2019-07-31
	public function getListViewMassActions($linkParams) {
        ...
        if(CPSocialIntegration_Config_Helper::isZaloEnabled()) {
			$zaloSyncLink = array(
				...
				'linklabel' => 'LBL_SOCIAL_INTEGRATION_SYNC_TO_ZALO_SHOP',
				'linkurl' => 'javascript:SocialHandler.triggerSyncProductsToZalo(this);',
				'linkicon' => ''
			);
            
			$massActionLinks['LISTVIEWMASSACTION'][] = Vtiger_Link_Model::getInstanceFromValues($zaloSyncLink);
		}
	...
	public function addSubProductsQuery($listQuery){
		...
		$splitQuery[1] .= " AND vtiger_seproductsrel.productid = $productId AND vtiger_products.active = 1";
	...
	public function getSubProducts($subProductId){
		...
		if(!empty($subProductId)){
			...
			$query = 'SELECT vtiger_seproductsrel.crmid from vtiger_seproductsrel
						...
						WHERE vtiger_seproductsrel.productid=? AND vtiger_products.active = 1';

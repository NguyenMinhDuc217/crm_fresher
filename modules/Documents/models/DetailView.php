...
class Documents_DetailView_Model extends Vtiger_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
        ...
        // Modified by Hieu Nguyen on 2020-06-26 to display download button only for uploaded file
        if ($recordModel->get('filestatus') && $recordModel->get('filename') && $recordModel->get('filesize') > 0) {
            $basicActionLink = array(
                    'linktype' => 'DETAILVIEW',
        ...
        // Modified by Hieu Nguyen on 2020-06-26 to display send email button only for uploaded file
		if ($recordModel->get('filestatus') && $recordModel->get('filename') && $recordModel->get('filesize') > 0) {
			...
			if ($currentUserModel->hasModulePermission($emailModuleModel->getId())) {
				$basicActionLink = array(
						...
						'linkurl' => "javascript:Documents_Detail_Js.triggerSendEmail('[". $recordModel->getId() ."]')",
						'linkicon' => ''
				);
				$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
			}

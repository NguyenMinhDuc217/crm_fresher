...
class Vtiger_Deprecated {
	...
	// Modified by Hieu Nguyen on 2021-09-29 to support fetching multiple language at the same time
	static function getModuleTranslationStrings($language, $module) {
		static $cache = [];

		if (empty($cache[$language])) {
			$cache[$language] = [];
		}

		if (!empty($cache[$language][$module])) {
			return $cache[$language][$module];
		}

		$moduleStrings = Vtiger_Language_Handler::getModuleStringsFromFile($language, $module);
		$cache[$language][$module] = $moduleStrings['languageStrings'];

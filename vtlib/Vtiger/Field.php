...
class Vtiger_Field extends Vtiger_FieldBasic {
	...
	function initialize($valuemap, $moduleInstance = false, $blockInstance = false) {
		parent::initialize($valuemap, $moduleInstance, $blockInstance);

		// Modified by Tung Nguyen on 2022.03.21
		if ($_REQUEST['parent'] == 'Settings') {
			if ($_REQUEST['module'] == 'LayoutEditor') {
				...
				$blockField = $GLOBALS['current_view'] == 'edit' ? 'editview_block' : 'block';
				$sequenceField = $GLOBALS['current_view'] == 'edit' ? 'editview_sequence' : 'sequence';
				$presenceField = $GLOBALS['current_view'] == 'edit' ? 'editview_presence' : 'presence';
			}
			else {
				$blockField = 'editview_block';
				$sequenceField = 'editview_sequence';
				$presenceField = 'editview_presence';
			}
		}
		// End by Tung Nguyen
		else {
		...
		// Added by Phuc on 2020.08.04 to allow loading field in detail view when module is report and mode is step 3
		if ($_REQUEST['module'] == 'Reports' && isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'step3') {
			$presenceField = 'presence';
		}
		// Ended by Phuc

		$this->block = $blockInstance ? $blockInstance : Vtiger_Block::getInstance($valuemap[$blockField], $moduleInstance);
		$this->sequence = $valuemap[$sequenceField];
		$this->presence = $valuemap[$presenceField];

		// Load label display in both english and vietnamese for editing
		require_once('include/utils/LangUtils.php');
		$moduleName = $this->block->module->name;
		$modStringsEn = LangUtils::readModStrings($moduleName, 'en_us');
		$modStringsVn = LangUtils::readModStrings($moduleName, 'vn_vn');

		$this->labelDisplayEn = $modStringsEn['languageStrings'][$this->label];
		$this->labelDisplayVn = $modStringsVn['languageStrings'][$this->label];
	}
	// End Hieu Nguyen
	...
	function setPicklistValues($values) {
        ...
        // Added by Hieu Nguyen on 2021-06-15 to create default label for new picklist values
        require_once('include/utils/LangUtils.php');
        $moduleName = $this->block->module->name;
	 ...
	 */
	 static function getAllForBlock($blockInstance, $moduleInstance=false) {
		...
		if($cache->getBlockFields($moduleInstance->name,$blockInstance->id)){
			...
			// Modified by Hieu Nguyen on 2018-07-27
			$sequenceField = $GLOBALS['current_view'] == 'edit' ? 'editview_sequence' : 'sequence';

			if($moduleInstance) {
				$query = "SELECT * FROM vtiger_field WHERE block=? AND tabid=? ORDER BY {$sequenceField}";
				$queryParams = Array($blockInstance->id, $moduleInstance->id);
			} else {
				$query = "SELECT * FROM vtiger_field WHERE block=? ORDER BY {$sequenceField}";
	...
	static function getAllForModule($moduleInstance) {
		...
		// Modified by Hieu Nguyen on 2018-07-27
		$sequenceField = $GLOBALS['current_view'] == 'edit' ? 'editview_sequence' : 'sequence';
		$query = "SELECT * FROM vtiger_field WHERE tabid=? ORDER BY {$sequenceField}";
		// End Hieu Nguyen

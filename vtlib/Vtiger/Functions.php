...
class Vtiger_Functions {
	...
	static function getModuleId($name) {
		...
		$moduleId = $moduleInfo ? $moduleInfo['tabid'] : NULL;
        $cache[$name] = $moduleId;
        return $moduleId;
	}
	...
	static function getModuleName($id) {
		...
		$moduleName = $moduleInfo ? $moduleInfo['name'] : NULL;
        $cache[$id] = $moduleName;
        return $moduleName;
	}
	...
	// Modified by Hieu Nguyen on 2021-08-04 to trim value before return
	static function getCRMRecordLabel($id, $default = '') {
		$metadata = self::getCRMRecordMetadata($id);
		return $metadata ? trim($metadata['label']) : $default;
	}
	...
	static function getCRMRecordLabels($module, $ids) {
		...
		if ($module == 'Users' || $module == 'Groups') {
			...
			foreach ($metadatas as $data) {
				$result[$data['crmid']] = trim($data['label']);	// Modified by Hieu Nguyen on 2021-08-04 to trim value before return
			}
			return $result;
		}
	}
		...
		if ($ids) {
			...
			if ($groups) {
				foreach ($groups as $id => $label) {
					$nameList[$id] = trim($label);	// Modified by Hieu Nguyen on 2021-08-04 to trim value before return
				}
			}
		}
			...
			if ($ids) {
				...
				if($result) {
					while ($row = $adb->fetch_array($result)) {
						...
						// Modified by Hieu Nguyen on 2021-08-04 to trim value before return
						$entityDisplay[$row['id']] = trim(implode(' ', $labelValues));
					}
				}
			}
	...
	static function validateImage($file_details) {
		...
		if (($filetype == "jpeg" ) || ($filetype == "png") || ($filetype == "jpg" ) || ($filetype == "pjpeg" ) || ($filetype == "x-png") || ($filetype == "gif") || ($filetype == 'bmp')) {
			...
			$saveimage = 'false';
			$_SESSION['image_type_error'] .= "" . $file_details[name] . "" . $app_strings['MSG_IS_NOT_UPLOADED'];
		}
		return $saveimage;
	}
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
        ...
        // Modified by Hieu Nguyen on 2020-12-29 to add DISTINCT keyword
	...
	public static function getMandatoryReferenceFields($module, $mode = 'label') {
		...
		foreach ($referenceFields as $field => $fieldModel) {
			...
			if ($fieldModel->isMandatory() && !empty($referenceModule) && !in_array($uiType, array(50, 51, 52))) {
				$recordName = 'Unknown';   // Modified by Hieu Nguyen on 2021-05-24 to map this value as unknown

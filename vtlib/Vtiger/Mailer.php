	...
	*/
	...
	function __AddToQueue($linktoid) {
		if($this->__initializeQueue()) {
			...
			// Updated by Phuc on 2019.11.15
			global $log;
			$result = $adb->pquery('INSERT INTO vtiger_mailer_queue(id,fromname,fromemail,content_type,subject,body,mailer,relcrmid) VALUES(?,?,?,?,?,?,?,?)',
			...
			if (!$result) {
				$log->info("Can not create mailer queue");
				exit();
			}
	...
	static function dispatchQueue(Vtiger_Mailer_Listener $listener=null) {
		global $adb, $emailQueueConfig; // Modified by Hieu Nguyen on 2019-10-14
		if(!Vtiger_Utils::CheckTable('vtiger_mailer_queue')) return;

		$mailer = new self();

        // Modified by Hieu Nguyen on 2019-10-14 to support balance processing
		$queue = $adb->pquery('SELECT * FROM vtiger_mailer_queue LIMIT ?', [$emailQueueConfig['batch_limit']]);
        // End Hieu Nguyen
		...
		if($adb->num_rows($queue)) {
			for($index = 0; $index < $adb->num_rows($queue); ++$index) {
				...
				$mailer->FromName = html_entity_decode($queue_record['fromname']); // Updated by Phuc on 2019.11.15 to fix error on loading email
				$mailer->Subject = decode_emptyspace_html($queue_record['subject']);
				...
				for($eidx = 0; $eidx < $adb->num_rows($emails); ++$eidx) {
					...
					// Updated by Phuc on 2020.01.02 to refactor code and fix error on entity decode
					switch ($email_record['type']) {
						case 'TO':
							$mailer->AddAddress($email_record['email'], html_entity_decode($email_record['name']));
							break;
						case 'CC':
							$mailer->AddCC($email_record['email'], html_entity_decode($email_record['name']));
							break;
						case 'BCC':
							$mailer->AddBCC($email_record['email'], html_entity_decode($email_record['name']));
							break;
						case 'RPLYTO':
							$mailer->AddReplyTo($email_record['email'], html_entity_decode($email_record['name']));
							break;
					}
					// Ended by Phuc
				}
				...
				if($sent) {
                    ...
                    // Modified by Hieu Nguyen on 2019-10-14
                    ...
                    // Modifed by Hieu Nguyen on 2019-10-14 to limit max attempts
                    if ($queue_record['attempt_count'] + 1 < $emailQueueConfig['max_attempts']) {
                        $adb->pquery(
                            'UPDATE vtiger_mailer_queue SET failed = ?, failreason = ?, attempt_count = attempt_count + 1 WHERE id = ?', 
                            [1, $mailer->ErrorInfo, $queueid]
                        );
                    }
                    else {
                        self::removeQueue($queueid);
    ...
    // Extreacted function by Hieu Nguyen on 2019-10-14 to be used in common
    static function removeQueue($queueId) {
        global $adb;

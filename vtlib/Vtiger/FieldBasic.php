...
class Vtiger_FieldBasic {
	...
	function initialize($valuemap, $moduleInstance=false, $blockInstance=false) {
        ...
        // Added by Hieu Nguyen on 2020-03-20 to be able to create new column with correct column type
        if (!empty($valuemap['columntype'])) {
            $this->columntype = $valuemap['columntype'];
        }
        // End Hieu Nguyen
	}
	...
	function __create($blockInstance) {
        ...
        $editViewBlockId = $this->getBlockId();

        if($this->editview_block_id) {
            $editViewBlockId = $this->editview_block_id;
        }

        // Not sure the block in detail view exists, so set this field to hide as default
		$detailViewPresence = 1;

		$sql = "SELECT 1 FROM vtiger_blocks WHERE tabid = {$this->getModuleId()} AND blockid = {$this->getBlockId()}";
		$detailViewBlockExists = $adb->getOne($sql);

		if($detailViewBlockExists) {
			$detailViewPresence = 2;	// Set to show if the detail view block is exist
		}

        $editViewSequence = $this->sequence;

        if($this->editview_sequence) {
            $editViewSequence = $this->editview_sequence;
        }

        // Field is added inside an existing block in edit view, so it should be displayed
        $editViewPresence = 2;  

        if($this->editview_presence) {
            $editViewPresence = $this->editview_presence;
        }
		...
		$adb->pquery("INSERT INTO vtiger_field (tabid, fieldid, columnname, tablename, generatedtype,
			...
			editview_block, editview_sequence, editview_presence)
			VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
			Array(
				$this->getModuleId(), $this->id, $this->column, $this->table, intval($this->generatedtype),
				$this->uitype, $this->name, $this->label, $this->readonly, $detailViewPresence, $this->defaultvalue,
                ...
                $editViewBlockId, $editViewSequence, $editViewPresence
			)
		);
		// End Hieu Nguyen

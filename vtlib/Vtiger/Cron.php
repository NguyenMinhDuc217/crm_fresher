...
class Vtiger_Cron {
	...
	function isRunnable() {
		...
		if (!$this->isDisabled()) {
			// Modified by Hieu Nguyen on 2018-08-18 to fix bug a job runs multiple times before it's finished
			// When job is stated but not ended yet then the new job will not be able to run
			if($this->getLastStart() > 0 && $this->getLastEnd() < $this->getLastStart()) {
				return false;
			}

			// When a job end and elapsed time passed the configured frequency then the new job will be able to run
	...
	*/
    ...
    // Implemented by Hieu Nguyen on 2020-06-25 to reset stuck services that run longer than 30 mins
    static function resetStuckServices() {
        global $adb;
        $sql = "UPDATE vtiger_cron_task SET laststart = 0, lastend = 0 
            WHERE status = ? 
                AND (laststart IS NOT NULL AND laststart != 0) 
                AND (UNIX_TIMESTAMP() - laststart) / 60 > 30";
		$adb->pquery($sql, [self::$STATUS_RUNNING]);
    }
}
?>
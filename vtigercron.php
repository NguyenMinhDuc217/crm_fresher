...
function vtigercron_detect_run_in_cli(){
     ...
     * Date: 2018-08-01
     */
    echo "[CRON] php_sapi_name: ".php_sapi_name()."\n";

    //return (!isset($_SERVER['SERVER_SOFTWARE']) && (php_sapi_name() == 'cli' ||  is_numeric($_SERVER['argc']) && $_SERVER['argc'] > 0));
    //return (!isset($_SERVER['SERVER_SOFTWARE']) && (php_sapi_name() == 'fpm-fcgi' ||  is_numeric($_SERVER['argc']) && $_SERVER['argc'] > 0));

    $php_sapi_name = 'fpm-fcgi';
    //$php_sapi_name = 'apache2handler';

    if( !isset($_SERVER['SERVER_SOFTWARE']) && (php_sapi_name() == $php_sapi_name) ||  (is_numeric($_SERVER['argc']) && $_SERVER['argc'] > 0) ){
        return true;
    }else{
        return false;
    }
...
//if(vtigercron_detect_run_in_cli() || (isset($_SESSION["authenticated_user_id"]) &&	isset($_SESSION["app_unique_key"]) && $_SESSION["app_unique_key"] == $application_unique_key)){
     ...
     * Date: 2018-08-01
     */
if(vtigercron_detect_run_in_cli() || !isset($_SERVER['REMOTE_ADDR']) || (isset($_SESSION["authenticated_user_id"]) &&   isset($_SESSION["app_unique_key"]) && $_SESSION["app_unique_key"] == $application_unique_key)){
    Vtiger_Cron::resetStuckServices();  // Added by Hieu Nguyen on 2020-06-25 to reset stuck services before doing anything
